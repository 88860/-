...
wg_status(){
  local file=$WG_CONF role enabled public ipv4 ipv6 peer_host peer_port peer_key endpoint_ips system_mode
  screen_clear
  if [ ! -f "$file" ]; then
  ...
  system_mode=$(jq -r '.endpoint.system // false' "$file" 2>/dev/null)
  ...
  case "$role" in
    client)
      ...
      if [ "$enabled" = true ] && { [ "$system_mode" != true ] || [ -d "/sys/class/net/$WG_IF" ]; }; then
      ...
      tell "外层IPv4|$(if printf "%s\n" "$endpoint_ips" | grep -Eq '^[0-9]+(\.[0-9]+){3}$'; then printf '%s正常%s' "$GREEN" "$PLAIN"; else printf '%s不可用%s' "$RED" "$PLAIN"; fi)      隧道IPv4|$(if [ -n "$ipv4" ] && [ "$enabled" = true ] && { [ "$system_mode" != true ] || [ -d "/sys/class/net/$WG_IF" ]; }; then printf '%s正常%s' "$GREEN" "$PLAIN"; else printf '%s不可用%s' "$RED" "$PLAIN"; fi)"
      tell "外层IPv6|$(if printf "%s\n" "$endpoint_ips" | grep -q ':'; then printf '%s正常%s' "$GREEN" "$PLAIN"; else printf '%s不可用%s' "$RED" "$PLAIN"; fi)      隧道IPv6|$(if [ -n "$ipv6" ] && [ "$enabled" = true ] && { [ "$system_mode" != true ] || [ -d "/sys/class/net/$WG_IF" ]; }; then printf '%s正常%s' "$GREEN" "$PLAIN"; else printf '%s不可用%s' "$RED" "$PLAIN"; fi)"
      tell ""
      tell "${CYAN}----------------------------------------${PLAIN}"
      ;;
    server)
...
menu_wireguard(){
  while :; do
    ...
    if [ ! -f "$WG_CONF" ]; then
      ...
      local role enabled state ms color system_mode
      role=$(jq -r '.role // ""' "$WG_CONF")
      enabled=$(jq -r '.enabled//false' "$WG_CONF")
      system_mode=$(jq -r '.endpoint.system // false' "$WG_CONF")
      if [ "$role" = client ] && [ "$enabled" = true ]; then
        if [ "$system_mode" = true ] && [ ! -d "/sys/class/net/$WG_IF" ]; then
        ...
        if [ "$enabled" = true ]; then
          if [ "$system_mode" != true ] || [ -d "/sys/class/net/$WG_IF" ]; then tell "  模式：服务端|${GREEN}已启动${PLAIN}"; else tell "  模式：服务端|${RED}异常${PLAIN}"; fi
        else
          tell "  模式：服务端|${RED}已停止${PLAIN}"
        fi
      else
        if [ "$enabled" = true ]; then
          if [ "$system_mode" != true ] || [ -d "/sys/class/net/$WG_IF" ]; then tell "  模式：客户端|${GREEN}已启动${PLAIN}"; else tell "  模式：客户端|${RED}异常${PLAIN}"; fi
        else
...
transparent_proxy_available(){
  ...
  local cap hex nibble
  hex=$(awk '$1=="CapEff:"{print $2; exit}' /proc/self/status 2>/dev/null)
  [ -n "$hex" ] || return 1
  nibble=$(printf '%s' "$hex" | sed 's/.*\(....\)$/\1/' | cut -c1)
  case "$nibble" in
    1|3|5|7|9|a|b|c|d|e|f|A|B|C|D|E|F) return 0 ;;
    *) return 1 ;;
  esac
}
...
build_config(){
  ...
  if [ "$selected" != direct ] && [ "$selected" != wireguard ] && [ -f "$PEER_DIR/$selected.json" ]; then
    ...
    local wg_system_mode; wg_system_mode=$(wg_system_enabled)
    wgob=$(jq -c --arg resolver "$dns_direct_tag" --arg name "$WG_IF" --argjson system "$wg_system_mode" '.endpoint | .system=$system | if $system then .name=$name else del(.name) end | if (.peers[0].address // "") != "" then .domain_resolver=$resolver else . end' "$wg_source") || return 1
...
# WireGuard environment helpers shared by both platform payloads.
is_lxc(){
...
wg_system_enabled(){
  if is_lxc; then printf '%s' false; else printf '%s' true; fi
}

wg_server_nat_apply(){ return 0; }
...
wg_server_nat_remove(){
  ...
  command -v sysctl >/dev/null 2>&1 && sysctl -w net.ipv4.ip_forward="$prev4" >/dev/null 2>&1 || true
  command -v sysctl >/dev/null 2>&1 && sysctl -w net.ipv6.conf.all.forwarding="$prev6" >/dev/null 2>&1 || true
...
wg_init_server(){
  ...
  local system_mode; system_mode=$(wg_system_enabled)
  body=$(jq -n --arg private "$WG_PRIVATE" --arg public "$WG_PUBLIC" --arg a4 "$WG_IPV4" --arg a6 "$WG_IPV6" --arg name "$WG_IF" --argjson port "$port" --argjson system "$system_mode" '{role:"server",enabled:false,private_key:$private,public_key:$public,address:[$a4,$a6],listen_port:$port,clients:[],endpoint:({type:"wireguard",tag:"wireguard",system:$system,mtu:1280,address:[$a4,$a6],private_key:$private,listen_port:$port,peers:[]} + (if $system then {name:$name} else {} end))}') || { tell_warn "配置生成失败"; wait_key; return; }
...
wg_init_client(){
  ...
  local system_mode; system_mode=$(wg_system_enabled)
  body=$(jq -n --arg private "$WG_PRIVATE" --arg public "$WG_PUBLIC" --arg a4 "$WG_IPV4" --arg a6 "$WG_IPV6" --arg peer4 "$WG_PEER_IPV4" --arg peer6 "$WG_PEER_IPV6" --arg host "$host" --arg peer_key "$peer_key" --arg name "$WG_IF" --argjson port "$port" --argjson system "$system_mode" '{role:"client",enabled:false,private_key:$private,public_key:$public,address:[$a4,$a6],listen_port:0,peer_public_key:$peer_key,peer_ip:$peer4,peer_ip6:$peer6,peer_host:$host,peer_port:$port,endpoint:({type:"wireguard",tag:"wireguard",system:$system,mtu:1280,address:[$a4,$a6],private_key:$private,peers:[{address:$host,port:$port,public_key:$peer_key,allowed_ips:["0.0.0.0/0","::/0"]}]} + (if $system then {name:$name} else {} end))}') || { tell_warn "配置生成失败"; wait_key; return; }
...
wg_reinit_prepare(){
  ...
  previous=$(state_get exit)
  if [ "$(jq -r '.endpoint.system // false' "$WG_CONF" 2>/dev/null)" = true ]; then
...
wg_init_server(){
  ...
  body=$(jq -n \
    ...
    --arg a4 "$WG_IPV4" --arg a6 "$WG_IPV6" --arg name "$WG_IF" \
    --argjson port "$port" --argjson system "$system_mode" \
    '{role:"server",enabled:false,private_key:$private,public_key:$public,
      address:[$a4,$a6],listen_port:$port,clients:[],
      endpoint:({type:"wireguard",tag:"wireguard",system:$system,mtu:1280,
        address:[$a4,$a6],private_key:$private,listen_port:$port,peers:[]} + (if $system then {name:$name} else {} end))}') || { tell_warn "配置生成失败"; wait_key; return; }
...
wg_init_client(){
  ...
  body=$(jq -n \
    ...
    --arg host "$host" --arg peer_key "$peer_key" --argjson port "$port" --argjson system "$system_mode" \
    '{role:"client",enabled:false,private_key:$private,public_key:$public,
      ...
      endpoint:({type:"wireguard",tag:"wireguard",system:$system,mtu:1280,
        ...
        peers:[{address:$host,port:$port,public_key:$peer_key,
          allowed_ips:["0.0.0.0/0","::/0"]}]} + (if $system then {name:$name} else {} end))}')
