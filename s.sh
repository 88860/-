...
detect_env() {
  if command -v systemctl >/dev/null 2>&1 && command -v apt-get >/dev/null 2>&1; then
    ...
    printf "${RED}[×] 系统不支持: 仅适配 Debian(Systemd) 或 Alpine(OpenRC) 环境${PLAIN}\n"
    exit 1
  fi
}
detect_env
...
check_dependencies(){
  if [ "$SYS_ENV" = "systemd" ]; then
    local missing=0 cmd
    for cmd in curl tar jq openssl nft ss ip ping wg; do
      command -v "$cmd" >/dev/null 2>&1 || missing=1
    done
    [ "$missing" = 1 ] && DEBIAN_FRONTEND=noninteractive apt-get update -q >/dev/null 2>&1
    local pkgs="curl tar jq openssl nftables iproute2 iputils-ping wireguard-tools"
    for pkg in $pkgs; do
...
remote_version(){
  if [ "$SYS_ENV" = "systemd" ]; then
    curl -fsSL -m 15 "$GH_API/releases/latest" 2>/dev/null | jq -r '.tag_name//""' | sed 's/^v//'
  else
    wget -qO- -T 10 "$GH_API/releases/latest" 2>/dev/null | jq -r '.tag_name // empty' | sed 's/^v//'
  fi
}
...
install_core(){
  local version url asset tmp found
  version=$(remote_version)
  [ -n "$version" ] || { tell_warn "获取版本信息失败或网络异常"; return 1; }

  if [ "$SYS_ENV" = "openrc" ]; then
    case $(uname -m) in
      x86_64|amd64) asset="linux-amd64-musl" ;;
      aarch64|arm64) asset="linux-arm64-musl" ;;
      armv7l|armv8l) asset="linux-armv7-musl" ;;
      armv6l) asset="linux-armv6" ;;
      i386|i686) asset="linux-386-musl" ;;
      *) tell_warn "未找到匹配架构: $(uname -m)"; return 1 ;;
    esac
    ...
    if wget -qO- -T 120 "$url" 2>/dev/null | tar -xzf - -C "/usr/local/bin" --strip-components=1 "sing-box-${version}-${asset}/sing-box" 2>/dev/null; then
      ...
      tell_warn "安装包下载或解压失败 (可能遭遇网络问题)"
      return 1
    fi
  else
    local json candidate
    json=$(curl -fsSL -m 15 "$GH_API/releases/latest" 2>/dev/null)
    if ! printf '%s' "$json" | grep -q '"tag_name"'; then tell_warn "获取版本信息失败或遭遇 API 限流"; return 1; fi
    
    for candidate in $(asset_candidates); do
      url=$(printf '%s' "$json" | jq -r --arg s "$candidate.tar.gz" '.assets[]?|select(.name|endswith($s))|.browser_download_url' | head -1)
    ...
    if ! curl -4 -fL --retry 3 --connect-timeout 15 -m 240 "$url" -o "$tmp/core.tar.gz" 2>/dev/null; then
      if ! curl -fL --retry 3 --connect-timeout 15 -m 240 "$url" -o "$tmp/core.tar.gz" 2>/dev/null; then
...
local_ipv4(){
  if [ "$SYS_ENV" = "openrc" ]; then
    local ip; ip=$(wget -qO- -T 5 http://ipv4.icanhazip.com 2>/dev/null | tr -d '\n ')
...
local_ipv6(){
  if [ "$SYS_ENV" = "openrc" ]; then
    local ip; ip=$(wget -qO- -T 5 http://ipv6.icanhazip.com 2>/dev/null | tr -d '\n ')
    [ -n "$ip" ] && printf '%s' "$ip" || ip -6 route get 2606:4700:4700::1111 2>/dev/null | awk '{for(i=1;i<=NF;i++) if($i=="src") {print $(i+1); exit}}'
  else
    ip -6 route get 2001:4860:4860::8888 2>/dev/null | awk '{for(i=1;i<=NF;i++) if($i=="src") {print $(i+1); exit}}'
  fi
}
...
clear_hopping_rules(){ 
  [ "$SYS_ENV" = "systemd" ] && nft delete table inet "$HOP_TABLE" 2>/dev/null
}
...
acme_options(){
  ...
  case $(state_get challenge) in
    ...
    dns_alidns) body=$(echo "$body" | jq --arg k "$(state_get ali_key)" --arg s "$(state_get ali_secret)" '.dns01_challenge={provider:"alidns",access_key_id:$k,access_key_secret:$s}') ;;
    dns_acmedns) body=$(echo "$body" | jq --arg u "$(state_get acmedns_user)" --arg p "$(state_get acmedns_pass)" --arg s "$(state_get acmedns_sub)" --arg r "$(state_get acmedns_url)" '.dns01_challenge={provider:"acmedns",username:$u,password:$p,subdomain:$s,server_url:$r}') ;;
...
build_config(){
  ...
  local peer_host tls_extra providers strategy ipv6 f
  ...
  if [ -f "$WG_CONF" ] && [ "$(jq -r '.enabled//false' "$WG_CONF")" = "true" ]; then
    ...
    if [ "$(jq -r .role "$WG_CONF")" = "client" ]; then
      ...
      if [ "$SYS_ENV" = "systemd" ]; then
        ...
        [ "$selected" = "wireguard" ] && { final="wireguard"; }
      fi
    fi
  fi
  ...
  if [ "$SYS_ENV" = "systemd" ]; then
    [ "$use_tun" = 1 ] && inbounds=$(jq -n --argjson list "$inbounds" --arg name "$TUN_IF" '
      [{type:"tun",tag:"tun-in",interface_name:$name,address:["172.19.0.1/30","fdfe:dcba:9876::1/126"],auto_route:true,strict_route:true,stack:"mixed",mtu:1400}] + $list')
    auto_if=$([ "$use_tun" = 1 ] && echo true || echo false)
  elif [ "$SYS_ENV" = "openrc" ]; then
    if [ "$selected" != "direct" ]; then
      inbounds=$(jq -n --argjson list "$inbounds" '[{type:"mixed",tag:"mixed-in",listen:"127.0.0.1",listen_port:2080}] + $list')
    fi
    auto_if=false
  fi
  ...
  dns_block=$(jq -n --arg detour "$final" --arg host "$peer_host" --arg strategy "$strategy" --arg direct_srv "$dns_direct_server" --arg remote_srv "$dns_remote_server" '
    {
      ...
      rules: [
        (if $host != "" and ($host | test("^[0-9]+\\.[0-9]+\\.[0-9]+\\.[0-9]+$") | not) and ($host | test("^[0-9a-fA-F:]+$") | not) then
          {domain: [$host], server: "dns-direct"} else empty end)
      ],
      final: "dns-remote",
  ...
  jq -n --argjson inbounds "$inbounds" --argjson outbounds "$outbounds" \
        ...
        --arg final "$final" --argjson auto_if "$auto_if" '
    {log:{level:"warn",timestamp:true},
     dns:$dns,
     inbounds:$inbounds,
     outbounds:$outbounds,
     route:{rules:$rules,final:$final,auto_detect_interface:$auto_if}}
...
apply_config(){
  ...
  if ! error=$("$CORE" check -c "$tmp" 2>&1); then
    out_warn "配置语法校验未通过 (未触发回滚，请仔细排查节点参数或端口语法):"
...
run_watchdog(){
  ...
  while :; do
    sleep 120
    if [ "$SYS_ENV" = "systemd" ]; then
      if curl -s -o /dev/null --connect-timeout 4 -m 6 "$PROBE_URL"; then fail_count=0; else fail_count=$((fail_count+1)); fi
    elif [ "$SYS_ENV" = "openrc" ]; then
...
validate_port(){
  ...
  if [ "$port" = 443 ] && [ "$(state_get challenge)" = "alpn" ]; then tell_warn "端口 443 已保留给 ALPN 证书签发"; return 1; fi
...
save_node(){
  ...
  else out_warn "配置应用失败，请检查运行日志。原修改文件已强制保留不回滚。"
  fi
  wait_key
}
...
create_hysteria2(){
  ...
  hopping=""
  if [ "$SYS_ENV" = "systemd" ] && prompt_yes "是否配置并开启端口跳跃防封锁机制"; then
...
menu_delete_protocol(){
  clear_screen; tell "${CYAN}========== 删除协议 ==========${PLAIN}"
  select_node || return
  ...
  apply_config && tell_ok "已删除" || tell_warn "移除节点文件成功但重启异常 (强制执行未回滚)"
  wait_key
}

menu_modify_protocol(){
  local kind value current_hop up_mbps down_mbps
  clear_screen; tell "${CYAN}========== 修改配置 ==========${PLAIN}"
  select_node || return
  while :; do
    ...
    [ "$kind" = "hysteria2" ] && [ "$SYS_ENV" = "systemd" ] && tell "  7. 端口跳跃机制 (仅Systemd)"
    tell "  0. 返回"
    tell "${CYAN}==============================${PLAIN}"
    ...
    case $(prompt "请选择") in
      ...
      7)
        [ "$kind" = "hysteria2" ] && [ "$SYS_ENV" = "systemd" ] || { tell_warn "输入无效"; sleep 1; continue; }
    ...
    else tell_warn "因配置冲突或语法异常应用失败 (文件已强制修改未回滚，请注意排查)"
    fi
    wait_key
  done
}
...
render_certificate_status(){
  ...
  if [ "$SYS_ENV" = "systemd" ]; then systemctl is-active --quiet sing-box || tell_warn "服务离线，无法自动续期"
  elif [ "$SYS_ENV" = "openrc" ]; then rc-service sing-box status >/dev/null 2>&1 || tell_warn "服务离线，无法自动续期"
  fi
}
...
menu_server_info(){
  ...
  if [ "$SYS_ENV" = "systemd" ]; then systemctl is-active --quiet sing-box && tell_ok "singbox: 运行中" || tell_warn "singbox: 未运行"
  elif [ "$SYS_ENV" = "openrc" ]; then rc-service sing-box status >/dev/null 2>&1 && tell_ok "singbox: 运行中" || tell_warn "singbox: 未运行"
  fi
...
menu_change_domain(){
  ...
  else tell_warn "部署应用失败 (强制修改未回滚)"; wait_key
  fi
}
      ...
      6) 
         if [ "$SYS_ENV" = "systemd" ]; then
           ...
           if systemctl is-active --quiet sing-box; then
             sync_bypass_rules; sync_hopping_rules; tell_ok "已重启"
           else tell_warn "重启失败"
           fi
         elif [ "$SYS_ENV" = "openrc" ]; then
           rc-service sing-box restart >/dev/null 2>&1
           if rc-service sing-box status >/dev/null 2>&1; then tell_ok "已重启"; else tell_warn "重启失败"; fi
         fi
         wait_key ;;
      7) 
         if [ "$SYS_ENV" = "systemd" ]; then systemctl stop sing-box && tell_ok "已停止" || tell_warn "操作异常"
         elif [ "$SYS_ENV" = "openrc" ]; then rc-service sing-box stop && tell_ok "已停止" || tell_warn "操作异常"
         fi
         wait_key ;;
      0) break ;;
      *) tell_warn "输入无效，请重新选择"; sleep 1 ;;
...
uri_to_outbound(){
  ...
  local username password congestion alpn obfs hop_range flow udp_mode
  ...
  case $URI_SCHEME in
    vless)
      ...
      if [ "$security" = "reality" ]; then
        outbound=$(printf '%s' "$outbound" | jq --arg sni "$sni" --arg fp "$fingerprint" --arg pbk "$(query_value pbk)" --arg sid "$(query_value sid)" \
        ...
        outbound=$(printf '%s' "$outbound" | jq --arg sni "$sni" --arg fp "$fingerprint" \
          '.tls={enabled:true,server_name:$sni,utls:{enabled:true,fingerprint:$fp}}')
      fi
      case $network in
        ws) outbound=$(printf '%s' "$outbound" | jq --arg path "${path:-/}" --arg host "$vhost" \
              '.transport=({type:"ws",path:(if $path=="" then "/" else $path end)}+(if $host=="" then {} else {headers:{Host:$host}} end))') ;;
        grpc) outbound=$(printf '%s' "$outbound" | jq --arg svc "$service" '.transport={type:"grpc",service_name:$svc}') ;;
        httpupgrade) outbound=$(printf '%s' "$outbound" | jq --arg path "${path:-/}" --arg host "$vhost" \
    ...
    hysteria2|hy2)
      ...
      [ -n "$obfs" ] && outbound=$(printf '%s' "$outbound" | jq --arg pw "$obfs" '.obfs={type:"salamander",password:$pw}')
      hop_range=$(query_value mport); [ -z "$hop_range" ] && hop_range=$(query_value ports)
      if [ -n "$hop_range" ]; then
        local r=$(echo "$hop_range" | sed 's/-/:/g')
        outbound=$(printf '%s' "$outbound" | jq --arg r "$r" '.server_ports=[$r]|del(.server_port)|.hop_interval="30s"')
      fi ;;
    tuic)
    ...
    trojan)
      ...
      if [ "$(query_value type)" = "ws" ]; then
        outbound=$(printf '%s' "$outbound" | jq --arg path "$(query_value path)" --arg host "$(query_value host)" \
          '.transport=({type:"ws",path:(if $path=="" then "/" else $path end)}+(if $host=="" then {} else {headers:{Host:$host}} end))')
      fi ;;
    anytls)
      outbound=$(jq -n --arg tag "$tag" --arg server "$URI_HOST" --argjson port "$URI_PORT" \
  ...
  if [ "$insecure" = "1" ] || [ "$insecure" = "true" ]; then
    outbound=$(printf '%s' "$outbound" | jq 'if .tls then .tls.insecure=true else . end')
  fi
  printf '%s' "$outbound"
}
...
peer_select(){
  local tag previous
  clear_screen; tell "${CYAN}========== 节点选择 ==========${PLAIN}"
  select_peer || return
  ...
  apply_config && tell_ok "已接管: $(jq -r .name "$PICKED")" || tell_warn "切换应用失败 (已强制保存未回滚)"
  wait_key
}

peer_delete(){
  clear_screen; tell "${CYAN}========== 删除节点 ==========${PLAIN}"
  select_peer || return
  [ "$(jq -r .tag "$PICKED")" = "$(state_get exit)" ] && state_set exit direct
  rm -f "$PICKED"
  apply_config && tell_ok "已删除" || tell_warn "删除节点已强制执行，但应用异常"
  ip link del "$WG_IF" 2>/dev/null
  wait_key
}

peer_stop(){
  clear_screen; state_set exit direct
...
get_ip_info(){
  ...
  local raw_ip
  if [ "$mode" = 6 ]; then raw_ip=$(env http_proxy="$proxy_env" wget -qO- -T 5 http://ipv6.icanhazip.com 2>/dev/null | tr -d '\n ')
  else raw_ip=$(env http_proxy="$proxy_env" wget -qO- -T 5 http://ipv4.icanhazip.com 2>/dev/null | tr -d '\n ')
  fi
  
  if [ -n "$raw_ip" ]; then
    res=$(env http_proxy="$proxy_env" wget -qO- -T 5 "http://ipwho.is/${raw_ip}" 2>/dev/null)
...
menu_client_status(){
  ...
  local exit_node proxy_name="直连"
  exit_node=$(state_get exit)
  if [ "$exit_node" != "direct" ]; then
    if [ "$exit_node" = "wireguard" ]; then proxy_name="WireGuard"
    else proxy_name="$(jq -r .outbound.type "$PEER_DIR/$exit_node.json" 2>/dev/null || echo '未知')"
    fi
  fi
  tell "  当前出口: ${CYAN}${proxy_name}${PLAIN}"
  [ "$SYS_ENV" = "openrc" ] && tell "  提示: LXC 无 TUN 环境目前仅支持全局 HTTP/SOCKS5 转发拦截"
  tell ""
  render_client_ip_status
  wait_key
}
...
wg_tunnel_state(){
  ...
  if [ "$SYS_ENV" = "systemd" ] && command -v ping >/dev/null 2>&1 && ping -c1 -W3 "$peer_ip" >/dev/null 2>&1; then 
    echo up; return; 
  fi
  
  local proxy_env=""
  [ "$SYS_ENV" = "openrc" ] && proxy_env="http://127.0.0.1:2080"
  
  if env http_proxy="$proxy_env" wget -q -O /dev/null -T 4 "$PROBE_URL" 2>/dev/null; then
    echo up; return;
  fi
  echo down
}
...
render_wg_info(){
  ...
  tell "  引擎: $([ "$(jq -r .enabled "$file")" = "true" ] && printf "${GREEN}运行中${PLAIN}" || printf "${RED}已挂起${PLAIN}")"
  echo ""
...
wg_setup(){
  ...
  local listen_port peer_host peer_port peer_key body sys_bool
  ...
  if [ "$role" = server ]; then
    ...
    body=$(jq -n --arg private "$private" --arg public "$public" --arg a4 "$address4" --arg a6 "$address6" \
          ...
          --arg peer4 "$peer_ip4" --arg peer6 "$peer_ip6" --arg iface "$WG_IF" --argjson sbool "$sys_bool" '
     {role:"server",enabled:false,private_key:$private,public_key:$public,
      ...
      endpoint:{type:"wireguard",tag:"wireguard",system:$sbool,name:$iface,mtu:1408,
    ...
    body=$(jq -n --arg private "$private" --arg public "$public" --arg a4 "$address4" --arg a6 "$address6" \
          ...
          --arg peer4 "$peer_ip4" --arg iface "$WG_IF" --argjson sbool "$sys_bool" '
...
wg_fill_peer_key(){
  local key role
  clear_screen; tell "${CYAN}========== 回填对端公钥 ==========${PLAIN}"
  ...
  if [ "$(jq -r .enabled "$WG_CONF")" = "true" ]; then apply_config && tell_ok "已生效" || tell_warn "配置应用失败"
  else tell_ok "已补录"
  fi
  wait_key
}
...
wg_toggle(){
  ...
  if [ "$(jq -r .enabled "$WG_CONF")" = "true" ]; then
    ...
    apply_config && tell_ok "已关闭隧道" || tell_warn "应用失败"
  else
    ...
    if apply_config; then sleep 3; render_wg_info "$WG_CONF"
    else tell_warn "配置应用失败 (未触发回滚)"
    fi
  fi
  wait_key
}
    ...
    case $(prompt "请选择") in
      ...
      5) clear_screen
         if prompt_yes "确认删除配置"; then
           ...
           apply_config && tell_ok "已清除" || tell_warn "强制清理发生异常"
           ip link del "$WG_IF" 2>/dev/null
           wait_key
         fi ;;
      0) break ;;
...
run_update(){
  ...
  if [ -z "$latest" ]; then
    tell_warn "无法获取最新版本，请检查网络"
  elif [ -z "$current" ] || [ "$current" != "$latest" ]; then
    if prompt_yes "发现新版本/无法读取版本，是否重新下载并安装 v$latest"; then
      if install_core; then
         apply_config && tell_ok "内核更新完成" || tell_warn "内核更新完成但应用配置失败"
      else
         tell_warn "内核安装由于网络或系统原因中止，未执行后续操作"
      fi
    fi
  else
    tell_ok "内核已是最新版本"
  ...
  if [ "$SYS_ENV" = "systemd" ]; then
    curl -4 -fL --retry 3 --connect-timeout 15 -m 30 "$script_url" -o "$script_tmp" 2>/dev/null || curl -fL --retry 3 --connect-timeout 15 -m 30 "$script_url" -o "$script_tmp" 2>/dev/null
  else
    wget -q -O "$script_tmp" -T 15 "$script_url" 2>/dev/null
  fi
  
  if grep -q "CYAN=" "$script_tmp" 2>/dev/null; then
    if cmp -s "$script_tmp" "$SELF" >/dev/null 2>&1; then
      tell_ok "脚本已是最新版本"
    else
      install -m700 "$script_tmp" "$SELF"
      tell_ok "脚本更新成功，请重新运行本脚本生效"
      rm -f "$script_tmp"
      exit 0
    fi
  else
    tell_warn "下载的脚本存在语法错误或获取失败，放弃更新"
  fi
  rm -f "$script_tmp"
  wait_key
}
...
menu_status(){
  while :; do
    ...
    local up_days=$((up_seconds / 86400)); local up_hours=$(( (up_seconds % 86400) / 3600 ))
    ...
    if [ "$SYS_ENV" = "systemd" ]; then systemctl is-active --quiet sing-box && s_state="${GREEN}正常运行${PLAIN}"
    elif [ "$SYS_ENV" = "openrc" ]; then rc-service sing-box status >/dev/null 2>&1 && s_state="${GREEN}正常运行${PLAIN}"; fi
    
    tell "  系统版本: ${os:-Unknown}"
    tell "  内核架构: ${core} (${arch})"
    tell "  内存状态: ${mem}"
    tell "  运行时间: ${up}"
...
case $1 in
  --sync) 
    init_dirs
    if [ "$SYS_ENV" = "systemd" ]; then sync_bypass_rules; sync_hopping_rules; elif [ "$SYS_ENV" = "openrc" ]; then sync_proxy_env; fi
    exit 0 ;;
...
while :; do
  ...
  tell "${CYAN}================================${PLAIN}"
  tell "${CYAN}             s管理              ${PLAIN}"
  tell "${CYAN}   [ 当前运行模式: ${ENV_DISP} ]   ${PLAIN}"
  tell "${CYAN}================================${PLAIN}"
  tell "  1. 服务端管理"
  tell "  2. 客户端管理"
  tell "  3. WireGuard 管理"
