...
build_config(){
  ...
  dns_block=$(jq -n --arg direct "$dns_direct_server" --arg strategy "$dns_strategy" --arg detour "$final" '
    {servers:[
      {type:"https",tag:"dns-direct",server:$direct,server_port:443,path:"/dns-query",tls:{enabled:true,server_name:"cloudflare-dns.com"},detour:"direct"},
      {type:"https",tag:"dns-remote",server:"1.1.1.1",server_port:443,path:"/dns-query",tls:{enabled:true,server_name:"cloudflare-dns.com"},detour:$detour}
    ],
    final:"dns-remote",
    strategy:$strategy}')
...
render_wg_info(){
  ...
  tell "  远端 IPv4: $(jq -r .peer_ip "$file")"
  tell "  远端 IPv6: $(jq -r '.peer_ip6 // "<未配置>"' "$file")"
  if [ "$role" = client ]; then
    if [ "$(jq -r .enabled "$file")" = true ]; then
      if [ "$(wg_tunnel_state "$(jq -r .peer_ip "$file")" "$(jq -r '.peer_ip6 // ""' "$file")")" = up ]; then tell_ok "连接成功"; else tell_warn "连接中断"; fi
    fi
  else
    echo ""; tell "  ${YELLOW}请在客户端填入:${PLAIN}"
    tell "  服务端: $NET_IPV4:$(jq -r .listen_port "$file")"
...
wg_setup(){
  ...
  if [ "$role" = server ]; then
    ...
    body=$(jq -n --arg private "$private" --arg public "$public" --arg a4 "$address4" --arg a6 "$address6" \
          ...
          --arg peer4 "$peer_ip4" --arg peer6 "$ipv6_prefix::1" --arg iface "$WG_IF" '
...
wg_toggle(){
  ...
  if [ "$(jq -r .enabled "$WG_CONF")" = true ]; then
    ...
    if apply_config; then tell_ok "已关闭隧道"; else tell_warn "配置应用失败，请手动检查"; fi
  else
