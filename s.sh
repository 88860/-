  ...
  case $kind in
    shadowsocks)
      ...
      if [[ "$ss_method" == 2022-* ]]; then
        ...
        ss_userinfo="$(uri_encode "$ss_method")%3A$(uri_encode "$ss_password")"
      ...
      # SIP002 has no standard network=tcp/udp parameter; do not emit a non-standard query.
      uri="${uri}#$(uri_encode "$name")" ;;
    vmess-tls|vmess-ws|vmess-grpc|vmess-http)
      local vmess_json vmess_net
      case "$kind" in
      ...
      vmess_json=$(jq -n --arg v "2" --arg ps "$name" --arg add "$host" --argjson port "$port" \
        ...
        --arg net "$vmess_net" --arg path "$(jq -r '.path//""' <<<"$meta")" \
...
create_shadowsocks(){
  ...
  body=$(jq -n --arg tag "$tag" --arg name "$name" --arg method "$method" --arg password "$password" \
        --arg network "$network" --arg proto "$proto" --argjson port "$port" '
