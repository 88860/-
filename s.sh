...
sync_wg_rules(){
  nft delete table inet sbm_wg 2>/dev/null
  if [ -f "$WG_CONF" ] && [ "$(jq -r '.role//""' "$WG_CONF")" = server ] && [ "$(jq -r '.enabled//false' "$WG_CONF")" = true ]; then
    sysctl -w net.ipv4.ip_forward=1 >/dev/null 2>&1
    sysctl -w net.ipv6.conf.all.forwarding=1 >/dev/null 2>&1
    nft add table inet sbm_wg 2>/dev/null
    nft add chain inet sbm_wg postrouting '{ type nat hook postrouting priority srcnat; policy accept; }' 2>/dev/null
    nft add rule inet sbm_wg postrouting iifname "$WG_IF" masquerade 2>/dev/null
  fi
}
...
build_config(){
  ...
  if [ -f "$WG_CONF" ] && [ "$(jq -r '.enabled//false' "$WG_CONF")" = true ]; then
    if [ "$(jq -r .role "$WG_CONF")" = server ]; then
      endpoints=$(jq '[.endpoint]' "$WG_CONF")
    elif [ "$(jq -r .role "$WG_CONF")" = client ]; then
      peer_host=$(jq -r '.peer_host//""' "$WG_CONF")
      [ "$selected" = wireguard ] && { 
        final=wireguard; use_tun=1
        outbounds=$(jq -n --argjson base "$outbounds" --slurpfile wg "$WG_CONF" '$base + [$wg[0].outbound]')
      }
    fi
  fi
...
arm_watchdog(){
  ...
  systemd-run --collect --unit=sbm-watchdog --on-active=30 /bin/bash "$SELF" --watchdog >/dev/null 2>&1 \
    || out_warn "看门狗部署失败"
}
...
run_watchdog(){
  ...
  sync_bypass_rules; sync_hopping_rules; sync_wg_rules
  exit 0
}
...
menu_modify_protocol(){
  ...
  case $(prompt "请选择") in
    ...
    4)
      [ "$kind" = socks ] || return
      value=$(prompt "鉴权账号" "$(jq -r .meta.username "$PICKED")"); [ -n "$value" ] || return
    ...
    6)
      ...
      if [ -z "$value" ]; then
        tell_ok "保持不变"; wait_key; return;
      fi
      if [ "$value" = "0" ]; then
        value=""
      else
        validate_range "$value" || { wait_key; return; }
        other=$(hopping_node) && [ "$other" != "$PICKED" ] && { tell_warn "已有节点开启跳跃"; wait_key; return; }
      fi
...
get_ip_info(){
  local mode=$1 res ip
  res=$(curl -sL -${mode} -m 5 -A "Mozilla/5.0" https://api.ip.sb/geoip 2>/dev/null)
  ip=$(jq -r '.ip // empty' <<<"$res" 2>/dev/null)
  
  if [ -n "$ip" ] && [ "$ip" != "null" ]; then
    IP_INFO_IP="$ip"
    IP_INFO_C=$(jq -r '.country_code // empty' <<<"$res" 2>/dev/null | tr 'A-Z' 'a-z')
    IP_INFO_ASN=$(jq -r '.asn // empty' <<<"$res" 2>/dev/null)
    [ -n "$IP_INFO_ASN" ] && [ "$IP_INFO_ASN" != "null" ] && IP_INFO_ASN="AS${IP_INFO_ASN#AS}"
    IP_INFO_NAME=$(jq -r '.organization // .isp // empty' <<<"$res" 2>/dev/null)
  else
    IP_INFO_IP=""
    IP_INFO_C=""
    IP_INFO_ASN=""
  ...
  if [ "$role" = server ]; then
    ...
    body=$(jq -n --arg private "$private" --arg public "$public" --arg a4 "$address4" --arg a6 "$address6" \
     ...
     '{role:"client",enabled:false,private_key:$private,public_key:$public,
      ...
      outbound:{type:"wireguard",tag:"wireguard",server:$host,server_port:$port,
        local_address:[$a4,$a6],private_key:$private,peer_public_key:$peer_key,mtu:1408}}')
  fi
...
wg_fill_peer_key(){
  ...
  [ -n "$key" ] || return
  json_edit "$WG_CONF" '.peer_public_key=$k | if .role == "server" then .endpoint.peers[0].public_key=$k else .outbound.peer_public_key=$k end' --arg k "$key" \
...
case $1 in
  --sync) init_dirs; sync_bypass_rules; sync_hopping_rules; sync_wg_rules; exit 0 ;;
  --clear-net) clear_hopping_rules; clear_wg_rules; ip link del "$TUN_IF" 2>/dev/null; exit 0 ;;
