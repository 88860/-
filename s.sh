...
prompt(){
  ...
  local value; value=$(read_line "$def")
  echo "${value:-$def}"
}
...
menu_create_protocol(){
  while :; do
    ...
    tell "  1. VLESS"; tell "  2. Trojan"; tell "  3. Hysteria2"; tell "  4. TUIC"
    tell "  5. Shadowsocks"; tell "  6. VMess"; tell "  7. AnyTLS"; tell "  8. SOCKS5"; tell "  9. Snell"
...
menu_create_vless(){
  while :; do
    clear; tell "${CYAN}╭─ VLESS 搭配 ─╮${PLAIN}"
    tell "  1. Reality"; tell "  2. TLS"; tell "  3. WebSocket"; tell "  4. gRPC"; tell "  0. 返回"
    case $(prompt "请选择") in
...
menu_create_trojan(){
  while :; do
    clear; tell "${CYAN}╭─ Trojan 搭配 ─╮${PLAIN}"
    tell "  1. TLS"; tell "  2. WebSocket"; tell "  3. gRPC"; tell "  0. 返回"
    case $(prompt "请选择") in
...
menu_create_shadowsocks(){
  while :; do
    clear; tell "${CYAN}╭─ Shadowsocks 加密 ─╮${PLAIN}"
    tell "  1. 2022-blake3-aes-128-gcm"; tell "  2. 2022-blake3-aes-256-gcm"
    tell "  3. 2022-blake3-chacha20-poly1305"; tell "  4. aes-128-gcm"; tell "  5. aes-256-gcm"
    tell "  6. chacha20-ietf-poly1305"; tell "  7. xchacha20-ietf-poly1305"; tell "  0. 返回"
    tell "${CYAN}╰──────────────────────────────────────╯${PLAIN}"
    case $(prompt "请选择") in
...
menu_create_vmess(){
  while :; do
    clear; tell "${CYAN}╭─ VMess 搭配 ─╮${PLAIN}"
    tell "  1. TLS"; tell "  2. WebSocket + TLS"; tell "  3. gRPC + TLS"; tell "  4. HTTP + TLS"; tell "  0. 返回"
    case $(prompt "请选择") in
...
menu_delete_protocol(){
  ...
  if server_delete_node_transaction "$file"; then
    ...
    if [ "$was_acme" = "acme" ]; then
      ...
      if [ "$acme_count" -eq 0 ] && [ -n "$(state_acme_get domain)" ]; then
        printf "\n"
...
render_certificate_status(){
  ...
  domain=$(state_acme_get domain) || return 1; [ -n "$domain" ] || return 0
  tell ""; tell "  全局域名: $domain | 验证: $(state_acme_get challenge)"
  crt=""
...
menu_server_info(){
  ...
  for file in "$NODE_DIR"/*.json; do
    [ -e "$file" ] || continue
    count=$((count+1)); port=$(server_node_port "$file") || return 1; proto=$(server_node_proto "$file") || return 1
    if [ "$proto" = u ]; then
...
menu_change_domain(){
  ...
  tell "  当前域名: $(state_acme_get domain || true)"; tell "  绑定节点:"
  for file in "$NODE_DIR"/*.json; do
    [ -e "$file" ] || continue
  ...
  if prompt_yes "同步重置验证机制"; then
    ...
    while :; do
      case $(prompt "请选择方式" 1) in
        1) mode=http; break ;;
        2) mode=alpn; break ;;
        3) mode=dns_cloudflare; state_acme_cf_token_set "$(prompt 'Cloudflare API Token')"; break ;;
        4) mode=dns_alidns; state_acme_ali_key_set "$(prompt 'AccessKeyId')"; state_acme_ali_secret_set "$(prompt 'AccessKeySecret')"; break ;;
        5) mode=dns_acmedns
           state_acme_acmedns_url_set "$(prompt 'server_url')"; state_acme_acmedns_user_set "$(prompt 'username')"
           state_acme_acmedns_pass_set "$(prompt 'password')"; state_acme_acmedns_sub_set "$(prompt 'subdomain')"; break ;;
        *) tell_warn "输入无效，请重新选择"; sleep 1 ;;
      esac
    done
    state_acme_set challenge "$mode" || return 1
    state_acme_set email "$email" || return 1
  fi
  ...
  while :; do
    ...
    case $(prompt "请选择") in
      ...
      6)
         ...
         wait_key ;;
      7)
         previous=$(state_exit_get) || { tell_warn "出口状态读取失败，未停止服务"; wait_key; continue; }
         state_exit_set direct || { tell_warn "出口状态更新失败，未停止服务"; wait_key; continue; }
         if ! stop_watchdog; then
           if ! state_exit_set "$previous" || ! sync_watchdog; then tell_warn "看门狗停止失败，且状态回滚未完成，请检查"; else tell_warn "看门狗停止失败，未停止服务"; fi
           wait_key; continue
         fi
         if timeout 15 systemctl stop sing-box 2>/dev/null; then
           local stop_tmp
           stop_tmp=$(mktemp) || {
             if ! state_exit_set "$previous" || ! sync_watchdog; then tell_warn "临时配置文件创建失败，且状态回滚未完成，请检查"; else tell_warn "临时配置文件创建失败，已回滚"; fi
             wait_key; continue
           }
           if build_config >"$stop_tmp" 2>/dev/null && [ -s "$stop_tmp" ] && json_write "$CONFIG" <"$stop_tmp"; then
             ...
             rm -f "$stop_tmp"
             if ! state_exit_set "$previous" || ! sync_watchdog; then
           ...
           if ! state_exit_set "$previous" || ! sync_watchdog; then tell_warn "操作异常，且状态回滚未完成，请检查"; else tell_warn "操作异常，已回滚"; fi
         fi
         wait_key ;;
      0) break ;;
      *) tell_warn "输入无效，请重新选择"; sleep 1 ;;
    ...
    if ! wg_config_exists; then
      ...
      if [ "$role" = client ] && [ "$enabled" = true ]; then
        if [ ! -d "/sys/class/net/$WG_IF" ]; then
          ...
          IFS='|' read -r peer4 peer6 <<<"$(wg_config_peer_ips)"
        ...
        if [ "$enabled" = true ]; then
          if [ -d "/sys/class/net/$WG_IF" ]; then tell "  模式：服务端|${GREEN}正常运行${PLAIN}"; else tell "  模式：服务端|${RED}异常${PLAIN}"; fi
        else
...
menu_status(){
  while :; do
    ...
    local sb_asset=$(state_core_asset_get)
...
stop_net_monitor(){
  if [ -f "$NET_MONITOR_PID" ]; then
    ...
    if [ -n "$pid" ] && kill -0 "$pid" 2>/dev/null && grep -zqx 'SBM_NETMON=1' "/proc/$pid/environ" 2>/dev/null; then
...
probe_network_stack(){
  ...
  IPV6_OK=0
  if [ -n "$ip6" ] && [ -n "$NET_IF_V6" ]; then
    if curl -6 -s -m 3 --interface "$NET_IF_V6" https://ipv6.icanhazip.com >/dev/null 2>&1; then
      IPV6_OK=1
    fi
  fi
  return "$rc"
...
start_net_monitor(){
  ...
  (
    ...
    while :; do
      ...
      done < <(ip monitor link route address 2>/dev/null)
      sleep 5
    done
  ) </dev/null >/dev/null 2>&1 &
  local pid=$! tmp_pid
  tmp_pid=$(mktemp "${NET_MONITOR_PID}.XXXXXX") || { kill "$pid" 2>/dev/null; return 1; }
  printf '%s\n' "$pid" >"$tmp_pid" && mv -f "$tmp_pid" "$NET_MONITOR_PID" || { rm -f "$tmp_pid"; kill "$pid" 2>/dev/null; return 1; }
}
render_share_uri(){
  ...
  if [ "$mode" = acme ]; then
    host=$(state_acme_get domain) || { tell_warn "证书域名状态读取失败，生成链接失败"; return 1; }
...
network_if_v4(){ printf '%s' "$NET_IF_V4"; }
network_if_v6(){ printf '%s' "$NET_IF_V6"; }
network_ipv6_ok(){ printf '%s' "$IPV6_OK"; }
network_refresh_interfaces(){
  NET_IF_V4=$(default_iface 4)
  NET_IF_V6=$(default_iface 6)
}
network_interfaces_changed(){
  local prev_if4=$1 prev_if6=$2
  [ "$NET_IF_V4" != "$prev_if4" ] || [ "$NET_IF_V6" != "$prev_if6" ]
}
network_cache_write(){
  local tmp
...
validate_port(){
  ...
  [ "$port" = 443 ] && [ "$(state_acme_get challenge)" = "alpn" ] && { tell_warn "端口 443 已保留给 ALPN 证书签发"; return 1; }
  [ "$port" = "$allow" ] && return 0
...
validate_domain(){
  ...
  case $(state_acme_get challenge) in
...
setup_certificate(){
  ...
  [ -n "$(state_acme_get domain)" ] && [ -n "$(state_acme_get email)" ] && certificate_ready "$(state_acme_get domain)" && return 0
  ...
  state_acme_set challenge "$mode" &&
  state_acme_set domain "$domain" &&
  state_acme_set email "$email" || return 1
  if ! validate_domain "$domain"; then
    prompt_yes "验证存在异常，是否强制继续" || { state_acme_set domain ""; state_acme_set email ""; return 1; }
  fi
  return 0
}
...
acme_options(){
  ...
  email=$(state_acme_get email) || return 1
  challenge=$(state_acme_get challenge) || return 1
  ...
  case "$challenge" in
    ...
    dns_cloudflare)
      token=$(state_acme_cf_token_get) || return 1
      body=$(jq --arg t "$token" '.dns01_challenge={provider:"cloudflare",api_token:$t}' <<<"$body") || return 1 ;;
    dns_alidns)
      key=$(state_acme_ali_key_get) || return 1; secret=$(state_acme_ali_secret_get) || return 1
      body=$(jq --arg k "$key" --arg s "$secret" '.dns01_challenge={provider:"alidns",access_key_id:$k,access_key_secret:$s}' <<<"$body") || return 1 ;;
    dns_acmedns)
      user=$(state_acme_acmedns_user_get) || return 1; pass=$(state_acme_acmedns_pass_get) || return 1
      sub=$(state_acme_acmedns_sub_get) || return 1; url=$(state_acme_acmedns_url_get) || return 1
...
config_build_tun_inbounds(){
  local inbounds=$1 name=$TUN_IF
  jq -n --argjson list "$inbounds" --arg name "$name" '
...
config_build_rules(){
  local peer_host=$1 ssh_rule_ports
  ssh_rule_ports=$(ssh_ports | jq -Rsc 'split("\n") | map(select(length > 0) | tonumber)')
  jq -n --arg host "$peer_host" --argjson ssh_ports "$ssh_rule_ports" '
...
config_build_dns(){
  local dns_direct_tag=$1 dns_strategy=$2 final=$3
  jq -n --arg direct_tag "$dns_direct_tag" --arg strategy "$dns_strategy" --arg detour "$final" '
...
build_config(){
  ...
  local node_block exit_block dns_direct_tag dns_strategy auto_detect providers

  selected=$(state_exit_get) || return 1
  domain=$(state_acme_get domain) || return 1
  dns_direct_tag="dns-direct-v4"
  dns_strategy="prefer_ipv4"
  if [ -n "$(network_if_v6)" ] && [ "$(network_ipv6_ok)" = 1 ]; then
  ...
  rules=$(config_build_rules "$peer_host") || return 1
  if [ "$use_tun" = 1 ]; then auto_detect="true"; else auto_detect="false"; fi
  dns_block=$(config_build_dns "$dns_direct_tag" "$dns_strategy" "$final") || return 1
...
build_uri(){
  ...
  case $kind in
    shadowsocks)
      local ss_method ss_password ss_userinfo ss_b64 ss_network
      ss_method=$(jq -r .method <<<"$meta"); ss_password=$(jq -r .password <<<"$meta"); ss_network=$(jq -r '.network//""' <<<"$meta")
      ss_userinfo=$(printf '%s:%s' "$ss_method" "$ss_password" | base64 -w0 2>/dev/null || printf '%s:%s' "$ss_method" "$ss_password" | base64 | tr -d '\n')
      ss_b64=$(printf '%s' "$ss_userinfo" | tr '+/' '-_' | tr -d '=')
      uri="ss://${ss_b64}@${host}:${port}/"
      [ -n "$ss_network" ] && uri="${uri}?network=${ss_network}"
      uri="${uri}#$(uri_encode "$name")" ;;
    vmess-tls|vmess-ws|vmess-grpc|vmess-http)
      local vmess_json
      vmess_json=$(jq -n --arg v "2" --arg ps "$name" --arg add "$host" --argjson port "$port" \
        --arg id "$(jq -r .uuid <<<"$meta")" --arg aid "0" --arg scy "$(jq -r '.security//"auto"' <<<"$meta")" \
        --arg net "$(jq -r '.transport//"tcp"' <<<"$meta")" --arg path "$(jq -r '.path//""' <<<"$meta")" \
        --arg hosth "$host" --arg service "$(jq -r '.service_name//""' <<<"$meta")" --arg tls "tls" --arg type "none" \
        '{v:$v,ps:$ps,add:$add,port:$port,id:$id,aid:$aid,scy:$scy,net:$net,path:$path,host:$hosth,serviceName:$service,tls:$tls,type:$type}')
      uri="vmess://$(printf '%s' "$vmess_json" | base64 -w0 2>/dev/null || printf '%s' "$vmess_json" | base64 | tr -d '\n')#$(uri_encode "$name")" ;;
    vless-reality)
    ...
    vless-tls)
      uri="vless://$(jq -r .uuid <<<"$meta")@$host:$port?encryption=none&flow=xtls-rprx-vision&security=tls&sni=$(uri_encode "$host")&fp=chrome&type=tcp&allowInsecure=0#$(uri_encode "$name")" ;;
    vless-ws)
      local vless_path
      vless_path=$(jq -r '.path//"/ws"' <<<"$meta")
      uri="vless://$(jq -r .uuid <<<"$meta")@$host:$port?encryption=none&security=tls&sni=$(uri_encode "$host")&fp=chrome&type=ws&path=$(uri_encode "$vless_path")&host=$(uri_encode "$host")&allowInsecure=0#$(uri_encode "$name")" ;;
    vless-grpc)
      local vless_service
      vless_service=$(jq -r '.service_name//"TunService"' <<<"$meta")
      uri="vless://$(jq -r .uuid <<<"$meta")@$host:$port?encryption=none&security=tls&sni=$(uri_encode "$host")&fp=chrome&type=grpc&serviceName=$(uri_encode "$vless_service")&allowInsecure=0#$(uri_encode "$name")" ;;
    hysteria2)
      ...
      up_mbps=$(jq -r '.up_mbps//0' <<<"$meta"); down_mbps=$(jq -r '.down_mbps//0' <<<"$meta")
      [ "$up_mbps" -gt 0 ] && bandwidth_str="${bandwidth_str}&up_mbps=${up_mbps}"
      [ "$down_mbps" -gt 0 ] && bandwidth_str="${bandwidth_str}&down_mbps=${down_mbps}"
      if [ -n "$hopping" ]; then
        uri="hysteria2://$(uri_encode "$(jq -r .password <<<"$meta")")@$host:$hopping?sni=$(uri_encode "$host")${bbr_str}${bandwidth_str}${obfs_str}${pkt_str}#$(uri_encode "$name")"
      else
        uri="hysteria2://$(uri_encode "$(jq -r .password <<<"$meta")")@$host:$port?sni=$(uri_encode "$host")${bbr_str}${bandwidth_str}${obfs_str}${pkt_str}#$(uri_encode "$name")"
      fi ;;
    tuic)
      local tuic_cc
      tuic_cc=$(jq -r '.congestion_control//"bbr"' <<<"$meta")
      uri="tuic://$(jq -r .uuid <<<"$meta"):$(uri_encode "$(jq -r .password <<<"$meta")")@$host:$port?congestion_control=$tuic_cc&alpn=h3&udp_relay_mode=native&sni=$(uri_encode "$host")&allow_insecure=0#$(uri_encode "$name")" ;;
    trojan)
      uri="trojan://$(uri_encode "$(jq -r .password <<<"$meta")")@$host:$port?security=tls&sni=$(uri_encode "$host")&type=tcp&allowInsecure=0#$(uri_encode "$name")" ;;
    trojan-ws)
      local trojan_path
      trojan_path=$(jq -r '.path//"/ws"' <<<"$meta")
      uri="trojan://$(uri_encode "$(jq -r .password <<<"$meta")")@$host:$port?security=tls&sni=$(uri_encode "$host")&type=ws&path=$(uri_encode "$trojan_path")&host=$(uri_encode "$host")&allowInsecure=0#$(uri_encode "$name")" ;;
    trojan-grpc)
      local trojan_service
      trojan_service=$(jq -r '.service_name//"TunService"' <<<"$meta")
      uri="trojan://$(uri_encode "$(jq -r .password <<<"$meta")")@$host:$port?security=tls&sni=$(uri_encode "$host")&type=grpc&serviceName=$(uri_encode "$trojan_service")&allowInsecure=0#$(uri_encode "$name")" ;;
    anytls)
      local a_pw a_meta
...
parse_uri(){
  ...
  if [ "$scheme" = hysteria2 ] || [ "$scheme" = hy2 ]; then
    ...
    port=${port_spec:-443}
    if ! [[ "$port" =~ ^[0-9]+$ ]] || [ "$port" -lt 1 ] || [ "$port" -gt 65535 ]; then port=443; fi
  fi
...
uri_to_outbound(){
  ...
  case $(jq -r .scheme <<<"$context") in
    ss|shadowsocks)
      local ss_method ss_password ss_network plugin plugin_opts ss_userinfo ss_decoded ss_pad
      ss_userinfo=$(jq -r .userinfo <<<"$context")
      ss_decoded=$(printf '%s' "$ss_userinfo" | tr '_-' '/+'; printf '\n')
      ...
      if [[ "$ss_decoded" == *:* ]]; then
        ...
        ss_method=${ss_userinfo%%:*}; ss_password=${ss_userinfo#*:}
      fi
      [ -n "$ss_method" ] && [ -n "$ss_password" ] || return 1
      ss_network=$(query_value "$context" network)
      ...
      outbound=$(jq -n --arg tag "$tag" --arg server "$(jq -r .host <<<"$context")" --argjson port "$(jq -r .port <<<"$context")" \
        --arg method "$ss_method" --arg password "$ss_password" --arg network "$ss_network" --arg plugin "$plugin" --arg plugin_opts "$plugin_opts" \
    ...
    vmess)
      local vmess_security vmess_net vmess_path vmess_host vmess_service vmess_raw vmess_json vmess_server vmess_port vmess_uuid vmess_pad
      ...
      outbound=$(jq -n --arg tag "$tag" --arg server "$vmess_server" --argjson port "$vmess_port" \
        ...
        '{type:"vmess",tag:$tag,server:$server,server_port:$port,uuid:$uuid,security:$security,alter_id:0,network:"tcp"}')
      case "$vmess_net" in
    ...
    hysteria2|hy2)
      ...
      bbr_profile=$(query_value "$context" bbr_profile); [ -n "$bbr_profile" ] || bbr_profile=standard
    ...
    anytls)
      client_meta=$(query_value "$context" client_metadata)
      outbound=$(jq -n --arg tag "$tag" --arg server "$(jq -r .host <<<"$context")" --argjson port "$(jq -r .port <<<"$context")" \
                       --arg password "$(jq -r .userinfo <<<"$context")" --arg sni "$sni" --arg meta "$client_meta" \
...
wg_server_nat_apply(){
  local prev4 prev6 legacy4 legacy6 nat_state rollback_rc=1
  ...
  if [ -f "$nat_state" ]; then
    ...
    wg_nat_state_write "$prev4" "$prev6" >/dev/null 2>&1 || return 1
    if [ -f "$WG_CONF" ]; then
      json_edit "$WG_CONF" 'del(.nat_prev_ipv4_forward,.nat_prev_ipv6_forward)' >/dev/null 2>&1 || return 1
    fi
  fi
  if [ -f /etc/sysctl.d/99-sbm-forward.conf ] && [ ! -f /etc/sysctl.d/99-sbm-forward.conf.sbm-backup ]; then
    cp -a /etc/sysctl.d/99-sbm-forward.conf /etc/sysctl.d/99-sbm-forward.conf.sbm-backup 2>/dev/null || return 1
  fi
  sysctl -w net.ipv4.ip_forward=1 >/dev/null 2>&1 || { wg_server_nat_remove || rollback_rc=2; return "$rollback_rc"; }
  sysctl -w net.ipv6.conf.all.forwarding=1 >/dev/null 2>&1 || { wg_server_nat_remove || rollback_rc=2; return "$rollback_rc"; }
  cat >/etc/sysctl.d/99-sbm-forward.conf <<EOF
net.ipv4.ip_forward=1
net.ipv6.conf.all.forwarding=1
EOF
  [ "$?" -eq 0 ] || { wg_server_nat_remove || rollback_rc=2; return "$rollback_rc"; }
  local out4 out6
  out4=$(default_iface 4); out6=$(default_iface 6)
  nft delete table ip sbm_nat 2>/dev/null || true
  nft add table ip sbm_nat 2>/dev/null || { wg_server_nat_remove || rollback_rc=2; return "$rollback_rc"; }
  nft add chain ip sbm_nat postrouting '{ type nat hook postrouting priority srcnat; policy accept; }' 2>/dev/null || { wg_server_nat_remove || rollback_rc=2; return "$rollback_rc"; }
  if [ -n "$out4" ]; then
    nft add rule ip sbm_nat postrouting oifname "$out4" masquerade 2>/dev/null || { wg_server_nat_remove || rollback_rc=2; return "$rollback_rc"; }
  fi
  nft delete table ip6 sbm_nat 2>/dev/null || true
  nft add table ip6 sbm_nat 2>/dev/null || { wg_server_nat_remove || rollback_rc=2; return "$rollback_rc"; }
  nft add chain ip6 sbm_nat postrouting '{ type nat hook postrouting priority srcnat; policy accept; }' 2>/dev/null || { wg_server_nat_remove || rollback_rc=2; return "$rollback_rc"; }
  if [ -n "$out6" ]; then
    nft add rule ip6 sbm_nat postrouting oifname "$out6" masquerade 2>/dev/null || { wg_server_nat_remove || rollback_rc=2; return "$rollback_rc"; }
  fi
}


wg_server_nat_remove(){
  local prev4 prev6 rc=0 nat_state
  nat_state=$(wg_nat_state_file)
  if [ -f "$nat_state" ]; then
    prev4=$(wg_nat_state_read ipv4_forward 2>/dev/null)
    prev6=$(wg_nat_state_read ipv6_forward 2>/dev/null)
...
wg_config_peer_ips(){
  [ -f "$WG_CONF" ] || { printf "|\n"; return 0; }
  jq -r "\(.peer_ip // "")|\(.peer_ip6 // "")" "$WG_CONF" 2>/dev/null
}
...
wg_reinit_prepare(){
  ...
  previous=$(state_exit_get) || return 1
  ...
  if [ "$previous" = wireguard ]; then
    state_exit_set direct || return 1
    stop_watchdog || { state_exit_set "$previous" || true; sync_watchdog; return 1; }
  fi
  return 0
}
...
wg_reinit_rollback(){
  ...
  state_exit_set "$previous" || return 1
...
wg_status(){
  ...
  clear
  if [ ! -f "$file" ]; then
    tell "${YELLOW}WireGuard 尚未配置，请先初始化。${PLAIN}"
    wait_key
    return
  fi
  ...
  case "$role" in
    client)
      ...
      tell "${CYAN}╭─ WireGuard 状态信息 ─╮${PLAIN}"
      if [ "$enabled" = true ] && [ -d "/sys/class/net/$WG_IF" ]; then
        tell "运行模式：客户端 状态：${GREEN}正常运行${PLAIN}"
    ...
    server)
      ...
      tell "${CYAN}╭─ WireGuard 状态信息 ─╮${PLAIN}"
      if [ "$enabled" = true ] && [ -d "/sys/class/net/$WG_IF" ]; then
        tell "运行模式：服务端 状态：${GREEN}正常运行${PLAIN}"
      elif [ "$enabled" = true ]; then
...
wg_init_server(){
  ...
  clear
  tell "${CYAN}╭─ WireGuard 服务端引导配置 ─╮${PLAIN}"
  local keypair
  ...
  previous_exit=$(state_exit_get) || { rm -rf "$backup_wg"; tell_warn "出口状态读取失败"; wait_key; return; }
...
wg_config_apply_transaction(){
  local body=$1 mode=${2:-} force_apply=${3:-false} old_body
  old_body=$(cat "$WG_CONF") || return 1
  wg_save "$body" || return 1
  if wg_config_enabled || [ "$force_apply" = true ]; then
    if [ -n "$mode" ]; then
      apply_config "$mode" || { printf '%s\n' "$old_body" | json_write "$WG_CONF"; apply_config_quiet; return 1; }
    else
      apply_config || { printf '%s\n' "$old_body" | json_write "$WG_CONF"; apply_config_quiet; return 1; }
    fi
  fi
  return 0
}
...
wg_delete(){
  ...
  previous_exit=$(state_exit_get) || { tell_warn "出口状态读取失败，取消删除"; wait_key; return; }
  ...
  if [ "$previous_exit" = wireguard ]; then state_exit_set direct || { wg_delete_rollback "$backup_wg" "$backup_conf" "$previous_exit" || true; rm -rf "$backup_wg" "$backup_conf"; wait_key; return; }; fi
...
wg_toggle_transaction(){
  ...
  if [ "$enabled" = true ]; then
    if [ "$role" = server ] && ! wg_server_nat_apply; then
      state_exit_set "$previous"
      sync_watchdog
      return 1
    fi
    if [ "$role" = client ]; then
      state_exit_set wireguard || return 1
      stop_watchdog || { state_exit_set "$previous" || true; sync_watchdog; return 1; }
    fi
    if wg_config_apply_transaction "$body"; then
      if [ "$role" = client ] && ! sync_watchdog; then
        state_exit_set "$previous" || true
    ...
    state_exit_set "$previous" || return 2
  ...
  if [ "$previous" = wireguard ]; then
    state_exit_set direct || { sync_watchdog; return 1; }
    stop_watchdog || { state_exit_set "$previous" || true; sync_watchdog; return 1; }
  fi
...
wg_toggle(){
  ...
  clear
  [ -f "$WG_CONF" ] || { tell_warn "请先初始化配置"; wait_key; return; }
  role=$(jq -r '.role' "$WG_CONF") || { tell_warn "WireGuard 配置读取失败"; wait_key; return; }; previous=$(state_exit_get) || { tell_warn "出口状态读取失败"; wait_key; return; }
...
protocol_set_name(){
  ...
  json_edit "$file" '.name=$v' --arg v "$value"
}

protocol_set_port(){
  local file=$1 value=$2
  json_edit "$file" '.port=$v|.inbound.listen_port=$v' --argjson v "$value"
}

protocol_set_uuid(){
  local file=$1 value=$2
  json_edit "$file" '.meta.uuid=$v|.inbound.users[0].uuid=$v' --arg v "$value"
}

protocol_set_password(){
  local file=$1 value=$2
  json_edit "$file" '.meta.password=$v|.inbound.users[0].password=$v' --arg v "$value"
}

protocol_set_username(){
  local file=$1 value=$2
  json_edit "$file" '.meta.username=$v|.inbound.users[0].username=$v' --arg v "$value"
}

protocol_set_snell_psk(){
  local file=$1 value=$2
  json_edit "$file" '.meta.psk=$v|.inbound.psk=$v' --arg v "$value"
}

protocol_set_snell_mode(){
  local file=$1 value=$2
  json_edit "$file" '.meta.mode=$v|.inbound.mode=$v' --arg v "$value"
}
...
protocol_set_hopping(){
  local file=$1 value=$2
  json_edit "$file" '.hopping=$v' --arg v "$value"
}

protocol_set_reality_target(){
  local file=$1 value=$2
  json_edit "$file" '.meta.target=$v|.inbound.tls.server_name=$v|.inbound.tls.reality.handshake.server=$v' --arg v "$value"
}

protocol_set_reality_short_id(){
  local file=$1 value=$2
  json_edit "$file" '.meta.short_id=$v|.inbound.tls.reality.short_id=[$v]' --arg v "$value"
}
...
protocol_clear_hysteria2_obfs(){
  local file=$1
  json_edit "$file" '.meta.obfs_type="" | .meta.obfs_password="" | del(.meta.min_packet_size,.meta.max_packet_size) | del(.inbound.obfs)'
}
...
protocol_edit_name(){
  ...
  [ -n "$value" ] || return 1
  protocol_set_name "$file" "$value" || { tell_warn 失败; wait_key; return 1; }
}
...
protocol_edit_port(){
  ...
  protocol_set_port "$file" "$value" || { tell_warn 失败; wait_key; return 1; }
}
...
protocol_edit_option3(){
  ...
  [ -n "$file" ] && [ -f "$file" ] || return 1
  case $kind in
    vless-reality|vless-tls|tuic)
      value=$(prompt "新通信 UUID (留空自动生成)")
      [ -z "$value" ] && value=$(random_uuid)
      protocol_set_uuid "$file" "$value" || { tell_warn 失败; wait_key; return 1; }
      ;;
    snell)
      while :; do
      ...
      protocol_set_snell_psk "$file" "$value" || { tell_warn 失败; wait_key; return 1; }
      ;;
    *)
      value=$(prompt "新连接密码 (留空自动生成)")
      [ -z "$value" ] && value=$(random_password)
      protocol_set_password "$file" "$value" || { tell_warn 失败; wait_key; return 1; }
      ;;
  esac
}

protocol_edit_option4(){
  local file=$1 kind=$2 value current_hop other
  [ -n "$file" ] && [ -f "$file" ] || return 1
  case $kind in
    socks)
      value=$(prompt "鉴权账号" "$(jq -r .meta.username "$file")"); [ -n "$value" ] || return 1
      protocol_set_username "$file" "$value" || { tell_warn 失败; wait_key; return 1; }
      ;;
    tuic)
    ...
    snell)
      ...
      while :; do
        case $(prompt "流量整形模式 (当前: $(jq -r '.meta.mode//\"default\"' "$file"))" 1) in
          1) value=default; break ;; 2) value=unshaped; break ;; 3) value=unsafe-raw; break ;;
          *) tell_warn "输入无效"; sleep 1 ;;
        esac
      done
      protocol_set_snell_mode "$file" "$value" || { tell_warn 失败; wait_key; return 1; }
      ;;
    anytls)
      value=$(prompt "客户端元数据 (留空则为空)" "$(jq -r '.meta.client_metadata//\"\"' "$file")")
      protocol_set_anytls_metadata "$file" "$value" || { tell_warn 失败; wait_key; return 1; }
      ;;
    hysteria2)
      current_hop=$(jq -r '.hopping//\"\"' "$file")
      value=$(prompt "跳跃范围 (当前: ${current_hop:-未开启}, 0 关闭)")
      [ -n "$value" ] || { tell_ok "保持不变"; wait_key; return 1; }
      if [ "$value" = 0 ]; then value=""; else
        validate_range "$value" || return 1
        other=$(hopping_node) && [ "$other" != "$file" ] && { tell_warn "已有节点开启跳跃"; wait_key; return 1; }
      fi
      protocol_set_hopping "$file" "$value" || { tell_warn 失败; wait_key; return 1; }
      ;;
    *)
      tell_warn "输入无效"; sleep 1; return 1 ;;
  esac
}

protocol_edit_option5(){
  local file=$1 kind=$2 value bbr_profile up_mbps down_mbps
  [ -n "$file" ] && [ -f "$file" ] || return 1
  if [ "$kind" = vless-reality ]; then
    ...
    protocol_set_reality_target "$file" "$value" || { tell_warn 失败; wait_key; return 1; }
    ...
    while :; do
      case $(prompt "请选择拥塞控制" 2) in
        ...
        4)
          bbr_profile=""
          while :; do
            up_mbps=$(prompt "上行带宽 (Mbps, 0为不限制)" "$(jq -r '.meta.up_mbps//0' "$file")")
            [[ $up_mbps =~ ^[0-9]+$ ]] || { tell_warn "输入无效"; continue; }
            down_mbps=$(prompt "下行带宽 (Mbps, 0为不限制)" "$(jq -r '.meta.down_mbps//0' "$file")")
    ...
    if [ -n "$bbr_profile" ]; then
      protocol_set_hysteria2_bbr "$file" "$bbr_profile" 0 0
    else
      protocol_set_hysteria2_bbr "$file" "" "$up_mbps" "$down_mbps"
    fi || { tell_warn 失败; wait_key; return 1; }
  else
    tell_warn "输入无效"; sleep 1; return 1
  fi
}
...
protocol_edit_option6(){
  local file=$1 kind=$2 value obfs_type obfs_password min_pkt max_pkt
  [ -n "$file" ] && [ -f "$file" ] || return 1
  if [ "$kind" = vless-reality ]; then
    ...
    if ! [[ $value =~ ^[0-9a-fA-F]{1,8}$ ]]; then tell_warn "short_id 必须为 1-8 位十六进制字符"; wait_key; return 1; fi
    protocol_set_reality_short_id "$file" "$value" || { tell_warn 失败; wait_key; return 1; }
  elif [ "$kind" = hysteria2 ]; then
    if prompt_yes "是否配置并开启协议混淆 (选择 N 则关闭混淆)"; then
      tell "  1. Salamander"; tell "  2. Gecko"
      while :; do
        case $(prompt "请选择混淆算法 (当前: $(jq -r '.meta.obfs_type//\"\"' "$file"))" 2) in
          1) obfs_type=salamander; break ;; 2) obfs_type=gecko; break ;;
          *) tell_warn "输入无效"; sleep 1 ;;
        esac
      done
      obfs_password=$(prompt "混淆密码 (留空与连接密码相同)" "$(jq -r '.meta.obfs_password//\"\"' "$file")")
      [ -z "$obfs_password" ] && obfs_password=$(jq -r '.meta.password' "$file")
      if [ "$obfs_type" = gecko ]; then
        while :; do
      ...
      protocol_clear_hysteria2_obfs "$file" || { tell_warn 失败; wait_key; return 1; }
    fi
  else
    tell_warn "输入无效"; sleep 1; return 1
  fi
...
menu_modify_protocol(){
  ...
  while :; do
    ...
    clear; tell "${CYAN}【$name】${PLAIN} ${DIM}[$kind]${PLAIN}"
    ...
    case $choice in
      ...
      4) protocol_edit_option4 "$file" "$kind" || continue ;;
      5) protocol_edit_option5 "$file" "$kind" || continue ;;
      6) protocol_edit_option6 "$file" "$kind" || continue ;;
...
install_core(){
  ...
  rm -rf "$tmp" || return 1
  core_cache_reset
  state_core_asset_set "$asset" || return 1
...
state_acme_cf_token_get(){ state_get cf_token; }
state_acme_cf_token_set(){ state_set cf_token "$1"; }
state_acme_ali_key_get(){ state_get ali_key; }
state_acme_ali_key_set(){ state_set ali_key "$1"; }
state_acme_ali_secret_get(){ state_get ali_secret; }
state_acme_ali_secret_set(){ state_set ali_secret "$1"; }
state_acme_acmedns_url_get(){ state_get acmedns_url; }
state_acme_acmedns_url_set(){ state_set acmedns_url "$1"; }
state_acme_acmedns_user_get(){ state_get acmedns_user; }
state_acme_acmedns_user_set(){ state_set acmedns_user "$1"; }
state_acme_acmedns_pass_get(){ state_get acmedns_pass; }
state_acme_acmedns_pass_set(){ state_set acmedns_pass "$1"; }
state_acme_acmedns_sub_get(){ state_get acmedns_sub; }
state_acme_acmedns_sub_set(){ state_set acmedns_sub "$1"; }

state_acme_clear_identity(){
  state_acme_set domain "" &&
  state_acme_set email "" &&
  state_acme_set challenge "http"
}

state_core_asset_get(){
  state_get asset
}

state_core_asset_set(){
  state_set asset "$1"
}
warp_require_wg(){
  command -v wg >/dev/null 2>&1 || { tell_warn "系统未安装 wg，无法使用 WARP"; return 1; }
  command -v ip >/dev/null 2>&1 || { tell_warn "系统未安装 ip，无法使用 WARP"; return 1; }
}
...
warp_api_request(){
  local method=$1 url=$2 token=${3:-} body=${4:-} out=$5 code
  [ -n "$out" ] || return 2
  if [ "$method" = POST ]; then
    code=$(curl -sS --location --tlsv1.3 --connect-timeout 8 --max-time 20 -o "$out" -w '%{http_code}' -X POST "$url" \
    ...
    code=$(curl "${args[@]}" 2>/dev/null) || return 1
  fi
  [[ "$code" =~ ^2[0-9][0-9]$ ]]
}
...
warp_refresh(){
  ...
  tmp=$(mktemp) || return 1
  if ! warp_api_request GET "$WARP_API/$id" "$token" '' "$tmp" || ! jq -e '.id and .token and .config.peers[0].public_key and .config.peers[0].endpoint.host and (.config.interface.addresses.v4 or .config.interface.addresses.v6)' "$tmp" >/dev/null 2>&1; then
    rm -f "$tmp"
    return 1
  fi
  current=$(cat "$WARP_CONF") || { rm -f "$tmp"; return 1; }
  mode=$(jq -r '.warp_mode // "dual"' <<<"$current")
  ...
  jq --arg pk "$private" --arg pub "$public" --arg iid "$iid" --arg mode "$mode" --arg registered "$registered_at" \
    '. + {private_key:$pk,public_key:$pub,install_id:$iid,warp_mode:$mode,registered_at:$registered} ' "$tmp" >"$tmp.new" 2>/dev/null || { rm -f "$tmp" "$tmp.new"; return 1; }
...
warp_mode_get(){
  local mode
  mode=$(warp_config_get '.warp_mode // "dual"')
  case "$mode" in ipv4|ipv6|dual) printf '%s' "$mode" ;; *) printf '%s' dual ;; esac
}

warp_mode_set(){
  case "$1" in ipv4|ipv6|dual) json_edit "$WARP_CONF" '.warp_mode=$mode' --arg mode "$1" ;; *) return 1 ;; esac
}

warp_mode_label(){
  case "$(warp_mode_get)" in ipv4) printf 'IPv4' ;; ipv6) printf 'IPv6' ;; *) printf '双栈' ;; esac
}

warp_status(){
  if ! warp_registered; then echo unconfigured; return; fi
  if ip link show "$WARP_IF" >/dev/null 2>&1 && wg show "$WARP_IF" >/dev/null 2>&1; then echo running; else echo stopped; fi
}
...
warp_register(){
  clear
  tell "${CYAN}✦ 注册 WARP${PLAIN}"
  if warp_registered; then
    tell "  当前状态：${GREEN}已注册${PLAIN}"
    tell ""
    tell "  WARP IPv4：$(warp_config_get '.config.interface.addresses.v4')"
    tell "  WARP IPv6：$(warp_config_get '.config.interface.addresses.v6')"
    tell ""
    prompt_yes "是否重新注册 WARP" || return
  else
    tell "  当前状态：${DIM}未注册${PLAIN}"
    tell ""
    prompt_yes "是否注册 WARP" || return
  fi
  warp_require_wg || { wait_key; return; }
  if [ "$(warp_status)" = running ]; then
    ip link del "$WARP_IF" 2>/dev/null || true
    warp_cleanup_routes
    [ "$(state_exit_get 2>/dev/null)" = warp ] && state_exit_set direct
  fi
  mkdir -p "$WARP_DIR" || { tell_warn "WARP 目录创建失败"; wait_key; return; }
  local private public install_id fcm tos body response
  private=$(wg genkey 2>/dev/null) || { tell_warn "WARP 密钥生成失败"; wait_key; return; }
  public=$(printf '%s' "$private" | wg pubkey 2>/dev/null) || { tell_warn "WARP 公钥生成失败"; wait_key; return; }
  install_id=$(tr -dc 'A-Za-z0-9' </dev/urandom | head -c 22)
  fcm="${install_id}:APA91b$(tr -dc 'A-Za-z0-9' </dev/urandom | head -c 134)"
  tos=$(date -u '+%Y-%m-%dT%H:%M:%S.000Z')
  body=$(jq -n --arg key "$public" --arg iid "$install_id" --arg fcm "$fcm" --arg tos "$tos" --arg model "PC" --arg serial "$install_id" '{key:$key,install_id:$iid,fcm_token:$fcm,tos:$tos,model:$model,serial_number:$serial,locale:"zh_CN"}') || { tell_warn "注册请求生成失败"; wait_key; return; }
  tell "  正在注册..."
  local response_file
  response_file=$(mktemp) || { tell_warn "临时响应文件创建失败"; wait_key; return; }
  if ! warp_api_request POST "$WARP_API" '' "$body" "$response_file" || ! jq -e '.account and .config.peers[0].public_key and (.config.interface.addresses.v4 or .config.interface.addresses.v6) and .token and .id' "$response_file" >/dev/null 2>&1; then
    rm -f "$response_file"
    tell_warn "WARP 注册失败"
    wait_key
    return
  fi
  jq --arg pk "$private" --arg pub "$public" --arg iid "$install_id" '{id:.id,token:.token,private_key:$pk,public_key:$pub,install_id:$iid,account:.account,config:.config,warp_mode:"dual",registered_at:(now|todateiso8601)}' "$response_file" | json_write "$WARP_CONF" || { rm -f "$response_file"; tell_warn "WARP 配置保存失败"; wait_key; return; }
  rm -f "$response_file"
  chmod 700 "$WARP_DIR"; chmod 600 "$WARP_CONF"
  tell_ok "注册成功"
  clear
  tell "${CYAN}✦ WARP 出口${PLAIN}"
  tell ""
  tell "  注册成功，请选择："
  tell ""
  tell "  1. 获取 IPv4"
  ...
  tell "  0. 返回"
  case $(prompt "请选择") in
    1) warp_mode_set ipv4 && tell "" && tell "  IPv4：$(warp_config_get '.config.interface.addresses.v4')" || tell_warn "模式保存失败"; wait_key ;;
    2) warp_mode_set ipv6 && tell "" && tell "  IPv6：$(warp_config_get '.config.interface.addresses.v6')" || tell_warn "模式保存失败"; wait_key ;;
    3) warp_mode_set dual && tell "" && tell "  IPv4：$(warp_config_get '.config.interface.addresses.v4')" && tell "  IPv6：$(warp_config_get '.config.interface.addresses.v6')" || tell_warn "模式保存失败"; wait_key ;;
  esac
}

warp_trace(){
  local family=$1 res ip loc colo warp
  case "$family" in 4) res=$(curl -4 -fsS --connect-timeout 5 --max-time 8 "$WARP_TRACE" 2>/dev/null) || return 1;;
    6) res=$(curl -6 -fsS --connect-timeout 5 --max-time 8 "$WARP_TRACE" 2>/dev/null) || return 1;;
    *) return 2;;
  esac
  ip=$(awk -F= '$1=="ip"{print $2;exit}' <<<"$res")
  loc=$(awk -F= '$1=="loc"{print $2;exit}' <<<"$res")
  colo=$(awk -F= '$1=="colo"{print $2;exit}' <<<"$res")
  warp=$(awk -F= '$1=="warp"{print $2;exit}' <<<"$res")
  [ -n "$ip" ] || return 1
  printf '%s\t%s\t%s\t%s\n' "$ip" "$loc" "$colo" "$warp"
}
...
warp_handshake_ok(){
  ...
  [ -n "$peer" ] || return 1
  last=$(wg show "$WARP_IF" latest-handshakes 2>/dev/null | awk -v p="$peer" '$1==p{print $2;exit}')
  [[ "$last" =~ ^[0-9]+$ ]] || return 1
  now=$(date +%s)
  [ "$last" -gt 0 ] && [ $((now-last)) -le 180 ]
}

warp_fetch_exit(){
  clear
  tell "${CYAN}✦ WARP 出口${PLAIN}"
  if ! warp_registered; then tell_warn "请先注册 WARP"; wait_key; return; fi
  [ "$(warp_status)" = running ] || { tell_warn "请先启动 WARP"; wait_key; return; }
  local mode info4 info6 ip4 ip6 loc4 loc6 colo4 colo6 warp4 warp6
  mode=$(warp_mode_get)
  warp_handshake_ok || tell_alert "WireGuard 最近没有检测到有效握手"
  case "$mode" in
    ipv4)
      info4=$(warp_trace 4); ip4=$(cut -f1 <<<"$info4"); loc4=$(cut -f2 <<<"$info4"); colo4=$(cut -f3 <<<"$info4"); warp4=$(cut -f4 <<<"$info4") ;;
    ipv6)
      info6=$(warp_trace 6); ip6=$(cut -f1 <<<"$info6"); loc6=$(cut -f2 <<<"$info6"); colo6=$(cut -f3 <<<"$info6"); warp6=$(cut -f4 <<<"$info6") ;;
    dual)
      info4=$(warp_trace 4); info6=$(warp_trace 6)
  ...
  if [ -z "$ip4" ] && [ -z "$ip6" ]; then tell_warn "Cloudflare 出口检测失败"; wait_key; return 1; fi
  tell "  接管：$(warp_mode_label)"
  tell ""
  [ -n "$ip4" ] && tell "  IPv4：$ip4 | ${loc4:-??} | ${colo4:-未知} | WARP=${warp4:-off}"
  [ -n "$ip6" ] && tell "  IPv6：$ip6 | ${loc6:-??} | ${colo6:-未知} | WARP=${warp6:-off}"
  if { [ "$mode" = ipv4 ] && [ "$warp4" != on ]; } || { [ "$mode" = ipv6 ] && [ "$warp6" != on ]; } || { [ "$mode" = dual ] && { [ "$warp4" != on ] || [ "$warp6" != on ]; }; }; then
    tell_warn "检测到所选地址族未确认经过 WARP"
    wait_key
    return 1
  fi
  wait_key
...
warp_cleanup_routes(){
  ip -4 rule del pref "$WARP_TABLE" 2>/dev/null || true
  ip -6 rule del pref "$WARP_TABLE" 2>/dev/null || true
  ip -4 route flush table "$WARP_TABLE" 2>/dev/null || true
  ip -6 route flush table "$WARP_TABLE" 2>/dev/null || true
}

warp_stop(){
  clear
  tell "${CYAN}✦ WARP${PLAIN}"
  tell "  正在停止..."
  local previous
  previous=$(state_exit_get) || previous=direct
  if ip link show "$WARP_IF" >/dev/null 2>&1; then
    ip link del "$WARP_IF" >/dev/null 2>&1 || { tell_warn "停止失败"; wait_key; return 1; }
  fi
  warp_cleanup_routes
  warp_registered && json_edit "$WARP_CONF" 'del(.started_at)' || true
  if [ "$previous" = warp ]; then
    state_exit_set direct || { tell_warn "停止失败"; wait_key; return 1; }
    stop_watchdog || true
    apply_config_quiet || true
  fi
  tell_ok "已停止"
...
warp_start(){
  ...
  tell "${CYAN}✦ WARP${PLAIN}"
  tell "  正在启动..."
  warp_require_wg || { wait_key; return 1; }
  warp_registered || { tell_warn "请先注册 WARP"; wait_key; return 1; }
  if [ "$(warp_status)" = running ]; then tell_ok "已在运行"; wait_key; return; fi
  if ! warp_refresh; then tell_warn "WARP 注册信息刷新失败，将使用本地缓存配置"; fi
  local private peer addr4 addr6 endpoint keyfile previous mode allowed_ips
  ...
  endpoint=$(warp_config_get '.config.peers[0].endpoint.host')
  mode=$(warp_mode_get)
  case "$mode" in
    ipv4) [ -n "$addr4" ] || { tell_warn "WARP IPv4 配置不存在"; wait_key; return 1; }; allowed_ips='0.0.0.0/0' ;;
    ipv6) [ -n "$addr6" ] || { tell_warn "WARP IPv6 配置不存在"; wait_key; return 1; }; allowed_ips='::/0' ;;
    dual) [ -n "$addr4" ] && [ -n "$addr6" ] || { tell_warn "WARP 双栈地址配置不完整"; wait_key; return 1; }; allowed_ips='0.0.0.0/0,::/0' ;;
  esac
  [ -n "$private" ] && [ -n "$peer" ] && [ -n "$endpoint" ] || { tell_warn "WARP 配置不完整"; wait_key; return 1; }
  previous=$(state_exit_get) || { tell_warn "出口状态读取失败"; wait_key; return 1; }
  if [ "$previous" != direct ] && [ "$previous" != warp ]; then
    ...
    if [ "$previous" = wireguard ] && wg_client_active; then
      ...
      state_exit_set direct || { tell_warn "当前接管状态切换失败"; wait_key; return 1; }
      apply_config_quiet || { state_exit_set "$previous"; tell_warn "当前接管状态切换失败"; wait_key; return 1; }
    fi
    previous=direct
  fi
  ip link del "$WARP_IF" 2>/dev/null || true
  warp_cleanup_routes
  keyfile=$(mktemp) || { tell_warn "临时密钥文件创建失败"; wait_key; return 1; }
  printf '%s\n' "$private" >"$keyfile"; chmod 600 "$keyfile"
  if ! ip link add dev "$WARP_IF" type wireguard || \
     ! wg set "$WARP_IF" private-key "$keyfile" fwmark "$WARP_TABLE" peer "$peer" allowed-ips "$allowed_ips" endpoint "$endpoint" persistent-keepalive 25 || \
     { [ -z "$addr4" ] || [ "$mode" = ipv6 ] || ip -4 address add "$addr4/32" dev "$WARP_IF"; } || \
     { [ -z "$addr6" ] || [ "$mode" = ipv4 ] || ip -6 address add "$addr6/128" dev "$WARP_IF"; } || \
     ! ip link set mtu 1280 up dev "$WARP_IF" || \
     { [ "$mode" = ipv6 ] || ip -4 route add default dev "$WARP_IF" table "$WARP_TABLE"; } || \
     { [ "$mode" = ipv6 ] || ip -4 rule add pref "$WARP_TABLE" not fwmark "$WARP_TABLE" table "$WARP_TABLE"; } || \
     { [ "$mode" = ipv4 ] || ip -6 route add default dev "$WARP_IF" table "$WARP_TABLE"; } || \
     { [ "$mode" = ipv4 ] || ip -6 rule add pref "$WARP_TABLE" not fwmark "$WARP_TABLE" table "$WARP_TABLE"; }; then
    rm -f "$keyfile"; ip link del "$WARP_IF" 2>/dev/null || true; warp_cleanup_routes
    tell_warn "启动失败"
    wait_key
    return 1
  fi
  rm -f "$keyfile"
  state_exit_set warp || { ip link del "$WARP_IF" 2>/dev/null || true; warp_cleanup_routes; tell_warn "出口状态更新失败"; wait_key; return 1; }
  json_edit "$WARP_CONF" '.started_at=$started' --arg started "$(date -u '+%Y-%m-%dT%H:%M:%SZ')" || true
  stop_watchdog || true
  apply_config_quiet || { state_exit_set "$previous"; ip link del "$WARP_IF" 2>/dev/null || true; warp_cleanup_routes; tell_warn "sing-box 配置更新失败"; wait_key; return 1; }
  tell_ok "启动成功（$(warp_mode_label)接管）"
  wait_key
}

warp_status_view(){
  clear
  tell "${CYAN}✦ WARP 状态${PLAIN}"
  case "$(warp_status)" in
    unconfigured) tell "  状态：${DIM}未配置${PLAIN}"; wait_key; return ;;
    stopped) tell "  状态：${YELLOW}已停止${PLAIN}"; wait_key; return ;;
  esac
  local mode info4 info6 ip4 ip6 loc4 loc6 colo4 colo6 warp4 warp6 created started now up_secs up_h up_m
  mode=$(warp_mode_get)
  warp_handshake_ok && tell "  WireGuard：${GREEN}握手正常${PLAIN}" || tell "  WireGuard：${YELLOW}暂无最近握手${PLAIN}"
  case "$mode" in
    ipv4) info4=$(warp_trace 4); ip4=$(cut -f1 <<<"$info4"); loc4=$(cut -f2 <<<"$info4"); colo4=$(cut -f3 <<<"$info4"); warp4=$(cut -f4 <<<"$info4") ;;
    ipv6) info6=$(warp_trace 6); ip6=$(cut -f1 <<<"$info6"); loc6=$(cut -f2 <<<"$info6"); colo6=$(cut -f3 <<<"$info6"); warp6=$(cut -f4 <<<"$info6") ;;
    dual) info4=$(warp_trace 4); info6=$(warp_trace 6); ip4=$(cut -f1 <<<"$info4"); loc4=$(cut -f2 <<<"$info4"); colo4=$(cut -f3 <<<"$info4"); warp4=$(cut -f4 <<<"$info4"); ip6=$(cut -f1 <<<"$info6"); loc6=$(cut -f2 <<<"$info6"); colo6=$(cut -f3 <<<"$info6"); warp6=$(cut -f4 <<<"$info6") ;;
  esac
  tell "  状态：${GREEN}运行中${PLAIN}"
  tell "  接管：$(warp_mode_label)"
  tell ""
  [ -n "$ip4" ] && tell "  IPv4：$ip4 | ${loc4:-??} | ${colo4:-未知} | WARP=${warp4:-off}"
  [ -n "$ip6" ] && tell "  IPv6：$ip6 | ${loc6:-??} | ${colo6:-未知} | WARP=${warp6:-off}"
  tell "  WARP 公钥：$(warp_config_get '.public_key')"
  tell "  账户状态：${GREEN}已注册${PLAIN}"
  started=$(warp_config_get '.started_at')
  if [ -n "$started" ]; then
    now=$(date +%s); created=$(date -d "$started" +%s 2>/dev/null || echo 0); up_secs=$((now-created)); [ "$up_secs" -lt 0 ] && up_secs=0
    up_h=$((up_secs/3600)); up_m=$(((up_secs%3600)/60)); tell "  运行时间：${up_h}h ${up_m}m"
  fi
  wait_key
}
...
warp_delete(){
  clear
  tell "${CYAN}✦ 删除 WARP 配置${PLAIN}"
  tell "  当前状态：$(case "$(warp_status)" in running) echo -e "${GREEN}运行中${PLAIN}";; stopped) echo -e "${YELLOW}已停止${PLAIN}";; *) echo -e "${DIM}未配置${PLAIN}";; esac)"
  tell ""
  tell "  ${YELLOW}⚠ 将删除 WARP 配置及注册信息${PLAIN}"
  tell ""
  prompt_yes "确认删除" || return
  if [ "$(warp_status)" = running ]; then
    ip link del "$WARP_IF" 2>/dev/null || true
    warp_cleanup_routes
  fi
  local id token response_file
  id=$(warp_config_get '.id'); token=$(warp_config_get '.token')
  if [ -n "$id" ] && [ -n "$token" ]; then
    response_file=$(mktemp) || { tell_warn "临时响应文件创建失败，未删除本地配置"; wait_key; return 1; }
    if ! warp_api_request DELETE "$WARP_API/$id" "$token" '' "$response_file"; then
      rm -f "$response_file"
      tell_warn "Cloudflare 注册删除失败，未删除本地配置"
      wait_key
      return 1
    fi
    rm -f "$response_file"
  ...
  rm -f "$WARP_CONF"
  if [ "$(state_exit_get 2>/dev/null)" = warp ]; then state_exit_set direct; stop_watchdog || true; apply_config_quiet || true; fi
  tell_ok "配置已删除"
  wait_key
}
...
menu_warp(){
  while :; do
    clear
    local st=$(warp_status)
    local label
    case "$st" in running) label="${GREEN}运行中${PLAIN}";; stopped) label="${YELLOW}已停止${PLAIN}";; *) label="${DIM}未配置${PLAIN}";; esac
    tell "${CYAN}✦ WARP 管理${PLAIN}"
    tell ""
    tell "  状态：$label"
    tell ""
    tell "  1. 注册 WARP"
    ...
    case $(prompt "请选择") in
      1) warp_register ;;
      2) warp_fetch_exit ;;
      3) [ "$st" = running ] && warp_stop || warp_start ;;
      4) warp_status_view ;;
      5) warp_delete ;;
      0) break ;;
      *) tell_warn "输入无效，请重新选择"; sleep 1 ;;
...
watchdog_needed(){ local exit_node; exit_node=$(state_exit_get) || return 2; [ "$exit_node" != "direct" ] && [ "$exit_node" != "warp" ]; }
...
run_watchdog(){
  ...
  sleep 3
  target_exit=$(state_exit_get) || exit 1
  while :; do
    current=$(state_exit_get) || exit 1
    ...
    if [ "$current" = "direct" ]; then
      ...
      if [ -n "$target_server" ] && probe_target_latency "$target_server" "$result" && [ "$(cat "$result" 2>/dev/null)" = ok ]; then
        fail_count=0
        if ! state_exit_set "$target_exit" || ! apply_config_quiet; then state_exit_set direct || exit 1; fi
      else
      ...
      if curl -s -o /dev/null --connect-timeout 4 -m 6 "$PROBE_URL" || curl -s -o /dev/null --connect-timeout 4 -m 6 "$probe2"; then fail_count=0; else
        ...
        if [ "$fail_count" -ge 3 ]; then
          fail_count=0
          if ! state_exit_set direct || ! apply_config_quiet; then state_exit_set "$target_exit" || exit 1; apply_config_quiet || exit 1; fi
        fi
      fi
    fi
    sleep 120
...
server_node_tag(){ jq -r '.tag // ""' "$1" 2>/dev/null; }
...
server_node_port(){ jq -r '.port // ""' "$1" 2>/dev/null; }
server_node_proto(){ jq -r '.proto // ""' "$1" 2>/dev/null; }
...
select_node(){
  local index target=$1 count=0 raw_data old_ifs node_file kind port name proto
  local -a files=()
  [ -n "$target" ] || return 2
  set -- "$NODE_DIR"/*.json
  ...
  if ! raw_data=$(jq -r '"\(input_filename)|\(.kind//"-")|\(.port//"-")|\(.name//"-")|\(.proto//"-")"' "$@" 2>/dev/null); then
    tell_warn "节点数据读取失败"; return 1
  fi
  if [ -n "$raw_data" ]; then
    old_ifs="$IFS"; IFS="|"
    while read -r node_file kind port name proto; do
      ...
      printf "  %b%2d%b  %b%-12s%b %s:%s %b%s%b\n" "$GREEN" "$count" "$PLAIN" "$CYAN" "$kind" "$PLAIN" "$name" "$port" "$color" "$status_text" "$PLAIN"
    done <<<"$raw_data"
    IFS="$old_ifs"
  fi
...
server_change_domain_transaction(){
  ...
  state_acme_set domain "$new_domain" || return 2
...
server_delete_node_rollback(){
  ...
  if [ "$was_exit" = 1 ]; then
    state_exit_set "$previous_exit" || rc=1
  fi
...
server_delete_node_transaction(){
  ...
  previous_exit=$(state_exit_get) || return 2
  [ "$(server_node_tag "$file")" = "$previous_exit" ] && was_exit=1

  if [ "$was_exit" = 1 ]; then
    state_exit_set direct || return 2
    stop_watchdog || { state_exit_set "$previous_exit" || true; sync_watchdog; return 2; }
  fi

  rm -f "$file" || {
    if [ "$was_exit" = 1 ]; then state_exit_set "$previous_exit" || true; sync_watchdog; fi
    return 2
  }
...
create_shadowsocks(){
  local method=${1:-2022-blake3-aes-256-gcm} name port password network tag body
  clear; tell "${CYAN}╭─ 创建 Shadowsocks ─╮${PLAIN}"
  ...
  tell "  1. TCP + UDP"; tell "  2. TCP"; tell "  3. UDP"
  while :; do
    case $(prompt "网络类型" 1) in
      1) network=""; break ;; 2) network=tcp; break ;; 3) network=udp; break ;;
      *) tell_warn "输入无效"; sleep 1 ;;
    esac
  done
  tag=$(unique_tag "$name" in- "$NODE_DIR")
  body=$(jq -n --arg tag "$tag" --arg name "$name" --arg method "$method" --arg password "$password" \
        --arg network "$network" --argjson port "$port" '
    {tag:$tag,name:$name,kind:"shadowsocks",port:$port,proto:"t",hopping:"",tls_mode:"none",alpn:null,
     meta:{method:$method,password:$password,network:$network},
...
create_vless_transport(){
  ...
  clear; tell "${CYAN}╭─ 创建 VLESS ─╮${PLAIN}"
...
create_vmess(){
  ...
  clear; tell "${CYAN}╭─ 创建 VMess ─╮${PLAIN}"
  name=$(prompt "节点名称" "VMess")
  setup_certificate || return
  port=$(prompt_port t "")
  uuid=$(prompt "通信 UUID (留空自动生成)" "$(random_uuid)")
  tell "  1. auto"; tell "  2. aes-128-gcm"; tell "  3. chacha20-poly1305"
  while :; do
...
create_tuic(){
  ...
  uuid=$(prompt "通信 UUID (留空自动生成)" "$(random_uuid)")
  password=$(prompt "连接密码 (留空自动生成)" "$(random_password)")
  tell "  1. cubic"; tell "  2. new_reno"; tell "  3. bbr"
  while :; do
...
client_peer_protocol(){ jq -r '.outbound.type // ""' "$1" 2>/dev/null; }
client_peer_server(){ jq -r '.outbound.server // ""' "$1" 2>/dev/null; }

peer_delete_rollback(){
  local file=$1 old_json=$2 previous_exit=$3 was_exit=$4 rc=0
  json_save "$file" "$old_json" || rc=1
  if [ "$was_exit" = 1 ]; then state_exit_set "$previous_exit" || rc=1; fi
...
peer_delete_transaction(){
  ...
  if [ "$was_exit" = 1 ]; then
    state_exit_set direct || return 2
    stop_watchdog || { state_exit_set "$previous_exit" || true; sync_watchdog || true; return 2; }
  fi
  rm -f "$file" || {
    if [ "$was_exit" = 1 ]; then state_exit_set "$previous_exit" || true; sync_watchdog; fi
    return 2
...
peer_select_transaction(){
  ...
  tag=$(client_peer_tag "$file"); target_name=$(client_peer_name "$file"); previous=$(state_exit_get) || return 1
  if [ "$previous" = warp ]; then
...
是否切换" || return 1
  ...
  state_exit_set "$previous" || tell_warn "出口状态恢复失败，请手动检查"
  if [ "$wg_was_active" = 1 ]; then wg_set_enabled true || tell_warn "WireGuard 状态恢复失败，请手动检查"; fi
  apply_config_quiet; sync_watchdog; return 1
}

peer_stop_transaction(){
  local previous; previous=$(state_exit_get) || return 1
  if [ "$previous" = warp ]; then
    ip link del "$WARP_IF" 2>/dev/null || true
    warp_cleanup_routes
  fi
...
list_peers(){
  local current; current=$(state_exit_get) || return 1
  local title=${1:-节点选择}
  ...
  if ! raw_data=$(jq -r '"\(input_filename)|\(.tag//"-")|\(.outbound.type//"-")|\(.name//"-")|\(.outbound.server_port // (if .outbound.server_ports then (.outbound.server_ports[0]|gsub(":";"-")) else null end) // .outbound.listen_port // "-")"' "${peer_files[@]}" 2>/dev/null); then
...
select_peer(){
  ...
  while :; do
    set -- "$PEER_DIR"/*.json
    [ ! -e "$1" ] && { clear; tell "${CYAN}╭─ $title ─╮${PLAIN}"; tell "  暂无外部节点"; wait_key; return 1; }
    peer_files=("$@")
...
exit_label(){
  local selected; selected=$(state_exit_get) || { echo "状态异常"; return 1; }
  case $selected in
    direct) echo "直连" ;;
...
render_client_ip_status(){
  ...
  if [ -n "$info" ]; then
    ...
    tell "  Cloudflare: ${CYAN}${name:-未知}${PLAIN} | WARP: ${PURPLE}${asn:-off}${PLAIN}"
...
menu_client_status(){
  ...
  local exit_node; exit_node=$(state_exit_get) || { tell_warn "出口状态读取失败"; return 1; }
  ...
  if [ "$exit_node" = "wireguard" ]; then
    ...
    protocol=$(client_peer_protocol "$PEER_DIR/$exit_node.json")
    case "$protocol" in
...
if [ "${BASH_SOURCE[0]}" = "$0" ]; then
  case $1 in
    ...
    --reapply)
      ...
      prev_if4=$(network_if_v4); prev_if6=$(network_if_v6)
      network_refresh_interfaces || { flock -u 9; exit 1; }
      if network_interfaces_changed "$prev_if4" "$prev_if6"; then
  ...
  while :; do
    ...
    tell "${CYAN}│          SYSTEMD · SING-BOX          │${PLAIN}"
    tell "${CYAN}╰──────────────────────────────────────╯${PLAIN}"
    tell "  1. 服务端管理"
    tell "  2. 客户端管理"
    tell "  3. WireGuard 管理"
