...
menu_change_domain(){
  ...
  TMP_FILES="$TMP_FILES $tmp"
  if ! jq -n --argjson p "$(acme_options "$new_domain")" '{log:{level:"warn",timestamp:true},certificate_providers:[($p+{type:"acme",tag:"acme-staging"})]}' >"$tmp" 2>/dev/null; then
...
probe_peer_latency(){
  local file=$1 result=$2 latency_file=${3:-} cfg port pid resolver node_tag
  ...
  if kill -0 "$pid" 2>/dev/null; then
    local start_ms now_ms remain_ms remain_sec rc
    start_ms=$(date +%s%3N)
    latency=$(curl -x "http://127.0.0.1:$port" -s -o /dev/null -w "%{time_total}" --connect-timeout 3 -m 3 "$PROBE_URL")
    rc=$?
    now_ms=$(date +%s%3N)
    remain_ms=$((3000-(now_ms-start_ms)))
    if [ "$rc" -eq 0 ]; then
      echo ok >"$result"
      [ -n "$latency_file" ] && awk -v t="$latency" 'BEGIN{printf "%.0f\n",t*1000}' >"$latency_file"
    elif [ "$remain_ms" -gt 0 ]; then
      remain_sec=$(awk -v ms="$remain_ms" 'BEGIN{printf "%.3f",ms/1000}')
      latency=$(curl -x "http://127.0.0.1:$port" -s -o /dev/null -w "%{time_total}" --connect-timeout "$remain_sec" -m "$remain_sec" "http://captive.apple.com/hotspot-detect.html")
      rc=$?
      if [ "$rc" -eq 0 ]; then
        echo ok >"$result"
        [ -n "$latency_file" ] && awk -v t="$latency" 'BEGIN{printf "%.0f\n",t*1000}' >"$latency_file"
