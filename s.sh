...
cleanup_old_domain_assets(){
  local domain=$1 dir
  [ -n "$domain" ] || return 0
  dir="$ACME_DIR/certificates/$domain"
  if [ -d "$dir" ]; then
    rm -rf -- "$dir"
  fi
}
...
menu_change_domain(){
  local new_domain old_state file count=0 email mode tmp pid log_file
...
run_uninstall(){
  ...
  clear_hopping_rules
  wg_server_nat_remove
  while read -r _ pref _ rule; do
    case "$rule" in
      sport) port=$(ip -4 rule show pref "$BYPASS_PREF" 2>/dev/null | sed -n 's/.*sport \([0-9]\+\).*/\1/p' | head -1); [ -n "$port" ] && ip -4 rule del pref "$BYPASS_PREF" sport "$port" lookup main 2>/dev/null || break ;;
      fwmark) ip -4 rule del pref "$BYPASS_PREF" fwmark 255 lookup main 2>/dev/null || break ;;
      *) break ;;
    esac
  done < <(ip -4 rule show pref "$BYPASS_PREF" 2>/dev/null)
  while read -r _ pref _ rule; do
    case "$rule" in
      sport) port=$(ip -6 rule show pref "$BYPASS_PREF" 2>/dev/null | sed -n 's/.*sport \([0-9]\+\).*/\1/p' | head -1); [ -n "$port" ] && ip -6 rule del pref "$BYPASS_PREF" sport "$port" lookup main 2>/dev/null || break ;;
      fwmark) ip -6 rule del pref "$BYPASS_PREF" fwmark 255 lookup main 2>/dev/null || break ;;
      *) break ;;
    esac
  done < <(ip -6 rule show pref "$BYPASS_PREF" 2>/dev/null)
