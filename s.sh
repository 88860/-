...
sync_bypass_rules(){
  local port new_ports owned4 owned6 old4 old6 changed=0
  new_ports=$(protected_ports)
  owned4=$(jq -r '.v4_ports[]? // empty' "$BYPASS_STATE" 2>/dev/null)
  owned6=$(jq -r '.v6_ports[]? // empty' "$BYPASS_STATE" 2>/dev/null)
  old4=$(ip -4 rule show pref "$BYPASS_PREF" 2>/dev/null | grep 'sport' | sed -n 's/.*sport \([0-9]\+\).*/\1/p' | sort -u)
  old6=$(ip -6 rule show pref "$BYPASS_PREF" 2>/dev/null | grep 'sport' | sed -n 's/.*sport \([0-9]\+\).*/\1/p' | sort -u)
  for port in $new_ports; do
    if ! grep -qx "$port" <<<"$old4"; then
      ip -4 rule add pref "$BYPASS_PREF" sport "$port" lookup main 2>/dev/null && { owned4=$(printf '%s\n%s\n' "$owned4" "$port" | grep -E '^[0-9]+$' | sort -un); changed=1; }
    fi
    if ! grep -qx "$port" <<<"$old6"; then
      ip -6 rule add pref "$BYPASS_PREF" sport "$port" lookup main 2>/dev/null && { owned6=$(printf '%s\n%s\n' "$owned6" "$port" | grep -E '^[0-9]+$' | sort -un); changed=1; }
    fi
  done
  for port in $owned4; do
    if ! grep -qx "$port" <<<"$new_ports"; then
      ip -4 rule del pref "$BYPASS_PREF" sport "$port" lookup main 2>/dev/null && { owned4=$(grep -vx "$port" <<<"$owned4" || true); changed=1; }
    fi
  done
  for port in $owned6; do
    if ! grep -qx "$port" <<<"$new_ports"; then
      ip -6 rule del pref "$BYPASS_PREF" sport "$port" lookup main 2>/dev/null && { owned6=$(grep -vx "$port" <<<"$owned6" || true); changed=1; }
    fi
  done
  if [ "$changed" = 1 ] || jq -e 'has("v4_fwmark") or has("v6_fwmark")' "$BYPASS_STATE" >/dev/null 2>&1; then
    jq -n --argjson v4 "$(printf '%s\n' "$owned4" | jq -Rsc 'split("\\n")|map(select(length>0)|tonumber)')" \
      --argjson v6 "$(printf '%s\n' "$owned6" | jq -Rsc 'split("\\n")|map(select(length>0)|tonumber)')" \
      '{v4_ports:$v4,v6_ports:$v6}' | json_write "$BYPASS_STATE"
  fi
}
...
clear_bypass_rules(){
  ...
  if [ -f "$BYPASS_STATE" ]; then
    while IFS= read -r p; do
      [ -n "$p" ] || continue
      ip -4 rule del pref "$BYPASS_PREF" sport "$p" lookup main 2>/dev/null || true
    done < <(jq -r '.v4_ports[]? // empty' "$BYPASS_STATE" 2>/dev/null)
    while IFS= read -r p; do
      [ -n "$p" ] || continue
      ip -6 rule del pref "$BYPASS_PREF" sport "$p" lookup main 2>/dev/null || true
    done < <(jq -r '.v6_ports[]? // empty' "$BYPASS_STATE" 2>/dev/null)
  fi
  printf '%s\n' '{"v4_ports":[],"v6_ports":[]}' | json_write "$BYPASS_STATE" >/dev/null 2>&1 || true
}
...
restore_config_runtime(){
  ...
  if [ -n "$prev_conf" ] && [ -s "$prev_conf" ]; then
    ...
    rm -f "$CONFIG"
    timeout 15 systemctl stop sing-box >/dev/null 2>&1 || true
  fi
  sync_bypass_rules
  sync_hopping_rules
  return 0
...
rollback_config_failure(){
  local prev_conf=$1 message=$2
  tell_warn "$message"
  if ! restore_config_runtime "$prev_conf"; then
...
_apply_config_locked(){
  ...
  if ! timeout 15 systemctl reload-or-restart sing-box >/dev/null 2>&1; then
    rollback_config_failure "$prev_conf" "sing-box 重载/重启失败，正在回滚配置"
    return 1
  fi
  ...
  if ! systemctl is-active --quiet sing-box; then
    ...
    rollback_config_failure "$prev_conf" "配置回滚中"
    return 1
  fi
...
menu_server(){
  ...
  while :; do
    ...
    tell "  5. 更换域名"; tell "  6. 重启服务"; tell "  7. 停止服务"; tell "  8. WireGuard 管理"; tell "  0. 返回"
    tell "${CYAN}================================${PLAIN}"
    case $(prompt "请选择") in
...
menu_client(){
  while :; do
    ...
    tell "  1. 添加节点"; tell "  2. 节点选择"; tell "  3. 删除节点"; tell "  4. 停止代理"; tell "  5. 客户端状态"; tell "  6. WireGuard 管理"; tell "  0. 返回"
    tell "${CYAN}================================${PLAIN}"
    case $(prompt "请选择") in
      1) peer_add ;; 2) peer_select ;; 3) peer_delete ;; 4) peer_stop ;; 5) menu_client_status ;; 6) menu_wireguard ;;
...
wg_toggle(){
  ...
  if [ "$(jq -r .enabled "$WG_CONF")" = true ]; then
    ...
    if [ "$role" = client ] && [ "$previous" = wireguard ]; then state_set exit direct; stop_watchdog; fi
    if apply_config; then
      tell_ok "已关闭隧道"
    else
      printf '%s\n' "$old_json" | json_write "$WG_CONF"
    ...
    json_edit "$WG_CONF" '.enabled=true' || { state_set exit "$previous"; tell_warn "启用状态写入失败"; wait_key; return; }
    if [ "$role" = server ]; then
...
run_uninstall(){
  ...
  if ip -4 rule show pref "$BYPASS_PREF" 2>/dev/null | grep -q 'sport ' || ip -6 rule show pref "$BYPASS_PREF" 2>/dev/null | grep -q 'sport '; then
