...
build_config(){
  ...
  local domain_resolver_obj wg_cidr
  ...
  if [ "$use_tun" = 1 ]; then
    ...
    [ -z "$ssh_client_ip" ] && [ -n "$SSH_CONNECTION" ] && \
      ssh_client_ip=$(awk '{print $1}' <<<"$SSH_CONNECTION")
    local -a exclude_list=()
    [ -n "$ssh_client_ip" ] && exclude_list+=("$ssh_client_ip/32")
    local _self_ip
  ...
  wg_cidr=""
  if [ -f "$WG_CONF" ] && [ "$(jq -r '.enabled//false' "$WG_CONF")" = true ] \
     && [ "$(jq -r '.role' "$WG_CONF")" = client ] \
     && [ "$final" = "wireguard" ]; then
    wg_cidr=$(jq -r '.peer_ip//""' "$WG_CONF" 2>/dev/null | sed 's|\.[0-9]*$|.0/24|')
  fi
  ...
  rules=$(jq -n --arg host "$peer_host" --arg dds "$dns_direct_server" --arg icmp "$icmp_out" \
    ...
    + (if $wg_cidr != "" then [{ip_cidr:[$wg_cidr],action:"route",outbound:"wireguard"}] else [] end)
  ...
  if [ "$use_tun" = 1 ] && [ "$final" != "wireguard" ]; then
    auto_detect="true"
  else
    auto_detect="false"
  fi
...
render_share_uri(){
  ...
  if [ -z "$NET_IPV4$NET_IPV6" ]; then
    out_warn "未识别到本机公网 IP，生成链接失败"
    return
  fi
  [ -n "$hopping" ] && out "跳跃配置: $hopping"
  if [ "$kind" = "snell" ]; then
  ...
  if [ -n "$raw_data" ]; then
    ...
    while read -r file tag type name port host; do
      ...
      (
        ...
        if [ -z "$ping_res" ]; then
          ping_res=$(timeout 1 $cmd -c 1 -W 1 -m 255 "$host" 2>/dev/null | awk -F'/' '/^rtt|^round-trip/{print $5}')
        fi
        if [ -n "$ping_res" ]; then
...
wg_fill_peer_key(){
  ...
  json_edit "$WG_CONF" '.peer_public_key=$k|.endpoint.peers[0].public_key=$k' --arg k "$key" \
    || { tell_warn "覆盖失败"; wait_key; return; }
