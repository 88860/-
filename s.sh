...
build_config(){
  ...
  rules=$(jq -n --arg host "$peer_host" --arg icmp "$icmp_out" '
    ...
    + [{action:"evaluate",server:"dns-direct-cf-v4",tag:"private-check"},
       {match_response:"private-check",ip_is_private:true,action:"route",server:"dns-direct-cf-v4"}]')
  ...
  (
  [
    {type:"udp",tag:"dns-cf-v4",server:$cf4,server_port:53,dialer:{detour:$dns_detour}},
    {type:"udp",tag:"dns-google-v4",server:$gg4,server_port:53,dialer:{detour:$dns_detour}},
    {type:"udp",tag:"dns-direct-cf-v4",server:$cf4,server_port:53,dialer:{detour:"direct"}},
    {type:"udp",tag:"dns-direct-google-v4",server:$gg4,server_port:53,dialer:{detour:"direct"}}
  ] + (if $has_v6 == 1 then [
    {type:"udp",tag:"dns-cf-v6",server:$cf6,server_port:53,dialer:{detour:$dns_detour}},
    {type:"udp",tag:"dns-google-v6",server:$gg6,server_port:53,dialer:{detour:$dns_detour}},
    {type:"udp",tag:"dns-direct-cf-v6",server:$cf6,server_port:53,dialer:{detour:"direct"}},
    {type:"udp",tag:"dns-direct-google-v6",server:$gg6,server_port:53,dialer:{detour:"direct"}}
  ] else [] end)
) as $servers |
    {
      "servers": $servers,
  ...
  jq -n --argjson inbounds "$inbounds" --argjson outbounds "$outbounds" \
    ...
    {log:{level:"warn",timestamp:true},
     ...
     route:{rules:$rules,final:$final,
            ...
            default_fallback_network_type:["udp"]}}
