...
menu_modify_protocol(){
  ...
  while :; do
    ...
    ui_title "$name${PLAIN} ${DIM}[$kind]"
    menu_item 1 "节点名称"; menu_item 2 "监听端口"
    case $kind in
      vless-reality|vless-tls) menu_item 3 "通信 UUID" ;;
      tuic) menu_item 3 "通信 UUID"; menu_item 4 "连接密码" ;;
      socks) menu_item 3 "鉴权密码"; menu_item 4 "鉴权账号" ;;
    ...
    [ "$kind" = vless-reality ] && menu_item 5 "握手目标域名"
    [ "$kind" = vless-reality ] && menu_item 6 "short_id"
    menu_item 0 "返回"
    choice=$(prompt "请选择")
    [ "$choice" = 0 ] && return
...
warp_refresh(){
  ...
  mode=$(warp_mode_get)
  WARP_API_INTERFACE=""; WARP_API_FAMILY=""
  case "$mode" in
    ipv4) WARP_API_FAMILY=4; WARP_API_INTERFACE="$NET_IF_V4" ;;
    ipv6) WARP_API_FAMILY=6; WARP_API_INTERFACE="$NET_IF_V6" ;;
    dual) if [ -n "$NET_IF_V4" ]; then WARP_API_FAMILY=4; WARP_API_INTERFACE="$NET_IF_V4"; else WARP_API_FAMILY=6; WARP_API_INTERFACE="$NET_IF_V6"; fi ;;
    *) return 1 ;;
  esac
  [ -n "$WARP_API_INTERFACE" ] || return 1
...
warp_register(){
  ...
  WARP_API_INTERFACE=""; WARP_API_FAMILY=""
  case "$new_ip_target" in
    ipv4) WARP_API_FAMILY=4; WARP_API_INTERFACE="$NET_IF_V4" ;;
    ipv6) WARP_API_FAMILY=6; WARP_API_INTERFACE="$NET_IF_V6" ;;
    dual) if [ -n "$NET_IF_V4" ]; then WARP_API_FAMILY=4; WARP_API_INTERFACE="$NET_IF_V4"; else WARP_API_FAMILY=6; WARP_API_INTERFACE="$NET_IF_V6"; fi ;;
  esac
  [ -n "$WARP_API_INTERFACE" ] || { tell_warn "没有可用的 WARP 上游网络"; wait_key; return 1; }
