...
tell_ok(){ echo -e "  ${BOLD}${GREEN}[✓] $*${PLAIN}"; }
tell_warn(){ echo -e "  ${BOLD}${RED}[✗] $*${PLAIN}"; }
tell_alert(){ echo -e "  ${BOLD}${YELLOW}[!] $*${PLAIN}"; }

ui_title(){ clear; printf "${BOLD}${CYAN}%s${PLAIN}\n" "$1"; printf "${DIM}────────────────────────────────────────${PLAIN}\n"; }
menu_item(){ if [ "$1" = 0 ]; then printf "  ${BOLD}${YELLOW}%2s  %s${PLAIN}\n" "$1" "$2"; else printf "  ${BOLD}${CYAN}%2s${PLAIN}  ${WHITE}%s${PLAIN}\n" "$1" "$2"; fi; }
...
menu_server_info(){
  ...
  for file in "$NODE_DIR"/*.json; do
    ...
    if [ "$(server_node_kind "$file")" = shadowsocks ]; then
      local ss_network
      ss_network=$(jq -r '.meta.network // ""' "$file" 2>/dev/null)
      case "$ss_network" in
        udp)
        ...
        *)
          if grep -qx "$port" <<<"$tcp_list" && grep -qx "$port" <<<"$udp_list"; then
            status="${GREEN}正常监听${PLAIN}"
          else
            status="${RED}未在监听${PLAIN}"
          fi
          ;;
      esac
    elif [ "$proto" = u ]; then
...
config_build_tun_inbounds(){
  local inbounds=$1 name=$TUN_IF mode=${2:-dual} preserve_management=${3:-0} route='[]' exclude='[]'
  case "$mode" in
  ...
  jq -n --argjson list "$inbounds" --arg name "$name" --argjson route "$route" --argjson exclude "$exclude" '
...
config_build_rules(){
  ...
  if [ "$selected" = warp ]; then
    # WARP native-family and management-peer protection is handled by TUN route exclusions.
    # No SSH port lookup is performed here, so WARP startup cannot depend on sshd configuration.
    jq -n --arg host "$peer_host" '
...
build_config(){
  ...
  if [ "$use_tun" = 1 ]; then
    if [ "$selected" = warp ]; then
      inbounds=$(config_build_tun_inbounds "$inbounds" "$(warp_mode_get)" 1) || return 1
    else
...
build_uri(){
  ...
  case $kind in
    ...
    vless-tls)
      uri="vless://$(jq -r .uuid <<<"$meta")@$host:$port?encryption=none&flow=xtls-rprx-vision&security=tls&sni=$(uri_encode "$sni_host")&fp=chrome&type=tcp&allowInsecure=0#$(uri_encode "$name")" ;;
    vless-ws)
      local vless_path
      vless_path=$(jq -r '.path//"/ws"' <<<"$meta")
      uri="vless://$(jq -r .uuid <<<"$meta")@$host:$port?encryption=none&security=tls&sni=$(uri_encode "$sni_host")&fp=chrome&type=ws&path=$(uri_encode "$vless_path")&host=$(uri_encode "$host")&allowInsecure=0#$(uri_encode "$name")" ;;
    vless-grpc)
      local vless_service
      vless_service=$(jq -r '.service_name//"TunService"' <<<"$meta")
      uri="vless://$(jq -r .uuid <<<"$meta")@$host:$port?encryption=none&security=tls&sni=$(uri_encode "$sni_host")&fp=chrome&type=grpc&serviceName=$(uri_encode "$vless_service")&allowInsecure=0#$(uri_encode "$name")" ;;
    hysteria2)
    ...
    trojan)
      uri="trojan://$(uri_encode "$(jq -r .password <<<"$meta")")@$host:$port?security=tls&sni=$(uri_encode "$sni_host")&type=tcp&allowInsecure=0#$(uri_encode "$name")" ;;
    trojan-ws)
      local trojan_path
      trojan_path=$(jq -r '.path//"/ws"' <<<"$meta")
      uri="trojan://$(uri_encode "$(jq -r .password <<<"$meta")")@$host:$port?security=tls&sni=$(uri_encode "$sni_host")&type=ws&path=$(uri_encode "$trojan_path")&host=$(uri_encode "$host")&allowInsecure=0#$(uri_encode "$name")" ;;
    trojan-grpc)
      local trojan_service
      trojan_service=$(jq -r '.service_name//"TunService"' <<<"$meta")
      uri="trojan://$(uri_encode "$(jq -r .password <<<"$meta")")@$host:$port?security=tls&sni=$(uri_encode "$sni_host")&type=grpc&serviceName=$(uri_encode "$trojan_service")&allowInsecure=0#$(uri_encode "$name")" ;;
    anytls)
      local a_pw a_meta
      a_pw=$(jq -r .password <<<"$meta"); a_meta=$(jq -r '.client_metadata//""' <<<"$meta")
      uri="anytls://$(uri_encode "$a_pw")@$host:$port/?sni=$(uri_encode "$sni_host")&insecure=0"
    ...
    ss|shadowsocks)
      ...
      outbound=$(jq -n --arg tag "$tag" --arg server "$(jq -r .host <<<"$context")" --argjson port "$(jq -r .port <<<"$context")" \
        ...
        '{type:"shadowsocks",tag:$tag,server:$server,server_port:$port,method:$method,password:$password}
         ...
         | if $network=="tcp" then .udp_over_tcp={enabled:true,version:2} else . end
...
protocol_set_ss_password(){
  local file=$1 value=$2
  json_edit "$file" '.meta.password=$v|.inbound.password=$v' --arg v "$value"
}
...
protocol_set_hysteria2_clear_obfs(){
  local file=$1
  json_edit "$file" '.meta.obfs_type="" | .meta.obfs_password="" | del(.meta.min_packet_size,.meta.max_packet_size) | del(.inbound.obfs)'
}

protocol_set_ss_network(){
  local file=$1 value=$2
  if [ "$value" = "" ]; then
    json_edit "$file" 'del(.meta.network,.inbound.network)|.proto="tu"'
  else
    json_edit "$file" '.meta.network=$v|.inbound.network=$v|.proto=$p' --arg v "$value" --arg p "$value"
  fi
}

protocol_set_ss_method(){
  local file=$1 value=$2
  json_edit "$file" '.meta.method=$m|.inbound.method=$m' --arg m "$value"
}

protocol_set_transport_path(){
  local file=$1 value=$2
  json_edit "$file" '.meta.path=$v|.inbound.transport.path=$v' --arg v "$value"
}

protocol_set_transport_service(){
  local file=$1 value=$2
  json_edit "$file" '.meta.service_name=$v|.inbound.transport.service_name=$v' --arg v "$value"
}

protocol_set_vmess_security(){
  local file=$1 value=$2
  json_edit "$file" '.meta.security=$v' --arg v "$value"
}

protocol_set_tuic_congestion(){
  local file=$1 value=$2
  json_edit "$file" '.meta.congestion_control=$v|.inbound.congestion_control=$v' --arg v "$value"
}
...
protocol_edit_option3(){
  local file=$1 kind=$2 value len method
  [ -n "$file" ] && [ -f "$file" ] || return 1
  case $kind in
    shadowsocks)
      menu_item 1 "TCP + UDP"; menu_item 2 "TCP"; menu_item 3 "UDP"
      while :; do
        case $(prompt "客户端连接方式" 1) in
          1) value=""; break ;; 2) value=tcp; break ;; 3) value=udp; break ;;
          *) tell_warn "输入无效，请重新选择"; sleep 1 ;;
        esac
      done
      protocol_set_ss_network "$file" "$value" || { tell_warn "修改失败"; wait_key; return 1; }
      ;;
    vless-reality|vless-tls|vless-ws|vless-grpc|vmess|tuic)
      value=$(prompt "新通信 UUID (留空自动生成)" "$(jq -r '.meta.uuid//""' "$file")")
      [ -z "$value" ] && value=$(random_uuid)
      protocol_set_uuid "$file" "$value" || { tell_warn "修改失败"; wait_key; return 1; }
      ;;
    snell)
      while :; do
      ...
      protocol_set_snell_psk "$file" "$value" || { tell_warn "修改失败"; wait_key; return 1; }
      ;;
    *)
      value=$(prompt "新连接密码 (留空自动生成)" "$(jq -r '.meta.password//""' "$file")")
      [ -z "$value" ] && value=$(random_password)
      protocol_set_password "$file" "$value" || { tell_warn "修改失败"; wait_key; return 1; }
      ;;
  esac
}

protocol_edit_option4(){
  local file=$1 kind=$2 value current_hop other security
  [ -n "$file" ] && [ -f "$file" ] || return 1
  case $kind in
    shadowsocks)
      menu_item 1 "2022-blake3-aes-128-gcm"; menu_item 2 "2022-blake3-aes-256-gcm"; menu_item 3 "2022-blake3-chacha20-poly1305"
      menu_item 4 "aes-128-gcm"; menu_item 5 "aes-256-gcm"; menu_item 6 "chacha20-ietf-poly1305"; menu_item 7 "xchacha20-ietf-poly1305"
      while :; do
        case $(prompt "加密方式" 2) in
      ...
      protocol_set_ss_method "$file" "$value" || { tell_warn "修改失败"; wait_key; return 1; }
    ...
    vless-ws|vless-grpc|trojan-ws|trojan-grpc|vmess-tls|vmess-ws|vmess-grpc|vmess-http)
      case $kind in
        *-ws) value=$(prompt "WebSocket Path" "$(jq -r '.meta.path//"/ws"' "$file")"); protocol_set_transport_path "$file" "$value" || return 1 ;;
        *-grpc) value=$(prompt "gRPC Service Name" "$(jq -r '.meta.service_name//"TunService"' "$file")"); protocol_set_transport_service "$file" "$value" || return 1 ;;
        vmess-http) value=$(prompt "HTTP Path" "$(jq -r '.meta.path//"/"' "$file")"); protocol_set_transport_path "$file" "$value" || return 1 ;;
        vmess-tls) menu_item 1 "Auto"; menu_item 2 "AES-128-GCM"; menu_item 3 "ChaCha20-Poly1305"
          while :; do case $(prompt "加密方式" 1) in 1) security=auto; break;;2) security=aes-128-gcm; break;;3) security=chacha20-poly1305; break;;*) tell_warn "输入无效，请重新选择";;esac; done
          protocol_set_vmess_security "$file" "$security" || return 1 ;;
      esac
      ;;
    tuic)
      menu_item 1 "cubic"; menu_item 2 "New Reno"; menu_item 3 "bbr"
      while :; do case $(prompt "拥塞控制" 3) in 1) value=cubic;break;;2) value=new_reno;break;;3) value=bbr;break;;*) tell_warn "输入无效，请重新选择";;esac; done
      protocol_set_tuic_congestion "$file" "$value" || { tell_warn "修改失败"; wait_key; return 1; }
      ;;
    socks)
      value=$(prompt "鉴权账号" "$(jq -r '.meta.username//""' "$file")"); [ -n "$value" ] || return 1
      protocol_set_username "$file" "$value" || { tell_warn "修改失败"; wait_key; return 1; }
      ;;
    snell)
      ...
      protocol_set_snell_mode "$file" "$value" || { tell_warn "修改失败"; wait_key; return 1; }
      ;;
    anytls)
    ...
    hysteria2)
      ...
      protocol_set_hopping "$file" "$value" || { tell_warn "修改失败"; wait_key; return 1; }
      ;;
    *)
...
protocol_edit_option5(){
  ...
  case $kind in
    vless-reality)
      ...
      protocol_set_reality_target "$file" "$value" || { tell_warn "修改失败"; wait_key; return 1; }
      ;;
    vmess)
      menu_item 1 "Auto"; menu_item 2 "AES-128-GCM"; menu_item 3 "ChaCha20-Poly1305"
      while :; do case $(prompt "加密方式" "$(jq -r '.meta.security//"auto"' "$file")") in 1|auto) security=auto;break;;2|aes-128-gcm) security=aes-128-gcm;break;;3|chacha20-poly1305) security=chacha20-poly1305;break;;*) tell_warn "输入无效，请重新选择";;esac; done
      protocol_set_vmess_security "$file" "$security" || { tell_warn "修改失败"; wait_key; return 1; }
      ;;
    hysteria2)
      menu_item 1 "BBR Conservative"; menu_item 2 "BBR Standard"; menu_item 3 "BBR Aggressive"; menu_item 4 "Brutal（手动带宽）"
      while :; do
...
protocol_edit_option6(){
  ...
  if [ "$kind" = vless-reality ]; then
    ...
    protocol_set_reality_short_id "$file" "$value" || { tell_warn "修改失败"; wait_key; return 1; }
    ...
    if prompt_yes "是否配置并开启协议混淆 (选择 N 则关闭混淆)"; then
      ...
      protocol_clear_hysteria2_obfs "$file"
...
protocol_edit_option7(){
  ...
  case $kind in
    shadowsocks)
      ...
      protocol_set_ss_password "$file" "$value" || return 1 ;;
    *) tell_warn "当前协议不支持该配置项"; sleep 1; return 1 ;;
  esac
}
...
menu_modify_protocol(){
  ...
  while :; do
    ...
    case $kind in
      shadowsocks) menu_item 3 "客户端连接方式"; menu_item 4 "加密方式"; menu_item 5 "连接密码" ;;
    ...
    case "$kind:$choice" in
      ...
      *:3) protocol_edit_option3 "$file" "$kind" || continue ;;
      shadowsocks:4) protocol_edit_option4 "$file" "$kind" || continue ;;
      shadowsocks:5) protocol_edit_option7 "$file" "$kind" || continue ;;
...
warp_tunnel_select(){
  ...
  WARP_TUNNEL_INTERFACE=""
  WARP_TUNNEL_FAMILY=""
  # Endpoint transport is independent of the requested WARP egress mode.
  ...
  [ -n "$WARP_TUNNEL_INTERFACE" ] || return 1
}
...
warp_delete(){
  ...
  if [ "$remote_delete_ok" -eq 1 ]; then
    ...
    tell_warn "本地 WARP 配置已删除，但 Cloudflare 注册信息未能远程删除；网络恢复后可重新清理"
  fi
  wait_key
}
  ...
  if [ -n "$raw_data" ]; then
    ...
    while read -r node_file kind port name proto; do
      ...
      if [ "$kind" = "shadowsocks" ]; then
        local ss_network
        ss_network=$(jq -r '.meta.network // ""' "$node_file" 2>/dev/null)
        case "$ss_network" in
          udp)
          ...
          *)
            if grep -qx "$port" <<<"$tcp_list" && grep -qx "$port" <<<"$udp_list"; then
              color="$GREEN"; status_text="● 正常"
            else
              color="$RED"; status_text="× 异常"
            fi
            ;;
        esac
      elif [ "$proto" = "u" ]; then
      ...
      printf "  %b%2d%b  %-13s %-21s %5s  %b%s%b\n" "$CYAN" "$count" "$PLAIN" "$display_kind" "$name_display" "$port" "$color" "$status_text" "$PLAIN"
    done <<<"$raw_data"
    IFS="$old_ifs"
  fi
...
create_shadowsocks(){
  local method=${1:-2022-blake3-aes-256-gcm} name port password network proto tag body
  ui_title "创建 Shadowsocks"
  name=$(prompt "节点名称" "Shadowsocks")
  menu_item 1 "TCP + UDP"; menu_item 2 "TCP"; menu_item 3 "UDP"
  while :; do
    case $(prompt "客户端连接方式" 1) in
      1) network=""; proto="tu"; break ;; 2) network=tcp; proto="t"; break ;; 3) network=udp; proto="u"; break ;;
      *) tell_warn "输入无效，请重新选择"; sleep 1 ;;
    esac
  done
  ...
  body=$(jq -n --arg tag "$tag" --arg name "$name" --arg method "$method" --arg password "$password" \
    ...
    {tag:$tag,name:$name,kind:"shadowsocks",port:$port,proto:$proto,hopping:"",tls_mode:"none",alpn:null,
...
list_peers(){
  ...
  for i in $(seq 1 $idx); do
    ...
    local current_text="" display_type name_display ms color status_text
    case "$type" in
      vless) display_type="VLESS" ;;
      trojan) display_type="Trojan" ;;
      hysteria2) display_type="Hysteria2" ;;
      tuic) display_type="TUIC" ;;
      shadowsocks) display_type="Shadowsocks" ;;
      vmess) display_type="VMess" ;;
      anytls) display_type="AnyTLS" ;;
      socks) display_type="SOCKS5" ;;
      snell) display_type="Snell" ;;
      *) display_type="$type" ;;
    esac
    name_display=$(LC_ALL=C.UTF-8 printf '%s' "$name" | cut -c1-20)
    [ "$name_display" != "$name" ] && name_display="${name_display}…"
    ...
    [ "$tag" = "$current" ] && current_text="当前"
    printf "  %b%2d%b  %-13s %-21s %7s  %b%-6s%b\n" "$CYAN" "$i" "$PLAIN" "$display_type" "$name_display" "$status_text" "$CYAN" "$current_text" "$PLAIN"
  done
