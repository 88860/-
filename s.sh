...
check_dependencies(){
  if [ "$SYS_ENV" = "systemd" ]; then
    ...
    if [ -n "$to_install" ]; then
      ...
      # LXC 内存优化: 安装完毕后立即清空 apk 缓存并同步释放内存
...
# 内存优化：舍弃 jq 解析庞大 release json，改用 grep+awk 纯流式轻量截取版本号，彻底防止 LXC 内存溢出
...
install_core(){
  ...
  [ -n "$version" ] || { tell_warn "获取最新版本信息失败，请检查网络"; return 1; }

  asset=$(asset_candidates)
  [ -n "$asset" ] || { tell_warn "未找到匹配架构: $(uname -m)"; return 1; }

  # 直接拼接地址，舍弃耗费内存的 json 定位解析
  ...
  if [ "$SYS_ENV" = "openrc" ]; then
    # LXC 内存及硬盘优化: 使用 wget 管道流式解压，不写入本地 tar.gz，防止小容量容器硬盘和内存双爆
...
build_config(){
  ...
  if [ "$SYS_ENV" = "systemd" ]; then
    # 注意: MTU 已从 9000 下调为 1400，防止 UDP/QUIC (Hysteria2等协议) 在部分网络架构中底层分片丢失
  ...
  # 1.14.0 官方规范强制适配：type/server 代替 address，移除 default_domain_resolver
...
render_wg_info(){
  ...
  tell "  引擎: $([ "$(jq -r .enabled "$file")" = "true" ] && echo "${GREEN}运行中${PLAIN}" || echo "${RED}已挂起${PLAIN}")"
  echo ""
