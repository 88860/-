...
build_config(){
  ...
  rules=$(jq -n --arg host "$peer_host" --arg dds "$dns_direct_server" --arg final "$final" --arg strategy "$strategy" '
    [{action:"sniff"},
     {protocol:"dns",action:"hijack-dns"},
     {port:53,action:"hijack-dns"}]
    + (if $final=="wg-direct" or $final=="wireguard" then [{action:"resolve",strategy:$strategy}] else [] end)
    + (if $host=="" then [] 
...
peer_add(){
  ...
  jq -n --argjson ob "$outbound" \
    '{log:{level:"error"},
      dns:{servers:[{tag:"dns-direct",type:"udp",server:"1.1.1.1"}]},
      outbounds:[$ob,{type:"direct",tag:"direct"}],
      route:{final:"direct", default_domain_resolver:"dns-direct"}}' >"$probe"
