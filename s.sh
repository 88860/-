...
probe_network_stack(){
  ...
  if [ -n "$ip6" ] && [ -n "$NET_IF_V6" ]; then
    if curl -6 -fsS --connect-timeout 2 -m 4 --interface "$NET_IF_V6" \
      --resolve 'cloudflare-dns.com:443:[2606:4700:4700::1111]' \
      -H 'accept: application/dns-json' \
      'https://cloudflare-dns.com/dns-query?name=cloudflare.com&type=A' >/dev/null 2>&1; then
      IPV6_OK=1
    fi
  fi
}
...
build_config(){
  ...
  dns_block=$(jq -n --arg direct_tag "$dns_direct_tag" --arg strategy "$dns_strategy" --arg detour "$final" '
    {servers:[
      ...
      {type:"https",tag:"dns-remote",server:"cloudflare-dns.com",server_port:443,path:"/dns-query",tls:{enabled:true,server_name:"cloudflare-dns.com"}}
    ]}
    | .servers[2] |= (if $detour != "direct" then . + {detour:$detour} else . end)
    | {servers:.servers,final:(if $detour == "direct" then $direct_tag else "dns-remote" end),strategy:$strategy}')
...
wg_status(){
  local file=$WG_CONF role state ms color enabled
  clear
  [ -f "$file" ] || { tell "模式：未配置"; wait_key; return; }
  role=$(jq -r '.role // ""' "$file")
  [ "$role" = client ] || [ "$role" = server ] || { tell "模式：未配置"; wait_key; return; }
  enabled=$(jq -r '.enabled//false' "$file")
  if [ "$enabled" != true ]; then
    [ "$role" = client ] && tell "客户端|已停止" || tell "服务端|已停止"
    wait_key
    return
  fi
  if [ ! -d "/sys/class/net/$WG_IF" ]; then
    [ "$role" = client ] && tell "客户端|异常" || tell "服务端|异常"
    wait_key
    return
  fi
  if [ "$role" = client ]; then
    state=$(wg_tunnel_state "$(jq -r '.peer_ip // empty' "$file")" "$(jq -r '.peer_ip6 // empty' "$file")")
    ms=${state#*|}
    if [[ "$state" == ok\|* ]] && [[ "$ms" =~ ^[0-9]+$ ]]; then
      if [ "$ms" -lt 100 ]; then color="$GREEN"; elif [ "$ms" -lt 150 ]; then color="$YELLOW"; else color="$BROWN"; fi
      tell "客户端|已启动|${color}${ms}ms${PLAIN}"
    else
      tell "客户端|异常"
    fi
  else
