...
NET_IPV4=""; NET_IPV6=""; NET_IF_V4=""; NET_IF_V6=""; NET_STACK=""; IPV6_OK=0; BIND_MODE=""; IPV6_V6ONLY=0
...
prompt(){
  ...
  local v; v=$(read_line); printf '%s\n' "${v:-$2}"
}

prompt_yes(){ local v; v=$(prompt "$1 (y/N)" "n"); case "$v" in y|Y|yes|YES) return 0 ;; *) return 1 ;; esac; }
prompt_yes_y(){ local v; v=$(prompt "$1 (Y/n)" "y"); case "$v" in n|N|no|NO) return 1 ;; *) return 0 ;; esac; }
...
json_write(){
  ...
  tmp=$(mktemp "$TMP_DIR/XXXXXX") || return 1
  cat >"$tmp" || rc=1
  [ -s "$tmp" ] || rc=1
...
init_dirs(){
  ...
  if [ -f "$STATE" ]; then
    if ! jq -e . "$STATE" >/dev/null 2>&1; then
      mv "$STATE" "$STATE.broken.$$" 2>/dev/null || true
      printf '%s\n' '{"exit":"direct","domain":"","email":"","challenge":"http","asset":"","acme_profile":""}' | json_write "$STATE"
    fi
  else
    printf '%s\n' '{"exit":"direct","domain":"","email":"","challenge":"http","asset":"","acme_profile":""}' | json_write "$STATE"
  fi
...
ensure_command(){
  ...
  [ "$pre" = 0 ] && ! grep -qF "${pkg}|owned" "$PKG_LOG" 2>/dev/null && echo "${pkg}|owned" >>"$PKG_LOG"
  return 0
}

check_dependencies(){
  local item cmd pkg
  local deps=("bash:bash" "curl:curl" "tar:tar" "jq:jq" "openssl:openssl" "ss:iproute2" "ip:iproute2" "ping:iputils-ping" "flock:util-linux" "pgrep:procps" "getent:libc-bin" "clear:ncurses-bin" "awk:mawk" "grep:grep" "sed:sed" "find:findutils" "shuf:coreutils" "timeout:coreutils" "systemctl:systemd")
  local miss=0
...
default_iface_v4(){
  ...
  case "$i" in "$TUN_IF") i="" ;; esac
  [ -z "$i" ] && i=$(ip -4 route show table main default 2>/dev/null | awk -v t="$TUN_IF" '$5!=t {print $5; exit}')
  printf '%s' "$i"
}
default_iface_v6(){
  local i
  ...
  case "$i" in "$TUN_IF") i="" ;; esac
  [ -z "$i" ] && i=$(ip -6 route show table main default 2>/dev/null | awk -v t="$TUN_IF" '$5!=t {print $5; exit}')
  printf '%s' "$i"
}
is_private_v4(){
  case "$1" in
    ""|10.*|192.168.*|127.*|172.1[6-9].*|172.2[0-9].*|172.3[0-1].*) return 0 ;;
    100.6[4-9].*|100.[7-9][0-9].*|100.1[0-2][0-7].*) return 0 ;;
    *) return 1 ;;
  esac
}
is_private_v6(){
...
ipv6_v6only(){
  ...
  if command -v sysctl >/dev/null 2>&1; then
    v=$(sysctl -n net.ipv6.bindv6only 2>/dev/null || true)
  fi
  [ -z "$v" ] && [ -r /proc/sys/net/ipv6/bindv6only ] && v=$(cat /proc/sys/net/ipv6/bindv6only 2>/dev/null || true)
...
probe_network_stack_async(){
  ( probe_network_stack
    ...
    printf '%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n' "$NET_IPV4" "$NET_IPV6" "$NET_IF_V4" "$NET_IF_V6" "$NET_STACK" "$IPV6_OK" "$BIND_MODE" "$IPV6_V6ONLY" >"$t" && mv -f "$t" "$NET_CACHE" || rm -f "$t"
  ) </dev/null >/dev/null 2>&1 &
}

load_net_cache(){
  if [ -f "$NET_CACHE" ]; then
    { IFS= read -r NET_IPV4; IFS= read -r NET_IPV6; IFS= read -r NET_IF_V4; IFS= read -r NET_IF_V6; IFS= read -r NET_STACK; IFS= read -r IPV6_OK; IFS= read -r BIND_MODE; IFS= read -r IPV6_V6ONLY; } <"$NET_CACHE"
  fi
  if [ -z "$NET_STACK" ] || [ -z "$IPV6_V6ONLY" ]; then
    probe_network_stack
    printf '%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n' "$NET_IPV4" "$NET_IPV6" "$NET_IF_V4" "$NET_IF_V6" "$NET_STACK" "$IPV6_OK" "$BIND_MODE" "$IPV6_V6ONLY" >"$NET_CACHE" 2>/dev/null
  fi
}
...
build_dns_block(){
  ...
  if $final != "direct" then
    .servers |= map(if .tag=="dns-remote" then .detour=$final | del(.domain_resolver) else . end)
  else . end'
}
...
build_route_rules(){
  local icmp_out="$1"
  local rules
  rules=$(jq -n --arg icmp "$icmp_out" '[
    { action:"sniff" },
    { protocol:"dns", action:"hijack-dns" },
    { action:"evaluate", server:"dns-remote" },
    { match_response:true, ip_is_private:true, action:"route", outbound:"direct" },
...
build_config(){
  ...
  for f in "${node_files[@]}"; do
    [ "$(jq -r .tls_mode "$f" 2>/dev/null)" = acme ] && { has_acme=1; break; }
  done

  if [ "$has_acme" = 1 ] && [ -n "$domain" ]; then
    local chal dns01="{}"
    chal=$(state_get challenge)
    case "$chal" in
      dns_cloudflare)
        dns01=$(jq -n --arg t "$(state_get cf_token)" '{provider:"cloudflare",api_token:$t}') ;;
      dns_alidns)
        dns01=$(jq -n --arg k "$(state_get ali_key)" --arg s "$(state_get ali_secret)" '{provider:"alidns",access_key_id:$k,access_key_secret:$s}') ;;
      dns_acmedns)
        dns01=$(jq -n --arg url "$(state_get acmedns_url)" --arg u "$(state_get acmedns_user)" --arg p "$(state_get acmedns_pass)" --arg sub "$(state_get acmedns_sub)" '{provider:"acmedns",server_url:$url,username:$u,password:$p,subdomain:$sub}') ;;
    esac
    providers=$(jq -n \
      --arg tag "$CERT_TAG" \
      --arg d "$domain" \
      --arg e "$(state_get email)" \
      --arg dir "$ACME_DIR" \
      --arg acme_profile "$(state_get acme_profile)" \
      --argjson dns01 "$dns01" \
      --arg chal "$chal" '
      [{
        type:"acme",
      ...
      | if $acme_profile != "" then .profile=$acme_profile else . end
      | if ($chal|startswith("dns_")) then
  ...
  if [ ${#node_files[@]} -gt 0 ]; then
    inbounds=$(jq -s --arg stack "$NET_STACK" --arg v6only "$IPV6_V6ONLY" 'map(
      . as $node |
      (if $node.tls_mode == "acme" then
        $node.inbound | .tls = ({enabled:true, certificate_provider:"acme-cert"})
      else
        $node.inbound
      end)
      | if $stack == "v4" then .listen="0.0.0.0"
        elif $stack == "v6" then .listen="::"
        elif $stack == "both" and $v6only == "1" then
          . as $in | [($in | .tag=(.tag+"-v4") | .listen="0.0.0.0"),
                       ($in | .tag=(.tag+"-v6") | .listen="::")]
        else .listen="::"
        end
    ) | flatten' "${node_files[@]}" 2>/dev/null) || return 1
  fi

  outbounds='[{"type":"direct","tag":"direct"}]'
  local dns_iface="" dns_direct=0
  case "$strat" in
    ipv6_only) dns_iface="$NET_IF_V6" ;;
    *) dns_iface="$NET_IF_V4" ;;
  esac
  [ -z "$dns_iface" ] && dns_iface="$NET_IF_V6"
  if [ -n "$dns_iface" ]; then
  ...
  if [ "$use_tun" = "1" ]; then
    ...
    inbounds=$(jq -n --argjson addr "$tun_addr" \
      ...
      '[{ type:"tun", tag:"tun-in", interface_name:"sbmtun",
         address:$addr, auto_route:true, strict_route:true, dns_mode:"disabled" }] + $list')
  fi
...
ssh_ports(){
  {
    ...
    ss -lntp 2>/dev/null | awk 'NR>1 && /sshd/{print $4}' | sed 's/.*://'
...
node_ports(){
  ...
  for f in "$NODE_DIR"/*.json; do
    [ -e "$f" ] || continue
    jq -r '.port' "$f" 2>/dev/null
    jq -r '.inbound.server_ports[]? // empty' "$f" 2>/dev/null | awk -F: '{print $1}'
  done | grep -E '^[1-9][0-9]*$'
}
protected_ports(){ { ssh_ports; node_ports; } | grep -E '^[1-9][0-9]*$' | sort -un; }
listening_ports(){ ss -ln"$1" 2>/dev/null | awk 'NR>1{print $4}' | sed 's/.*://' | grep -E '^[0-9]+$' | sort -u; }

clear_rules(){
  while ip -4 rule del pref "$PREF_MAIN" 2>/dev/null; do :; done
  while ip -6 rule del pref "$PREF_MAIN" 2>/dev/null; do :; done
}

sync_mgmt_rules(){
  local p new_ports rc=0
  new_ports=$(protected_ports)
  local tmp_pref=$((PREF_MAIN + 10))
  while ip -4 rule del pref "$tmp_pref" 2>/dev/null; do :; done
  while ip -6 rule del pref "$tmp_pref" 2>/dev/null; do :; done
  if [ -n "$NET_IF_V4" ]; then
    for p in $new_ports; do
      ip -4 rule add pref "$tmp_pref" sport "$p" lookup main 2>/dev/null || rc=1
    done
  fi
  if [ -n "$NET_IF_V6" ]; then
    for p in $new_ports; do
      ip -6 rule add pref "$tmp_pref" sport "$p" lookup main 2>/dev/null || rc=1
    done
  fi
  while ip -4 rule del pref "$PREF_MAIN" 2>/dev/null; do :; done
  while ip -6 rule del pref "$PREF_MAIN" 2>/dev/null; do :; done
  if [ -n "$NET_IF_V4" ]; then
    for p in $new_ports; do
      ip -4 rule add pref "$PREF_MAIN" sport "$p" lookup main 2>/dev/null || rc=1
    done
  fi
  if [ -n "$NET_IF_V6" ]; then
    for p in $new_ports; do
      ip -6 rule add pref "$PREF_MAIN" sport "$p" lookup main 2>/dev/null || rc=1
    done
  fi
  while ip -4 rule del pref "$tmp_pref" 2>/dev/null; do :; done
  while ip -6 rule del pref "$tmp_pref" 2>/dev/null; do :; done
...
_apply_config_locked(){
  ...
  tmp=$(mktemp "$TMP_DIR/XXXXXX") || { tell_warn "无法创建临时配置"; return 1; }
  if [ -f "$CONFIG" ]; then
    prev_exists=1
    prev_conf=$(mktemp "$TMP_DIR/XXXXXX") || { rm -f "$tmp"; tell_warn "无法备份旧配置"; return 1; }
...
setup_certificate(){
  ...
  local od oe oc ocf oak oas oau oap oasub oaurl
  ...
  oaurl=$(state_get acmedns_url)
  if [ -n "$od" ] && [ -n "$oe" ]; then
    tell "当前域名: $od, 邮箱: $oe, 验证: $oc"
    if [ -z "$suggest" ]; then
      prompt_yes_y "是否继续使用当前证书配置" && return 0
    else
      prompt_yes "是否继续使用当前证书配置" && return 0
    fi
  fi
    ...
    case $(prompt "请选择" 1) in
      ...
      5) mode=dns_acmedns
         if ! state_set acmedns_url "$(prompt 'server_url')" || ! state_set acmedns_user "$(prompt 'username')" || ! state_set acmedns_pass "$(prompt 'password')" || ! state_set acmedns_sub "$(prompt 'subdomain')"; then
           tell_warn "凭据写入失败"; state_set acmedns_url "$oaurl" || true; state_set acmedns_user "$oau" || true; state_set acmedns_pass "$oap" || true; state_set acmedns_sub "$oasub" || true; return 1
         fi; break ;;
  ...
  if ! state_set challenge "$mode" || ! state_set domain "$domain" || ! state_set email "$email"; then
    ...
    state_set acmedns_url "$oaurl" || true; state_set acmedns_user "$oau" || true; state_set acmedns_pass "$oap" || true; state_set acmedns_sub "$oasub" || true
    return 1
  fi
...
validate_port(){
  ...
  case "$proto" in
    tu|both)
      if grep -qx "$p" <<<"$(listening_ports t)" || grep -qx "$p" <<<"$(listening_ports u)"; then
        tell_warn "端口占用"; return 1
      fi ;;
    u)
      grep -qx "$p" <<<"$(listening_ports u)" && { tell_warn "端口占用"; return 1; } ;;
    *)
      grep -qx "$p" <<<"$(listening_ports t)" && { tell_warn "端口占用"; return 1; } ;;
  esac
  return 0
}
...
build_uri(){
  ...
  case $kind in
    vless-reality)
      uri=$(jq -r --arg n "$name" --arg h "$host" --arg p "$port" 'def u(s): s|@uri; "vless://\(.uuid)@\($h):\($p)?encryption=none&flow=xtls-rprx-vision&security=reality&sni=\(.target)&fp=chrome&pbk=\(.public_key)&sid=\(.short_id)&spx=%2F&type=tcp#" + u($n)' <<<"$meta") ;;
    vless-tls)
      uri=$(jq -r --arg n "$name" --arg h "$host" --arg p "$port" 'def u(s): s|@uri; "vless://\(.uuid)@\($h):\($p)?encryption=none&flow=xtls-rprx-vision&security=tls&sni=\($h)&fp=chrome&type=tcp&allowInsecure=0#" + u($n)' <<<"$meta") ;;
    hysteria2)
      uri=$(jq -r --arg n "$name" --arg h "$host" --arg p "$port" --arg hop "$hopping" '
        def u(s): s|@uri;
        "hysteria2://" + u(.password) + "@\($h):\($p)?sni=\($h)&alpn=h3"
        + (if .bbr_profile then "&bbr_profile=" + u(.bbr_profile) else "" end)
        + (if (.up_mbps//0)>0 or (.down_mbps//0)>0 then "&up_mbps=\(.up_mbps//0)&down_mbps=\(.down_mbps//0)" else "" end)
        + (if $hop != "" then "&mport=\($hop)" else "" end)
        + (if .obfs_type then "&obfs=\(.obfs_type)&obfs-password=" + u(.obfs_password) else "" end)
        + (if .min_packet_size then "&min_packet_size=\(.min_packet_size)" else "" end)
        + (if .max_packet_size then "&max_packet_size=\(.max_packet_size)" else "" end)
        + "#" + u($n)
      ' <<<"$meta") ;;
    tuic)
      uri=$(jq -r --arg n "$name" --arg h "$host" --arg p "$port" 'def u(s): s|@uri; "tuic://\(.uuid):" + u(.password) + "@\($h):\($p)?congestion_control=bbr&alpn=h3&udp_relay_mode=native&sni=\($h)&allow_insecure=0#" + u($n)' <<<"$meta") ;;
    trojan)
      uri=$(jq -r --arg n "$name" --arg h "$host" --arg p "$port" 'def u(s): s|@uri; "trojan://" + u(.password) + "@\($h):\($p)?security=tls&sni=\($h)&type=tcp&allowInsecure=0#" + u($n)' <<<"$meta") ;;
    anytls)
      uri=$(jq -r --arg n "$name" --arg h "$host" --arg p "$port" 'def u(s): s|@uri; "anytls://" + u(.password) + "@\($h):\($p)?sni=\($h)&insecure=0" + (if .client_metadata then "&client_metadata=" + u(.client_metadata) else "" end) + "#" + u($n)' <<<"$meta") ;;
    socks)
      uri=$(jq -r --arg n "$name" --arg h "$host" --arg p "$port" 'def u(s): s|@uri; "socks5://" + u(.username) + ":" + u(.password) + "@\($h):\($p)#" + u($n)' <<<"$meta") ;;
    snell)
      uri=$(jq -r --arg n "$name" --arg h "$host" --arg p "$port" 'def u(s): s|@uri; "snell://" + u(.psk) + "@\($h):\($p)?version=6&mode=\(.mode//"default")#" + u($n)' <<<"$meta") ;;
  esac
  printf '%s' "$uri"
}
...
list_nodes(){
  ...
  if [ -n "$raw" ]; then
    ...
    while read -r file kind port name proto; do
      ...
      local st c
      case "$proto" in
        u) grep -qx "$port" <<<"$udp" && { c="$GREEN"; st="[正常]"; } || { c="$RED"; st="[异常]"; } ;;
        tu)
          if grep -qx "$port" <<<"$tcp" && grep -qx "$port" <<<"$udp"; then c="$GREEN"; st="[正常]"
          elif grep -qx "$port" <<<"$tcp" || grep -qx "$port" <<<"$udp"; then c="$YELLOW"; st="[部分]"
          else c="$RED"; st="[异常]"; fi ;;
        *) grep -qx "$port" <<<"$tcp" && { c="$GREEN"; st="[正常]"; } || { c="$RED"; st="[异常]"; } ;;
      esac
  ...
  body=$(jq -n --arg tag "$tag" --arg name "$name" --argjson port "$port" --arg pw "$pw" --arg hop "$hop" \
   ...
   {tag:$tag,name:$name,kind:"hysteria2",port:$port,proto:"u",hopping:$hop,tls_mode:"acme",alpn:["h3"],
    ...
    inbound:({type:"hysteria2",tag:$tag,listen:"::",listen_port:$port,users:[{password:$pw}]}
      | if $hop!="" then .server_ports=[($hop|gsub("-";":"))] | del(.listen_port) | .hop_interval="30s" else . end
...
create_socks(){
  ...
  body=$(jq -n --arg tag "$tag" --arg name "$name" --argjson port "$port" --arg user "$user" --arg pw "$pw" '
   {tag:$tag,name:$name,kind:"socks",port:$port,proto:"tu",hopping:"",tls_mode:"none",alpn:null,
    meta:{username:$user,password:$pw},
    inbound:{type:"socks",tag:$tag,listen:"::",listen_port:$port,network:"tcp,udp",users:[{username:$user,password:$pw}]}}')
  save_node "$NODE_DIR/$tag.json" "$body"
}
  ...
  if apply_config; then
    ...
    if [ "$was" = acme ]; then
      ...
      if [ "$c" -eq 0 ] && [ -n "$(state_get domain)" ]; then
        tell_gap
        prompt_yes "是否连同域名和证书一起清理" && {
          state_set domain ""
          state_set email ""
          state_set challenge http
          state_set cf_token ""
          state_set ali_key ""
          state_set ali_secret ""
          state_set acmedns_url ""
          state_set acmedns_user ""
          state_set acmedns_pass ""
          state_set acmedns_sub ""
    ...
    case $(prompt "请选择") in
      ...
      4)
        if [ "$kind" = socks ]; then
          ...
          value=$(prompt "跳跃范围 (当前: ${cur_hop:-未开启}, 输入 0 关闭)")
          [ -z "$value" ] && { tell_ok "保持不变"; wait_key; continue; }
          if [ "$value" = "0" ]; then
            json_edit "$PICKED" '
              .hopping="" |
              del(.inbound.server_ports,.inbound.hop_interval) |
              .inbound.listen_port=.port
            ' || { tell_warn 失败; wait_key; continue; }
            tell_ok "已关闭端口跳跃"
          else
            [[ $value =~ ^[0-9]+-[0-9]+$ ]] || { tell_warn "格式错误 (示例 20000-30000)"; wait_key; continue; }
            local lo=${value%-*} hi=${value#*-}
            { [ "$lo" -ge 1 ] && [ "$hi" -le 65535 ] && [ "$lo" -lt "$hi" ]; } || { tell_warn "范围不合法"; wait_key; continue; }
            json_edit "$PICKED" '
              .hopping=$v |
              .inbound.server_ports=[($v|gsub("-";":"))] |
              del(.inbound.listen_port) |
              .inbound.hop_interval="30s"
            ' --arg v "$value" || { tell_warn 失败; wait_key; continue; }
            tell_ok "端口跳跃已更新: $value"
          fi
...
uri_to_outbound(){
  ...
  local bbr dp sm cm mp pk up down
  ...
  case $URI_SCHEME in
    ...
    hysteria2|hy2)
      ...
      up=$(query_value up_mbps); down=$(query_value down_mbps)
      if [ -n "$up" ] || [ -n "$down" ]; then
        outbound=$(jq --argjson u "${up:-0}" --argjson d "${down:-0}" 'if $u>0 then .up_mbps=$u else . end|if $d>0 then .down_mbps=$d else . end' <<<"$outbound")
      fi
  ...
  if ! [[ "$URI_HOST" =~ ^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+$ ]] && ! [[ "$URI_HOST" == *:* ]]; then
    outbound=$(jq --arg r "dns-bootstrap" '.domain_resolver={server:$r}' <<<"$outbound")
  fi
  printf '%s' "$outbound"
}
...
peer_add(){
  ...
  probe=$(mktemp "$TMP_DIR/XXXXXX")
  jq -n --argjson o "$outbound" '{
    log:{level:"error"},
    dns:{
      servers:[{
        type:"https",tag:"dns-bootstrap",
  ...
  if "$CORE" check -c "$probe" >/dev/null 2>&1; then
    if json_save "$PEER_DIR/$tag.json" "$(jq -n --arg tag "$tag" --arg name "$name" --arg uri "$uri" --argjson o "$outbound" '{tag:$tag,name:$name,uri:$uri,outbound:$o}')"; then
      tell_ok "挂载完成: $name"
...
list_peers(){
  ...
  local tmp; tmp=$(mktemp -d "$TMP_DIR/XXXXXX")
  ...
  if [ -n "$raw" ]; then
    ...
    while read -r file tag type name port host; do
      ...
      (
        ...
        if [[ "$host" =~ : ]]; then iface="$NET_IF_V6"; cmd="ping -6"; else iface="$NET_IF_V4"; cmd="ping"; fi
...
exit_label(){
  ...
  case $s in direct) echo "直连" ;; *) [ -f "$PEER_DIR/$s.json" ] && jq -r .name "$PEER_DIR/$s.json" || echo "$s" ;; esac
}
...
install_core(){
  ...
  state_set asset "$asset" || tell_warn "资产状态写入失败（继续）"
...
render_certificate_status(){
  ...
  while IFS= read -r f; do
    if openssl x509 -in "$f" -noout -checkhost "$domain" >/dev/null 2>&1; then
      crt="$f"; break
    fi
  done < <(find "$ACME_DIR" -type f \( -name '*.crt' -o -name '*.pem' -o -name '*.cert' \) 2>/dev/null)
  ...
  if [ "$days" -lt 0 ]; then
    tell "  到期时间: $expiry | ${RED}已过期${PLAIN}"
  else
...
menu_server_info(){
  ...
  for file in "$NODE_DIR"/*.json; do
    ...
    case "$proto" in
      u) grep -qx "$port" <<<"$udp" && status="${GREEN}正常监听${PLAIN}" || status="${RED}未在监听${PLAIN}" ;;
      tu)
        if grep -qx "$port" <<<"$tcp" && grep -qx "$port" <<<"$udp"; then status="${GREEN}正常监听${PLAIN}"
        elif grep -qx "$port" <<<"$tcp" || grep -qx "$port" <<<"$udp"; then status="${YELLOW}部分监听${PLAIN}"
        else status="${RED}未在监听${PLAIN}"; fi ;;
      *) grep -qx "$port" <<<"$tcp" && status="${GREEN}正常监听${PLAIN}" || status="${RED}未在监听${PLAIN}" ;;
    esac
...
menu_change_domain(){
  ...
  [ "$nd" = "$od" ] && { tell_warn "新域名与当前域名相同"; wait_key; return; }
  ...
  if prompt_yes "同步重置验证机制"; then
    ...
    if ! state_set domain "$nd"; then
      tell_warn "写入失败"
      rm -rf "$ACME_DIR"
      [ "$acme_old" = 1 ] && mv "$acme_bak/old" "$ACME_DIR"
      rm -rf "$acme_bak"
      wait_key
      return
    fi
...
update_core_transaction(){
  ...
  cb=$(mktemp "$TMP_DIR/XXXXXX") || { tell_warn "无法创建备份"; return 1; }
  ...
  if [ "$sa" = 1 ]; then
    ...
    if systemctl is-active --quiet sing-box; then
      rm -f "$cb"; tell_warn "无法停止服务"; return 1
    fi
  fi
...
menu_status(){
  while :; do
    ...
    [ "$um" -gt 0 ] && up="${up}${um}分钟"
    [ -z "$up" ] && up="不足1分钟"
    local sv=$(core_version) sasset=$(state_get asset) svc_status="${RED}未运行${PLAIN}"
    systemctl is-active --quiet sing-box && svc_status="${GREEN}正常运行${PLAIN}"
    tell "  系统版本: ${os}"
    tell "  内核架构: ${krnl} (${arch})"
    tell "  内存状态: ${mem}"
    tell "  运行时间: ${up}"
    tell "  singbox : ${svc_status}"
...
EOF
  ...
  [ ! -f "$CONFIG" ] && { build_config | json_write "$CONFIG" || exit 1; }
