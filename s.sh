...
read_line(){
  local value def="${1:-}"
  if [ -c /dev/tty ]; then
    if [ -n "$def" ]; then IFS= read -r -e -i "$def" value </dev/tty || value=""
    else IFS= read -r -e value </dev/tty || value=""; fi
  elif [ -n "$def" ]; then
    IFS= read -r -e -i "$def" value || value=""
  else
    IFS= read -r -e value || value=""
  fi
  value="${value%$'\r'}"
  printf '%s' "$value"
}
...
wg_next_client_address(){
  ...
  for n in $(seq 2 254); do
    ...
    if [ "$used4" = 0 ] && [ "$used6" = 0 ]; then
      WG_NEXT_IPV4="$base4.$n/24"
      WG_NEXT_IPV6="$base6:$n/64"
      return 0
    fi
  done
  return 1
