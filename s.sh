...
is_alpine(){ [ -f /etc/alpine-release ] && command -v apk >/dev/null 2>&1; }
openrc_available(){ command -v rc-service >/dev/null 2>&1 && command -v rc-update >/dev/null 2>&1; }
install_deps(){
  local miss=() c
  if is_alpine; then
    for c in jq openssl timeout ip ping tar gzip sha256sum ss flock getent wget; do
      case "$c" in
        timeout|sha256sum) command -v "$c" >/dev/null 2>&1 || miss+=("coreutils");;
        ip|ss) command -v "$c" >/dev/null 2>&1 || miss+=("iproute2");;
        ping) command -v ping >/dev/null 2>&1 || miss+=("iputils");;
        flock) command -v flock >/dev/null 2>&1 || miss+=("util-linux");;
        getent) command -v getent >/dev/null 2>&1 || miss+=("musl-utils");;
        *) command -v "$c" >/dev/null 2>&1 || miss+=("$c");;
      esac
    done
    if ((${#miss[@]})); then
      apk update >/dev/null 2>&1 || { log_err "apk 软件源更新失败"; return 1; }
      for c in $(printf '%s\n' "${miss[@]}" | awk '!seen[$0]++'); do
        apk add --quiet --no-cache "$c" >/dev/null 2>&1 || { log_err "组件 $c 安装失败，请检查网络"; return 1; }
      done
...
remote_version(){
  ...
  if cmd_exists curl; then
    v=$(env -u http_proxy -u https_proxy -u HTTP_PROXY -u HTTPS_PROXY -u all_proxy -u ALL_PROXY curl -fsSL --connect-timeout 10 --max-time 20 "https://api.github.com/repos/SagerNet/sing-box/releases/latest" 2>/dev/null | jq -r '.tag_name // empty')
  else
    v=$(env -u http_proxy -u https_proxy -u HTTP_PROXY -u HTTPS_PROXY -u all_proxy -u ALL_PROXY wget -qO- -T 10 "https://api.github.com/repos/SagerNet/sing-box/releases/latest" 2>/dev/null | jq -r '.tag_name // empty')
  fi
  v=${v#v}
  printf '%s' "$v"
}
install_singbox(){
  local force="${1:-0}" version arch tmpdir f url
  if [ -x "$BIN" ] && [ "$force" != 1 ]; then
    VERSION=$("$BIN" version 2>/dev/null | awk '/version/{print $3; exit}')
    [ -n "$VERSION" ] || VERSION="unknown"
    return 0
  fi
  version=$(remote_version)
  [ -n "$version" ] || { log_err "获取最新稳定版信息失败，请检查网络"; return 1; }
  if is_alpine; then
    case "$(uname -m)" in
      x86_64|amd64) arch="linux-amd64-musl" ;;
      aarch64|arm64) arch="linux-arm64-musl" ;;
      armv7l|armv8l|armv7) arch="linux-armv7-musl" ;;
      armv6l) arch="linux-armv6" ;;
      i386|i686) arch="linux-386-musl" ;;
      *) log_err "未找到匹配架构的安装包: $(uname -m)"; return 1 ;;
    esac
  else
    case "$(uname -m)" in
      x86_64|amd64) arch="linux-amd64" ;;
      aarch64|arm64) arch="linux-arm64" ;;
      armv7l|armv8l|armv7) arch="linux-armv7" ;;
      armv6l) arch="linux-armv6" ;;
  ...
  if [ -x "$BIN" ] && "$BIN" version 2>/dev/null | grep -q "version ${version}"; then VERSION="$version"; return 0; fi
  tmpdir=$(mktemp -d) || return 1
  TMP_FILES="$TMP_FILES $tmpdir"
  if is_alpine; then
    if ! wget -qO- -T 120 "$url" 2>/dev/null | tar -xzf - -C "$tmpdir" 2>/dev/null; then
      rm -rf "$tmpdir"; log_err "内核下载或解压失败"; return 1
    fi
  else
    local archive="$tmpdir/package.tgz"
    if ! curl -fsSL --connect-timeout 15 --max-time 120 "$url" -o "$archive"; then
      rm -rf "$tmpdir"; log_err "内核下载失败"; return 1
    fi
    tar -xzf "$archive" -C "$tmpdir" >/dev/null 2>&1 || { rm -rf "$tmpdir"; log_err "内核解压失败"; return 1; }
  fi
  f=$(find "$tmpdir" -type f -name sing-box -perm -111 | head -n1)
  [ -n "$f" ] || { rm -rf "$tmpdir"; log_err "安装包中未找到 sing-box"; return 1; }
  service_stop >/dev/null 2>&1 || true
  install -m755 "$f" "$BIN" || { rm -rf "$tmpdir"; log_err "内核安装失败"; return 1; }
  rm -rf "$tmpdir"
  VERSION="$version"
  log_ok "sing-box 内核已安装: v${version} [${arch#linux-}]"
  return 0
}
write_service(){
...
command="/usr/bin/env"
command_args="GOMEMLIMIT=15MiB GOGC=20 $BIN -D /var/lib/sing-box -c $CONFIG run"
output_log="/dev/null"
error_log="/dev/null"
respawn_delay=0
respawn_max=0
...
service_active(){
  if openrc_available; then rc-service sing-box status >/dev/null 2>&1
  elif systemd_available; then systemctl is-active --quiet "$SERVICE"
  else local pid; pid=$(cat "$PIDFILE" 2>/dev/null || true); [[ "$pid" =~ ^[0-9]+$ ]] && kill -0 "$pid" 2>/dev/null
  fi
}
service_stop(){
  if openrc_available; then rc-service sing-box stop >/dev/null 2>&1 || true
  elif systemd_available; then systemctl stop "$SERVICE" >/dev/null 2>&1 || true
  else
...
service_start(){
  if openrc_available; then rc-service sing-box start >/dev/null 2>&1
  elif systemd_available; then systemctl start "$SERVICE" >/dev/null 2>&1
  else
    service_stop
    nohup "$BIN" run -c "$CONFIG" >>"$LOGFILE" 2>&1 & echo $! > "$PIDFILE"
    sleep 1
    service_active
  fi
}
...
apply_config(){
  local new="$1" old="$DATA/.config-before.$$" had_old=0
  [ -f "$CONFIG" ] && had_old=1
...
update_core(){
  echo '  >> 更新内核 <<'; echo
  local old="$VERSION" latest backup="$DATA/.sing-box.old.$$"
  latest=$(remote_version)
  [ -n "$latest" ] || { log_err '获取最新稳定版失败'; pause; return; }
  printf '  当前内核：singbox%s\n  最新内核：singbox%s\n\n' "$old" "$latest"
  [ "$old" = "$latest" ] && { echo '  当前为最新版本，无需更新'; pause; return; }
  cp -f "$BIN" "$backup" 2>/dev/null || { log_err '无法备份当前内核'; pause; return; }
  if ! install_singbox 1; then rm -f "$backup"; pause; return; fi
  if [ -f "$CONFIG" ] && ! "$BIN" check -c "$CONFIG" >/dev/null 2>&1; then
    install -m755 "$backup" "$BIN" 2>/dev/null || true
    rm -f "$backup"
    VERSION="$old"
    log_err '新内核检查现有配置失败，已恢复旧内核'
    pause
    return
  fi
  if ! service_restart; then
    ...
    VERSION="$old"
    service_restart >/dev/null 2>&1 || true
    log_err '新内核启动失败，已恢复旧内核'
    pause
    return
  fi
  rm -f "$backup"
  log_ok "内核已更新至 v$VERSION"
  pause
}
