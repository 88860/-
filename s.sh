...
start_net_monitor(){
  ...
  (
    ...
    local last_if4 last_if6 cur_if4 cur_if6
    last_if4=$(default_iface 4)
    last_if6=$(default_iface 6)
    while :; do
      last_trigger=0
      while IFS= read -r line; do
        ...
        cur_if4=$(default_iface 4)
        cur_if6=$(default_iface 6)
        if [ "$cur_if4" != "$last_if4" ] || [ "$cur_if6" != "$last_if6" ]; then
          last_if4=$cur_if4; last_if6=$cur_if6
          now=$(date +%s)
...
config_build_exit(){
  ...
  if [ "$selected" = warp ]; then
    ...
    # Direct traffic used by DNS/endpoint resolution must stay on the verified
    # physical upstream; auto_redirect may re-capture unbound direct sockets.
    outbounds=$(jq -c --arg bind "$WARP_TUNNEL_INTERFACE" 'map(if .tag=="direct" then . + {bind_interface:$bind} else . end)' <<<"$outbounds") || return 1
...
config_build_tun_inbounds(){
  local inbounds=$1 name=$TUN_IF mode=${2:-dual} preserve_management=${3:-0} strict_override=${4:-} route='[]' exclude='[]'
  case "$mode" in
...
build_config(){
  ...
  local node_block exit_block dns_direct_tag dns_remote_tag dns_strategy auto_detect default_interface providers warp_mode
  ...
  if [ "$use_tun" = 1 ]; then
    if [ "$selected" = warp ]; then
      ...
      inbounds=$(config_build_tun_inbounds "$inbounds" "$(warp_mode_get)" 1 false) || return 1
    else
  ...
  # WARP is the transparent exit. Bind non-WARP direct connections to the
  # same verified upstream interface; auto_redirect can otherwise defeat
  # auto_detect_interface on Linux and re-capture direct traffic into TUN.
  if [ "$selected" = warp ]; then
    [ -n "$WARP_TUNNEL_INTERFACE" ] || return 1
    auto_detect="false"
    default_interface="$WARP_TUNNEL_INTERFACE"
  elif [ "$use_tun" = 1 ]; then
    auto_detect="true"
    default_interface=""
  else
  ...
  jq -n --argjson inbounds "$inbounds" --argjson outbounds "$outbounds" \
        ...
        --arg final "$final" --arg direct_tag "$dns_direct_tag" \
        --argjson auto_detect "$auto_detect" --arg default_interface "$default_interface" '
    {log:{level:"warn",timestamp:true},
     ...
     | if $default_interface != "" then .route.default_interface=$default_interface else . end
...
warp_endpoint(){
  ...
  local resolver_strategy=ipv4_only
  [ "$WARP_TUNNEL_FAMILY" = 6 ] && resolver_strategy=ipv6_only
  jq -n --arg resolver "$resolver" --arg strategy "$resolver_strategy" --arg host "$host" --argjson port "$port" --arg private "$private" --arg peer "$peer" \
    ...
    '{type:"wireguard",tag:"warp",system:true,name:"warp",mtu:1280,address:$addresses,private_key:$private,
      ...
      domain_resolver:{server:$resolver,strategy:$strategy}} | if $bind != "" then . + {bind_interface:$bind} else . end'
}
