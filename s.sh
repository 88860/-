...
build_config(){
  ...
  [ "$use_tun" = 1 ] && inbounds=$(jq -n --argjson list "$inbounds" --arg name "$TUN_IF" '
    [{type:"tun",tag:"tun-in",interface_name:$name,
      ...
      auto_route:true,strict_route:true,stack:"mixed",mtu:9000}] + $list')
...
list_nodes(){
  local file index=0 node_port kind name
  NODE_FILES=()
  for file in "$NODE_DIR"/*.json; do
...
select_node(){
  ...
  while :; do
    ...
    [ -z "$index" ] && continue
    [ "$index" = 0 ] && return 1
    [[ $index =~ ^[0-9]+$ ]] && [ "$index" -ge 1 ] && [ "$index" -le ${#NODE_FILES[@]} ] && break
...
menu_modify_protocol(){
  local kind value other current_hop up_mbps down_mbps old_json choice
  ...
  while :; do
    ...
    [ "$kind" = hysteria2 ] && tell "  6. 修改端口跳跃机制"
    [ "$kind" = hysteria2 ] && tell "  7. 修改带宽限制"
    tell "  0. 返回上级"
    tell "${CYAN}==========================================${PLAIN}"
...
render_certificate_status(){
  ...
  if openssl x509 -checkend 0 -noout -in "$crt" 2>/dev/null; then
    local mtime; mtime=$(stat -c %Y "$crt" 2>/dev/null || echo 0)
    days=$(( (mtime + 7776000 - $(date +%s)) / 86400 ))
    [ "$days" -lt 0 ] && days=0
  else
    days=0
  fi
  tell "  到期时间: $expiry | 剩余:大约 ${days} 天"
...
select_peer(){
  ...
  while :; do
    ...
    [ -z "$index" ] && continue
    [ "$index" = 0 ] && return 1
    [[ $index =~ ^[0-9]+$ ]] && [ "$index" -ge 1 ] && [ "$index" -le ${#PEER_FILES[@]} ] && break
    tell_warn "序号无效，请重新输入"
  done
