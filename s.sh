      ...
      if [ "$selected" = "direct" ] || [ "$selected" = "wireguard" ]; then
        if [ "$wg_endpoint" = "yes" ]; then
          ...
          if printf '%s' "$peer_host" | grep -Eq '^[0-9]+(\.[0-9]+){3}$'; then
            ...
            local resolved_wg_ips
            resolved_wg_ips=$(timeout 3 getent ahosts "$peer_host" 2>/dev/null | awk '{print $1}' | sort -u) || resolved_wg_ips=""
            if [ -n "$resolved_wg_ips" ]; then
              wg_route_excludes=$(printf '%s\n' "$resolved_wg_ips" | awk 'NF{if($0 ~ /:/) print $0 "/128"; else print $0 "/32"}' | jq -Rsc 'split("\n")|map(select(length>0))') || return 1
            fi
          fi
          final="wireguard"
          use_tun=1
...
wg_tunnel_state(){
  ...
  if ! command -v wg >/dev/null 2>&1; then echo down; return; fi
  handshake=$(wg show "$WG_IF" latest-handshakes 2>/dev/null | awk 'NR==1{print $2}')
  case "$handshake" in
    ''|*[!0-9]*) handshake=0 ;;
  esac
  now=$(date +%s)
  if [ "$handshake" -gt 0 ] && [ "$handshake" -le "$now" ] && [ $((now-handshake)) -le 30 ]; then
    if timeout 5 curl --interface "$WG_IF" -fsS -o /dev/null --connect-timeout 3 -m 5 "$PROBE_URL"; then echo up; return; fi
  fi
  if [ -n "$peer_ip" ] && timeout 4 ping -I "$WG_IF" -c1 -W2 "$peer_ip" >/dev/null 2>&1; then
    if timeout 5 curl --interface "$WG_IF" -fsS -o /dev/null --connect-timeout 3 -m 5 "$PROBE_URL"; then echo up; return; fi
  fi
  echo down
}
...
wg_setup(){
  ...
  if [ "$role" = server ]; then
    ...
    body=$(jq -n --arg private "$private" --arg public "$public" --arg a4 "$address4" --arg a6 "$address6" \
     ...
     {role:"server",enabled:false,private_key:$private,public_key:$public,
      ...
      endpoint:{type:"wireguard",tag:"wireguard",system:true,name:$iface,mtu:1408,
        ...
        peers:(if $peer_key!="" then [{public_key:$peer_key,allowed_ips:[($peer4+"/32"),($peer6+"/128")]}] else [] end)}}') || { tell_warn "WireGuard 配置生成失败"; wait_key; return; }
  else
    ...
    body=$(jq -n --arg private "$private" --arg public "$public" --arg a4 "$address4" --arg a6 "$address6" \
          --arg host "$peer_host" --argjson port "$peer_port" --arg peer_key "$peer_key" --arg peer4 "$peer_ip4" --arg iface "$WG_IF" \
...
wg_fill_peer_key(){
  ...
  if [ "$(jq -r '.role' "$WG_CONF" 2>/dev/null)" = server ]; then
    local peer_ip peer_ip6
    peer_ip=$(jq -r '.peer_ip // ""' "$WG_CONF" 2>/dev/null) || { tell_warn "读取客户端地址失败"; wait_key; return; }
    peer_ip6=$(jq -r '.endpoint.address[1] // ""' "$WG_CONF" 2>/dev/null) || { tell_warn "读取 WireGuard IPv6 地址失败"; wait_key; return; }
    [ -n "$peer_ip" ] || { tell_warn "缺少客户端 IPv4 地址"; wait_key; return; }
    peer_ip6=${peer_ip6%%/*}
    if [ -n "$peer_ip6" ]; then
      json_edit "$WG_CONF" '.peer_public_key=$k|if (.endpoint.peers|length)>0 then .endpoint.peers[0].public_key=$k|.endpoint.peers[0].allowed_ips=[($ip+"/32"),($ip6+"/128")] else .endpoint.peers=[{public_key:$k,allowed_ips:[($ip+"/32"),($ip6+"/128")]}] end' --arg k "$key" --arg ip "$peer_ip" --arg ip6 "$peer_ip6" || { tell_warn "覆盖失败"; wait_key; return; }
    else
      json_edit "$WG_CONF" '.peer_public_key=$k|if (.endpoint.peers|length)>0 then .endpoint.peers[0].public_key=$k|.endpoint.peers[0].allowed_ips=[($ip+"/32")] else .endpoint.peers=[{public_key:$k,allowed_ips:[($ip+"/32")]}] end' --arg k "$key" --arg ip "$peer_ip" || { tell_warn "覆盖失败"; wait_key; return; }
    fi
  else
