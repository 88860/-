...
build_uri(){
  ...
  case $kind in
    ...
    hysteria2)
      local ot op os bs mp pk ps up down
      ...
      up=$(jq -r '.up_mbps//0' <<<"$meta"); down=$(jq -r '.down_mbps//0' <<<"$meta")
      os=""; [ -n "$ot" ] && os="&obfs=${ot}&obfs-password=$(uri_encode "$op")"
      local bbs=""; [ -n "$bs" ] && bbs="&bbr_profile=$(uri_encode "$bs")"
      local bws=""; { [ "$up" -gt 0 ] || [ "$down" -gt 0 ]; } && bws="&up_mbps=${up}&down_mbps=${down}"
      ps=""; [ -n "$mp" ] && ps="${ps}&min_packet_size=${mp}"; [ -n "$pk" ] && ps="${ps}&max_packet_size=${pk}"
      uri="hysteria2://$(uri_encode "$(jq -r .password <<<"$meta")")@$host:$port?sni=$host&alpn=h3${bbs}${bws}${hopping:+&mport=$hopping}${os}${ps}#$(uri_encode "$name")" ;;
    tuic)
