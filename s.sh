...
ensure_core_compatible(){
  local tmp rc
  [ -x "$CORE" ] || { install_core || return 1; }
  tmp=$(mktemp) || return 1
  printf '%s\n' '{"log":{"level":"error"},"inbounds":[{"type":"vless","tag":"core-probe","listen":"127.0.0.1","listen_port":1,"users":[{"uuid":"00000000-0000-0000-0000-000000000001"}]}],"outbounds":[{"type":"direct","tag":"direct"}]}' >"$tmp"
  if "$CORE" check -c "$tmp" >/dev/null 2>&1; then rm -f "$tmp"; return 0; fi
  rm -f "$tmp"
  tell "${CYAN}当前 sing-box 内核不支持所需协议，正在更新到最新版本...${PLAIN}"
  install_core || return 1
  tmp=$(mktemp) || return 1
  printf '%s\n' '{"log":{"level":"error"},"inbounds":[{"type":"vless","tag":"core-probe","listen":"127.0.0.1","listen_port":1,"users":[{"uuid":"00000000-0000-0000-0000-000000000001"}]}],"outbounds":[{"type":"direct","tag":"direct"}]}' >"$tmp"
  "$CORE" check -c "$tmp" >/dev/null 2>&1; rc=$?
  rm -f "$tmp"
  return "$rc"
}
...
save_node(){
  ...
  ensure_core_compatible || { tell_warn "当前 sing-box 内核不兼容，无法创建节点"; wait_key; return 1; }
...
warp_file(){ printf '%s/warp.json' "$SBM_DIR"; }

warp_enabled(){
  [ -s "$(warp_file)" ] && jq -e '.enabled == true' "$(warp_file)" >/dev/null 2>&1
}

warp_mode(){ jq -r '.mode // "dual"' "$(warp_file)" 2>/dev/null; }

warp_reserved_from_client_id(){
  local client_id=$1
  printf '%s' "$client_id" | base64 -d 2>/dev/null | od -An -tu1 2>/dev/null | awk 'BEGIN{printf "["} {for(i=1;i<=NF;i++) printf "%s%s", (i>1?",":""), $i} END{print "]"}'
}

warp_generate_keypair(){
  local pair
  pair=$("$CORE" generate wg-keypair 2>/dev/null) || return 1
  WARP_PRIVATE_KEY=$(printf '%s\n' "$pair" | awk -F': ' '/^PrivateKey:/{print $2; exit}')
  WARP_PUBLIC_KEY=$(printf '%s\n' "$pair" | awk -F': ' '/^PublicKey:/{print $2; exit}')
  [ -n "$WARP_PRIVATE_KEY" ] && [ -n "$WARP_PUBLIC_KEY" ]
}

warp_register(){
  local install_id fcm response reserved tmp
  warp_generate_keypair || { tell_warn "WARP 密钥生成失败"; return 1; }
  install_id=$(tr -dc 'a-z0-9' </dev/urandom | head -c 22)
  fcm_token="${install_id}:APA91b$(tr -dc 'A-Za-z0-9' </dev/urandom | head -c 134)"
  response=$(curl -fsSL --connect-timeout 5 --max-time 15 --retry 1 --tlsv1.3 \
    -H 'User-Agent: okhttp/3.12.1' \
    -H 'CF-Client-Version: a-6.10-2158' \
    -H 'Content-Type: application/json' \
    --data "{\"key\":\"$WARP_PUBLIC_KEY\",\"install_id\":\"$install_id\",\"fcm_token\":\"$fcm_token\",\"tos\":\"$(date -u +%Y-%m-%dT%H:%M:%S.000Z)\",\"model\":\"PC\",\"serial_number\":\"$install_id\",\"locale\":\"zh_CN\"}" \
    'https://api.cloudflareclient.com/v0a2158/reg' 2>/dev/null) || { tell_warn "WARP 注册请求失败"; return 1; }
  printf '%s' "$response" | jq -e '.id and .token and .config.client_id and .config.interface.addresses.v4 and .config.peers[0].public_key and .config.peers[0].endpoint.host' >/dev/null 2>&1 || { tell_warn "WARP 注册返回数据无效"; return 1; }
  reserved=$(warp_reserved_from_client_id "$(printf '%s' "$response" | jq -r '.config.client_id')")
  [ -n "$reserved" ] || { tell_warn "WARP reserved 数据生成失败"; return 1; }
  tmp=$(mktemp) || return 1
  jq --arg pk "$WARP_PRIVATE_KEY" --argjson reserved "$reserved" '.config.reserved=$reserved | .private_key=$pk | .enabled=false | .mode="dual"' <<EOF >"$tmp"
$response
EOF
  if ! jq -e . "$tmp" >/dev/null 2>&1 || ! install -m600 "$tmp" "$(warp_file)"; then
    rm -f "$tmp"; tell_warn "WARP 数据保存失败"; return 1
  fi
  rm -f "$tmp"
  return 0
}
...
warp_endpoint_json(){
  local mode=${1:-$(warp_mode)} file=$(warp_file) address_list allowed endpoint_host endpoint_port peer_key reserved
  [ -s "$file" ] || return 1
  case "$mode" in
    4) address_list=$(jq -c '[.config.interface.addresses.v4 + "/32"]' "$file") ; allowed='["0.0.0.0/0"]' ;;
    6) address_list=$(jq -c '[.config.interface.addresses.v6 + "/128"]' "$file") ; allowed='["::/0"]' ;;
    dual) address_list=$(jq -c '[.config.interface.addresses.v4 + "/32", .config.interface.addresses.v6 + "/128"]' "$file"); allowed='["0.0.0.0/0","::/0"]' ;;
    *) return 1 ;;
  esac
  endpoint_host=$(jq -r '.config.peers[0].endpoint.host' "$file")
  endpoint_port=${endpoint_host##*:}
  endpoint_host=${endpoint_host%:*}
  peer_key=$(jq -r '.config.peers[0].public_key' "$file")
  reserved=$(jq -c '.config.reserved // [0,0,0]' "$file")
  jq -n --arg tag cf-warp --arg host "$endpoint_host" --argjson port "$endpoint_port" --argjson address "$address_list" --arg key "$peer_key" --arg private_key "$(jq -r '.private_key' "$file")" --argjson allowed "$allowed" --argjson reserved "$reserved" '{type:"wireguard",tag:$tag,system:false,mtu:1280,address:$address,private_key:$private_key,peers:[{address:$host,port:$port,public_key:$key,allowed_ips:$allowed,persistent_keepalive_interval:30,reserved:$reserved}],domain_resolver:"dns-direct-v4"}'
}

warp_probe(){
  local mode=$1 port tmp_conf tmp_log pid res url4 url6 v4 v6 i endpoint
  [ -s "$(warp_file)" ] || { tell_warn "尚未获取 WARP 数据"; return 1; }
  mkdir -p "$SBM_DIR/tmp" 2>/dev/null || return 1
  ensure_core_compatible || return 1
  command -v curl >/dev/null 2>&1 || { tell_warn "缺少 curl"; return 1; }
  endpoint=$(warp_endpoint_json "$mode") || return 1
  port=$((20000 + $(od -An -N2 -tu2 /dev/urandom 2>/dev/null | tr -d ' ') % 20000)); [ "$port" -ge 20000 ] 2>/dev/null || port=20873
  tmp_conf=$(mktemp "$SBM_DIR/tmp/warp-probe.XXXXXX") || return 1
  tmp_log=$(mktemp "$SBM_DIR/tmp/warp-probe-log.XXXXXX") || { rm -f "$tmp_conf"; return 1; }
  jq -n --argjson port "$port" --argjson endpoint "$endpoint" '{log:{level:"error"},dns:{servers:[{type:"https",tag:"dns-direct-v4",server:"1.1.1.1",server_port:443,path:"/dns-query",tls:{enabled:true,server_name:"cloudflare-dns.com"}}],final:"dns-direct-v4"},inbounds:[{type:"mixed",tag:"warp-probe",listen:"127.0.0.1",listen_port:$port}],outbounds:[{type:"direct",tag:"direct"}],endpoints:[$endpoint],route:{rules:[{inbound:["warp-probe"],action:"route",outbound:"cf-warp"}],final:"direct",default_domain_resolver:"dns-direct-v4",auto_detect_interface:true}}' >"$tmp_conf" 2>/dev/null || { rm -f "$tmp_conf" "$tmp_log"; return 1; }
  "$CORE" check -c "$tmp_conf" >/dev/null 2>&1 || { rm -f "$tmp_conf" "$tmp_log"; tell_warn "WARP 临时配置校验失败"; return 1; }
  "$CORE" run -c "$tmp_conf" >"$tmp_log" 2>&1 & pid=$!
  sleep 1
  if ! kill -0 "$pid" 2>/dev/null; then
    rm -f "$tmp_conf" "$tmp_log"; tell_warn "WARP 出口启动失败"; return 1
  fi
  url4=https://api4.ipify.org/?format=json; url6=https://api6.ipify.org/?format=json
  case "$mode" in
    4) res=$(env -u http_proxy -u https_proxy -u HTTP_PROXY -u HTTPS_PROXY -u all_proxy -u ALL_PROXY curl -4 -fsS --socks5-hostname "127.0.0.1:$port" -m 8 "$url4" 2>/dev/null) ;;
    6) res=$(env -u http_proxy -u https_proxy -u HTTP_PROXY -u HTTPS_PROXY -u all_proxy -u ALL_PROXY curl -6 -fsS --socks5-hostname "127.0.0.1:$port" -m 8 "$url6" 2>/dev/null) ;;
    dual)
      local v4 v6
      v4=$(env -u http_proxy -u https_proxy -u HTTP_PROXY -u HTTPS_PROXY -u all_proxy -u ALL_PROXY curl -4 -fsS --socks5-hostname "127.0.0.1:$port" -m 8 "$url4" 2>/dev/null)
      v6=$(env -u http_proxy -u https_proxy -u HTTP_PROXY -u HTTPS_PROXY -u all_proxy -u ALL_PROXY curl -6 -fsS --socks5-hostname "127.0.0.1:$port" -m 8 "$url6" 2>/dev/null)
      res=$(jq -n --arg v4 "$(printf '%s' "$v4" | jq -r '.ip // empty' 2>/dev/null)" --arg v6 "$(printf '%s' "$v6" | jq -r '.ip // empty' 2>/dev/null)" '{v4:$v4,v6:$v6}')
      ;;
  esac
  kill "$pid" 2>/dev/null || true
  i=0; while kill -0 "$pid" 2>/dev/null && [ "$i" -lt 10 ]; do i=$((i+1)); sleep .2; done
  kill -9 "$pid" 2>/dev/null || true; wait "$pid" 2>/dev/null || true
  rm -f "$tmp_conf" "$tmp_log"
  case "$mode" in
    4)
      v4=$(printf '%s' "$res" | jq -r '.ip // empty' 2>/dev/null)
      printf '%s' "$v4" | grep -Eq '^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+$' || return 1
      jq --arg ip "$v4" --arg mode 4 '.public_ipv4=$ip|.mode=$mode' "$(warp_file)" >"$SBM_DIR/tmp/warp-save.$$" && install -m600 "$SBM_DIR/tmp/warp-save.$$" "$(warp_file)" && rm -f "$SBM_DIR/tmp/warp-save.$$" || return 1
      printf '%s' "$v4" ;;
    6)
      v6=$(printf '%s' "$res" | jq -r '.ip // empty' 2>/dev/null)
      printf '%s' "$v6" | grep -q ':' || return 1
      jq --arg ip "$v6" --arg mode 6 '.public_ipv6=$ip|.mode=$mode' "$(warp_file)" >"$SBM_DIR/tmp/warp-save.$$" && install -m600 "$SBM_DIR/tmp/warp-save.$$" "$(warp_file)" && rm -f "$SBM_DIR/tmp/warp-save.$$" || return 1
      printf '%s' "$v6" ;;
    dual)
      v4=$(printf '%s' "$res" | jq -r '.v4 // empty' 2>/dev/null)
      v6=$(printf '%s' "$res" | jq -r '.v6 // empty' 2>/dev/null)
      printf '%s' "$v4" | grep -Eq '^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+$' || return 1
      printf '%s' "$v6" | grep -q ':' || return 1
      jq --arg v4 "$v4" --arg v6 "$v6" --arg mode dual '.public_ipv4=$v4|.public_ipv6=$v6|.mode=$mode' "$(warp_file)" >"$SBM_DIR/tmp/warp-save.$$" && install -m600 "$SBM_DIR/tmp/warp-save.$$" "$(warp_file)" && rm -f "$SBM_DIR/tmp/warp-save.$$" || return 1
      printf '%s|%s' "$v4" "$v6" ;;
  esac
}

warp_set_enabled(){
  local enabled=$1 mode=${2:-$(warp_mode)} old tmp
  [ -s "$(warp_file)" ] || { tell_warn "请先获取 WARP 出口"; wait_key; return 1; }
  old=$(cat "$(warp_file)")
  if ! json_edit "$(warp_file)" '.enabled=$e|.mode=$m' --argjson e "$enabled" --arg m "$mode"; then tell_warn "WARP 状态保存失败"; return 1; fi
  if apply_config; then
    [ "$enabled" = true ] && tell_ok "WARP 已启用" || tell_ok "WARP 已关闭"
    return 0
  fi
  printf '%s\n' "$old" | json_write "$(warp_file)" >/dev/null 2>&1 || true
  apply_config_quiet
  tell_warn "WARP 配置应用失败，已恢复原状态"
  return 1
}

warp_change_ip(){
  local file=$(warp_file) id token mode tmp
  [ -s "$file" ] || { tell_warn "尚未获取 WARP 数据"; wait_key; return 1; }
  mode=$(warp_mode)
  if warp_enabled; then
    warp_set_enabled false || { wait_key; return 1; }
  fi
  id=$(jq -r '.id // empty' "$file")
  token=$(jq -r '.token // empty' "$file")
  if [ -n "$id" ] && [ -n "$token" ]; then
    curl -fsS --connect-timeout 5 --max-time 10 -X DELETE \
      -H "User-Agent: okhttp/3.12.1" \
      -H "CF-Client-Version: a-6.10-2158" \
      -H "Authorization: Bearer $token" \
      "https://api.cloudflareclient.com/v0a2158/reg/$id" >/dev/null 2>&1 || { tell_warn "旧 WARP 注册删除失败，未更换 IP"; wait_key; return 1; }
  fi
  rm -f "$file"
  warp_register || { tell_warn "新 WARP 出口获取失败"; wait_key; return 1; }
  tmp=$(mktemp) || { tell_warn "WARP 模式保存失败"; wait_key; return 1; }
  jq --arg mode "$mode" '.mode=$mode|.enabled=false|del(.public_ipv4,.public_ipv6)' "$file" >"$tmp" && install -m600 "$tmp" "$file" || { rm -f "$tmp"; tell_warn "WARP 模式保存失败"; wait_key; return 1; }
  rm -f "$tmp"
  screen_clear
  tell "正在更换 WARP IP..."
  case "$mode" in
    4) if v=$(warp_probe 4); then tell_ok "WARP IPv4：$v"; else tell_warn "WARP IPv4 出口获取失败"; fi ;;
    6) if v=$(warp_probe 6); then tell_ok "WARP IPv6：$v"; else tell_warn "WARP IPv6 出口获取失败"; fi ;;
    dual) if v=$(warp_probe dual); then printf '%s\n' "$v" | awk -F'|' '{printf "  WARP IPv4：%s\n  WARP IPv6：%s\n",$1,$2}'; else tell_warn "WARP 双栈出口获取失败"; fi ;;
  esac
  wait_key
}
...
menu_warp(){
  while :; do
    screen_clear; tell "${BRIGHT_CYAN}${BOLD}>> WARP <<${PLAIN}"
    if [ -s "$(warp_file)" ]; then
      if warp_enabled; then tell "  状态：${GREEN}已启用${PLAIN}"; else tell "  状态：${YELLOW}已关闭${PLAIN}"; fi
    else tell "  状态：${DIM}未配置${PLAIN}"; fi
    tell ""; tell "  1. 获取出口"; tell "  2. 启用 / 关闭"; tell "  3. WARP 信息"; tell "  4. 更换 IP"; tell "  5. 清除数据"; tell "  0. 返回"
    tell "${CYAN}--------------------------------${PLAIN}"
    case $(prompt "请选择") in
      1)
        tell ""; tell "  >> 获取 WARP 出口 <<"; tell ""; tell "  1. IPv4 出口"; tell "  2. IPv6 出口"; tell "  3. 双栈出口"; tell "  0. 返回"
        case $(prompt "请选择") in
          1) warp_register_if_needed=0; [ -s "$(warp_file)" ] || warp_register || { wait_key; continue; }; screen_clear; tell "正在获取 WARP IPv4 出口..."; if v=$(warp_probe 4); then tell_ok "WARP IPv4：$v"; else tell_warn "WARP IPv4 出口获取失败"; fi; wait_key ;;
          2) warp_register_if_needed=0; [ -s "$(warp_file)" ] || warp_register || { wait_key; continue; }; screen_clear; tell "正在获取 WARP IPv6 出口..."; if v=$(warp_probe 6); then tell_ok "WARP IPv6：$v"; else tell_warn "WARP IPv6 出口获取失败"; fi; wait_key ;;
          3) warp_register_if_needed=0; [ -s "$(warp_file)" ] || warp_register || { wait_key; continue; }; screen_clear; tell "正在获取 WARP 双栈出口..."; if v=$(warp_probe dual); then printf '%s\n' "$v" | awk -F'|' '{printf "  WARP IPv4：%s\n  WARP IPv6：%s\n",$1,$2}'; else tell_warn "WARP 双栈出口获取失败"; fi; wait_key ;;
          0) ;;
          *) tell_warn "输入无效"; sleep 1 ;;
        esac ;;
      2)
        if warp_enabled; then warp_set_enabled false; else warp_set_enabled true "$(warp_mode)"; fi; wait_key ;;
      3)
        screen_clear; tell "${BRIGHT_CYAN}${BOLD}>> WARP 信息 <<${PLAIN}"; if [ -s "$(warp_file)" ]; then
          tell "  状态：$([ "$(warp_enabled && echo 1 || echo 0)" = 1 ] && printf "%s" "${GREEN}已启用${PLAIN}" || printf "%s" "${YELLOW}已关闭${PLAIN}")"
          tell "  模式：$(warp_mode_label)"; tell "  出口 IPv4：$(jq -r '.public_ipv4 // "-"' "$(warp_file)")"; tell "  出口 IPv6：$(jq -r '.public_ipv6 // "-"' "$(warp_file)")"; tell "  WARP 地址 IPv4：$(jq -r '.config.interface.addresses.v4 // "-"' "$(warp_file)")"; tell "  WARP 地址 IPv6：$(jq -r '.config.interface.addresses.v6 // "-"' "$(warp_file)")"; tell "  连接端点：$(jq -r '.config.peers[0].endpoint.host // "-"' "$(warp_file)")"; tell "  客户端 ID：$(jq -r '.config.client_id // "-"' "$(warp_file)")"
        else tell "  未配置 WARP"; fi; wait_key ;;
      4) warp_change_ip ;;
      5)
        if [ -s "$(warp_file)" ]; then
          prompt_yes "确认清除 WARP 数据" || continue
          warp_set_enabled false >/dev/null 2>&1 || true
          id=$(jq -r '.id // empty' "$(warp_file)"); token=$(jq -r '.token // empty' "$(warp_file)")
          [ -n "$id" ] && [ -n "$token" ] && curl -fsS --connect-timeout 5 --max-time 10 -X DELETE -H "User-Agent: okhttp/3.12.1" -H "CF-Client-Version: a-6.10-2158" -H "Authorization: Bearer $token" "https://api.cloudflareclient.com/v0a2158/reg/$id" >/dev/null 2>&1 || true
          rm -f "$(warp_file)"; apply_config_quiet; tell_ok "WARP 数据已清除"; wait_key
        else tell "  未配置 WARP"; wait_key; fi ;;
      0) break ;;
      *) tell_warn "输入无效"; sleep 1 ;;
    esac
  done
...
init_dirs(){
  mkdir -p "$SBM_DIR/tmp" "$NODE_DIR" "$PEER_DIR" "$ACME_DIR" "$SB_DIR" /var/lib/sing-box /usr/local/bin
  chmod 700 "$SBM_DIR" "$NODE_DIR" "$PEER_DIR" "$ACME_DIR" "$SB_DIR" /var/lib/sing-box
  rm -rf "$SBM_DIR/wg"
  [ -f "$STATE" ] || printf '%s\n' '{"exit":"direct","domain":"","email":"","challenge":"http","asset":""}' | json_write "$STATE"
  _exit=$(state_get exit 2>/dev/null); [ "$_exit" = direct ] || [ -f "$PEER_DIR/$_exit.json" ] || state_set exit direct
  [ -f "$PKG_LOG" ] || : >"$PKG_LOG"
}
...
menu_server_info(){
  ...
  get_ip_info 4 >/dev/null 2>&1 && [ -n "$IP_INFO_IP" ] && tell "本机IPv4：${GREEN}${IP_INFO_IP}${PLAIN}" || tell "本机IPv4：${RED}无或不可用${PLAIN}"
  get_ip_info 6 >/dev/null 2>&1 && [ -n "$IP_INFO_IP" ] && tell "本机IPv6：${GREEN}${IP_INFO_IP}${PLAIN}" || tell "本机IPv6：${RED}无或不可用${PLAIN}"
...
get_ip_info(){
  local mode=$1 selected=${2:-} port tmp_conf tmp_log pid res ip country asn name url family_url i outbound
  ...
  if [ "$selected" = warp ]; then
    ...
    mkdir -p "$SBM_DIR/tmp" 2>/dev/null || return 1
    local endpoint port tmp_conf tmp_log pid i
    endpoint=$(warp_endpoint_json "$(warp_mode)") || return 1
    port=$((20000 + $(od -An -N2 -tu2 /dev/urandom 2>/dev/null | tr -d ' ') % 20000)); [ "$port" -ge 20000 ] 2>/dev/null || port=20873
    tmp_conf=$(mktemp "$SBM_DIR/tmp/status-warp.XXXXXX") || return 1
    tmp_log=$(mktemp "$SBM_DIR/tmp/status-warp-log.XXXXXX") || { rm -f "$tmp_conf"; return 1; }
    jq -n --argjson port "$port" --argjson endpoint "$endpoint" '{log:{level:"warn"},inbounds:[{type:"mixed",tag:"status-in",listen:"127.0.0.1",listen_port:$port}],outbounds:[{type:"direct",tag:"direct"}],endpoints:[$endpoint],route:{rules:[{inbound:["status-in"],action:"route",outbound:"cf-warp"}],final:"direct",auto_detect_interface:true}}' >"$tmp_conf" 2>/dev/null || { rm -f "$tmp_conf" "$tmp_log"; return 1; }
    "$CORE" check -c "$tmp_conf" >/dev/null 2>&1 || { rm -f "$tmp_conf" "$tmp_log"; return 1; }
    "$CORE" run -c "$tmp_conf" >"$tmp_log" 2>&1 & pid=$!; i=0
    while [ "$i" -lt 10 ]; do kill -0 "$pid" 2>/dev/null || break; res=$(env -u http_proxy -u https_proxy -u HTTP_PROXY -u HTTPS_PROXY -u all_proxy -u ALL_PROXY curl -sS -"$mode" -m 3 --socks5-hostname "127.0.0.1:$port" "$family_url" 2>/dev/null) && break; i=$((i+1)); sleep .3; done
    kill "$pid" 2>/dev/null || true; i=0; while kill -0 "$pid" 2>/dev/null && [ "$i" -lt 10 ]; do i=$((i+1)); sleep .2; done; kill -9 "$pid" 2>/dev/null || true; wait "$pid" 2>/dev/null || true; rm -f "$tmp_conf" "$tmp_log"
  elif [ -n "$selected" ] && [ "$selected" != direct ]; then
    [ -s "$CONFIG" ] || return 1
    ...
    tmp_log=$(mktemp "$SBM_DIR/tmp/status-proxy-log.XXXXXX") || { rm -f "$tmp_conf"; return 1; }
      [ -s "$PEER_DIR/$selected.json" ] || { rm -f "$tmp_conf" "$tmp_log"; return 1; }
        outbound=$(jq -c --arg tag "$selected" '.outbound | .tag=$tag | .domain_resolver="dns-direct-v4"' "$PEER_DIR/$selected.json" 2>/dev/null) || { rm -f "$tmp_conf" "$tmp_log"; return 1; }
    ...
    while [ "$i" -lt 4 ]; do
      ...
      res=$(env -u http_proxy -u https_proxy -u HTTP_PROXY -u HTTPS_PROXY -u all_proxy -u ALL_PROXY \
        curl -sS --socks5-hostname "127.0.0.1:$port" -m 2 "$family_url" 2>/dev/null) && break
      i=$((i+1)); sleep 0.25
    done
    kill "$pid" 2>/dev/null || true
    i=0
  ...
  if command -v curl >/dev/null 2>&1; then
    res=$(env -u http_proxy -u https_proxy -u HTTP_PROXY -u HTTPS_PROXY -u all_proxy -u ALL_PROXY \
      curl -sS -m 2 "https://ipwho.is/$ip" 2>/dev/null)
  elif command -v wget >/dev/null 2>&1; then
    res=$(wget -qO- -T 2 "https://ipwho.is/$ip" 2>/dev/null)
  else
    res=""
  fi
...
render_client_ip_status(){
  ...
  if warp_enabled; then exit_node=warp; fi
...
render_status_ip(){
  ...
  if warp_enabled; then mode="WARP"; exit_node=warp; fi
...
menu_status(){
  while :; do
    ...
    if warp_enabled; then tell "  WARP出口: ${GREEN}已启用${PLAIN}"; else [ -s "$(warp_file)" ] && tell "  WARP出口: ${YELLOW}已关闭${PLAIN}" || tell "  WARP出口: ${DIM}未配置${PLAIN}"; fi
    tell ""
    render_status_ip
    tell ""
    tell "  1. 更新内核"
...
default_iface(){
  ...
  case "$iface" in "$TUN_IF") iface="" ;; esac
  [ -n "$iface" ] || iface=$(ip -"$family" route show table main default 2>/dev/null | awk -v t="$TUN_IF" '$5!=t {print $5; exit}')
  printf '%s' "$iface"
}
...
resolve_addresses(){
  ...
  if command -v curl >/dev/null 2>&1 && command -v jq >/dev/null 2>&1; then
    encoded=$(uri_encode "$host") || return 1
    if [ -n "${NET_IF_V4:-}" ]; then
      ...
      out=$(env -u http_proxy -u https_proxy -u HTTP_PROXY -u HTTPS_PROXY -u all_proxy -u ALL_PROXY curl -4 -fsS --connect-timeout 2 -m 4 --resolve cloudflare-dns.com:443:1.1.1.1 -H 'accept: application/dns-json' "https://cloudflare-dns.com/dns-query?name=$encoded&type=A" 2>/dev/null)
    fi
    printf '%s\n' "$out" | jq -r '.Answer[]?.data // empty' 2>/dev/null
    if [ -n "${NET_IF_V6:-}" ] && ipv6_egress_available; then
  ...
  if command -v getent >/dev/null 2>&1; then
    { getent ahostsv4 "$host"; getent ahostsv6 "$host"; } 2>/dev/null | awk '{print $1}' | sort -u
  elif command -v nslookup >/dev/null 2>&1; then
    nslookup "$host" 2>/dev/null | awk '/^Address/{print $NF}' | sort -u
  fi
}
...
latency_route_iface(){
  ...
  iface=$(ip -"$family" route get "$target" 2>/dev/null | awk '{for(i=1;i<=NF;i++) if($i=="dev"){print $(i+1); exit}}')
  case "$iface" in
    ""|"$TUN_IF") iface=$(default_iface "$family") ;;
  esac
  case "$iface" in
    ""|"$TUN_IF") return 1 ;;
  esac
...
latency_resolve_addresses(){
  local host=$1 encoded iface out
  case "$host" in
    *:*) printf '%s\n' "$host"; return 0 ;;
    *[!0-9.]*) ;;
    *) printf '%s\n' "$host"; return 0 ;;
  ...
  encoded=$(uri_encode "$host") || return 1
  iface=$(default_iface 6)
  if [ -n "$iface" ]; then
    out=$(env -u http_proxy -u https_proxy -u HTTP_PROXY -u HTTPS_PROXY -u all_proxy -u ALL_PROXY curl -6 -fsS --connect-timeout 2 -m 4 --interface "$iface" --resolve 'cloudflare-dns.com:443:[2606:4700:4700::1111]' -H 'accept: application/dns-json' "https://cloudflare-dns.com/dns-query?name=$encoded&type=AAAA" 2>/dev/null)
    printf '%s\n' "$out" | jq -r '.Answer[]?.data // empty' 2>/dev/null
  fi
  iface=$(default_iface 4)
  if [ -n "$iface" ]; then
    out=$(env -u http_proxy -u https_proxy -u HTTP_PROXY -u HTTPS_PROXY -u all_proxy -u ALL_PROXY curl -4 -fsS --connect-timeout 2 -m 4 --interface "$iface" --resolve cloudflare-dns.com:443:1.1.1.1 -H 'accept: application/dns-json' "https://cloudflare-dns.com/dns-query?name=$encoded&type=A" 2>/dev/null)
    printf '%s\n' "$out" | jq -r '.Answer[]?.data // empty' 2>/dev/null
  fi
}

probe_peer_latency(){
  local file=$1 result=$2 latency_file=${3:-} server targets target iface out ms
  server=$(jq -r '.outbound.server // ""' "$file" 2>/dev/null) || { echo fail >"$result"; return 1; }
  [ -n "$server" ] || { echo fail >"$result"; return 1; }
  case "$server" in
    *:*) targets=$server ;;
    *[!0-9.]*) targets=$(latency_resolve_addresses "$server") ;;
    *) targets=$server ;;
  esac
  [ -n "$targets" ] || { echo fail >"$result"; return 1; }
  while IFS= read -r target; do
    [ -n "$target" ] || continue
    iface=$(latency_route_iface "$target") || continue
    case "$target" in
      *:*) out=$(timeout 3 ping -6 -I "$iface" -c 1 -W 2 "$target" 2>/dev/null) ;;
      *.*) out=$(timeout 3 ping -4 -I "$iface" -c 1 -W 2 "$target" 2>/dev/null) ;;
      *) out="" ;;
    esac
    if [ $? -eq 0 ]; then
...
protected_ports(){
  { ssh_ports; node_ports; } | grep -E '^[1-9][0-9]*$' | sort -un
}

build_config(){
  local selected domain listen_addr dns_direct_tag dns_strategy final use_tun auto_detect
  local inbounds outbounds rules dns_block providers='[]' tls_extra='{}' peer_host endpoints='[]'
  local f acme_json pending_json target_resolver ssh_rule_ports warp_final
  selected=${BUILD_SELECTED_OVERRIDE:-$(state_get exit)}
  domain=${BUILD_DOMAIN_OVERRIDE:-$(state_get domain)}
  listen_addr='0.0.0.0'
  ipv6_listen_available && listen_addr='::'
  final=direct; use_tun=0; peer_host=''; inbounds='[]'; outbounds='[{"type":"direct","tag":"direct"}]'
  dns_direct_tag=dns-direct-v4; dns_strategy=prefer_ipv4
  ipv6_egress_available && { dns_direct_tag=dns-direct-v6; dns_strategy=prefer_ipv6; }
  ...
  if [ -n "$ACME_PENDING_DOMAIN" ] && [ "$ACME_PENDING_DOMAIN" != "$domain" ]; then
    ...
    providers=$(printf '%s\n' "$providers" | jq -c --arg tag "${CERT_TAG}-pending" --argjson options "$pending_json" '. + [$options+{type:"acme",tag:$tag}]') || return 1
  fi
  ...
  if [ -e "$1" ]; then
    inbounds=$(for f do
      [ -e "$f" ] || continue
      if [ -n "${NODE_OVERRIDE_TARGET:-}" ] && [ "$f" = "$NODE_OVERRIDE_TARGET" ] && [ -f "${NODE_OVERRIDE_FILE:-}" ]; then cat "$NODE_OVERRIDE_FILE"; else cat "$f"; fi
    done | jq -s --arg d "$domain" --arg listen "$listen_addr" --argjson x "$tls_extra" '
      [ .[] |
        (if .kind=="hysteria2" and ((.inbound.up_mbps//0)>0 or (.inbound.down_mbps//0)>0) then .meta=(.meta|del(.bbr_profile)) else . end) |
        .tls_mode as $tls_mode | .alpn as $alpn | .inbound as $in |
        ($in | if .type=="hysteria2" and ((.up_mbps//0)>0 or (.down_mbps//0)>0) then del(.bbr_profile) else . end) as $in2 |
        .inbound = ($in2 | .listen=$listen |
          if $tls_mode=="acme" then .tls=({enabled:true} + (if $d!="" then {server_name:$d} else {} end) + (if $alpn then {alpn:$alpn} else {} end) + $x) else . end) ]') || return 1
  fi

  if warp_enabled; then
    endpoints=$(warp_endpoint_json "$(warp_mode)") || return 1
    endpoints="[$endpoints]"
    final=cf-warp
    peer_host=""
  elif [ "$selected" != direct ] && [ -f "$PEER_DIR/$selected.json" ]; then
    outbounds=$(jq -n --argjson base "$outbounds" --slurpfile peer "$PEER_DIR/$selected.json" --arg resolver "$dns_direct_tag" --arg tag "$selected" '$base + [($peer[0].outbound + {tag:$tag,domain_resolver:$resolver})]') || return 1
    final=$selected; peer_host=$(jq -r '.outbound.server//""' "$PEER_DIR/$selected.json")
    transparent_proxy_available && use_tun=1
  fi

  if [ "$use_tun" = 1 ]; then
    inbounds=$(jq -n --argjson list "$inbounds" --arg name "$TUN_IF" '[{type:"tun",tag:"tun-in",interface_name:$name,address:["172.19.0.1/30","fdfe:dcba:9876::1/126"],dns_mode:"hijack",dns_address:["172.19.0.2","fdfe:dcba:9876::2"],auto_route:true,auto_redirect:true,strict_route:true,mtu:1500}] + $list') || return 1
  fi

  ssh_rule_ports=$(ssh_ports | jq -Rsc 'split("\n") | map(select(length > 0) | tonumber)') || return 1
  rules=$(jq -n --arg host "$peer_host" --argjson ssh_ports "$ssh_rule_ports" '
    (if ($ssh_ports|length)>0 then [{network:"tcp",port:$ssh_ports,action:"route",outbound:"direct"}] else [] end)
    + [{network:"icmp",action:"route",outbound:"direct"},{action:"sniff"},{protocol:"dns",action:"hijack-dns"}]
    + (if $host=="" then [] elif ($host|test("^[0-9]+\\.[0-9]+\\.[0-9]+\\.[0-9]+$")) then [{ip_cidr:[($host+"/32")],action:"route",outbound:"direct"}] elif ($host|test("^[0-9a-fA-F:]+$")) then [{ip_cidr:[($host+"/128")],action:"route",outbound:"direct"}] else [{domain:[$host],action:"route",outbound:"direct"}] end)') || return 1

  auto_detect=false; [ "$use_tun" = 1 ] && auto_detect=true
  dns_block=$(jq -n --arg strategy "$dns_strategy" --arg final "$final" --arg direct "$dns_direct_tag" '
    {servers:[
      ...
      {type:"https",tag:"dns-remote",server:"cloudflare-dns.com",server_port:443,path:"/dns-query",tls:{enabled:true,server_name:"cloudflare-dns.com"},domain_resolver:$direct}]}
     | .strategy=$strategy | .final="dns-remote"') || return 1
  target_resolver=$dns_direct_tag
  [ "$final" = direct ] || target_resolver=dns-direct-v4
  jq -n --argjson inbounds "$inbounds" --argjson outbounds "$outbounds" --argjson rules "$rules" --argjson dns "$dns_block" --argjson providers "$providers" --argjson endpoints "$endpoints" --arg final "$final" --arg resolver "$target_resolver" --argjson auto_detect "$auto_detect" '
    {log:{level:"warn",timestamp:true},dns:$dns,inbounds:$inbounds,outbounds:$outbounds,endpoints:$endpoints,route:{rules:$rules,final:$final,default_domain_resolver:$resolver,auto_detect_interface:$auto_detect}}
...
remote_version(){
  curl -fsSL --connect-timeout 5 --max-time 10 "https://api.github.com/repos/SagerNet/sing-box/releases/latest" 2>/dev/null | jq -r '.tag_name // empty' | sed 's/^v//'
}

is_lxc(){
  grep -qaE '(^|/)(lxc|liblxc)(/|$)' /proc/1/cgroup /proc/1/mountinfo 2>/dev/null && return 0
...
install_core(){
  ...
  if curl -fsSL --connect-timeout 5 --max-time 120 "$url" 2>/dev/null | tar -xzf - -C "$tmpdir" 2>/dev/null; then
...
check_dependencies(){
  local cmd pkg
  local deps=(
    "curl:curl" "tar:tar" "jq:jq" "openssl:openssl"
...
start_net_monitor(){
  ...
  (
    ...
    while :; do
      ...
      ip monitor link route address 2>/dev/null | while IFS= read -r line; do
        case "$line" in
          *" lo "*|*"lo:"*|*"docker"*|*"veth"*|*"br-"*|*"virbr"*|*"$TUN_IF"*|*"tailscale"*|*"zt"*) continue ;;
        esac
...
sync_hopping_rules(){
  ...
  nft add rule inet "$HOP_TABLE" prerouting iifname != "lo" iifname != "$TUN_IF" udp dport "$range" redirect to :"$port" 2>/dev/null
}
...
4)
  ...
  hopping=""
  if ! is_lxc && prompt_yes "是否配置并开启端口跳跃防封锁机制"; then
...
render_share_uri(){
  ...
  if [ -n "$(current_public_ipv4)" ]; then
    uri=$(build_uri "$kind" "$name" "$port" "$meta" "$hopping" "$(current_public_ipv4)")
    tell "${GREEN}$uri${PLAIN}"
  fi
  if [ -n "$(current_public_ipv6)" ]; then
    uri=$(build_uri "$kind" "$name" "$port" "$meta" "$hopping" "[$(current_public_ipv6)]")
    tell "${GREEN}$uri${PLAIN}"
  fi
  if [ -z "$(current_public_ipv4)$(current_public_ipv6)" ]; then tell_warn "未识别到本机公网 IP，生成链接失败"; return; fi
  [ -n "$hopping" ] && tell "跳跃配置: $hopping"
