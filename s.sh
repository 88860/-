...
probe_network_stack_async(){
  (
    ...
    printf '%s\n%s\n%s\n%s\n%s\n' \
      ...
      > "$tmp" && mv -f "$tmp" "$NET_CACHE" || rm -f "$tmp"
...
self_ips(){
  ...
  local pub4 pub6
  pub4=$(curl -4 -s -m 3 --noproxy '*' https://ipv4.icanhazip.com 2>/dev/null | tr -d '\n ')
  pub6=$(curl -6 -s -m 3 --noproxy '*' https://ipv6.icanhazip.com 2>/dev/null | tr -d '\n ')
  [ -n "$pub4" ] && echo "$pub4"
  [ -n "$pub6" ] && echo "$pub6"
}
