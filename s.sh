...
build_config(){
  ...
  if [ -f "$WG_CONF" ] && [ "$(jq -r '.enabled//false' "$WG_CONF")" = true ]; then
    if [ "$(jq -r .role "$WG_CONF")" = client ]; then
      ...
      local wg_out
      wg_out=$(jq -n --arg name "$WG_IF" --slurpfile cfg "$WG_CONF" '
        $cfg[0] | {
          ...
          system: true, interface_name: $name, mtu: 1408
        }
      ')
      outbounds=$(jq -n --argjson base "$outbounds" --argjson wg "$wg_out" '$base + [$wg]')
...
get_ip_info(){
  ...
  res=$(curl -s -${mode} -m 5 https://ifconfig.co/json 2>/dev/null)
  ip=$(jq -r '.ip // empty' <<<"$res" 2>/dev/null)
  if [ -n "$ip" ]; then
    country=$(jq -r '.country_iso // empty' <<<"$res" 2>/dev/null)
    asn=$(jq -r '.asn // empty' <<<"$res" 2>/dev/null)
    name=$(jq -r '.asn_org // empty' <<<"$res" 2>/dev/null)
    IP_INFO_IP="$ip"
    IP_INFO_C="$country"
    IP_INFO_ASN="$asn"
    IP_INFO_NAME="$name"
...
wg_fill_peer_key(){
  ...
  if [ "$role" = server ]; then
    ...
    json_edit "$WG_CONF" '.peer_public_key=$k' --arg k "$key" \
      || { tell_warn "覆写失败"; wait_key; return; }
  fi
