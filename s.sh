...
config_build_tun_inbounds(){
  local inbounds=$1 name=$TUN_IF mode=${2:-dual} ssh_exclude=${3:-[]} route='[]' exclude='[]'
  case "$mode" in
  ...
  # Preserve the current management peer by destination address, never by SSH port.
  # This is only a narrow exception for the existing control connection.
  if [ -n "$SSH_CONNECTION" ]; then
    ...
    [ -n "$ssh_cidr" ] && exclude=$(jq -c --arg x "$ssh_cidr" '. + [$x] | unique' <<<"$exclude") || true
  fi
...
config_build_rules(){
  local peer_host=$1 mode=${2:-} selected=${3:-direct} ssh_rule_ports ssh_client_ips
  ...
  # WARP native-family preservation is handled by route_exclude_address at the TUN layer.
  # There is deliberately no SSH port rule and no family-specific forced Direct rule here.
...
build_config(){
  ...
  if [ "$use_tun" = 1 ]; then
    local ssh_peer_exclude='[]'
    if [ "$selected" = warp ] && [ -n "$SSH_CONNECTION" ]; then
      ssh_peer_exclude=$(printf '%s\n' "$SSH_CONNECTION" | awk '{print $1}' | awk '''NF{if ($0 ~ /:/) print $0"/128"; else print $0"/32"}''' | jq -Rsc '''split("\n")|map(select(length>0))''') || return 1
    fi
    if [ "$selected" = warp ]; then
      inbounds=$(config_build_tun_inbounds "$inbounds" "$(warp_mode_get)" "$ssh_peer_exclude") || return 1
    else
