...
build_config(){
  ...
  local inbounds outbounds rules dns_block providers='[]' tls_extra='{}' peer_host
  local f acme_json pending_json target_resolver ssh_rule_ports
  ...
  if [ "$selected" != direct ] && [ -f "$PEER_DIR/$selected.json" ]; then
  ...
  jq -n --argjson inbounds "$inbounds" --argjson outbounds "$outbounds" --argjson rules "$rules" --argjson dns "$dns_block" --argjson providers "$providers" --arg final "$final" --arg resolver "$target_resolver" --argjson auto_detect "$auto_detect" '
