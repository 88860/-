...
wg_latest_handshake(){
  ...
  command -v wg >/dev/null 2>&1 || { printf '%s' ""; return; }
  ts=$(wg show "$WG_IF" latest-handshakes 2>/dev/null | awk -v k="$pub" '$1==k{print $2;exit}')
...
local current4 current6 value
  ...
  while :; do
    ...
    [[ "$value" == 0 || "$value" != *:*/* ]] || { tell_warn "IPv4 地址无效"; continue; }
    WG_EDIT_IPV4="$value"
    break
  done
  while :; do
...
wg_status(){
  ...
  case "$role" in
    client)
      local state ms color handshake
      ...
      if [[ "$state" == ok\|* ]] && [[ "$ms" =~ ^[0-9]+$ ]]; then
        ...
        tell "服务器延迟：${RED}超时${PLAIN}"
      fi
      tell "服务器公钥：$peer_key"
      handshake=$(wg_latest_handshake "$peer_key")
      tell "最近握手：${handshake:-无}"
      tell "本机公钥：$public"
      tell ""
      tell "Endpoint："
      ...
      ;;
    server)
      local peer_count peer_index peer_name peer_ipv4 peer_ipv6 peer_pub handshake
      tell "${CYAN}========== WireGuard 状态信息 ==========${PLAIN}"
      if [ "$enabled" = true ] && [ -d "/sys/class/net/$WG_IF" ]; then
      ...
      actual_port=$(wg_actual_listen_port)
      ...
      for ((peer_index=0; peer_index<peer_count; peer_index++)); do
        ...
        tell "IPv6：$peer_ipv6"
        tell "公钥：$peer_pub"
        handshake=$(wg_latest_handshake "$peer_pub")
        tell "最近握手：${handshake:-无}"
      done
      tell ""
      tell "${CYAN}========================================${PLAIN}"
...
wg_edit_address(){
  ...
  echo ""
  wg_prompt_address
  [ "$WG_EDIT_IPV4" = 0 ] && [ "$WG_EDIT_IPV6" = 0 ] && { tell_warn "IPv4 和 IPv6 不能同时禁用"; wait_key; return; }
  body=$(jq --arg a4 "$WG_EDIT_IPV4" --arg a6 "$WG_EDIT_IPV6" '.address=([$a4,$a6]|map(select(. != "0")))|.endpoint.address=([$a4,$a6]|map(select(. != "0")))' "$WG_CONF") || { tell_warn "地址生成失败"; wait_key; return; }
...
wg_client_add(){
  ...
  while :; do
    ipv4=$(prompt "隧道 IPv4")
    [ "$ipv4" = 0 ] || { wg_valid_address "$ipv4" && [[ "$ipv4" != *:*/* ]]; } && break
    tell_warn "IPv4 地址无效"
  done
  while :; do
    ipv6=$(prompt "隧道 IPv6")
    [ "$ipv6" = 0 ] || { wg_valid_address "$ipv6" && [[ "$ipv6" == *:*/* ]]; } && break
    tell_warn "IPv6 地址无效"
  done
  [ "$ipv4" = 0 ] && [ "$ipv6" = 0 ] && { tell_warn "IPv4 和 IPv6 不能同时禁用"; wait_key; return; }
  while :; do
    pub=$(prompt "客户端公钥")
  ...
  body=$(jq --arg name "$name" --arg ipv4 "$ipv4" --arg ipv6 "$ipv6" --arg pub "$pub" \
    '.clients += [{name:$name,ipv4:$ipv4,ipv6:$ipv6,public_key:$pub}] |
     .endpoint.peers += [{public_key:$pub,allowed_ips:([$ipv4,$ipv6]|map(select(. != "0")))}]' "$WG_CONF") || { tell_warn "配置生成失败"; wait_key; return; }
...
wg_client_edit(){
  ...
  { [ "$ipv4" = 0 ] || { wg_valid_address "$ipv4" && [[ "$ipv4" != *:*/* ]]; }; } || { tell_warn "IPv4 地址无效"; wait_key; return; }
  ipv6=$(prompt "隧道 IPv6" "$(jq -r ".clients[$index].ipv6 // \"\"" "$WG_CONF")")
  { [ "$ipv6" = 0 ] || { wg_valid_address "$ipv6" && [[ "$ipv6" == *:*/* ]]; }; } || { tell_warn "IPv6 地址无效"; wait_key; return; }
  [ "$ipv4" = 0 ] && [ "$ipv6" = 0 ] && { tell_warn "IPv4 和 IPv6 不能同时禁用"; wait_key; return; }
  ...
  body=$(jq --argjson i "$index" --arg name "$name" --arg ipv4 "$ipv4" --arg ipv6 "$ipv6" --arg pub "$pub" \
    '.clients[$i]={name:$name,ipv4:$ipv4,ipv6:$ipv6,public_key:$pub} |
     .endpoint.peers[$i]={public_key:$pub,allowed_ips:([$ipv4,$ipv6]|map(select(. != "0")))}' "$WG_CONF") || { tell_warn "配置生成失败"; wait_key; return; }
  wg_server_client_apply "$body" || { wait_key; return; }
  tell_ok "客户端已修改"
  wait_key
}
