...
EOF
  ...
  local low=0 high=36500 mid seconds
  while [ "$low" -lt "$high" ]; do
    mid=$(((low + high + 1) / 2))
    seconds=$((mid * 86400))
    if openssl x509 -in "$crt" -noout -checkend "$seconds" >/dev/null 2>&1; then
      low=$mid
    else
      high=$((mid - 1))
    fi
...
wg_tunnel_state(){
  local file=$WG_CONF enabled peer_ip4 peer_ip6 out ms
  enabled=$(jq -r '.enabled // false' "$file" 2>/dev/null)
  [ "$enabled" = true ] || { echo disconnected; return; }
  if [ "$(jq -r '.endpoint.system // false' "$file" 2>/dev/null)" = true ] && [ -d "/sys/class/net/$WG_IF" ]; then
    peer_ip4=$(jq -r '.peer_ip // empty' "$file" 2>/dev/null); peer_ip6=$(jq -r '.peer_ip6 // empty' "$file" 2>/dev/null)
    if [ -n "$peer_ip6" ]; then
      out=$(ping -6 -I "$WG_IF" -c 1 -W 2 "$peer_ip6" 2>/dev/null)
      if [ $? -eq 0 ]; then ms=$(printf '%s\n' "$out" | awk -F'time=' '/time=/{gsub(/ ms.*/,"",$2); printf "%.0f\n",$2; exit}'); printf '%s' "$ms" | grep -Eq '^[0-9]+$' && { printf 'ok|%s\n' "$ms"; return; }; fi
    fi
    if [ -n "$peer_ip4" ]; then
  ...
  get_ip_info 4 wireguard >/dev/null 2>&1 && echo connected || echo disconnected
}
    ...
    client)
      ...
      case "$state" in
        ok\|*)
          ...
          else tell "服务器状态：${GREEN}已连接${PLAIN}"; fi
          ;;
        connected) tell "服务器状态：${GREEN}已连接${PLAIN}" ;;
        *) tell "服务器状态：${RED}未连接${PLAIN}" ;;
      esac
      tell "服务器公钥：$peer_key"
      tell "本机公钥：$public"
      tell ""
      endpoint_ips=$(resolve_addresses "$peer_host" 2>/dev/null || true)
      tell "外层IPv4|$(if printf "%s\n" "$endpoint_ips" | grep -Eq '^[0-9]+(\.[0-9]+){3}$'; then printf '%s正常%s' "$GREEN" "$PLAIN"; else printf '%s不可用%s' "$RED" "$PLAIN"; fi)      隧道IPv4|$(if [ -n "$ipv4" ] && [ "$enabled" = true ] && true; then printf '%s正常%s' "$GREEN" "$PLAIN"; else printf '%s不可用%s' "$RED" "$PLAIN"; fi)"
      tell "外层IPv6|$(if printf "%s\n" "$endpoint_ips" | grep -q ':'; then printf '%s正常%s' "$GREEN" "$PLAIN"; else printf '%s不可用%s' "$RED" "$PLAIN"; fi)      隧道IPv6|$(if [ -n "$ipv6" ] && [ "$enabled" = true ] && true; then printf '%s正常%s' "$GREEN" "$PLAIN"; else printf '%s不可用%s' "$RED" "$PLAIN"; fi)"
      tell ""
      tell "${CYAN}----------------------------------------${PLAIN}"
      ;;
    server)
      ...
      if [ "$role" = client ] && [ "$enabled" = true ]; then
        ...
        case "$state" in
          ok\|*)
            ...
            else tell "  模式：客户端|${GREEN}已连接${PLAIN}"; fi
            ;;
          connected) tell "  模式：客户端|${GREEN}已连接${PLAIN}" ;;
          *) tell "  模式：客户端|${RED}未连接${PLAIN}" ;;
        esac
      elif [ "$role" = server ]; then
        if [ "$enabled" = true ]; then
          tell "  模式：服务端|${GREEN}已启动${PLAIN}"
        else
          tell "  模式：服务端|${RED}已停止${PLAIN}"
        fi
      else
        if [ "$enabled" = true ]; then
          tell "  模式：客户端|${GREEN}已启动${PLAIN}"
        else
...
validate_domain(){
  local domain=$1 resolved mode=${2:-$(state_get challenge)}
...
wait_for_certificate(){
  local domain=$1 pid=${2:-} log_file=${3:-} elapsed=0
...
latency_route_iface(){
  ...
  iface=$(ip -"$family" route get "$target" 2>/dev/null | awk '{for(i=1;i<=NF;i++) if($i=="dev"){print $(i+1); exit}}')
  case "$iface" in
    ""|"$TUN_IF"|"$WG_IF") iface=$(default_iface "$family") ;;
  esac
  case "$iface" in
    ""|"$TUN_IF"|"$WG_IF") return 1 ;;
  esac
  printf '%s' "$iface"
}
...
build_config(){
  ...
  if [ "$selected" != direct ] && [ "$selected" != wireguard ] && [ -f "$PEER_DIR/$selected.json" ]; then
    ...
    wgob=$(jq -c --arg resolver "$dns_direct_tag" '.endpoint | .system=false | del(.name) | if (.peers[0].address // "") != "" then .domain_resolver=$resolver else . end' "$wg_source") || return 1
  ...
  if [ "$use_tun" = 1 ]; then
    local tun_redirect=false
    wg_tun_auto_redirect && tun_redirect=true
    inbounds=$(jq -n --argjson list "$inbounds" --arg name "$TUN_IF" --argjson redirect "$tun_redirect" '[{type:"tun",tag:"tun-in",interface_name:$name,address:["172.19.0.1/30","fdfe:dcba:9876::1/126"],dns_mode:"hijack",dns_address:["172.19.0.2","fdfe:dcba:9876::2"],auto_route:true,auto_redirect:$redirect,strict_route:true,mtu:1500}] + $list') || return 1
  fi
  ...
  dns_block=$(jq -n --arg strategy "$dns_strategy" --arg final "$final" --arg direct "$dns_direct_tag" '
    {servers:[
     ...
     | .servers[2] |= (if $final!="direct" then . + {detour:$final} else . end)
...
menu_client_status(){
  ...
  if transparent_proxy_available && ip link show "$TUN_IF" >/dev/null 2>&1; then
    tell "  网络接管：${CYAN}${proxy_name}${PLAIN}"
  elif [ "$exit_node" != direct ]; then
    tell "  网络接管：${CYAN}${proxy_name}${PLAIN}"
  else
    tell "  网络接管：${CYAN}直连${PLAIN}"
  fi
  tell ""
  render_client_ip_status; wait_key
}
# WireGuard core shared by OpenRC and systemd.
is_lxc(){
  if command -v systemd-detect-virt >/dev/null 2>&1 && [ "$(systemd-detect-virt --container 2>/dev/null)" = lxc ]; then return 0; fi
  grep -qaE '(^|/)(lxc|liblxc)(/|$)' /proc/1/cgroup /proc/1/mountinfo 2>/dev/null && return 0
  [ -r /run/systemd/container ] && grep -qx 'lxc' /run/systemd/container 2>/dev/null && return 0
...
wg_tun_capable(){
  ...
  local test_if="sbmtun-test.$$"
  ip tuntap add dev "$test_if" mode tun 2>/dev/null || return 1
  ip link del "$test_if" 2>/dev/null || true
  return 0
}
transparent_proxy_available(){ wg_tun_capable; }
wg_tun_auto_redirect(){ wg_tun_capable && command -v nft >/dev/null 2>&1; }

wg_apply_body(){
  local body=$1 tmp old_conf old_exit had_old=0
  tmp=$(mktemp) || return 1
  old_conf=$(mktemp) || { rm -f "$tmp"; return 1; }
  if [ -f "$WG_CONF" ]; then cp "$WG_CONF" "$old_conf" || { rm -f "$tmp" "$old_conf"; return 1; }; had_old=1; fi
  old_exit=$(state_get exit)
  printf '%s\n' "$body" >"$tmp" || { rm -f "$tmp" "$old_conf"; return 1; }
  WG_OVERRIDE_FILE="$tmp"
  if apply_config; then
    install -m600 "$tmp" "$WG_CONF" || { WG_OVERRIDE_FILE=""; rm -f "$tmp" "$old_conf"; return 1; }
    WG_OVERRIDE_FILE=""
    rm -f "$tmp" "$old_conf"
    return 0
  fi
  WG_OVERRIDE_FILE=""
  if [ "$had_old" = 1 ]; then install -m600 "$old_conf" "$WG_CONF"; else rm -f "$WG_CONF"; fi
  state_set exit "$old_exit" >/dev/null 2>&1 || true
  apply_config_quiet >/dev/null 2>&1 || true
  rm -f "$tmp" "$old_conf"
  return 1
}
...
wg_init_server(){
  ...
  tell_ok "配置完成"; printf '\n  %b配置完成，按回车启用服务端...%b' "${CYAN}" "${PLAIN}" >&2; read_line >/dev/null
...
wg_client_add(){
  ...
  screen_clear; tell "${BRIGHT_CYAN}${BOLD}>> 添加客户端 <<${PLAIN}"; name=$(prompt "客户端名称"); [ -n "$name" ] || { tell_warn "客户端名称不能为空"; wait_key; return; }
  wg_next_client_address || { tell_warn "没有可用的客户端隧道地址"; wait_key; return; }
  while :; do ipv4=$(prompt "隧道 IPv4" "$WG_NEXT_IPV4"); wg_valid_address "$ipv4" && ! printf '%s' "$ipv4" | grep -q ':' || { tell_warn "IPv4 地址无效"; continue; }; ip4=${ipv4%/*}; jq -e --arg ip "$ip4" 'any(.clients[]?; ((.ipv4 // "") | split("/")[0]) == $ip)' "$WG_CONF" >/dev/null 2>&1 && { tell_warn "IPv4 地址已被其他客户端使用"; continue; }; break; done
  while :; do ipv6=$(prompt "隧道 IPv6" "$WG_NEXT_IPV6"); printf '%s' "$ipv6" | grep -q ':' && wg_valid_address "$ipv6" || { tell_warn "IPv6 地址无效"; continue; }; ip6=${ipv6%/*}; jq -e --arg ip "$ip6" 'any(.clients[]?; ((.ipv6 // "") | split("/")[0]) == $ip)' "$WG_CONF" >/dev/null 2>&1 && { tell_warn "IPv6 地址已被其他客户端使用"; continue; }; break; done
  while :; do pub=$(prompt "客户端公钥"); [ -n "$pub" ] || { tell_warn "客户端公钥不能为空"; continue; }; jq -e --arg k "$pub" '.endpoint.peers[]? | select(.public_key==$k)' "$WG_CONF" >/dev/null 2>&1 && { tell_warn "客户端公钥已存在"; continue; }; break; done
  allowed4="${ipv4%/*}/32"; allowed6="${ipv6%/*}/128"
  body=$(jq --arg name "$name" --arg ipv4 "$ipv4" --arg ipv6 "$ipv6" --arg pub "$pub" --arg allowed4 "$allowed4" --arg allowed6 "$allowed6" '.clients += [{name:$name,ipv4:$ipv4,ipv6:$ipv6,public_key:$pub}] | .endpoint.peers += [{public_key:$pub,allowed_ips:[$allowed4,$allowed6]}]' "$WG_CONF") || { tell_warn "配置生成失败"; wait_key; return; }
  wg_apply_body "$body" || { wait_key; return; }; tell_ok "客户端已添加"; wait_key
}

wg_client_edit(){
  local index count name ipv4 ipv6 pub body
  count=$(jq '.endpoint.peers | length' "$WG_CONF" 2>/dev/null); count=${count:-0}; [ "$count" -gt 0 ] || { tell_warn "暂无客户端"; wait_key; return; }
  screen_clear; tell "${BRIGHT_CYAN}${BOLD}>> 修改客户端 <<${PLAIN}"; jq -r '.clients | to_entries[] | "  \(.key+1). \(.value.name) | \(.value.ipv4) | \(.value.ipv6)"' "$WG_CONF" 2>/dev/null; tell "  0. 返回"; index=$(prompt "请选择"); [ "$index" = 0 ] && return
  printf '%s' "$index" | grep -Eq '^[0-9]+$' && [ "$index" -ge 1 ] && [ "$index" -le "$count" ] || { tell_warn "序号无效"; wait_key; return; }; index=$((index-1))
  name=$(prompt "客户端名称" "$(jq -r ".clients[$index].name // \"\"" "$WG_CONF")"); ipv4=$(prompt "隧道 IPv4" "$(jq -r ".clients[$index].ipv4 // \"\"" "$WG_CONF")")
  wg_valid_address "$ipv4" && ! printf '%s' "$ipv4" | grep -q ':' || { tell_warn "IPv4 地址无效"; wait_key; return; }
  ipv6=$(prompt "隧道 IPv6" "$(jq -r ".clients[$index].ipv6 // \"\"" "$WG_CONF")"); printf '%s' "$ipv6" | grep -q ':' && wg_valid_address "$ipv6" || { tell_warn "IPv6 地址无效"; wait_key; return; }
  jq -e --arg ip "${ipv4%/*}" --argjson i "$index" '.clients | to_entries[] | select(.key != $i) | (.value.ipv4 // "") | split("/")[0] == $ip' "$WG_CONF" >/dev/null 2>&1 && { tell_warn "IPv4 地址已被其他客户端使用"; wait_key; return; }
  jq -e --arg ip "${ipv6%/*}" --argjson i "$index" '.clients | to_entries[] | select(.key != $i) | (.value.ipv6 // "") | split("/")[0] == $ip' "$WG_CONF" >/dev/null 2>&1 && { tell_warn "IPv6 地址已被其他客户端使用"; wait_key; return; }
  pub=$(prompt "客户端公钥" "$(jq -r ".clients[$index].public_key // \"\"" "$WG_CONF")"); [ -n "$pub" ] || { tell_warn "客户端公钥不能为空"; wait_key; return; }
  body=$(jq --argjson i "$index" --arg name "$name" --arg ipv4 "$ipv4" --arg ipv6 "$ipv6" --arg pub "$pub" --arg allowed4 "${ipv4%/*}/32" --arg allowed6 "${ipv6%/*}/128" '.clients[$i]={name:$name,ipv4:$ipv4,ipv6:$ipv6,public_key:$pub} | .endpoint.peers[$i]={public_key:$pub,allowed_ips:[$allowed4,$allowed6]}' "$WG_CONF") || { tell_warn "配置生成失败"; wait_key; return; }
  wg_apply_body "$body" || { wait_key; return; }; tell_ok "客户端已修改"; wait_key
}

wg_toggle(){
  local role previous previous_name body
  screen_clear; [ -f "$WG_CONF" ] || { tell_warn "请先初始化配置"; wait_key; return; }
  ...
  if [ "$(jq -r '.enabled//false' "$WG_CONF")" = true ]; then
    ...
    [ "$previous" = wireguard ] && BUILD_SELECTED_OVERRIDE=direct
    if wg_apply_body "$body"; then [ "$previous" = wireguard ] && state_set exit direct && sync_proxy_env; tell_ok "已停止"; else tell_warn "配置应用失败"; fi
    BUILD_SELECTED_OVERRIDE=""
  else
    if [ "$role" = client ] && [ -z "$(jq -r '.peer_public_key // ""' "$WG_CONF")" ]; then tell_warn "缺少服务端公钥"; wait_key; return; fi
    if [ "$role" = client ] && [ "$previous" != direct ]; then prompt_yes "当前 ${previous_name} 正在接管网络。\n启动 WireGuard 后，网络出口将切换至 WireGuard。\n是否切换" || return; fi
...
check_dependencies(){
  ...
  local deps=(
    ...
    "systemctl:systemd" "journalctl:systemd" "ping:iputils-ping" "fuser:psmisc" "cmp:diffutils"
  )
  local missing=0
  for item in "${deps[@]}"; do
    cmd=${item%%:*}
...
sync_hopping_rules(){
  ...
  nft add chain inet "$HOP_TABLE" prerouting '{ type nat hook prerouting priority dstnat; policy accept; }' 2>/dev/null || { nft delete table inet "$HOP_TABLE" 2>/dev/null; return 0; }
  nft add rule inet "$HOP_TABLE" prerouting iifname != "lo" iifname != "$WG_IF" iifname != "$TUN_IF" udp dport "$range" redirect to :"$port" 2>/dev/null || { nft delete table inet "$HOP_TABLE" 2>/dev/null; return 0; }
}

clear_hopping_rules(){ command -v nft >/dev/null 2>&1 && nft delete table inet "$HOP_TABLE" 2>/dev/null || true; }
