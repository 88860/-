...
latency_tcp_connect(){
  local target=$1 port=$2 iface=$3 family url value ms rc
  [ -n "$target" ] && [ -n "$port" ] && [ -n "$iface" ] || return 1
  case "$target" in
    *:*) family=6; url="telnet://[$target]:$port" ;;
    *.*) family=4; url="telnet://$target:$port" ;;
    *) return 1 ;;
  esac
  value=$(env -u http_proxy -u https_proxy -u HTTP_PROXY -u HTTPS_PROXY -u all_proxy -u ALL_PROXY timeout 2 curl -sS -o /dev/null -w '%{time_connect}' --connect-timeout 1 -m 1 -"$family" --interface "$iface" "$url" 2>/dev/null)
  rc=$?
  printf '%s' "$value" | grep -Eq '^[0-9]+([.][0-9]+)?$' || return 1
  ms=$(awk -v t="$value" 'BEGIN{printf "%.0f",t*1000}')
  [ "$ms" -gt 0 ] || [ "$rc" -eq 0 ] || return 1
  [ "$ms" -le 2000 ] || return 1
  printf '%s' "$ms"
}

latency_proxy_probe(){
  local file=$1 target=$2 iface=$3 result=$4 latency_file=${5:-} outbound port tmp_conf tmp_log pid i elapsed_ms value
  [ -s "$file" ] || return 1
  [ -n "$target" ] && [ -n "$iface" ] || return 1
  outbound=$(jq -c --arg target "$target" --arg iface "$iface" --arg original "$(jq -r '.outbound.server // ""' "$file" 2>/dev/null)" \
    '.outbound | .tag="latency-out" | .server=$target | .bind_interface=$iface | if (.tls.enabled == true and (.tls.server_name // "") == "" and ($original|test("^[0-9a-fA-F:.]+$")|not)) then .tls.server_name=$original else . end' \
    "$file" 2>/dev/null) || return 1
  [ -n "$outbound" ] && [ "$outbound" != null ] || return 1
  port=$((20000 + $(od -An -N2 -tu2 /dev/urandom 2>/dev/null | tr -d ' ') % 20000))
  [ "$port" -ge 20000 ] 2>/dev/null || port=20873
  tmp_conf=$(mktemp "$SBM_DIR/tmp/latency-conf.XXXXXX") || return 1
  tmp_log=$(mktemp "$SBM_DIR/tmp/latency-log.XXXXXX") || { rm -f "$tmp_conf"; return 1; }
  TMP_FILES="$TMP_FILES $tmp_conf $tmp_log"
  jq -n --argjson port "$port" --argjson outbound "$outbound" --arg iface "$iface" '
    {log:{level:"error"},dns:{servers:[{type:"https",tag:"latency-dns",server:"1.1.1.1",server_port:443,path:"/dns-query",tls:{enabled:true,server_name:"cloudflare-dns.com"},bind_interface:$iface}],final:"latency-dns",strategy:"prefer_ipv6"},inbounds:[{type:"mixed",tag:"latency-in",listen:"127.0.0.1",listen_port:$port}],outbounds:[$outbound],route:{final:"latency-out",auto_detect_interface:false,default_interface:$iface,default_domain_resolver:"latency-dns"}}' >"$tmp_conf" 2>/dev/null || { rm -f "$tmp_conf" "$tmp_log"; return 1; }
  "$CORE" check -c "$tmp_conf" >/dev/null 2>&1 || { rm -f "$tmp_conf" "$tmp_log"; return 1; }
  "$CORE" run -c "$tmp_conf" >"$tmp_log" 2>&1 & pid=$!
  i=0
  while [ "$i" -lt 20 ]; do
    kill -0 "$pid" 2>/dev/null || break
    value=$(env -u http_proxy -u https_proxy -u HTTP_PROXY -u HTTPS_PROXY -u all_proxy -u ALL_PROXY curl -fsS -o /dev/null -w '%{time_total}' --socks5-hostname "127.0.0.1:$port" --connect-timeout 1 -m 2 "http://cp.cloudflare.com/generate_204" 2>/dev/null) || value=""
    if printf '%s' "$value" | grep -Eq '^[0-9]+([.][0-9]+)?$'; then
      elapsed_ms=$(awk -v t="$value" 'BEGIN{printf "%.0f",t*1000}')
      [ "$elapsed_ms" -ge 0 ] && [ "$elapsed_ms" -le 3000 ] || elapsed_ms=3000
      printf 'ok\n' >"$result"
...
probe_peer_latency(){
  local file=$1 result=$2 latency_file=${3:-} server port type targets target iface ms out icmp_v4=0 icmp_v6=0 icmp_ready
  if command -v ping >/dev/null 2>&1; then
    ping -4 -c 1 -W 1 127.0.0.1 >/dev/null 2>&1 && icmp_v4=1
    ping -6 -c 1 -W 1 ::1 >/dev/null 2>&1 && icmp_v6=1
  fi
  server=$(jq -r '.outbound.server // ""' "$file" 2>/dev/null) || { echo fail >"$result"; return 1; }
  port=$(jq -r '.outbound.server_port // (.outbound.server_ports[0] // "")' "$file" 2>/dev/null)
  type=$(jq -r '.outbound.type // ""' "$file" 2>/dev/null)
  ...
  while IFS= read -r target; do
    ...
    case "$type" in
      vless|vmess|trojan|anytls|socks|http|shadowsocks|snell|naive|ssh)
        ms=$(latency_tcp_connect "$target" "$port" "$iface" 2>/dev/null) || ms=""
        if printf '%s' "$ms" | grep -Eq '^[0-9]+$'; then
          echo ok >"$result"
          [ -n "$latency_file" ] && printf '%s\n' "$ms" >"$latency_file"
          return 0
      ...
      hysteria|hysteria2|hy2|tuic)
        if latency_proxy_probe "$file" "$target" "$iface" "$result" "$latency_file"; then return 0; fi
        ;;
      *)
        ms=$(latency_tcp_connect "$target" "$port" "$iface" 2>/dev/null) || ms=""
        if printf '%s' "$ms" | grep -Eq '^[0-9]+$'; then
...
menu_modify_protocol(){
  ...
  cp "$PICKED" "$NODE_EDIT_FILE" || { rm -f "$NODE_EDIT_FILE"; NODE_EDIT_FILE=""; return; }NODE_OVERRIDE_TARGET="$PICKED"
...
acme_options(){
  ...
  body=$(jq -n --arg d "$domain" --arg e "$email" --arg dir "$ACME_DIR" '{domain:[$d],email:$e,data_directory:$dir,key_type:"p256"}') || return 1case "$mode" in
    alpn) body=$(echo "$body" | jq '.disable_http_challenge=true') || return 1 ;;
    dns_cloudflare) body=$(echo "$body" | jq --arg t "$cf_token" '.dns01_challenge={provider:"cloudflare",api_token:$t}') || return 1 ;;
    dns_alidns) body=$(echo "$body" | jq --arg k "$ali_key" --arg s "$ali_secret" '.dns01_challenge={provider:"alidns",access_key_id:$k,access_key_secret:$s}') || return 1 ;;
    dns_acmedns) body=$(echo "$body" | jq --arg u "$acmedns_user" --arg p "$acmedns_pass" --arg s "$acmedns_sub" --arg r "$acmedns_url" '.dns01_challenge={provider:"acmedns",username:$u,password:$p,subdomain:$s,server_url:$r}') || return 1 ;;
...
wg_reinit_rollback(){
  ...
  chmod 700 "$WG_DIR"
  role=$(jq -r '.role // ""' "$WG_CONF" 2>/dev/null)
  state_set exit "$previous" && sync_proxy_envif [ "$role" = server ] && [ "$(jq -r '.enabled // false' "$WG_CONF" 2>/dev/null)" = true ]; then
    wg_server_nat_apply
  fi
  apply_config_quiet
}
