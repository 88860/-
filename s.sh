...
restore_bypass_snapshot(){
  ...
  while IFS= read -r p; do
    [ -n "$p" ] && ip -4 rule add pref "$BYPASS_PREF" sport "$p" lookup main 2>/dev/null || true
  done <<< "$old4"
  while IFS= read -r p; do
    [ -n "$p" ] && ip -6 rule add pref "$BYPASS_PREF" sport "$p" lookup main 2>/dev/null || true
  done <<< "$old6"
  while IFS= read -r item; do
    [ -n "$item" ] || continue
    ip=${item%|*}; p=${item##*|}
    ip -4 rule add pref "$((BYPASS_PREF-1))" to "$ip" dport "$p" lookup main 2>/dev/null || true
  done <<< "$oldd4"
  while IFS= read -r item; do
    [ -n "$item" ] || continue
    ip=${item%|*}; p=${item##*|}
    ip -6 rule add pref "$((BYPASS_PREF-1))" to "$ip" dport "$p" lookup main 2>/dev/null || true
  done <<< "$oldd6"
}
...
case $1 in
  ...
  --reapply)
    ...
    _new_conf=$(mktemp); TMP_FILES+=("$_new_conf")
