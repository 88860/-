...
menu_delete_protocol(){
  ...
  was_acme=$(jq -r '.tls_mode' "$PICKED") || { tell_warn "无法读取节点配置"; wait_key; return; }
  old_json=$(cat "$PICKED") || { tell_warn "无法读取节点配置"; wait_key; return; }
  rm -f "$PICKED" || { tell_warn "节点文件删除失败"; wait_key; return; }
...
select_peer(){
  ...
  while :; do
    index=$(prompt "请选择")
    [ -z "$index" ] && { tell_warn "输入无效，请重新选择"; continue; }
    [ "$index" = 0 ] && return 1
...
wg_setup(){
  ...
  local listen_port peer_host peer_port peer_key body management_port
  while :; do
  ...
  if [ "$role" = server ]; then
    ...
    body=$(jq -n --arg private "$private" --arg public "$public" --arg a4 "$address4" --arg a6 "$address6" \
     ...
     {role:"client",enabled:false,private_key:$private,public_key:$public,
      address:[$a4,$a6],listen_port:0,management_port:$management_port,
