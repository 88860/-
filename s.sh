...
list_peers(){
  ...
  while ss -Hln | grep -q ":$dir_port\b"; do dir_port=$((dir_port+1)); done
  while ss -Hln | grep -q ":$base_port\b"; do base_port=$((base_port+100)); done
  ...
  jq -s --argjson dp "$dir_port" --argjson bp "$base_port" '
    ...
    {
      log: {level: "fatal"},
      dns: {servers:[{type:"https",tag:"dns-direct",server:"223.5.5.5",detour:"direct"}], rules:[], final:"dns-direct"},
    ...
    while read -r file tag type name port host; do
      ...
      (
        ...
        if [ "$type" != "hysteria2" ] && [ "$type" != "tuic" ]; then
          res=$(curl -x socks5h://127.0.0.1:$dir_port -w "%{time_total}" -s -o /dev/null -m 2 "http://$host:$port" 2>/dev/null)
        fi
        
        if [ -z "$res" ] || [ "$res" = "0.000" ]; then
           res=$(curl -x socks5h://127.0.0.1:$dir_port -w "%{time_total}" -s -o /dev/null -m 2 "http://$host:22" 2>/dev/null)
        fi
        
        if [ -z "$res" ] || [ "$res" = "0.000" ]; then
           res=$(curl -x socks5h://127.0.0.1:$dir_port -w "%{time_total}" -s -o /dev/null -m 2 "http://$host:443" 2>/dev/null)
        fi
