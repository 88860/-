...
build_dns_block(){
  ...
  [ "$NET_STACK" = "v6" ] && bserver="2606:4700:4700::1111"
  jq -n --arg final "$final" --arg bserver "$bserver" --arg strat "$strat" --argjson dns_direct "$dns_direct" '{
    servers: [
      {
  ...
  if $final != "direct" then
    .servers |= map(if .tag=="dns-remote" then .detour=$final else . end)
  else . end'
}
...
build_config(){
  ...
  if [ "$selected" != direct ]; then
    if [ -f "$PEER_DIR/$selected.json" ]; then
      local out
      out=$(jq -c --arg tag "$selected" --arg strat "$strat" \
        '.outbound | .tag=$tag | .domain_resolver={server:"dns-bootstrap",strategy:$strat}' \
...
sync_mgmt_rules(){
  ...
  [ -n "$NET_IF_V4" ] && for p in $new_ports; do
    ip -4 rule add pref "$tmp_pref" sport "$p" lookup main 2>/dev/null || true
  done
  [ -n "$NET_IF_V6" ] && for p in $new_ports; do
    ip -6 rule add pref "$tmp_pref" sport "$p" lookup main 2>/dev/null || true
  done
  while ip -4 rule del pref "$PREF_MAIN" 2>/dev/null; do :; done
  while ip -6 rule del pref "$PREF_MAIN" 2>/dev/null; do :; done
  [ -n "$NET_IF_V4" ] && for p in $new_ports; do
    ip -4 rule add pref "$PREF_MAIN" sport "$p" lookup main 2>/dev/null || true
  done
  [ -n "$NET_IF_V6" ] && for p in $new_ports; do
    ip -6 rule add pref "$PREF_MAIN" sport "$p" lookup main 2>/dev/null || true
  done
  while ip -4 rule del pref "$tmp_pref" 2>/dev/null; do :; done
  while ip -6 rule del pref "$tmp_pref" 2>/dev/null; do :; done
}
...
setup_certificate(){
  ...
  if [ -n "$od" ] && [ -n "$oe" ]; then
    ...
    if prompt_yes "是否继续使用当前证书配置"; then
      return 0
    fi
  fi
