...
release_json(){ curl -fsSL -m 15 "$GH_API/releases/tags/v1.14.1" 2>/dev/null; }
remote_version(){ printf "%s" "1.14.1"; }

default_iface_v4(){
  local iface
  iface=$(ip -4 route get 1.1.1.1 2>/dev/null | awk '{for(i=1;i<=NF;i++)if($i=="dev"){print $(i+1);exit}}')
  case "$iface" in "$TUN_IF"|"$WG_IF"|lo) iface="" ;; esac
  if [ -z "$iface" ]; then
    iface=$(ip -4 route show table main default 2>/dev/null | awk -v t="$TUN_IF" -v w="$WG_IF" '{for(i=1;i<=NF;i++)if($i=="dev"){d=$(i+1);if(d!=t&&d!=w&&d!="lo"){print d;exit}}}')
  fi
  printf '%s' "$iface"
}
default_iface_v6(){
  ...
  case "$iface" in "$TUN_IF"|"$WG_IF"|lo) iface="" ;; esac
  if [ -z "$iface" ]; then
    iface=$(ip -6 route show table main default 2>/dev/null | awk -v t="$TUN_IF" -v w="$WG_IF" '{for(i=1;i<=NF;i++)if($i=="dev"){d=$(i+1);if(d!=t&&d!=w&&d!="lo"){print d;exit}}}')
  fi
  printf '%s' "$iface"
}
...
probe_network_stack(){
  local tmp4 tmp6 ip4 ip6 pid4 pid6
  NET_IPV4=""; NET_IPV6=""; NET_IF_V4=""; NET_IF_V6=""; NET_STACK="none"; IPV4_OK=0; IPV6_OK=0
  ...
  (curl -4 -fsS --connect-timeout 3 -m 6 https://ipv4.icanhazip.com 2>/dev/null | tr -d '
 ' >"$tmp4") & pid4=$!
  (curl -6 -fsS --connect-timeout 3 -m 6 https://ipv6.icanhazip.com 2>/dev/null | tr -d '
 ' >"$tmp6") & pid6=$!
  ...
  [ -n "$ip4" ] && IPV4_OK=1
  [ -n "$ip6" ] && IPV6_OK=1
  if [ "$IPV4_OK" = 1 ] && [ "$IPV6_OK" = 1 ]; then NET_STACK="both"
  elif [ "$IPV4_OK" = 1 ]; then NET_STACK="v4"
  elif [ "$IPV6_OK" = 1 ]; then NET_STACK="v6"
...
probe_network_stack_async(){
  (
    ...
    printf '%s\n%s\n%s\n%s\n%s\n%s\n%s\n' "$NET_IPV4" "$NET_IPV6" "$NET_IF_V4" "$NET_IF_V6" "$NET_STACK" "$IPV4_OK" "$IPV6_OK" > "$tmp" && mv -f "$tmp" "$NET_CACHE" || rm -f "$tmp"
...
load_net_cache(){
  if [ -f "$NET_CACHE" ]; then
    { IFS= read -r NET_IPV4; IFS= read -r NET_IPV6
      ...
      IFS= read -r NET_STACK; IFS= read -r IPV4_OK; IFS= read -r IPV6_OK; } < "$NET_CACHE"
    [ "$IPV4_OK" = 1 ] || [ -z "$NET_IPV4" ] || IPV4_OK=1
    [ "$IPV6_OK" = 1 ] || [ -z "$NET_IPV6" ] || IPV6_OK=1
  fi
  if [ -z "$NET_STACK" ]; then
    probe_network_stack
    printf '%s\n%s\n%s\n%s\n%s\n%s\n%s\n' "$NET_IPV4" "$NET_IPV6" "$NET_IF_V4" "$NET_IF_V6" "$NET_STACK" "$IPV4_OK" "$IPV6_OK" > "$NET_CACHE" 2>/dev/null
  fi
}
...
protected_ports(){
  {
    ssh_ports
    node_ports
    [ -f "$WG_CONF" ] && jq -r '.listen_port//0' "$WG_CONF" | grep -E '^[1-9][0-9]*$'
  } | grep -E '^[1-9][0-9]*$' | sort -un
}
...
wg_server_nat_apply(){
  local prev4 prev6 backup=/etc/sysctl.d/99-sbm-forward.conf.sbm-backup
  local owned=/etc/sysctl.d/99-sbm-forward.conf.sbm-owned
  local state=/etc/sysctl.d/99-sbm-forward.conf.sbm-state
  local target=/etc/sysctl.d/99-sbm-forward.conf
  local tmp out4 out6
  prev4=$(sysctl -n net.ipv4.ip_forward 2>/dev/null) || return 1
  prev6=$(sysctl -n net.ipv6.conf.all.forwarding 2>/dev/null) || return 1
  if [ -f "$target" ]; then
    if [ ! -f "$backup" ]; then
      cp -a "$target" "$backup" 2>/dev/null || return 1
    fi
  elif [ ! -f "$owned" ]; then
    : > "$owned" 2>/dev/null || return 1
  fi
  if [ ! -f "$state" ]; then
    {
  ...
  tmp=$(mktemp /etc/sysctl.d/.99-sbm-forward.conf.XXXXXX) || return 1
  printf '%s\n' 'net.ipv4.ip_forward=1' 'net.ipv6.conf.all.forwarding=1' >"$tmp" || { rm -f "$tmp"; return 1; }
  chmod 644 "$tmp" 2>/dev/null || { rm -f "$tmp"; return 1; }
  if ! sysctl -w net.ipv4.ip_forward=1 >/dev/null 2>&1 || ! sysctl -w net.ipv6.conf.all.forwarding=1 >/dev/null 2>&1; then
    sysctl -w net.ipv4.ip_forward="$prev4" >/dev/null 2>&1 || true
    sysctl -w net.ipv6.conf.all.forwarding="$prev6" >/dev/null 2>&1 || true
    rm -f "$tmp"
    return 1
  fi
  if ! install -m644 "$tmp" "$target" 2>/dev/null; then
    rm -f "$tmp"
    sysctl -w net.ipv4.ip_forward="$prev4" >/dev/null 2>&1 || true
    sysctl -w net.ipv6.conf.all.forwarding="$prev6" >/dev/null 2>&1 || true
    return 1
  ...
  if [ -n "$out4" ]; then
    nft add rule ip sbm_nat postrouting oifname "$out4" masquerade 2>/dev/null || { wg_server_nat_remove; return 1; }
  fi
  ...
  if [ -n "$out6" ]; then
    nft add rule ip6 sbm_nat postrouting oifname "$out6" masquerade 2>/dev/null || { wg_server_nat_remove; return 1; }
  fi
  return 0
}
...
wg_server_nat_remove(){
  ...
  nft delete table ip sbm_nat 2>/dev/null
  nft delete table ip6 sbm_nat 2>/dev/null
  prev4=$(sed -n 's/^net.ipv4.ip_forward=//p' "$state" 2>/dev/null | head -n1)
  prev6=$(sed -n 's/^net.ipv6.conf.all.forwarding=//p' "$state" 2>/dev/null | head -n1)
  if [ -z "$prev4" ] && [ -f "$backup" ]; then prev4=$(sed -n 's/^[[:space:]]*net.ipv4.ip_forward[[:space:]]*=[[:space:]]*//p' "$backup" 2>/dev/null | tail -n1); fi
  if [ -z "$prev6" ] && [ -f "$backup" ]; then prev6=$(sed -n 's/^[[:space:]]*net.ipv6.conf.all.forwarding[[:space:]]*=[[:space:]]*//p' "$backup" 2>/dev/null | tail -n1); fi
  if [ "$prev4" = 0 ] || [ "$prev4" = 1 ]; then sysctl -w net.ipv4.ip_forward="$prev4" >/dev/null 2>&1 || true; fi
  if [ "$prev6" = 0 ] || [ "$prev6" = 1 ]; then sysctl -w net.ipv6.conf.all.forwarding="$prev6" >/dev/null 2>&1 || true; fi
  if [ -f "$backup" ]; then
...
build_config(){
  ...
  local dns_direct_server dns_strategy auto_detect icmp_out
  ...
  if [ "$NET_STACK" = "v6" ] || { [ "$NET_STACK" = "both" ] && [ "$IPV6_OK" = "1" ]; }; then
  ...
  if [ ${#node_files[@]} -gt 0 ]; then
    ...
    inbounds=$(jq -s --arg d "$domain" --argjson x "$tls_extra" --arg stack "$NET_STACK" '
      [ .[] |
        ...
        (if .tls_mode=="acme" then
          $in2 * {tls: ({enabled:true,server_name:$d}
                       ...
                       + $x)}
         else $in2 end) as $base |
        if $stack=="v4" then [$base | .listen="0.0.0.0" | .tag=.tag+"-v4"]
        elif $stack=="v6" then [$base | .listen="::" | .tag=.tag+"-v6"]
        elif $stack=="both" then [$base | .listen="0.0.0.0" | .tag=.tag+"-v4", $base | .listen="::" | .tag=.tag+"-v6"]
        else [] end
      ] | add' "${node_files[@]}") || return 1
  fi

  outbounds='[{"type":"direct","tag":"direct","domain_resolver":"dns-direct"}]'
  ...
  if [ -f "$WG_CONF" ] && [ "$(jq -r '.enabled//false' "$WG_CONF")" = true ]; then
    if [ "$(jq -r .role "$WG_CONF")" = client ]; then
      if [ "$selected" = "direct" ] || [ "$selected" = "wireguard" ]; then
        endpoints=$(jq '[.endpoint | .domain_resolver = (.domain_resolver // "dns-direct")]' "$WG_CONF")
  ...
  if [ "$selected" != direct ] && [ "$selected" != wireguard ]; then
    if [ -f "$PEER_DIR/$selected.json" ]; then
      outbounds=$(jq -n --argjson base "$outbounds" --slurpfile peer "$PEER_DIR/$selected.json" \
        '$base + [$peer[0].outbound + {domain_resolver:"dns-direct"}]')
  ...
  dns_block=$(jq -n --arg direct "$dns_direct_server" --arg strategy "$dns_strategy" --arg detour "$final" '
    {servers:[
      {type:"udp",tag:"dns-direct",server:$direct,server_port:53,detour:"direct"},
      {type:"udp",tag:"dns-remote-v6-1",server:"2606:4700:4700::1111",server_port:53,detour:$detour},
      {type:"udp",tag:"dns-remote-v6-2",server:"2001:4860:4860::8888",server_port:53,detour:$detour},
      {type:"udp",tag:"dns-remote-v4-1",server:"1.1.1.1",server_port:53,detour:$detour},
      {type:"udp",tag:"dns-remote-v4-2",server:"8.8.8.8",server_port:53,detour:$detour}
    ],
    rules:[
      {action:"evaluate",server:"dns-remote-v6-1",timeout:"3s"},
      {match_response:true,response_rcode:"NOERROR",action:"respond"},
      {match_response:true,response_rcode:"NXDOMAIN",action:"respond"},
      {action:"evaluate",server:"dns-remote-v6-2",timeout:"3s"},
      {match_response:true,response_rcode:"NOERROR",action:"respond"},
      {match_response:true,response_rcode:"NXDOMAIN",action:"respond"},
      {action:"evaluate",server:"dns-remote-v4-1",timeout:"3s"},
      {match_response:true,response_rcode:"NOERROR",action:"respond"},
      {match_response:true,response_rcode:"NXDOMAIN",action:"respond"},
      {action:"evaluate",server:"dns-remote-v4-2",timeout:"3s"},
      {match_response:true,response_rcode:"NOERROR",action:"respond"},
      {match_response:true,response_rcode:"NXDOMAIN",action:"respond"},
      {match_response:true,action:"respond"}
    ],
    final:"dns-remote-v6-1",
    strategy:$strategy}')
...
apply_config(){
  ...
  if ! install -m600 "$tmp" "$CONFIG"; then
    rm -f "$tmp"; tell_warn "新配置安装失败，未切换现有配置"; return 1
  fi
  rm -f "$tmp"
  if ! timeout 15 systemctl reload-or-restart sing-box >/dev/null 2>&1; then
    ...
    if [ -n "$prev_conf" ] && [ -s "$prev_conf" ]; then
      if ! install -m600 "$prev_conf" "$CONFIG" || ! timeout 15 systemctl restart sing-box >/dev/null 2>&1 || ! systemctl is-active --quiet sing-box; then
        tell_warn "旧配置回滚后服务仍未恢复，请手动检查 sing-box"
      fi
    else
      rm -f "$CONFIG"; timeout 15 systemctl stop sing-box >/dev/null 2>&1 || true
    fi
    return 1
  fi
...
save_node(){
  ...
  if apply_config; then tell_ok "协议应用成功"; printf "\n"; render_share_uri "$file"
    ...
    if [ "$was_acme" = "acme" ]; then
      ...
      if [ "$acme_count" -eq 0 ] && [ -n "$(state_get domain)" ]; then
        ...
        if prompt_yes "是否连同域名和证书一起清理"; then
          ...
          find "$ACME_DIR" -mindepth 1 -delete 2>/dev/null; apply_config || tell_warn "证书清理后的配置重新应用失败，请检查服务状态"; tell_ok "相关配置已清理"
        fi
      fi
    fi
  else
  ...
  if "$CORE" check -c "$probe" >/dev/null 2>&1; then
    if json_save "$PEER_DIR/$tag.json" "$(jq -n --arg tag "$tag" --arg name "$name" --arg uri "$uri" --argjson ob "$outbound" '{tag:$tag,name:$name,uri:$uri,outbound:$ob}')"; then
      if apply_config; then
        ...
        rm -f "$PEER_DIR/$tag.json"
        apply_config_quiet
        tell_warn "节点已写入但配置应用失败，已回滚"
      fi
    else
      tell_warn "节点保存失败"
    fi
  ...
  if [ -n "$raw_data" ]; then
    ...
    while read -r file tag type name port host; do
      ...
      (
        ...
        if [ -n "$iface" ]; then ping_res=$(timeout 1 $cmd -I "$iface" -c 1 -W 1 -m 255 "$host" 2>/dev/null | awk -F'/' '/^rtt|^round-trip/{print $5}'); fi
        if [ -z "$ping_res" ]; then ping_res=$(timeout 1 $cmd -c 1 -W 1 -m 255 "$host" 2>/dev/null | awk -F'/' '/^rtt|^round-trip/{print $5}'); fi
...
wg_tunnel_state(){
  ...
  if [ -n "$peer_ip4" ] && command -v ping >/dev/null 2>&1 && ping -c1 -W3 "$peer_ip4" >/dev/null 2>&1; then echo up; return; fi
...
wg_valid_ipv4(){
  local value=$1 ip prefix part n
  [[ $value == */* ]] || return 1
  ...
  [ "${#part[@]}" -eq 4 ] || return 1
  for n in "${part[@]}"; do
    [[ $n =~ ^[0-9]+$ ]] && [ "$n" -le 255 ] || return 1
  done
}

wg_ipv6_part_units(){
  local part=$1 allow_ipv4=$2 token units=0 i
  [ -n "$part" ] || { printf '0'; return 0; }
  IFS=: read -r -a _wg6_tokens <<<"$part"
  for ((i=0; i<${#_wg6_tokens[@]}; i++)); do
    token=${_wg6_tokens[i]}
    [ -n "$token" ] || return 1
    if [[ $token == *.* ]]; then
      [ "$allow_ipv4" = 1 ] || return 1
      [ "$i" -eq $((${#_wg6_tokens[@]}-1)) ] || return 1
      wg_valid_ipv4 "${token}/32" || return 1
      units=$((units+2))
    else
      [[ $token =~ ^[0-9A-Fa-f]{1,4}$ ]] || return 1
      units=$((units+1))
    fi
  done
  printf '%s' "$units"
...
wg_valid_ipv6(){
  local value=$1 ip prefix left right lc rc units
  [[ $value == */* ]] || return 1
  ip=${value%/*}; prefix=${value#*/}
  ...
  if [[ $ip == *::* ]]; then
    left=${ip%%::*}; right=${ip#*::}
    lc=$(wg_ipv6_part_units "$left" 0) || return 1
    rc=$(wg_ipv6_part_units "$right" 1) || return 1
    units=$((lc+rc))
    [ "$units" -lt 8 ] || return 1
  elif [[ $ip == *:* ]]; then
    left=$ip
    lc=$(wg_ipv6_part_units "$left" 1) || return 1
    [ "$lc" -eq 8 ] || return 1
  else
    return 1
  fi
...
wg_delete(){
  ...
  if build_config >"$tmp" 2>/dev/null && "$CORE" check -c "$tmp" >/dev/null 2>&1; then
    ...
    if systemctl is-active --quiet sing-box; then
      if ! timeout 15 systemctl reload sing-box >/dev/null 2>&1; then
        ...
        timeout 15 systemctl reload sing-box >/dev/null 2>&1 || true
...
run_uninstall(){
  ...
  clear
  tell_warn "警告: 卸载将清空所有配置、服务、网络规则和脚本状态"
  [ "$(prompt '输入 yes 确认')" = yes ] || return
  mapfile -t packages < <(grep -v '^[[:space:]]*$' "$PKG_LOG" 2>/dev/null)
  stop_net_monitor
  timeout 20 systemctl disable --now sing-box >/dev/null 2>&1 || true
  timeout 20 systemctl disable --now sbm-watchdog.service >/dev/null 2>&1 || true
  systemctl reset-failed 'sbm-watchdog*' 2>/dev/null || true
  rm -f /etc/systemd/system/multi-user.target.wants/sing-box.service
  rm -f /etc/systemd/system/multi-user.target.wants/sbm-watchdog.service
  rm -f /etc/systemd/system/sing-box.service.wants/*
  rm -f /etc/systemd/system/sbm-watchdog.service.wants/*
  ...
  wg_server_nat_remove
  while ip rule del pref "$BYPASS_PREF" 2>/dev/null; do guard=$((guard+1)); [ "$guard" -gt 256 ] && break; done
  guard=0
  while ip -6 rule del pref "$BYPASS_PREF" 2>/dev/null; do guard=$((guard+1)); [ "$guard" -gt 256 ] && break; done
  ip link del "$WG_IF" 2>/dev/null || true
  ...
  rm -f "$NET_MONITOR_PID" "$REAPPLY_LOCK"
  rm -f /etc/sysctl.d/99-sbm-forward.conf.sbm-owned
  if [ -f /etc/sysctl.d/99-sbm-forward.conf.sbm-backup ]; then
    mv -f /etc/sysctl.d/99-sbm-forward.conf.sbm-backup /etc/sysctl.d/99-sbm-forward.conf 2>/dev/null || true
  fi
  rm -f /etc/sysctl.d/99-sbm-forward.conf.sbm-state
  rm -rf "$SB_DIR" "$SBM_DIR" /var/lib/sing-box "$CORE" "$SHORTCUT"
  rm -f /root/.cache/sing-box/* 2>/dev/null || true
  if [ ${#packages[@]} -gt 0 ]; then
    tell "脚本曾安装过: [ ${packages[*]} ]"
    apt-get remove -y -q "${packages[@]}" >/dev/null 2>&1 || true
...
bootstrap(){
  ...
  probe_network_stack
  local net_tmp
  net_tmp=$(mktemp "${NET_CACHE}.XXXXXX") || exit 1
  printf '%s\n%s\n%s\n%s\n%s\n%s\n%s\n' "$NET_IPV4" "$NET_IPV6" "$NET_IF_V4" "$NET_IF_V6" "$NET_STACK" "$IPV4_OK" "$IPV6_OK" > "$net_tmp" && mv -f "$net_tmp" "$NET_CACHE" || { rm -f "$net_tmp"; exit 1; }
  if [ ! -x "$CORE" ]; then
...
case $1 in
  ...
  --reapply)
    ...
    prev_ip4="$NET_IPV4"; prev_ip6="$NET_IPV6"; prev_stack="$NET_STACK"
    prev_ok4="$IPV4_OK"; prev_ok6="$IPV6_OK"
    probe_network_stack
    [ -n "$NET_IF_V4" ] || NET_IF_V4=$(default_iface_v4)
    [ -n "$NET_IF_V6" ] || NET_IF_V6=$(default_iface_v6)
    if [ "$NET_STACK" = "none" ] && [ -n "$prev_stack" ] && [ "$prev_stack" != "none" ]; then
      NET_IPV4="$prev_ip4"; NET_IPV6="$prev_ip6"; NET_STACK="$prev_stack"
      IPV4_OK="$prev_ok4"; IPV6_OK="$prev_ok6"
      [ -n "$NET_IF_V4" ] || NET_IF_V4="$prev_if4"
      [ -n "$NET_IF_V6" ] || NET_IF_V6="$prev_if6"
    fi
    _cache_tmp=$(mktemp "${NET_CACHE}.XXXXXX") || { flock -u 9; exit 1; }
    if ! printf '%s\n%s\n%s\n%s\n%s\n%s\n%s\n' \
      "$NET_IPV4" "$NET_IPV6" "$NET_IF_V4" "$NET_IF_V6" "$NET_STACK" "$IPV4_OK" "$IPV6_OK" \
      > "$_cache_tmp" || ! mv -f "$_cache_tmp" "$NET_CACHE"; then
    ...
    if ! timeout 15 systemctl reload-or-restart sing-box >/dev/null 2>&1; then
      if [ -n "$_prev_conf" ] && [ -s "$_prev_conf" ]; then
        if ! install -m600 "$_prev_conf" "$CONFIG" || ! timeout 15 systemctl restart sing-box >/dev/null 2>&1 || ! systemctl is-active --quiet sing-box; then
          rm -f "$_prev_conf"; flock -u 9; exit 2
        fi
      else
        rm -f "$CONFIG"
        timeout 15 systemctl stop sing-box >/dev/null 2>&1
