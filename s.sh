...
probe_network_stack(){
  ...
  if [ -n "$local_v4" ]; then
    ...
    (local pref
     pref=$(ip rule show 2>/dev/null | awk 'BEGIN{for(i=1000;i<9000;i++) free[i]=1} /^[0-9]+:/{p=$1; sub(/:$/, "", p); delete free[p]} END{for(i=1000;i<9000;i++) if(free[i]){print i; exit}}')
     [ -n "$pref" ] || exit 1
  ...
  if [ -n "$local_v6" ]; then
    ...
    (local pref
     pref=$(ip -6 rule show 2>/dev/null | awk 'BEGIN{for(i=1000;i<9000;i++) free[i]=1} /^[0-9]+:/{p=$1; sub(/:$/, "", p); delete free[p]} END{for(i=1000;i<9000;i++) if(free[i]){print i; exit}}')
     [ -n "$pref" ] || exit 1
