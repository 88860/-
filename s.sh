...
detect_env() {
  ...
  elif command -v apk >/dev/null 2>&1 && command -v rc-service >/dev/null 2>&1; then
    SYS_ENV="lxc"
    SERVICE_UNIT=/etc/init.d/sing-box
  else
    echo -e "${RED}[×] 系统不支持: 仅适配 Debian(KVM/Systemd) 或 Alpine(LXC/OpenRC) 环境${PLAIN}"
    exit 1
  fi
}
detect_env
...
install_core(){
  ...
  if [ "$SYS_ENV" = "kvm" ]; then systemctl stop sing-box >/dev/null 2>&1; else rc-service sing-box stop >/dev/null 2>&1; fi
...
build_config(){
  ...
  if [ "$selected" != direct ] && [ "$selected" != wireguard ] && [ -f "$PEER_DIR/$selected.json" ]; then
    ...
    final=$selected; [ "$SYS_ENV" = "kvm" ] && use_tun=1
  fi
  
  if [ "$SYS_ENV" = "kvm" ]; then
    # 注意: MTU 从 9000 下调为 1400，防止 UDP/QUIC(Hysteria2 等协议) 底层分片丢失
  ...
  # 修正了 DNS 参数规范，使用 address 而非 server，避免泄漏
  ...
  # 移除了会导致 DNS 泄漏的 default_domain_resolver
...
EOF
    ...
    /bin/bash "$SELF" --watchdog </dev/null >/dev/null 2>&1 &
    echo $! > "$WATCHDOG_PID"
  fi
}
...
run_watchdog(){
  ...
  while :; do
    ...
    if [ "$fail_count" -ge 3 ] && [ "$(state_get exit)" != "direct" ]; then
      ...
      [ "$SYS_ENV" = "kvm" ] && { sync_bypass_rules; sync_hopping_rules; } || sync_proxy_env
      exit 0
    fi
  done
}
...
validate_port(){
  ...
  [ "$port" = "$allow" ] && return 0
  grep -qx "$port" <<<"$(protected_ports)" && { tell_warn "已被节点占用"; return 1; }
...
save_node(){
  ...
  else out_warn "配置应用失败，请检查运行日志。原文件已保留。"
  fi
  wait_key
}
...
create_hysteria2(){
  ...
  hopping=""
  if [ "$SYS_ENV" = "kvm" ] && prompt_yes "是否配置并开启端口跳跃防封锁机制"; then
...
render_share_uri(){
  ...
  case $kind in
    ...
    hysteria2) uri="hysteria2://$(uri_encode "$(jq -r .password <<<"$meta")")@$host:$port/?sni=$host&alpn=h3${hopping:+&mport=$hopping}#$(uri_encode "$name")" ;;
...
menu_delete_protocol(){
  ...
  apply_config && tell_ok "已删除" || tell_warn "移除失败或存在遗留冲突配置"
  wait_key
}
...
menu_modify_protocol(){
  ...
  while :; do
    ...
    [ "$kind" = hysteria2 ] && [ "$SYS_ENV" = "kvm" ] && tell "  7. 端口跳跃机制 (仅KVM)"
    tell "  0. 返回"
    tell "${CYAN}==============================${PLAIN}"
    ...
    case $(prompt "请选择") in
      ...
      7)
        [ "$kind" = hysteria2 ] && [ "$SYS_ENV" = "kvm" ] || { tell_warn "输入无效"; sleep 1; continue; }
    ...
    else tell_warn "因配置冲突应用失败，但文件已强制修改 (未回滚)"
    fi
    wait_key
  done
}
...
render_certificate_status(){
  ...
  if [ "$SYS_ENV" = "kvm" ]; then systemctl is-active --quiet sing-box || tell_warn "服务离线，无法自动续期"
  else rc-service sing-box status >/dev/null 2>&1 || tell_warn "服务离线，无法自动续期"
  fi
}
...
menu_server_info(){
  ...
  if [ "$SYS_ENV" = "kvm" ]; then systemctl is-active --quiet sing-box && tell_ok "singbox: 运行中" || tell_warn "singbox: 未运行"
  else rc-service sing-box status >/dev/null 2>&1 && tell_ok "singbox: 运行中" || tell_warn "singbox: 未运行"
  fi
  
  for file in "$NODE_DIR"/*.json; do
    count=$((count+1))
    port=$(jq -r .port "$file"); proto=$(jq -r .proto "$file")
    if [ "$proto" = u ]; then grep -qx "$port" <<<"$udp_list" && status="${GREEN}正常监听${PLAIN}" || status="${RED}未在监听${PLAIN}"
    else grep -qx "$port" <<<"$tcp_list" && status="${GREEN}正常监听${PLAIN}" || status="${RED}未在监听${PLAIN}"
    fi
    tell ""
...
menu_change_domain(){
  ...
  else tell_warn "部署应用失败 (未回滚配置)"; wait_key
  fi
}
  ...
  while :; do
    ...
    case $(prompt "请选择") in
      ...
      6) 
         ...
         wait_key ;;
      7) 
         if [ "$SYS_ENV" = "kvm" ]; then systemctl stop sing-box && tell_ok "已停止" || tell_warn "操作异常"
         else rc-service sing-box stop && tell_ok "已停止" || tell_warn "操作异常"
         fi
         wait_key ;;
      0) break ;;
      *) tell_warn "输入无效，请重新选择"; sleep 1 ;;
...
peer_select(){
  ...
  apply_config && tell_ok "已接管: $(jq -r .name "$PICKED")" || tell_warn "切换应用失败 (配置未回滚)"
  wait_key
}
...
peer_delete(){
  ...
  rm -f "$PICKED"
  apply_config && tell_ok "已删除" || tell_warn "删除失败或应用异常"
  ip link del "$WG_IF" 2>/dev/null
  wait_key
}

peer_stop(){
  clear; state_set exit direct
...
get_ip_info(){
  ...
  [ "$SYS_ENV" = "lxc" ] && [ "$(state_get exit)" != "direct" ] && proxy_env="http://127.0.0.1:2080"
...
menu_client_status(){
  ...
  [ "$SYS_ENV" = "lxc" ] && tell "  提示: LXC无TUN环境目前仅支持全局 HTTP/SOCKS5 接管(不支持UDP转发)"
  tell ""
  render_client_ip_status
  wait_key
}
...
wg_tunnel_state(){
  local peer_ip=$1
  [ "$SYS_ENV" = "kvm" ] && [ ! -d "/sys/class/net/$WG_IF" ] && { echo down; return; }
  
  if [ "$SYS_ENV" = "kvm" ] && command -v ping >/dev/null 2>&1 && ping -c1 -W3 "$peer_ip" >/dev/null 2>&1; then 
    echo up; return; 
  fi
  
  local proxy_env=""
  [ "$SYS_ENV" = "lxc" ] && proxy_env="http://127.0.0.1:2080"
...
wg_setup(){
  ...
  sys_bool=$([ "$SYS_ENV" = "kvm" ] && echo true || echo false)
...
wg_toggle(){
  ...
  if [ "$(jq -r .enabled "$WG_CONF")" = true ]; then
    ...
    apply_config && tell_ok "已关闭隧道" || tell_warn "应用失败，强制阻断"
  else
...
run_uninstall(){
  ...
  if [ "$SYS_ENV" = "kvm" ]; then
    ...
    rm -f "$SERVICE_UNIT" /etc/profile.d/sbm_proxy.sh
  fi
...
menu_status(){
  while :; do
    ...
    if [ "$SYS_ENV" = "kvm" ]; then systemctl is-active --quiet sing-box && s_state="${GREEN}正常运行${PLAIN}"; else rc-service sing-box status >/dev/null 2>&1 && s_state="${GREEN}正常运行${PLAIN}"; fi
...
case $1 in
  --sync) 
    init_dirs
    if [ "$SYS_ENV" = "kvm" ]; then sync_bypass_rules; sync_hopping_rules; else sync_proxy_env; fi
    exit 0 ;;
  --clear-hopping) [ "$SYS_ENV" = "kvm" ] && clear_hopping_rules; exit 0 ;;
...
while :; do
  ...
  tell "${CYAN}================================${PLAIN}"
  tell "${CYAN}             s管理              ${PLAIN}"
  tell "${CYAN}   [ 当前运行环境: ${SYS_ENV^^} 模式 ]   ${PLAIN}"
  tell "${CYAN}================================${PLAIN}"
  tell "  1. 服务端管理"
  tell "  2. 客户端管理"
  tell "  3. WireGuard 管理"
