...
build_config(){
  ...
  dns_block=$(jq -n --arg host "$peer_host" \
    ...
    [
      {type:"udp",tag:"dns-cf-v4",server:$cf4},
      {type:"udp",tag:"dns-cf-v6",server:$cf6},
      {type:"udp",tag:"dns-google-v4",server:$gg4},
      {type:"udp",tag:"dns-google-v6",server:$gg6},
      {type:"udp",tag:"dns-direct-cf-v4",server:$cf4},
      {type:"udp",tag:"dns-direct-cf-v6",server:$cf6},
      {type:"udp",tag:"dns-direct-google-v4",server:$gg4},
      {type:"udp",tag:"dns-direct-google-v6",server:$gg6}
    ] as $servers |
    {
      servers: $servers,
      rules: ($node_rules + [
...
create_hysteria2(){
  ...
  while :; do up_mbps=$(prompt "上行带宽 (Mbps, 0为不限制)" "0"); [[ $up_mbps =~ ^[0-9]+$ ]] && break || tell_warn "输入无效"; done
  while :; do down_mbps=$(prompt "下行带宽 (Mbps, 0为不限制)" "0"); [[ $down_mbps =~ ^[0-9]+$ ]] && break || tell_warn "输入无效"; done
  ...
  else tell "已设置宽带上限，将启用 Brutal，BBR profile 不生效"; fi
  hopping=""
...
build_uri(){
  ...
  case $kind in
    ...
    anytls)
      ...
      uri="anytls://$(uri_encode "$a_pw")@$host:$port/?sni=$host"
    ...
    case $(prompt "请选择") in
      ...
      6)
        if [ "$kind" = vless-reality ]; then
          ...
          while :; do up_mbps=$(prompt "上行带宽 (Mbps, 0为关闭限制)" "$(jq -r '.meta.up_mbps//0' "$PICKED")"); [[ $up_mbps =~ ^[0-9]+$ ]] && break || tell_warn "输入无效"; done
          while :; do down_mbps=$(prompt "下行带宽 (Mbps, 0为关闭限制)" "$(jq -r '.meta.down_mbps//0' "$PICKED")"); [[ $down_mbps =~ ^[0-9]+$ ]] && break || tell_warn "输入无效"; done
          if [ "$up_mbps" -gt 0 ] || [ "$down_mbps" -gt 0 ]; then
            ...
            json_edit "$PICKED" '.meta.up_mbps=0 | .meta.down_mbps=0 | del(.inbound.up_mbps,.inbound.down_mbps)' || { tell_warn 失败; wait_key; continue; }
            tell "  1. conservative"; tell "  2. standard"; tell "  3. aggressive"
            while :; do
              case $(prompt "BBR 拥塞控制" 2) in
