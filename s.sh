...
warp_trace(){
  local family=$1 mode res ip loc colo warp
  mode=$(warp_mode_get)
  case "$family" in
    4)
      if [ "$mode" = ipv4 ] || [ "$mode" = dual ]; then
        res=$(curl -4 --interface "$TUN_IF" -fsS --connect-timeout 5 --max-time 8 "$WARP_TRACE" 2>/dev/null) || return 1
      else
        res=$(curl -4 -fsS --connect-timeout 5 --max-time 8 "$WARP_TRACE" 2>/dev/null) || return 1
      fi
      ;;
    6)
      if [ "$mode" = ipv6 ] || [ "$mode" = dual ]; then
        res=$(curl -6 --interface "$TUN_IF" -fsS --connect-timeout 5 --max-time 8 "$WARP_TRACE" 2>/dev/null) || return 1
      else
        res=$(curl -6 -fsS --connect-timeout 5 --max-time 8 "$WARP_TRACE" 2>/dev/null) || return 1
      fi
      ;;
    *) return 2 ;;
  esac
...
warp_select_mode_and_start(){
  ...
  if warp_start; then
    ...
    tell ""
    tell_ok "启用成功，当前出口：$(warp_mode_label)"
    tell ""
    warp_show_exit
    wait_key
    return 0
