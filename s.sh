...
probe_peer_latency(){
  local file=$1 result=$2 latency_file=${3:-} server iface family out ms families
  ...
  for family in $families; do
    if [ "$family" = 6 ]; then
      ...
      iface=$NET_IF_V6
      out=$(ping -6 -I "$iface" -c 1 -W 2 "$server" 2>/dev/null)
    else
      [ -n "$NET_IF_V4" ] || continue
      iface=$NET_IF_V4
      out=$(ping -4 -I "$iface" -c 1 -W 2 "$server" 2>/dev/null)
    fi
    if [ $? -eq 0 ]; then
...
wg_tunnel_state(){
  local peer_ip4=$1 peer_ip6=$2 out
