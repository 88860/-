...
get_ip_info(){
  ...
  if [ -n "$selected" ] && [ "$selected" != direct ]; then
    ...
    if [ "$selected" = wireguard ]; then
      ...
      jq -n --argjson port "$port" --arg final "$selected" --argjson endpoint "$(jq -c '.endpoint' "$WG_CONF" 2>/dev/null)" '
        {log:{level:"warn"},dns:{servers:[{type:"https",tag:"dns-direct-v4",server:"1.1.1.1",server_port:443,path:"/dns-query",tls:{enabled:true,server_name:"cloudflare-dns.com"},detour:"direct"}],final:"dns-direct-v4",strategy:"prefer_ipv4"},inbounds:[{type:"mixed",tag:"status-in",listen:"127.0.0.1",listen_port:$port}],outbounds:[{type:"direct",tag:"direct"},($endpoint|.tag=$final|.domain_resolver="dns-direct-v4")],endpoints:[],route:{final:$final,default_domain_resolver:"dns-direct-v4",auto_detect_interface:false}}
      ...
      jq -n --argjson port "$port" --arg final "$selected" --argjson outbound "$outbound" '
        {log:{level:"warn"},dns:{servers:[{type:"https",tag:"dns-direct-v4",server:"1.1.1.1",server_port:443,path:"/dns-query",tls:{enabled:true,server_name:"cloudflare-dns.com"},detour:"direct"}],final:"dns-direct-v4",strategy:"prefer_ipv4"},inbounds:[{type:"mixed",tag:"status-in",listen:"127.0.0.1",listen_port:$port}],outbounds:[{type:"direct",tag:"direct"},$outbound],endpoints:[],route:{final:$final,default_domain_resolver:"dns-direct-v4",auto_detect_interface:false}}
      ' >"$tmp_conf" 2>/dev/null || { rm -f "$tmp_conf" "$tmp_log"; return 1; }
    fi
    if ! timeout 5 "$CORE" check -c "$tmp_conf" >/dev/null 2>&1; then
    ...
    if kill -0 "$pid" 2>/dev/null; then
      res=$(env -u http_proxy -u https_proxy -u HTTP_PROXY -u HTTPS_PROXY -u all_proxy -u ALL_PROXY \
        curl -sS --socks5-hostname "127.0.0.1:$port" --connect-timeout 2 -m 4 "$family_url" 2>/dev/null || true)
    fi
    kill "$pid" 2>/dev/null || true
    timeout 2 sh -c "wait $pid" 2>/dev/null || true
    rm -f "$tmp_conf" "$tmp_log"
  else
  ...
  if command -v curl >/dev/null 2>&1; then
    res=$(env -u http_proxy -u https_proxy -u HTTP_PROXY -u HTTPS_PROXY -u all_proxy -u ALL_PROXY \
      curl -sS --connect-timeout 2 -m 3 "https://ipwho.is/$ip" 2>/dev/null)
  elif command -v wget >/dev/null 2>&1; then
    res=$(wget -qO- -T 3 "https://ipwho.is/$ip" 2>/dev/null)
  else
    res=""
  fi
...
EOF
  ...
  dns=$(jq -n --arg strategy "$dns_strategy" --arg final "$final" --argjson v6ok "$dns_v6" '
    ...
    {servers:[
      {type:"https",tag:"dns-bootstrap-v4",server:"1.1.1.1",server_port:443,path:"/dns-query",tls:{enabled:true,server_name:"cloudflare-dns.com"},detour:"direct"},
      {type:"https",tag:"dns-bootstrap-v6",server:"2606:4700:4700::1111",server_port:443,path:"/dns-query",tls:{enabled:true,server_name:"cloudflare-dns.com"},detour:"direct"},
