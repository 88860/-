  ...
  if apply_config; then
    if state_set exit "$tag" && sync_proxy_env; then
      if transparent_proxy_available && ip link show "$TUN_IF" >/dev/null 2>&1; then
        ...
        tell_ok "已选择节点: $target_name（当前环境未启用 TUN 全局接管）"
      fi
    else
...
menu_client_status(){
  ...
  if transparent_proxy_available && ip link show "$TUN_IF" >/dev/null 2>&1; then
    ...
    tell "  网络接管：${YELLOW}未启用 TUN（已选择节点：${proxy_name}）${PLAIN}"
  else
...
wg_server_nat_apply(){
  command -v nft >/dev/null 2>&1 || { tell_warn "当前环境无 nftables，WireGuard 服务端 NAT 未启用"; return 1; }
  local prev4 prev6
