...
build_config(){
  ...
  local dns_direct_server dns_direct_tag dns_strategy auto_detect icmp_out
  ...
  outbounds=$(jq -n --arg resolver "$dns_direct_tag" '[{type:"direct",tag:"direct",domain_resolver:$resolver}]')
  ...
  dns_block=$(jq -n --arg direct "$dns_direct_server" --arg direct_tag "$dns_direct_tag" --arg strategy "$dns_strategy" --arg detour "$final" '
    {servers:[
      ...
      {type:"https",tag:"dns-remote",server:"cloudflare-dns.com",server_port:443,path:"/dns-query",tls:{enabled:true,server_name:"cloudflare-dns.com"},domain_resolver:$direct_tag,detour:$detour}
    ],
    final:"dns-remote",
    strategy:$strategy}')
