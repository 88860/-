...
build_config(){
  ...
  jq -n --argjson dns "$dns" --argjson tun "$tun" --argjson fallback "$fallback" --argjson rules "$rules" --argjson nodes "$node_inbounds" --argjson warp "$warp_ep" --arg direct "$ROUTE_DIRECT_OUTBOUND" --arg warp_tag "$WARP_ENDPOINT_TAG" '
    {log:{level:"warn",timestamp:true},dns:$dns,inbounds:$nodes,outbounds:[{type:"direct",tag:$direct}],route:{rules:$rules,final:$direct,auto_detect_interface:true,default_domain_resolver:"dns-control"}}
...
parse_uri(){
  ...
  if [ "$URI_SCHEME" = hysteria2 ] || [ "$URI_SCHEME" = hy2 ]; then
    ...
    URI_PORT=${port_spec:-443}; [[ "$URI_PORT" =~ ^[0-9]+$ ]] && [ "$URI_PORT" -ge 1 ] && [ "$URI_PORT" -le 65535 ] || URI_PORT=443
  fi
}
...
uri_to_outbound(){
  ...
  case $URI_SCHEME in
    ...
    vmess)
      ...
      outbound=$(jq -n --arg tag "$tag" --arg server "$URI_HOST" --argjson port "$URI_PORT" --arg uuid "$username" --arg alter "$password" '{type:"vmess",tag:$tag,server:$server,server_port:$port,uuid:$uuid,security:"auto"}')
    ...
    trojan)
      ...
      [ "$(query_value type)" = ws ] && outbound=$(jq --arg path "$(query_value path)" --arg host "$(query_value host)" '.transport=({type:"ws",path:(if $path=="" then "/" else $path end)}+(if $host=="" then {} else {headers:{Host:$host}} end))' <<<"$outbound")
      ;;
    hysteria2|hy2)
    ...
    snell) outbound=$(jq -n --arg tag "$tag" --arg server "$URI_HOST" --argjson port "$URI_PORT" --arg psk "$URI_USERINFO" '{type:"snell",tag:$tag,server:$server,server_port:$port,version:6,psk:$psk}');;
    shadowsocks|ss) outbound=$(jq -n --arg tag "$tag" --arg server "$URI_HOST" --argjson port "$URI_PORT" --arg method "$(query_value method)" --arg password "$URI_USERINFO" '{type:"shadowsocks",tag:$tag,server:$server,server_port:$port,method:(if $method=="" then "aes-256-gcm" else $method end),password:$password}');;
    *) return 1;;
  esac
  printf '%s' "$outbound"
}
...
peer_add(){
  ...
  outbound=$(uri_to_outbound "$tag") || { ui_warn '无法解析'; ui_wait; return; }
  probe=$(mktemp); printf '%s\n' "{\"log\":{\"level\":\"error\"},\"outbounds\":[$outbound,{\"type\":\"direct\",\"tag\":\"direct\"}],\"route\":{\"final\":\"direct\"}}" >"$probe"
  if "${SING_BOX_BIN:-$CORE}" check -c "$probe" >/dev/null 2>&1; then
    jq -n --arg tag "$tag" --arg name "$name" --arg uri "$uri" --argjson ob "$outbound" --arg hosts "$resolved_hosts" '{tag:$tag,name:$name,uri:$uri,outbound:$ob,probe_addresses:($hosts|split("\n")|map(select(length>0)))}' >"$PEER_ROOT/$tag.json"
    chmod 600 "$PEER_ROOT/$tag.json"; ui_ok "挂载完成: $name"
  else ui_warn '校验拦截'; "${SING_BOX_BIN:-$CORE}" check -c "$probe" 2>&1 | head -3 >&2; fi
  rm -f "$probe"; ui_wait
}
...
warp_has(){ warp_init; [ -s "$WARP_FILE" ] && jq -e '.private_key and .public_key and .config.interface.addresses.v4 and .config.peers[0].public_key' "$WARP_FILE" >/dev/null 2>&1; }
...
warp_keypair(){
  local out priv pub
  out=$(sing-box generate wg-keypair 2>/dev/null) || return 1
...
warp_register(){
  ...
  body=$(jq -n --arg key "$WARP_PUBLIC_KEY" --arg iid "$install_id" --arg fcm "$fcm" --arg tos "$tos" \
    '{key:$key,install_id:$iid,fcm_token:$fcm,tos:$tos,type:"PC",model:"s管理",serial_number:$iid,locale:"zh_CN",warp_enabled:true}')
...
warp_endpoint_json(){
  warp_has || return 1
  local a4 a6 peer endpoint ep_host ep_port reserved
  ...
  if [ -n "$endpoint" ] && [[ "$endpoint" == *:* ]]; then
    ep_host=${endpoint%:*}; ep_port=${endpoint##*:}; [ "$ep_port" = 0 ] && ep_port=2408
  else
    ep_host=162.159.192.1; ep_port=2408
  fi
  reserved=$(jq -c '.config.reserved // [0,0,0]' "$WARP_FILE")
...
warp_get_exit(){
  ...
  if ! "$CORE" check -c "$cfg" >/dev/null 2>&1; then rm -f "$cfg"; ui_warn '当前 sing-box 不支持 WARP WireGuard endpoint'; ui_wait; return 1; fi
  "$CORE" run -c "$cfg" >/dev/null 2>&1 & pid=$!
...
warp_show_config(){
  ...
  printf '  reserved: %s\n' "$(jq -c '.config.reserved // [0,0,0]' "$WARP_FILE")"
  ui_wait
}
warp_delete_config(){
  warp_has || { ui_warn 'WARP 尚未注册'; ui_wait; return 1; }
...
node_port(){
  ...
  while :; do
    ...
    ss -Hlnptu 2>/dev/null | awk '{print $5}' | grep -Eq "[:.]$p$" || { printf '%s' "$p"; return; }
  done
}
...
server_prompt_acme(){
  ...
  [[ "$(sing-box version 2>/dev/null)" == *with_acme* ]] || { ui_warn '当前 sing-box 未包含 with_acme 构建标签'; return 1; }
  state_set domain "$d"; state_set email "$e"; TLS_MODE=acme
}
server_prompt_tls(){
  local mode
...
server_tls_json(){
  if [ "${TLS_MODE:-cert}" = acme ]; then
    jq -n --arg d "$(state_get domain)" --arg e "$(state_get email)" \
      '{enabled:true,server_name:$d,certificate_provider:{type:"acme",domain:[$d],email:$e,data_directory:"/var/lib/kvm-new/acme"}}'
  else
...
protocol_transport_json(){
  case ${1:-tcp} in
    ...
    grpc) local svc host; svc=$(ui_prompt 'gRPC ServiceName' 'grpc'); host=$(ui_prompt 'CDN Host' "$(state_get domain)"); jq -n --arg s "$svc" --arg h "$host" '{type:"grpc",service_name:$s,headers:{Host:$h}}' ;;
    httpupgrade) local path host; path=$(ui_prompt 'HTTPUpgrade 路径' '/'); host=$(ui_prompt 'CDN Host' "$(state_get domain)"); jq -n --arg p "$path" --arg h "$host" '{type:"httpupgrade",path:$p,headers:{Host:$h}}' ;;
    *) return 1 ;;
  esac
}
protocol_apply_transport(){
  ...
  jq --argjson t "$t" '.inbound.transport=$t' "$f" >"$f.tmp" && mv -f "$f.tmp" "$f"
}
...
create_vless_tcp_tls(){
  local name port uuid tag f tls
  ui_title 'VLESS / TCP + TLS'; name=$(ui_prompt '节点名称' 'VLESS-TLS'); server_prompt_tls || return
  port=$(ui_prompt '监听端口' "$(node_port)"); uuid=$(ui_prompt 'UUID' "$(node_uuid)"); tag=$(protocol_new_file "$name") || return; f="$NODE_DIR/$tag.json"; tls=$(server_tls_json)
  jq -n --arg tag "$tag" --arg name "$name" --arg uuid "$uuid" --argjson port "$port" --argjson tls "$tls" \
    '{tag:$tag,name:$name,kind:"vless",port:$port,tls_mode:"cert",meta:{uuid:$uuid,transport:"tcp"},inbound:{type:"vless",tag:$tag,listen:"::",listen_port:$port,users:[{uuid:$uuid,flow:"xtls-rprx-vision"}],tls:$tls}}' >"$f"
  server_save_and_apply "$f" tcp
}
create_vless_reality(){
  local name port uuid target kp priv pub sid tag f
  ui_title 'VLESS / TCP + Reality'; name=$(ui_prompt '节点名称' 'VLESS-Reality'); port=$(ui_prompt '监听端口' "$(node_port)"); uuid=$(ui_prompt 'UUID' "$(node_uuid)"); target=$(ui_prompt 'Reality 握手域名' 'www.microsoft.com')
...
create_vless_transport(){
  ...
  ui_title "VLESS / $mode"; name=$(ui_prompt '节点名称' "VLESS-${mode}"); server_prompt_tls || return; port=$(ui_prompt '监听端口' "$(node_port)"); uuid=$(ui_prompt 'UUID' "$(node_uuid)"); tag=$(protocol_new_file "$name") || return; f="$NODE_DIR/$tag.json"; tls=$(server_tls_json)
...
create_vmess_tcp(){
  local name port uuid tag f; ui_title 'VMess / TCP'; name=$(ui_prompt '节点名称' 'VMess-TCP'); port=$(ui_prompt '监听端口' "$(node_port)"); uuid=$(ui_prompt 'UUID' "$(node_uuid)"); tag=$(protocol_new_file "$name") || return; f="$NODE_DIR/$tag.json"
...
create_vmess_transport(){
  local mode=$1 name port uuid tag f tls; ui_title "VMess / $mode"; name=$(ui_prompt '节点名称' "VMess-${mode}"); server_prompt_tls || return; port=$(ui_prompt '监听端口' "$(node_port)"); uuid=$(ui_prompt 'UUID' "$(node_uuid)"); tag=$(protocol_new_file "$name") || return; f="$NODE_DIR/$tag.json"; tls=$(server_tls_json)
...
create_trojan_tcp_tls(){
  local name port pw tag f tls; ui_title 'Trojan / TCP + TLS'; name=$(ui_prompt '节点名称' 'Trojan-TLS'); server_prompt_tls || return; port=$(ui_prompt '监听端口' "$(node_port)"); pw=$(ui_prompt '密码' "$(node_secret_base64)"); tag=$(protocol_new_file "$name") || return; f="$NODE_DIR/$tag.json"; tls=$(server_tls_json)
  jq -n --arg tag "$tag" --arg name "$name" --arg pw "$pw" --argjson port "$port" --argjson tls "$tls" '{tag:$tag,name:$name,kind:"trojan",port:$port,tls_mode:"cert",meta:{password:$pw,transport:"tcp"},inbound:{type:"trojan",tag:$tag,listen:"::",listen_port:$port,users:[{password:$pw}],tls:$tls}}' >"$f"; server_save_and_apply "$f" tcp
}
create_trojan_transport(){
  local mode=$1 name port pw tag f tls; ui_title "Trojan / $mode"; name=$(ui_prompt '节点名称' "Trojan-${mode}"); server_prompt_tls || return; port=$(ui_prompt '监听端口' "$(node_port)"); pw=$(ui_prompt '密码' "$(node_secret_base64)"); tag=$(protocol_new_file "$name") || return; f="$NODE_DIR/$tag.json"; tls=$(server_tls_json)
...
create_ss(){
  local method_label=$1 method=$2 name port pw tag f
  ui_title "Shadowsocks / $method_label"; name=$(ui_prompt '节点名称' "SS-${method_label}"); port=$(ui_prompt '监听端口' "$(node_port)")
  ...
  jq -n --arg tag "$tag" --arg name "$name" --arg pw "$pw" --arg method "$method" --argjson port "$port" '{tag:$tag,name:$name,kind:"shadowsocks",port:$port,tls_mode:"none",meta:{method:$method,password:$pw},inbound:{type:"shadowsocks",tag:$tag,listen:"::",listen_port:$port,method:$method,password:$pw}}' >"$f"; server_save_and_apply "$f" tcp
}
create_shadowtls(){
  local version=$1 name port stpw method sspw target tag ssport f
  ui_title "ShadowTLS + Shadowsocks / v${version}"; name=$(ui_prompt '节点名称' "ShadowTLS-v${version}"); port=$(ui_prompt 'ShadowTLS 监听端口' "$(node_port)"); target=$(ui_prompt '握手域名' 'www.microsoft.com'); stpw=$(ui_prompt 'ShadowTLS 密码' "$(node_secret)")
  method=$(ui_prompt 'Shadowsocks 加密方式' 'aes-256-gcm'); sspw=$(ui_prompt 'Shadowsocks 密码' "$(node_secret)"); ssport=$((port+1)); tag=$(protocol_new_file "$name") || return; f="$NODE_DIR/$tag.json"
  jq -n --arg tag "$tag" --arg ss "$tag-ss" --arg name "$name" --arg target "$target" --arg stpw "$stpw" --arg method "$method" --arg sspw "$sspw" --argjson port "$port" --argjson ssport "$ssport" --argjson version "$version" \
    '{tag:$tag,name:$name,kind:"shadowtls-shadowsocks",port:$port,tls_mode:"shadowtls",meta:{shadowtls_password:$stpw,method:$method,password:$sspw,handshake:$target,version:$version},inbound:{type:"shadowtls",tag:$tag,listen:"::",listen_port:$port,version:$version,handshake:{server:$target,server_port:443},detour:$ss},embedded_inbounds:[{type:"shadowsocks",tag:$ss,listen:"127.0.0.1",listen_port:$ssport,method:$method,password:$sspw}]}' >"$f"
  server_save_and_apply "$f" tcp
}
create_snell(){
  local version=$1 mode name port psk tag f; ui_title "Snell / v${version}"; name=$(ui_prompt '节点名称' "Snell-v${version}"); port=$(ui_prompt '监听端口' "$(node_port)"); psk=$(ui_prompt 'PSK (12-255 字节)' "$(node_secret)")
  if [ "$version" = 5 ]; then mode=$(ui_prompt 'HTTP 混淆' 'none'); else mode=$(ui_prompt '流量整形模式' 'default'); fi
  tag=$(protocol_new_file "$name") || return; f="$NODE_DIR/$tag.json"
  if [ "$version" = 5 ]; then jq -n --arg tag "$tag" --arg name "$name" --arg psk "$psk" --arg mode "$mode" --argjson port "$port" '{tag:$tag,name:$name,kind:"snell",port:$port,tls_mode:"none",meta:{psk:$psk,version:5,obfs_mode:$mode},inbound:{type:"snell",tag:$tag,listen:"::",listen_port:$port,version:5,psk:$psk,obfs_mode:$mode}}' >"$f"; else jq -n --arg tag "$tag" --arg name "$name" --arg psk "$psk" --arg mode "$mode" --argjson port "$port" '{tag:$tag,name:$name,kind:"snell",port:$port,tls_mode:"none",meta:{psk:$psk,version:6,mode:$mode},inbound:{type:"snell",tag:$tag,listen:"::",listen_port:$port,version:6,psk:$psk,mode:$mode}}' >"$f"; fi
  server_save_and_apply "$f" tcp
}
create_socks5(){
  local name user pw port tag f; ui_title 'SOCKS5'; name=$(ui_prompt '节点名称' 'SOCKS5'); user=$(ui_prompt '用户名' 'admin'); pw=$(ui_prompt '密码' "$(node_secret)"); port=$(ui_prompt '监听端口' "$(node_port)"); tag=$(protocol_new_file "$name") || return; f="$NODE_DIR/$tag.json"
...
create_hysteria2(){
  local profile=$1 name port pw tag f tls up down
  ui_title "Hysteria2 / $profile"; name=$(ui_prompt '节点名称' "Hysteria2-${profile}"); server_prompt_tls || return; port=$(ui_prompt '监听端口' "$(node_port)"); pw=$(ui_prompt '密码' "$(node_secret)"); tag=$(protocol_new_file "$name") || return; f="$NODE_DIR/$tag.json"; tls=$(server_tls_json)
  if [ "$profile" = brutal ]; then
    up=$(ui_prompt '上行带宽 Mbps' '100'); down=$(ui_prompt '下行带宽 Mbps' '100')
    jq -n --arg tag "$tag" --arg name "$name" --arg pw "$pw" --argjson up "$up" --argjson down "$down" --argjson port "$port" --argjson tls "$tls" \
      '{tag:$tag,name:$name,kind:"hysteria2",port:$port,tls_mode:"cert",meta:{password:$pw,profile:"brutal",up_mbps:$up,down_mbps:$down},inbound:{type:"hysteria2",tag:$tag,listen:"::",listen_port:$port,users:[{password:$pw}],up_mbps:$up,down_mbps:$down,tls:$tls}}' >"$f"
  else
    jq -n --arg tag "$tag" --arg name "$name" --arg pw "$pw" --arg profile "$profile" --argjson port "$port" --argjson tls "$tls" \
      '{tag:$tag,name:$name,kind:"hysteria2",port:$port,tls_mode:"cert",meta:{password:$pw,bbr_profile:$profile},inbound:{type:"hysteria2",tag:$tag,listen:"::",listen_port:$port,users:[{password:$pw}],bbr_profile:$profile,tls:$tls}}' >"$f"
  fi
  server_save_and_apply "$f" tcp
}
create_hysteria2_obfs(){
  local name port pw obfs opw tag f tls; ui_title 'Hysteria2 / 混淆'; name=$(ui_prompt '节点名称' 'Hysteria2-Obfs'); server_prompt_tls || return; port=$(ui_prompt '监听端口' "$(node_port)"); pw=$(ui_prompt '密码' "$(node_secret)"); obfs=$(ui_prompt '混淆类型' 'salamander'); opw=$(ui_prompt '混淆密码' "$(node_secret)"); tag=$(protocol_new_file "$name") || return; f="$NODE_DIR/$tag.json"; tls=$(server_tls_json)
  jq -n --arg tag "$tag" --arg name "$name" --arg pw "$pw" --arg obfs "$obfs" --arg opw "$opw" --argjson port "$port" --argjson tls "$tls" '{tag:$tag,name:$name,kind:"hysteria2",port:$port,tls_mode:"cert",meta:{password:$pw,obfs:$obfs},inbound:{type:"hysteria2",tag:$tag,listen:"::",listen_port:$port,users:[{password:$pw}],obfs:{type:$obfs,password:$opw},tls:$tls}}' >"$f"; server_save_and_apply "$f" tcp
}
create_tuic(){
  local cc=$1 name port uuid pw tag f tls; ui_title "TUIC / $cc"; name=$(ui_prompt '节点名称' "TUIC-${cc}"); server_prompt_tls || return; port=$(ui_prompt '监听端口' "$(node_port)"); uuid=$(ui_prompt 'UUID' "$(node_uuid)"); pw=$(ui_prompt '密码' "$(node_secret)"); tag=$(protocol_new_file "$name") || return; f="$NODE_DIR/$tag.json"; tls=$(server_tls_json)
  cc=$(tr '[:upper:]' '[:lower:]' <<<"$cc"); [ "$cc" = 'new reno' ] && cc='new_reno'
  jq -n --arg tag "$tag" --arg name "$name" --arg uuid "$uuid" --arg pw "$pw" --arg cc "$cc" --argjson port "$port" --argjson tls "$tls" '{tag:$tag,name:$name,kind:"tuic",port:$port,tls_mode:"cert",meta:{uuid:$uuid,password:$pw,congestion_control:$cc},inbound:{type:"tuic",tag:$tag,listen:"::",listen_port:$port,users:[{uuid:$uuid,password:$pw}],congestion_control:$cc,tls:$tls}}' >"$f"; server_save_and_apply "$f" tcp
}
create_anytls(){
  local name port pw tag f tls; ui_title 'AnyTLS'; name=$(ui_prompt '节点名称' 'AnyTLS'); server_prompt_tls || return; port=$(ui_prompt '监听端口' "$(node_port)"); pw=$(ui_prompt '密码' "$(node_secret)"); tag=$(protocol_new_file "$name") || return; f="$NODE_DIR/$tag.json"; tls=$(server_tls_json)
  jq -n --arg tag "$tag" --arg name "$name" --arg pw "$pw" --argjson port "$port" --argjson tls "$tls" '{tag:$tag,name:$name,kind:"anytls",port:$port,tls_mode:"cert",meta:{password:$pw},inbound:{type:"anytls",tag:$tag,listen:"::",listen_port:$port,users:[{password:$pw}],tls:$tls}}' >"$f"; server_save_and_apply "$f" tcp
}
...
menu_create_protocol(){
  while :; do
    ...
    ui_back; ui_footer
    case $(ui_prompt '请选择') in
      1) menu_vless ;; 2) menu_vmess ;; 3) menu_trojan ;; 4) menu_shadowsocks ;; 5) menu_shadowtls ;; 6) menu_snell ;; 7) create_socks5 ;; 8) menu_hysteria2 ;; 9) menu_tuic ;; 10) create_anytls ;; 0) break ;; *) ui_invalid ;;
    esac
  done
}
menu_vless(){ while :; do ui_title 'VLESS'; ui_menu 1 'TCP + TLS' 2 'TCP + Reality' 3 'WS + CDN' 4 'gRPC + CDN' 5 'HTTPUpgrade + CDN'; ui_back; ui_footer; case $(ui_prompt '请选择') in 1) create_vless_tcp_tls;;2) create_vless_reality;;3) create_vless_transport ws;;4) create_vless_transport grpc;;5) create_vless_transport httpupgrade;;0) break;;*) ui_invalid;;esac; done; }
menu_vmess(){ while :; do ui_title 'VMess'; ui_menu 1 'TCP' 2 'WS + CDN' 3 'gRPC + CDN'; ui_back; ui_footer; case $(ui_prompt '请选择') in 1) create_vmess_tcp;;2) create_vmess_transport ws;;3) create_vmess_transport grpc;;0) break;;*) ui_invalid;;esac; done; }
menu_trojan(){ while :; do ui_title 'Trojan'; ui_menu 1 'TCP + TLS' 2 'WS + CDN' 3 'gRPC + CDN'; ui_back; ui_footer; case $(ui_prompt '请选择') in 1) create_trojan_tcp_tls;;2) create_trojan_transport ws;;3) create_trojan_transport grpc;;0) break;;*) ui_invalid;;esac; done; }
menu_shadowsocks(){ while :; do ui_title 'Shadowsocks'; ui_menu 1 'SS2022 AES-128-GCM' 2 'SS2022 AES-256-GCM' 3 'SS2022 ChaCha20-Poly1305' 4 'AES-128-GCM' 5 'AES-192-GCM' 6 'AES-256-GCM' 7 'ChaCha20-Poly1305'; ui_back; ui_footer; case $(ui_prompt '请选择') in 1) create_ss 'SS2022 AES-128-GCM' '2022-blake3-aes-128-gcm';;2) create_ss 'SS2022 AES-256-GCM' '2022-blake3-aes-256-gcm';;3) create_ss 'SS2022 ChaCha20-Poly1305' '2022-blake3-chacha20-poly1305';;4) create_ss 'AES-128-GCM' 'aes-128-gcm';;5) create_ss 'AES-192-GCM' 'aes-192-gcm';;6) create_ss 'AES-256-GCM' 'aes-256-gcm';;7) create_ss 'ChaCha20-Poly1305' 'chacha20-ietf-poly1305';;0) break;;*) ui_invalid;;esac; done; }
menu_shadowtls(){ while :; do ui_title 'ShadowTLS + Shadowsocks'; ui_menu 1 'ShadowTLS v1' 2 'ShadowTLS v2' 3 'ShadowTLS v3'; ui_back; ui_footer; case $(ui_prompt '请选择') in 1) create_shadowtls 1;;2) create_shadowtls 2;;3) create_shadowtls 3;;0) break;;*) ui_invalid;;esac; done; }
menu_snell(){ while :; do ui_title 'Snell'; ui_menu 1 'Snell v5' 2 'Snell v6'; ui_back; ui_footer; case $(ui_prompt '请选择') in 1) create_snell 5;;2) create_snell 6;;0) break;;*) ui_invalid;;esac; done; }
menu_socks5(){ while :; do ui_title 'SOCKS5'; ui_tell '  用户名 / 密码'; ui_back; ui_footer; case $(ui_prompt '请选择') in 0) break;;*) create_socks5;;esac; done; }
menu_hysteria2(){ while :; do ui_title 'Hysteria2'; ui_menu 1 'conservative' 2 'standard' 3 'aggressive' 4 'brutal' 5 '混淆'; ui_back; ui_footer; case $(ui_prompt '请选择') in 1) create_hysteria2 conservative;;2) create_hysteria2 standard;;3) create_hysteria2 aggressive;;4) create_hysteria2 brutal;;5) create_hysteria2_obfs;;0) break;;*) ui_invalid;;esac; done; }
menu_tuic(){ while :; do ui_title 'TUIC'; ui_menu 1 'CUBIC' 2 'New Reno' 3 'BBR'; ui_back; ui_footer; case $(ui_prompt '请选择') in 1) create_tuic cubic;;2) create_tuic 'New Reno';;3) create_tuic bbr;;0) break;;*) ui_invalid;;esac; done; }

# ===== MODULE: server/menu.sh =====

select_node(){
  local files=() file i=1 n
...
prompt_port(){
  ...
  while :; do
    ...
    [[ "$p" =~ ^[0-9]+$ ]] && [ "$p" -ge 1 ] && [ "$p" -le 65535 ] && { printf '%s' "$p"; return 0; }
    ...
    case $(ui_prompt '请选择') in
      ...
      7)
        ...
        if core_stop >/dev/null 2>&1; then
          if apply_config_quiet; then ui_ok '已停止'; else ui_warn '服务已停止，但配置同步失败'; fi
        else
...
menu_server_info(){
  ...
  for file in "$NODE_DIR"/*.json; do
    [ -e "$file" ] || continue
    count=$((count+1)); port=$(jq -r .port "$file" 2>/dev/null); proto=$(jq -r .proto "$file" 2>/dev/null)
    if [ "$proto" = u ]; then
...
menu_change_domain(){
  server_prompt_tls || { ui_wait; return; }
}
  ...
  while :; do
    ...
    case $(ui_prompt '请选择') in
      1)
        ...
        ;;
      2)
        value=$(prompt_port "$(jq -r '.proto // "t"' "$PICKED")" "$(jq -r '.port // .inbound.listen_port' "$PICKED")") || continue
      ...
      4)
        ...
        [ -n "$value" ] || continue
        jq --arg v "$value" 'if .kind=="socks5" then .meta.username=$v|.inbound.users[0].username=$v else .meta.password=$v|.inbound.users[0].password=$v end' "$PICKED" >"$PICKED.tmp" && mv -f "$PICKED.tmp" "$PICKED" || { ui_warn '修改失败'; ui_wait; continue; }
        ;;
      0) return ;;
      *) ui_invalid; continue ;;
    esac
