...
wg_endpoint_route_excludes(){
  ...
  if wg_endpoint_valid_ip "$host"; then
    ...
    if [[ "$host" == *:* ]]; then printf '%s/128\n' "$host"; else printf '%s/32\n' "$host"; fi
    return 0
  fi
  raw=''
  if [ -n "$v4_iface" ]; then
    raw=$(curl -sS -f --connect-timeout 3 -m 4 -G \
      -H 'Accept: application/dns-json' \
      --data-urlencode "name=$host" --data-urlencode 'type=A' \
      --resolve 'cloudflare-dns.com:443:1.1.1.1' \
      --interface "$v4_iface" \
      'https://cloudflare-dns.com/dns-query' 2>/dev/null) || raw=''
  fi
  if [ -n "$raw" ] && jq -e '.Status==0' >/dev/null 2>&1 <<<"$raw"; then
    while IFS= read -r a; do
      [[ "$a" =~ ^([0-9]{1,3}\.){3}[0-9]{1,3}$ ]] || continue
      wg_route_ok "$a" || continue
      printf '%s/32\n' "$a"
      found=1
    done < <(jq -r '.Answer[]? | select(.type==1) | .data' <<<"$raw" 2>/dev/null)
  fi
  raw=''
  if [ -n "$v6_iface" ]; then
    raw=$(curl -sS -f --connect-timeout 3 -m 4 -G \
      -H 'Accept: application/dns-json' \
      --data-urlencode "name=$host" --data-urlencode 'type=AAAA' \
      --resolve 'cloudflare-dns.com:443:[2606:4700:4700::1111]' \
      --interface "$v6_iface" \
      'https://cloudflare-dns.com/dns-query' 2>/dev/null) || raw=''
  fi
  if [ -n "$raw" ] && jq -e '.Status==0' >/dev/null 2>&1 <<<"$raw"; then
    while IFS= read -r a; do
      ...
      printf '%s/128\n' "$a"
      found=1
    done < <(jq -r '.Answer[]? | select(.type==28) | .data' <<<"$raw" 2>/dev/null)
  fi
  [ "$found" = 1 ]
}
        ...
        if [ "$wg_endpoint" = "yes" ]; then
          ...
          if [ -n "$peer_host" ]; then
            wg_exclude_addresses=$(wg_endpoint_route_excludes "$peer_host" | sort -u | jq -Rsc 'split("\n")|map(select(length>0))') || {
              tell_warn "WireGuard 服务端地址解析或物理路由检查失败，保留当前运行配置" >&2
              return 1
            }
            [ "$(jq 'length' <<<"$wg_exclude_addresses")" -gt 0 ] || {
              tell_warn "WireGuard 服务端没有可用的物理路由地址" >&2
              return 1
            }
          fi
          final="wireguard"
