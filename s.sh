...
render_share_uri(){
  ...
  if [ -n "$NET_IF_V4" ]; then
    physical_v4=$(curl -4 -fsS --connect-timeout 2 --max-time 4 --interface "$NET_IF_V4" https://ipv4.icanhazip.com 2>/dev/null | tr -d '\n ')
  fi
  if [ -n "$NET_IF_V6" ]; then
    physical_v6=$(curl -6 -fsS --connect-timeout 2 --max-time 4 --interface "$NET_IF_V6" https://ipv6.icanhazip.com 2>/dev/null | tr -d '\n ')
  fi
