...
sync_hopping_rules(){
  ...
  nft add rule inet "$HOP_TABLE" prerouting iifname != { "lo", "$WG_IF", "$TUN_IF" } udp dport "$range" redirect to :"$port" 2>/dev/null
}
...
uri_to_outbound(){
  ...
  if [ "$URI_SCHEME" = "vless" ] || [ "$URI_SCHEME" = "trojan" ] || [ "$URI_SCHEME" = "anytls" ]; then
    outbound=$(jq '.multiplex={enabled:true,protocol:"smux",max_connections:4,padding:true}' <<<"$outbound")
  fi
