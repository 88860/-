...
build_config(){
  ...
  dns_cf_v4="1.1.1.1"; dns_cf_v6="2606:4700:4700::1111"
  dns_google_v4="8.8.8.8"; dns_google_v6="2001:4860:4860::8888"
  ...
  dns_block=$(jq -n --arg host "$peer_host" \
    ...
    (if $host != "" and ($host | test("^[0-9]+\\.[0-9]+\\.[0-9]+\\.[0-9]+$") | not) and ($host | test("^[0-9a-fA-F:]+$") | not) then
      [{action:"evaluate",server:"dns-direct-cf-v4",tag:"node-a"},
       {action:"evaluate",server:"dns-direct-cf-v6",tag:"node-aaaa"},
       {action:"evaluate",server:"dns-direct-google-v4",tag:"node-gg-a"},
       {action:"evaluate",server:"dns-direct-google-v6",tag:"node-gg-aaaa"},
       {match_response:"node-a",action:"route",server:"dns-direct-cf-v4",race:true},
       {match_response:"node-aaaa",action:"route",server:"dns-direct-cf-v6",race:true},
       {match_response:"node-gg-a",action:"route",server:"dns-direct-google-v4",race:true},
       {match_response:"node-gg-aaaa",action:"route",server:"dns-direct-google-v6",race:true}]
    else [] end) as $node_rules |
    [
      {type:"udp",tag:"dns-cf-v4",server:$cf4,detour:$final},
      {type:"udp",tag:"dns-cf-v6",server:$cf6,detour:$final},
      {type:"udp",tag:"dns-google-v4",server:$gg4,detour:$final},
      {type:"udp",tag:"dns-google-v6",server:$gg6,detour:$final},
      {type:"udp",tag:"dns-direct-cf-v4",server:$cf4,detour:"direct"},
      {type:"udp",tag:"dns-direct-cf-v6",server:$cf6,detour:"direct"},
      {type:"udp",tag:"dns-direct-google-v4",server:$gg4,detour:"direct"},
      {type:"udp",tag:"dns-direct-google-v6",server:$gg6,detour:"direct"}
    ] as $servers |
    {
      servers: $servers,
      rules: ($node_rules + [
        {action:"evaluate",server:"dns-cf-v4",tag:"remote-a"},
        {action:"evaluate",server:"dns-cf-v6",tag:"remote-aaaa"},
        {action:"evaluate",server:"dns-google-v4",tag:"remote-gg-a"},
        {action:"evaluate",server:"dns-google-v6",tag:"remote-gg-aaaa"},
        {match_response:"remote-a",action:"route",server:"dns-cf-v4",race:true},
        {match_response:"remote-aaaa",action:"route",server:"dns-cf-v6",race:true},
        {match_response:"remote-gg-a",action:"route",server:"dns-google-v4",race:true},
...
create_hysteria2(){
  ...
  up_mbps=0; down_mbps=0; bbr_profile=""
  tell ""
  tell "  1. BBR     (自动调节，无需带宽)"
  tell "  2. Brutal  (需指定带宽上下行)"
  while :; do
    case $(prompt "请选择拥塞控制方式" 1) in
      1)
        tell ""
        tell "  1. conservative"; tell "  2. standard (默认)"; tell "  3. aggressive"
        while :; do
        ...
        tell ""
        tell "  已启用 BBR (${bbr_profile})，无需设置带宽"
        break ;;
      2)
        tell ""
        while :; do
        ...
        tell ""
        tell "  已启用 Brutal (上行 ${up_mbps} / 下行 ${down_mbps} Mbps，BBR 已关闭)"
        break ;;
      *) tell_warn "输入无效，请重新选择"; sleep 1 ;;
    esac
  done
...
build_uri(){
  ...
  case $kind in
    ...
    hysteria2)
      local obfs_type obfs_pw obfs_str hop_str bbr up_mbps down_mbps bbr_str
      obfs_type=$(jq -r '.obfs_type//""' <<<"$meta")
      obfs_pw=$(jq -r '.obfs_password//""' <<<"$meta")
      bbr=$(jq -r '.bbr_profile//""' <<<"$meta")
      up_mbps=$(jq -r '.up_mbps//0' <<<"$meta")
      down_mbps=$(jq -r '.down_mbps//0' <<<"$meta")
      obfs_str=""
      [ -n "$obfs_type" ] && obfs_str="&obfs=${obfs_type}&obfs-password=$(uri_encode "$obfs_pw")"
      hop_str=""
      [ -n "$hopping" ] && hop_str="&mport=$hopping"
      bbr_str=""
      if [ -n "$bbr" ] && [ "$up_mbps" = "0" ] && [ "$down_mbps" = "0" ]; then
        bbr_str="&bbr_profile=$(uri_encode "$bbr")"
      fi
      uri="hysteria2://$(uri_encode "$(jq -r .password <<<"$meta")")@$host:$port/?sni=$host&alpn=h3${bbr_str}${obfs_str}${hop_str}#$(uri_encode "$name")" ;;
    tuic)
...
menu_modify_protocol(){
  ...
  local bbr_profile min_pkt max_pkt cur_up cur_down cur_bbr cur_mode new_bbr
          ...
          while :; do
            case $(prompt "请选择拥塞控制方式" "$cur_mode") in
              1)
                ...
                while :; do
                  case $(prompt "BBR 拥塞控制" "2") in
                    1) new_bbr="conservative"; break ;;
                    2) new_bbr="standard"; break ;;
                    3) new_bbr="aggressive"; break ;;
                    *) tell_warn "输入无效"; sleep 1 ;;
                  esac
                done
                json_edit "$PICKED" '
                  .meta.up_mbps=0 | .meta.down_mbps=0 | .meta.bbr_profile=$b |
                  del(.inbound.up_mbps,.inbound.down_mbps) | .inbound.bbr_profile=$b
                ' --arg b "$new_bbr" || { tell_warn 失败; wait_key; continue 2; }
                tell_ok "已启用 BBR (${new_bbr})"
                break ;;
              2)
                tell ""
                while :; do
                ...
                json_edit "$PICKED" '
                  .meta.up_mbps=$up | .meta.down_mbps=$down | del(.meta.bbr_profile) |
                  .inbound.up_mbps=$up | .inbound.down_mbps=$down | del(.inbound.bbr_profile)
...
uri_to_outbound(){
  ...
  case $URI_SCHEME in
    ...
    hysteria2|hy2)
      ...
      [ -n "$bbr_profile" ] && outbound=$(jq --arg bbr "$bbr_profile" '.bbr_profile=$bbr' <<<"$outbound")
