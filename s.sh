  ...
  (
    ...
    while :; do
      ...
      while IFS= read -r line; do
        case "$line" in
          *" lo "*|*"lo:"*|*"docker"*|*"veth"*|*"br-"*|*"virbr"*|*"$TUN_IF"*|*"$WG_IF"*|*"$WARP_IF"*|*"tailscale"*|*"zt"*) continue ;;
        esac
...
config_build_tun_inbounds(){
  local inbounds=$1 name=$TUN_IF route='[]'
  case "${2:-dual}" in
    ipv4) route='["0.0.0.0/0"]' ;;
    ipv6) route='["::/0"]' ;;
    dual) route='["0.0.0.0/0","::/0"]' ;;
  esac
  jq -n --argjson list "$inbounds" --arg name "$name" --argjson route "$route" '
    [{type:"tun",tag:"tun-in",interface_name:$name,
      ...
      mtu:1500} + (if ($route|length)>0 then {route_address:$route} else {} end)] + $list'
}
...
build_config(){
  ...
  if [ "$use_tun" = 1 ]; then
    if [ "$selected" = warp ]; then
      inbounds=$(config_build_tun_inbounds "$inbounds" "$(warp_mode_get)") || return 1
    else
...
install_core(){
  ...
  if [ -n "$digest" ]; then
    ...
    expected=${digest#sha256:}; actual=$(sha256sum "$tmp/core.tar.gz" | awk '{print $1}')
...
warp_registered(){
  [ -s "$WARP_CONF" ] && jq -e '.id and .token and .private_key and .config.peers[0].public_key and .config.peers[0].endpoint.host and (.config.peers[0].endpoint.port|tonumber) and (.config.interface.addresses.v4 or .config.interface.addresses.v6)' "$WARP_CONF" >/dev/null 2>&1
}
...
warp_refresh(){
  ...
  tmp=$(mktemp) || return 1
  if ! warp_api_request GET "$WARP_API/$id" "$token" '' "$tmp" || \
     ! jq -e --arg id "$id" '.id==$id and .config.peers[0].public_key and .config.peers[0].endpoint.host and (.config.peers[0].endpoint.port|tonumber) and (.config.interface.addresses.v4 or .config.interface.addresses.v6)' "$tmp" >/dev/null 2>&1; then
    rm -f "$tmp"; return 1
  fi
...
warp_endpoint(){
  local resolver=$1 mode private peer addr4 addr6 host port allowed addresses
  mode=$(warp_mode_get)
  ...
  jq -n --arg resolver "$resolver" --arg host "$host" --argjson port "$port" --arg private "$private" --arg peer "$peer" \
    --argjson addresses "$addresses" --argjson allowed "$allowed" \
...
warp_status(){
  ...
  if systemctl is-active --quiet sing-box 2>/dev/null && ip link show "$WARP_IF" >/dev/null 2>&1; then echo running; else echo stopped; fi
}

warp_register(){
  clear; tell "${CYAN}✦ 注册 WARP${PLAIN}"
  if warp_registered; then
    tell "  当前状态：${GREEN}已注册${PLAIN}"; tell ""; tell "  WARP IPv4：$(warp_config_get '.config.interface.addresses.v4')"; tell "  WARP IPv6：$(warp_config_get '.config.interface.addresses.v6')"; tell ""
    prompt_yes "是否重新注册 WARP" || return
  else
    tell "  当前状态：${DIM}未注册${PLAIN}"; tell ""; prompt_yes "是否注册 WARP" || return
  fi
  if [ "$(warp_status)" = running ]; then
    local previous_exit
    previous_exit=$(state_exit_get) || previous_exit=warp
    state_exit_set direct || { tell_warn "出口状态切换失败"; wait_key; return; }
    stop_watchdog || { state_exit_set "$previous_exit" || true; tell_warn "看门狗停止失败"; wait_key; return; }
    if ! apply_config_quiet; then
      state_exit_set "$previous_exit" || true; apply_config_quiet || true
      tell_warn "停止当前 WARP 失败"; wait_key; return
    fi
  fi
  mkdir -p "$WARP_DIR" || { tell_warn "WARP 目录创建失败"; wait_key; return; }
  local old_id old_token keypair private public install_id fcm tos body response_file
  old_id=$(warp_config_get '.id'); old_token=$(warp_config_get '.token')
  keypair=$("$CORE" generate wg-keypair 2>/dev/null) || { tell_warn "WARP 密钥生成失败"; wait_key; return; }
  private=$(awk '/PrivateKey/{print $2}' <<<"$keypair"); public=$(awk '/PublicKey/{print $2}' <<<"$keypair")
  [ -n "$private" ] && [ -n "$public" ] || { tell_warn "WARP 密钥生成失败"; wait_key; return; }
  ...
  tell "  正在注册..."; response_file=$(mktemp) || { tell_warn "临时响应文件创建失败"; wait_key; return; }
  if ! warp_api_request POST "$WARP_API" '' "$body" "$response_file" || ! jq -e '.account and .config.peers[0].public_key and .config.peers[0].endpoint.host and (.config.peers[0].endpoint.port|tonumber) and (.config.interface.addresses.v4 or .config.interface.addresses.v6) and .token and .id' "$response_file" >/dev/null 2>&1; then
    rm -f "$response_file"; tell_warn "WARP 注册失败"; wait_key; return
  fi
  if ! jq --arg pk "$private" --arg pub "$public" --arg iid "$install_id" '{id:.id,token:.token,private_key:$pk,public_key:$pub,install_id:$iid,account:.account,config:.config,warp_mode:"dual",registered_at:(now|todateiso8601)}' "$response_file" | json_write "$WARP_CONF"; then
    rm -f "$response_file"; tell_warn "WARP 配置保存失败"; wait_key; return
  fi
  ...
  if [ -n "$old_id" ] && [ -n "$old_token" ] && [ "$old_id" != "$(warp_config_get '.id')" ]; then
    ...
    if [ -n "$old_response" ] && ! warp_api_request DELETE "$WARP_API/$old_id" "$old_token" '' "$old_response"; then
      tell_warn "新注册已成功，但旧 WARP 注册删除失败"
    fi
    rm -f "$old_response" 2>/dev/null || true
  fi
  tell_ok "注册成功"
  clear; tell "${CYAN}✦ WARP 出口${PLAIN}"; tell ""; tell "  1. 获取 IPv4"; tell "  2. 获取 IPv6"; tell "  3. 获取双栈"; tell "  0. 返回"
  case $(prompt "请选择") in
    1) warp_mode_set ipv4 || tell_warn "模式保存失败"; wait_key ;;
    2) warp_mode_set ipv6 || tell_warn "模式保存失败"; wait_key ;;
    3) warp_mode_set dual || tell_warn "模式保存失败"; wait_key ;;
  esac
}

warp_trace(){
  local family=$1 res ip loc colo warp
  case "$family" in 4) res=$(curl -4 -fsS --connect-timeout 5 --max-time 8 "$WARP_TRACE" 2>/dev/null) || return 1;; 6) res=$(curl -6 -fsS --connect-timeout 5 --max-time 8 "$WARP_TRACE" 2>/dev/null) || return 1;; *) return 2;; esac
...
warp_handshake_ok(){
  local mode info4 info6
  mode=$(warp_mode_get)
  case "$mode" in
    ipv4) info4=$(warp_trace 4) || return 1; [ "$(cut -f4 <<<"$info4")" = on ] ;;
    ipv6) info6=$(warp_trace 6) || return 1; [ "$(cut -f4 <<<"$info6")" = on ] ;;
    dual) info4=$(warp_trace 4) || return 1; info6=$(warp_trace 6) || return 1; [ "$(cut -f4 <<<"$info4")" = on ] && [ "$(cut -f4 <<<"$info6")" = on ] ;;
  esac
}

warp_fetch_exit(){
  clear; tell "${CYAN}✦ WARP 出口${PLAIN}"
  ...
  mode=$(warp_mode_get); warp_handshake_ok || tell_alert "Cloudflare 尚未确认 WARP 出口"
  case "$mode" in
  ...
  tell "  接管：$(warp_mode_label)"; tell ""; [ -n "$ip4" ] && tell "  IPv4：$ip4 | ${loc4:-??} | ${colo4:-未知} | WARP=${warp4:-off}"; [ -n "$ip6" ] && tell "  IPv6：$ip6 | ${loc6:-??} | ${colo6:-未知} | WARP=${warp6:-off}"
  if { [ "$mode" = ipv4 ] && [ "$warp4" != on ]; } || { [ "$mode" = ipv6 ] && [ "$warp6" != on ]; } || { [ "$mode" = dual ] && { [ "$warp4" != on ] || [ "$warp6" != on ]; }; }; then tell_warn "检测到所选地址族未确认经过 WARP"; wait_key; return 1; fi
  wait_key
}

warp_stop(){
  clear; tell "${CYAN}✦ WARP${PLAIN}"; tell "  正在停止..."
  local previous; previous=$(state_exit_get) || previous=direct
  if [ "$previous" = warp ]; then state_exit_set direct || { tell_warn "出口状态更新失败"; wait_key; return 1; }; stop_watchdog || true; apply_config_quiet || { tell_warn "sing-box 停止 WARP 失败"; wait_key; return 1; }
  else
    apply_config_quiet || true
  fi
...
warp_start(){
  clear; tell "${CYAN}✦ WARP${PLAIN}"; tell "  正在启动..."
  ...
  local previous mode endpoint previous_wg=0
  mode=$(warp_mode_get); endpoint=$(warp_endpoint "$( [ "$(network_ipv6_ok)" = 1 ] && printf dns-direct-v6 || printf dns-direct-v4 )") || { tell_warn "WARP 配置不完整"; wait_key; return 1; }
  ...
  state_exit_set warp || { tell_warn "出口状态更新失败"; wait_key; return 1; }
  json_edit "$WARP_CONF" '.started_at=$started' --arg started "$(date -u '+%Y-%m-%dT%H:%M:%SZ')" || true
  stop_watchdog || true
  if ! apply_config_quiet; then
    state_exit_set "$previous" || true
    [ "$previous_wg" = 1 ] && wg_set_enabled true || true
    apply_config_quiet || true
    tell_warn "sing-box WARP 配置启动失败"; wait_key; return 1
  fi
  tell_ok "启动成功（$(warp_mode_label)，由 sing-box 接管 WireGuard）"; wait_key
}

warp_status_view(){
  clear; tell "${CYAN}✦ WARP 状态${PLAIN}"
  case "$(warp_status)" in unconfigured) tell "  状态：${DIM}未配置${PLAIN}"; wait_key; return;; stopped) tell "  状态：${YELLOW}已停止${PLAIN}"; wait_key; return;; esac
  local mode info4 info6 ip4 ip6 loc4 loc6 colo4 colo6 warp4 warp6 started now created up_secs up_h up_m
  mode=$(warp_mode_get); warp_handshake_ok && tell "  WireGuard：${GREEN}Cloudflare 已确认${PLAIN}" || tell "  WireGuard：${YELLOW}尚未确认${PLAIN}"
  case "$mode" in
    ipv4) info4=$(warp_trace 4); ip4=$(cut -f1 <<<"$info4"); loc4=$(cut -f2 <<<"$info4"); colo4=$(cut -f3 <<<"$info4"); warp4=$(cut -f4 <<<"$info4") ;;
    ipv6) info6=$(warp_trace 6); ip6=$(cut -f1 <<<"$info6"); loc6=$(cut -f2 <<<"$info6"); colo6=$(cut -f3 <<<"$info6"); warp6=$(cut -f4 <<<"$info6") ;;
    dual) info4=$(warp_trace 4); info6=$(warp_trace 6); ip4=$(cut -f1 <<<"$info4"); loc4=$(cut -f2 <<<"$info4"); colo4=$(cut -f3 <<<"$info4"); warp4=$(cut -f4 <<<"$info4"); ip6=$(cut -f1 <<<"$info6"); loc6=$(cut -f2 <<<"$info6"); colo6=$(cut -f3 <<<"$info6"); warp6=$(cut -f4 <<<"$info6") ;;
  esac
  tell "  状态：${GREEN}运行中${PLAIN}"; tell "  接管：$(warp_mode_label)"; tell ""
  [ -n "$ip4" ] && tell "  IPv4：$ip4 | ${loc4:-??} | ${colo4:-未知} | WARP=${warp4:-off}"; [ -n "$ip6" ] && tell "  IPv6：$ip6 | ${loc6:-??} | ${colo6:-未知} | WARP=${warp6:-off}"
...
warp_delete(){
  clear; tell "${CYAN}✦ 删除 WARP 配置${PLAIN}"
  ...
  if [ "$previous" = warp ]; then
    state_exit_set direct || { tell_warn "出口状态更新失败"; wait_key; return 1; }
    stop_watchdog || { state_exit_set warp || true; tell_warn "看门狗停止失败"; wait_key; return 1; }
    if ! apply_config_quiet; then
      state_exit_set warp || true; apply_config_quiet || true
  ...
  if [ -n "$id" ] && [ -n "$token" ]; then
    ...
    if ! warp_api_request DELETE "$WARP_API/$id" "$token" '' "$response_file"; then
      rm -f "$response_file"; tell_warn "Cloudflare 注册删除失败，未删除本地配置"; wait_key; return 1
    fi
    rm -f "$response_file"
  fi
  rm -f "$WARP_CONF"
  tell_ok "配置已删除"; wait_key
}
...
render_client_ip_status(){
  local info ip country colo warp
  info=$(get_ip_info 4)
  if [ -n "$info" ]; then
    IFS=$'\t' read -r ip country colo warp <<<"$info"
    tell "  IPv4: ${GREEN}${ip}${PLAIN} | 地区: ${YELLOW}${country:-??}${PLAIN}"
    tell "  Cloudflare: ${CYAN}${colo:-未知}${PLAIN} | WARP: ${PURPLE}${warp:-off}${PLAIN}"
