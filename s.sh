...
get_ip_info(){
  ...
  if [ -n "$selected" ] && [ "$selected" != direct ]; then
    ...
    if ! jq --argjson port "$port" --arg final "$selected" '
      ...
      | .outbounds |= map(if .tag==$final then .domain_resolver="dns-direct-v4" else select(.tag=="direct") end)
      | .endpoints = ((.endpoints // []) | map(.domain_resolver="dns-direct-v4"))
      | .dns={servers:[{type:"https",tag:"dns-direct-v4",server:"1.1.1.1",server_port:443,path:"/dns-query",tls:{enabled:true,server_name:"cloudflare-dns.com"}}],final:"dns-direct-v4",strategy:"prefer_ipv4"}
      | .route={final:$final,default_domain_resolver:"dns-direct-v4",auto_detect_interface:false}
