...
build_dns_block(){
  local strat="$1" final="$2"
  local bserver="1.1.1.1"
  [ "$strat" = "ipv6_only" ] && bserver="2606:4700:4700::1111"
  
  jq -n --arg strat "$strat" --arg final "$final" --arg bserver "$bserver" '{
    servers: [
      {
...
build_config(){
  ...
  if [ ${#node_files[@]} -gt 0 ] && [ -n "$domain" ]; then
    ...
    case "$chal" in
      dns_cloudflare) chal_ext=$(jq -n --arg t "$(state_get cf_token)" '{dns01_challenge:{provider:"cloudflare",api_token:$t}}') ;;
      dns_alidns)     chal_ext=$(jq -n --arg k "$(state_get ali_key)" --arg s "$(state_get ali_secret)" '{dns01_challenge:{provider:"alidns",access_key_id:$k,access_key_secret:$s}}') ;;
      dns_acmedns)    chal_ext=$(jq -n --arg url "$(state_get acmedns_url)" --arg u "$(state_get acmedns_user)" --arg p "$(state_get acmedns_pass)" --arg sub "$(state_get acmedns_sub)" '{dns01_challenge:{provider:"acmedns",server_url:$url,username:$u,password:$p,subdomain:$sub}}') ;;
      http)           chal_ext='{"disable_tls_alpn_challenge":true}' ;;
      alpn)           chal_ext='{"disable_http_challenge":true}' ;;
    esac
    providers=$(jq -n --arg d "$domain" --arg e "$(state_get email)" --arg dir "$ACME_DIR" --argjson ext "$chal_ext" '[{
  ...
  local dns_block
  dns_block=$(build_dns_block "$strat" "$final")
