...
probe_peer_latency(){
  local file=$1 result=$2 latency_file=${3:-} server targets target family iface out ms
  ...
  case "$server" in
    *:*) targets=$server ;;
    *[!0-9.]*) targets=$(resolve_addresses "$server") ;;
    *) targets=$server ;;
  esac
  ...
  while IFS= read -r target; do
    [ -n "$target" ] || continue
    case "$target" in
      *:*) family=6; iface=${NET_IF_V6:-} ;;
      *.*) family=4; iface=${NET_IF_V4:-} ;;
      *) continue ;;
    esac
    [ -n "$iface" ] || continue
    out=$(timeout 3 ping -"$family" -I "$iface" -c 1 -W 3 "$target" 2>/dev/null)
    if [ $? -eq 0 ]; then
      ms=$(printf '%s\n' "$out" | awk -F'time=' '/time=/{gsub(/ ms.*/,"",$2); printf "%.0f\n",$2; exit}')
