...
ssh_ports(){
  {
    ...
    ss -Hlntu 2>/dev/null | awk '/sshd/{print $4}' | sed 's/.*://'
    ...
    while read -r file tag type name port host; do
      ...
      (
        ...
        if [ "$type" != "hysteria2" ] && [ "$type" != "tuic" ]; then
          res=$(curl -x socks5h://127.0.0.1:$dir_port -w "%{time_connect}" -s -o /dev/null -m 2 "http://$host:$port" 2>/dev/null)
        fi
        
        if [ -z "$res" ] || [ "$res" = "0.000" ]; then
           res=$(curl -x socks5h://127.0.0.1:$dir_port -w "%{time_connect}" -s -o /dev/null -m 2 "http://$host:22" 2>/dev/null)
        fi
        
        if [ -z "$res" ] || [ "$res" = "0.000" ]; then
           res=$(curl -x socks5h://127.0.0.1:$dir_port -w "%{time_connect}" -s -o /dev/null -m 2 "http://$host:443" 2>/dev/null)
        fi
