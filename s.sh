...
build_config(){
  ...
  # 1.14 官方规范彻底修复：完全抛弃老的 type、server 格式，只能使用 address 进行标准路由分发
  ...
  # 1.14 官方规范彻底修复：补齐 default_domain_resolver，否则无法进行域名请求匹配
  jq -n --argjson inbounds "$inbounds" --argjson outbounds "$outbounds" \
    ...
    {log:{level:"warn",timestamp:true},
     ...
     inbounds:$inbounds,
     outbounds:$outbounds,
     route:{rules:$rules,final:$final,auto_detect_interface:$auto_if,default_domain_resolver:"dns-direct"}}
