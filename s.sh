...
STATE_LOCK=$SBM_DIR/state.lock
HOP_LOCK=$SBM_DIR/hopping.lock
...
init_dirs(){
  mkdir -p "$NODE_DIR" "$PEER_DIR" "$ACME_DIR" "$SB_DIR" /var/lib/sing-box
...
state_set(){
  local k=$1 v=$2
  exec 8>"$STATE_LOCK"
  flock -w 10 8 || { flock -u 8; return 1; }
  json_edit "$STATE" '.[$k]=$v' --arg k "$k" --arg v "$v"
  local rc=$?
  flock -u 8
  return $rc
}
...
default_iface_v4(){
  ...
  if [ -z "$iface" ]; then
    iface=$(ip -4 route show table main default 2>/dev/null | awk -v t="$TUN_IF" \
      '$5!=t {print $5; exit}')
  fi
  printf '%s' "$iface"
}
default_iface_v6(){
  ...
  if [ -z "$iface" ]; then
    iface=$(ip -6 route show table main default 2>/dev/null | awk -v t="$TUN_IF" \
      '$5!=t {print $5; exit}')
  fi
  printf '%s' "$iface"
}
...
load_net_cache(){
  local old4 old6 oldif4 oldif6 oldstack
  if [ -f "$NET_CACHE" ]; then
    { IFS= read -r old4; IFS= read -r old6
      IFS= read -r oldif4; IFS= read -r oldif6
      IFS= read -r oldstack; } < "$NET_CACHE"
  fi
  NET_IPV4=""; NET_IPV6=""; NET_IF_V4=""; NET_IF_V6=""; NET_STACK=""
  probe_network_stack
  if [ "$NET_STACK" = "none" ] || [ -z "$NET_STACK" ]; then
    NET_IPV4="$old4"; NET_IPV6="$old6"; NET_IF_V4="$oldif4"; NET_IF_V6="$oldif6"; NET_STACK="$oldstack"
  fi
  if [ -n "$NET_STACK" ] && [ "$NET_STACK" != "none" ]; then
    printf '%s\n%s\n%s\n%s\n%s\n'       "$NET_IPV4" "$NET_IPV6" "$NET_IF_V4" "$NET_IF_V6" "$NET_STACK"       > "$NET_CACHE" 2>/dev/null || true
    chmod 600 "$NET_CACHE" 2>/dev/null || true
  fi
}
start_net_monitor(){
  stop_net_monitor
...
install_core(){
  ...
  tmp=$(mktemp -d)
  TMP_FILES="$TMP_FILES $tmp"
  if ! curl -4 -fL --retry 3 --retry-delay 2 --connect-timetell 15 -m 240 "$url" -o "$tmp/core.tar.gz" 2>/dev/null; then
    if ! curl -fL --retry 3 --retry-delay 2 --connect-timetell 15 -m 240 "$url" -o "$tmp/core.tar.gz" 2>/dev/null; then
...
protected_ports(){ { ssh_ports; node_ports; echo 2081; } | grep -E '^[1-9][0-9]*$' | sort -un; }

listening_ports(){
  local proto=$1
  case "$proto" in
    t) ss -Hlnpt 2>/dev/null ;;
    u) ss -Hlnpu 2>/dev/null ;;
    *) return 1 ;;
  esac | awk '{print $4}' | sed -n 's/.*:\([0-9][0-9]*\)$/\1/p' | grep -E '^[1-9][0-9]*$' | sort -un
}
...
hopping_node(){
  ...
  for file in "$NODE_DIR"/*.json; do
    [ -f "$file" ] || continue
    hop=$(jq -r '.hopping//""' "$file" 2>/dev/null)
    if [[ "$hop" =~ ^[0-9]+-[0-9]+$ ]]; then
      printf '%s\n' "$file"
      return 0
    fi
...
sync_hopping_rules(){
  exec 6>"$HOP_LOCK"
  flock -w 10 6 || { flock -u 6; return 1; }
  local file range port rc=0
  file=$(hopping_node) || { nft delete table inet "$HOP_TABLE" 2>/dev/null || true; flock -u 6; return 0; }
  range=$(jq -r '.hopping//""' "$file" 2>/dev/null) || { flock -u 6; return 1; }
  port=$(jq -r '.port//0' "$file" 2>/dev/null) || { flock -u 6; return 1; }
  [[ "$range" =~ ^[0-9]+-[0-9]+$ ]] || { flock -u 6; return 1; }
  [[ "$port" =~ ^[1-9][0-9]*$ ]] || { flock -u 6; return 1; }
  nft delete table inet "$HOP_TABLE" 2>/dev/null || true
  nft -f - <<EOF
add table inet $HOP_TABLE
add chain inet $HOP_TABLE prerouting { type nat hook prerouting priority dstnat; policy accept; }
add rule inet $HOP_TABLE prerouting iifname != "$TUN_IF" udp dport $range redirect to :$port
EOF
  rc=$?
  flock -u 6
  return $rc
...
build_config(){
  local selected domain inbounds outbounds rules dns_block final use_tun
  local peer_host node_files tls_extra providers strategy probe_target
  local dns_direct_server dns_remote_server dns_remote_v6_server auto_detect
  local domain_resolver_obj
  ...
  [ -z "$probe_target" ] && [ "$final" != "direct" ] && probe_target="$final"
  if [ -n "$probe_target" ] && [ "$probe_target" != "direct" ] && [ -f "$PEER_DIR/$probe_target.json" ]; then
  ...
  if [ "$use_tun" = 1 ]; then
    ...
    inbounds=$(jq -n --argjson list "$inbounds" --arg name "$TUN_IF" --argjson excl "$exclude_addrs" '
      [{type:"tun",tag:"tun-in",interface_name:$name,
        ...
        auto_route:true,strict_route:true,dns_mode:"hijack",auto_redirect:true,mtu:9000,
  ...
  jq -n --argjson inbounds "$inbounds" --argjson outbounds "$outbounds" \
        --argjson rules "$rules" --argjson dns "$dns_block" \
        --argjson providers "$providers" --arg final "$final" \
        --argjson auto_detect "$auto_detect" --argjson domain_resolver "$domain_resolver_obj" '
...
apply_config_locked(){
  ...
  if ! error=$("$CORE" check -c "$tmp" 2>&1); then
    tell_warn "配置校验未通过:"
    while IFS= read -r line; do tell "    $line"; done <<<"$(head -10 <<<"$error")"
    rm -f "$tmp"; return 1
  fi
  install -m600 "$tmp" "$CONFIG"; rm -f "$tmp"
  if ! sync_hopping_rules; then
    tell_warn "端口跳跃规则应用失败，回滚配置"
    if [ -n "$prev_conf" ] && [ -s "$prev_conf" ]; then
      install -m600 "$prev_conf" "$CONFIG"
      sync_hopping_rules || true
      timetell 15 systemctl restart sing-box >/dev/null 2>&1
    else
      nft delete table inet "$HOP_TABLE" 2>/dev/null || true
      rm -f "$CONFIG"
      timetell 15 systemctl stop sing-box >/dev/null 2>&1
    fi
    return 1
  fi
  if ! timetell 15 systemctl restart sing-box >/dev/null 2>&1; then
...
apply_config(){
  exec 7>"$CONFIG_LOCK"
  flock -w 30 7 || { tell_warn "配置操作正在进行，请稍后重试"; flock -u 7; return 1; }
  apply_config_locked
  local rc=$?
  flock -u 7
  return $rc
...
stop_watchdog(){
  timetell 15 systemctl stop sbm-watchdog.service 2>/dev/null
...
run_watchdog(){
  ...
  local need_tun=0
  [ "$(state_get exit)" != "direct" ] && need_tun=1
  if [ "$need_tun" = 1 ]; then
    local i=0
  ...
  while :; do
    ...
    if [ "$target_exit" = "direct" ]; then
      if curl -s -o /dev/null --connect-timetell 4 -m 6 "$PROBE_URL" || \
         curl -s -o /dev/null --connect-timetell 4 -m 6 "$probe2"; then
        fail_count=0
      else
    ...
    if [ "$current" = "direct" ]; then
      ...
      if env http_proxy="$proxy" curl -s -o /dev/null --connect-timetell 4 -m 6 "$PROBE_URL" || \
         env http_proxy="$proxy" curl -s -o /dev/null --connect-timetell 4 -m 6 "$probe2"; then
        fail_count=0
...
probe_handshake_target(){
  ...
  result=$(echo | timetell 10 openssl s_client -connect "$target:443" -servername "$target" -alpn h2 -tls1_3 2>/dev/null)
...
validate_domain(){
  ...
  case $(state_get challenge) in
    dns_cloudflare|dns_alidns|dns_acmedns)
      [[ "$domain" =~ ^([A-Za-z0-9-]+\.)+[A-Za-z]{2,63}$ ]] || { tell_warn "域名格式无效"; return 1; }
      tell_ok "DNS-01 验证方式无需域名解析到本机"
      return 0
      ;;
  esac
...
save_node(){
  ...
  if apply_config; then
    tell_ok "协议应用成功"; out_gap; render_share_uri "$file"
  else
...
render_share_uri(){
  ...
  if [ -z "$pub4$pub6" ]; then
    tell_warn "未识别到本机原生公网 IP，生成链接失败"
    return
  fi
  [ -n "$hopping" ] && tell "跳跃配置: $hopping"
  if [ "$kind" = "snell" ]; then
    tell_warn "Snell 分享链接可能不被所有客户端识别，请手动复制 PSK"
  fi
}
...
render_certificate_status(){
  ...
  while IFS= read -r file; do
    if openssl x509 -in "$file" -notell -checkhost "$domain" >/dev/null 2>&1; then
      crt="$file"
      break
    fi
  done < <(find "$ACME_DIR" -type f -name '*.crt' 2>/dev/null)
  [ -n "$crt" ] || { tell "  ${YELLOW}证书状态: 未找到匹配域名的证书${PLAIN}"; return 0; }
  expiry=$(openssl x509 -enddate -notell -in "$crt" 2>/dev/null | cut -d= -f2)
  ...
  while :; do
    ...
    case $(prompt "请选择") in
      ...
      6)
         ...
         if systemctl is-active --quiet sing-box; then
           if sync_hopping_rules; then
             ...
             tell_warn "singbox 已重启，但端口跳跃规则应用失败"
           fi
         else
      ...
      7)
         ...
         state_set exit direct
         stop_watchdog
         if timetell 15 systemctl stop sing-box 2>/dev/null; then
  ...
  if [ -n "$raw_data" ]; then
    ...
    while read -r file tag type name port host; do
      ...
      (
        ...
        if [ -n "$iface" ]; then
          ping_res=$(timetell 1 $cmd -I "$iface" -c 1 -W 1 -m 255 "$host" 2>/dev/null | awk -F'/' '/^rtt|^round-trip/{print $5}')
        fi
        if [ -n "$ping_res" ]; then
...
run_update(){
  ...
  if ! curl -4 -fL --retry 3 --retry-delay 2 --connect-timetell 15 -m 30 "$script_url" -o "$script_tmp" 2>/dev/null; then
    if ! curl -fL --retry 3 --retry-delay 2 --connect-timetell 15 -m 30 "$script_url" -o "$script_tmp" 2>/dev/null; then
...
menu_status(){
  while :; do
    ...
    local up_seconds up
    up_seconds=$(awk '{printf "%d", $1}' /proc/uptime 2>/dev/null)
    if [ -n "$up_seconds" ]; then
      local up_days=$((up_seconds/86400)) up_hours=$((up_seconds%86400/3600)) up_minutes=$((up_seconds%3600/60))
      if [ "$up_days" -gt 0 ]; then
        up="${up_days}天 ${up_hours}小时 ${up_minutes}分钟"
      elif [ "$up_hours" -gt 0 ]; then
        up="${up_hours}小时 ${up_minutes}分钟"
      else
        up="${up_minutes}分钟"
      fi
    else
...
bootstrap(){
  ...
  if [ ! -f "$CONFIG" ] || ! "$CORE" check -c "$CONFIG" >/dev/null 2>&1; then
    local bootstrap_conf
    bootstrap_conf=$(mktemp) || exit 1
    TMP_FILES="$TMP_FILES $bootstrap_conf"
    if ! build_config >"$bootstrap_conf" 2>/dev/null || ! "$CORE" check -c "$bootstrap_conf" >/dev/null 2>&1; then
      rm -f "$bootstrap_conf"
      echo -e "  ${RED}[×] sing-box 配置无效且无法自动重建${PLAIN}"
      exit 1
    fi
    install -m600 "$bootstrap_conf" "$CONFIG" || exit 1
    rm -f "$bootstrap_conf"
  fi
...
case $1 in
  ...
  --reapply)
    ...
    init_dirs
    exec 7>"$CONFIG_LOCK"
    flock -w 30 7 || { flock -u 7; flock -u 9; exit 0; }
    load_net_cache
    ...
    if [ "$NET_IF_V4" != "$prev_if4" ] || [ "$NET_IF_V6" != "$prev_if6" ]; then
      ...
      [ -n "$NET_IF_V6" ] || NET_IF_V6="$prev_if6"
      if [ -n "$NET_STACK" ] && [ "$NET_STACK" != "none" ]; then
        _cache_tmp=$(mktemp "${NET_CACHE}.XXXXXX") || { flock -u 7; flock -u 9; exit 1; }
        printf '%s\n%s\n%s\n%s\n%s\n' \
          "$NET_IPV4" "$NET_IPV6" "$NET_IF_V4" "$NET_IF_V6" "$NET_STACK" \
          > "$_cache_tmp" && mv -f "$_cache_tmp" "$NET_CACHE" || { rm -f "$_cache_tmp"; flock -u 7; flock -u 9; exit 1; }
      fi
    fi
    _new_conf=$(mktemp); TMP_FILES="$TMP_FILES $_new_conf"
    build_config > "$_new_conf" 2>/dev/null || { flock -u 7; flock -u 9; exit 1; }
    [ -s "$_new_conf" ] || { flock -u 7; flock -u 9; exit 1; }
    ...
    _prev_conf=""
    if [ -f "$CONFIG" ]; then
      _prev_conf=$(mktemp) || { flock -u 7; flock -u 9; exit 1; }
      cp "$CONFIG" "$_prev_conf" || { rm -f "$_prev_conf"; flock -u 7; flock -u 9; exit 1; }
    fi
    install -m600 "$_new_conf" "$CONFIG" || { rm -f "$_prev_conf"; flock -u 7; flock -u 9; exit 1; }
    if ! timetell 15 systemctl reload-or-restart sing-box >/dev/null 2>&1; then
