...
config_build_exit(){
  ...
  if [ "$selected" = warp ]; then
    outbounds=$(jq -n --arg v4 "$NET_IF_V4" --arg v6 "$NET_IF_V6" '
      [{type:"direct",tag:"direct"}]
      + (if $v4 != "" then [{type:"direct",tag:"direct-v4",bind_interface:$v4}] else [] end)
      + (if $v6 != "" then [{type:"direct",tag:"direct-v6",bind_interface:$v6}] else [] end)') || return 1
  fi
...
config_build_rules(){
  ...
  case "$mode" in
    ipv4)
      if [ -n "$NET_IF_V6" ]; then
        family_rules='[{"ip_cidr":["::/0"],"action":"route","outbound":"direct-v6"}]'
      else
        family_rules='[]'
      fi
      ;;
    ipv6)
      if [ -n "$NET_IF_V4" ]; then
        family_rules='[{"ip_cidr":["0.0.0.0/0"],"action":"route","outbound":"direct-v4"}]'
      else
        family_rules='[]'
      fi
      ;;
