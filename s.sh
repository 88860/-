...
probe_network_stack(){
  ...
  if [ -n "$local_v4" ]; then
    ...
    (local pref=10000
     while ip rule show 2>/dev/null | awk '{print $1}' | grep -qx "${pref}:"; do pref=$((pref+1)); done
  ...
  if [ -n "$local_v6" ]; then
    ...
    (local pref=10000
     while ip -6 rule show 2>/dev/null | awk '{print $1}' | grep -qx "${pref}:"; do pref=$((pref+1)); done
