...
render_wg_info(){
  ...
  if [ "$role" = client ]; then
    ...
    tell "  远端: $(local_ipv4):$(jq -r .listen_port "$file")"
...
wg_setup(){
  ...
  private=$(awk '/PrivateKey/{print $2}' <<<"$keypair")
  public=$(awk '/PublicKey/{print $2}' <<<"$keypair")
  [ -n "$private" ] || { tell_warn "失败"; wait_key; return; }
  ...
  if [ "$role" = server ]; then
    ...
    peer_host=$(prompt "出口端 IP")
    peer_port=$(prompt "出口端端口")
    peer_key=$(prompt "出口端公钥")
    [ -n "$peer_host" ] && [ -n "$peer_port" ] && [ -n "$peer_key" ] \
      || { tell_warn "数据不完整"; wait_key; return; }
    [[ $peer_port =~ ^[0-9]+$ ]] || { tell_warn "端口报错"; wait_key; return; }
    body=$(jq -n --arg private "$private" --arg public "$public" --arg a4 "$address4" --arg a6 "$address6" \
...
bootstrap(){
  ...
  if [ -n "${BASH_SOURCE[0]}" ] && [ -f "${BASH_SOURCE[0]}" ] && [[ ! "${BASH_SOURCE[0]}" =~ ^/dev/fd/ ]]; then
    s_path="$(readlink -f "${BASH_SOURCE[0]}")"
  elif [ -n "$0" ] && [ -f "$0" ] && [[ ! "$0" =~ ^/dev/fd/ ]] && [ "$0" != "bash" ] && [ "$0" != "sh" ]; then
    s_path="$(readlink -f "$0")"
  fi
  
  if [ -n "$s_path" ] && [ -s "$s_path" ] && [ "$s_path" != "$SELF" ]; then
    cp -f "$s_path" "$SELF" 2>/dev/null
  elif [ -s /proc/$$/fd/255 ] && [[ ! "$(readlink -f /proc/$$/fd/255 2>/dev/null)" =~ ^/dev/fd/ ]]; then
  ...
  if [ -s "$SELF" ]; then
    ...
    rm -f "$SELF" "$SHORTCUT" 2>/dev/null
    tell_warn "由于纯管道运行(如 curl|bash)且 Linux 底层限制，无法自动截获源码落盘。"
    tell_warn "如果您希望以后能直接输入 s 启动，建议通过实体文件方式拉取："
    tell_warn "wget -qO s.sh <链接> && bash s.sh"
    out_gap
  fi
