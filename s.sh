...
remote_version(){
  release_json | jq -r '.tag_name // ""' 2>/dev/null | sed 's/^v//'
}
...
probe_network_stack(){
  ...
  local local_v4="" local_v6="" v4_ok=0 v6_ok=0
  ...
  if [ -n "$local_v4" ] && curl --interface "$NET_IF_V4" -4 -s -m 3 --connect-timeout 2 "$PROBE_URL" >/dev/null 2>&1; then
    v4_ok=1
  fi
  if [ -n "$local_v6" ] && curl --interface "$NET_IF_V6" -6 -s -m 3 --connect-timeout 2 "$PROBE_URL" >/dev/null 2>&1; then
    v6_ok=1
  fi

  NET_IPV4=""; NET_IPV6=""
  if [ "$v4_ok" = 1 ]; then
    NET_IPV4=$(curl --interface "$NET_IF_V4" -4 -s -m 3 --connect-timeout 2 https://ipv4.icanhazip.com 2>/dev/null | tr -d '\n ')
    case "$NET_IPV4" in
      ""|10.*|192.168.*|127.*|172.1[6-9].*|172.2[0-9].*|172.3[0-1].*) NET_IPV4="" ;;
      *) is_cgnat_v4 "$NET_IPV4" && NET_IPV4="" ;;
    esac
  fi
  if [ "$v6_ok" = 1 ]; then
    NET_IPV6=$(curl --interface "$NET_IF_V6" -6 -s -m 3 --connect-timeout 2 https://ipv6.icanhazip.com 2>/dev/null | tr -d '\n ')
    case "$NET_IPV6" in ""|fe80:*|fc*|fd*|::1) NET_IPV6="" ;; esac
  fi

  if [ "$v4_ok" = 1 ] && [ "$v6_ok" = 1 ]; then NET_STACK="both"
  elif [ "$v4_ok" = 1 ]; then NET_STACK="v4"
  elif [ "$v6_ok" = 1 ]; then NET_STACK="v6"
...
build_config(){
  ...
  local dns_strategy auto_detect icmp_out
  ...
  [ "$NET_STACK" = "v6" ] && dns_strategy="prefer_ipv6"
  ...
  if [ "$final" = "direct" ]; then
    dns_block=$(jq -n --arg strategy "$dns_strategy" '
      {servers:[{type:"local",tag:"dns-direct"}],final:"dns-direct",strategy:$strategy}')
  elif [ "$NET_STACK" = "v6" ]; then
    dns_block=$(jq -n --arg strategy "$dns_strategy" --arg detour "$final" '
      {servers:[{type:"local",tag:"dns-direct"},
        {type:"https",tag:"dns-remote-v6",server:"2606:4700:4700::1111",server_port:443,path:"/dns-query",tls:{enabled:true,server_name:"cloudflare-dns.com"},detour:$detour}
      ],final:"dns-remote-v6",strategy:$strategy}')
  elif [ "$NET_STACK" = "both" ]; then
    dns_block=$(jq -n --arg strategy "$dns_strategy" --arg detour "$final" '
      {servers:[{type:"local",tag:"dns-direct"},
        {type:"https",tag:"dns-remote-v4",server:"1.1.1.1",server_port:443,path:"/dns-query",tls:{enabled:true,server_name:"cloudflare-dns.com"},detour:$detour},
        {type:"https",tag:"dns-remote-v6",server:"2606:4700:4700::1111",server_port:443,path:"/dns-query",tls:{enabled:true,server_name:"cloudflare-dns.com"},detour:$detour}
      ],rules:[
        {ip_version:4,action:"route",server:"dns-remote-v4"},
        {ip_version:6,action:"route",server:"dns-remote-v6"}
      ],final:"dns-remote-v4",strategy:$strategy}')
  else
    dns_block=$(jq -n --arg strategy "$dns_strategy" --arg detour "$final" '
      {servers:[{type:"local",tag:"dns-direct"},
        {type:"https",tag:"dns-remote-v4",server:"1.1.1.1",server_port:443,path:"/dns-query",tls:{enabled:true,server_name:"cloudflare-dns.com"},detour:$detour}
...
apply_config(){
  ...
  if ! timeout 15 systemctl reload-or-restart sing-box >/dev/null 2>&1; then
    ...
    out "    systemctl status:"
    systemctl status sing-box --no-pager -n 8 2>&1 | sed 's/^/    /' | head -20
    out "    journal:"
    journalctl -u sing-box -n 12 --no-pager 2>&1 | sed 's/^/    /' | head -30
...
render_share_uri(){
  ...
  [ -n "$hopping" ] && out "跳跃配置: $hopping"
  if [ "$kind" = "snell" ]; then out_warn "Snell 分享链接可能不被所有客户端识别，请手动复制 PSK"; fi
}
...
uri_to_outbound(){
  ...
  outbound=$(jq '.domain_resolver = "dns-direct"' <<<"$outbound")
  printf '%s' "$outbound"
}
...
peer_add(){
  ...
  jq -n --argjson ob "$outbound" '{log:{level:"error"},dns:{servers:[{type:"local",tag:"dns-direct"}],final:"dns-direct"},outbounds:[$ob,{type:"direct",tag:"direct"}],route:{final:"direct",default_domain_resolver:"dns-direct"}}' >"$probe"
...
get_ip_info(){
  local mode=$1 proxy=${2:-} res ip country asn name
  if [ -n "$proxy" ]; then
    res=$(curl -s -${mode} -x "$proxy" --proxy-insecure -m 8 https://ipwho.is/ 2>/dev/null)
  else
...
render_client_ip_status(){
  local exit_node proxy=""
  exit_node=$(state_get exit)
  if [ "$exit_node" != "direct" ]; then
    if ss -Hln 2>/dev/null | grep -q ':2081'; then
      proxy="http://127.0.0.1:2081"
    elif [ "$exit_node" = "wireguard" ]; then
      WATCHDOG_PROBE="$exit_node" apply_config_quiet >/dev/null 2>&1
      ss -Hln 2>/dev/null | grep -q ':2081' && proxy="http://127.0.0.1:2081"
    fi
  fi
  if [ "$exit_node" != "direct" ] && [ -z "$proxy" ]; then
    tell_warn "当前出口已选择，但状态检测通道尚未就绪"
    tell "  请稍后重新打开客户端状态"
    return
  fi
  get_ip_info 4 "$proxy"
...
wg_tunnel_state(){
  ...
  if command -v ping >/dev/null 2>&1 && ping -I "$WG_IF" -c1 -W3 "$peer_ip" >/dev/null 2>&1; then echo up; return; fi
  if command -v ping6 >/dev/null 2>&1; then
    local peer_ip6
    peer_ip6=$(jq -r '.peer_ip6 // empty' "$WG_CONF" 2>/dev/null)
    [ -n "$peer_ip6" ] && ping6 -I "$WG_IF" -c1 -W3 "$peer_ip6" >/dev/null 2>&1 && echo up && return
  fi
...
render_wg_info(){
  ...
  if [ "$role" = client ]; then
    local display_host
    display_host=$(jq -r '.peer_host // ""' "$file")
    if [[ $display_host == *:* ]]; then display_host="[$display_host]"; fi
    tell "  远端地址: ${display_host}:$(jq -r .peer_port "$file")"
  fi
  tell "  远端公钥: $(jq -r '.peer_public_key // "" | if .=="" then "<待补录>" else . end' "$file")"
  tell "  远端 IPv4: $(jq -r '.peer_ip4 // .peer_ip // ""' "$file")"
  tell "  远端 IPv6: $(jq -r '.peer_ip6 // ""' "$file")"
  if [ "$role" = client ]; then
    if [ "$(jq -r .enabled "$file")" = true ]; then
      if [ "$(wg_tunnel_state "$(jq -r .peer_ip "$file")")" = up ]; then tell_ok "连接成功"; else tell_warn "连接中断"; fi
    fi
  else
    echo ""; tell "  ${YELLOW}请在客户端填入:${PLAIN}"
    tell "  服务端 IPv4: $(local_ipv4):$(jq -r .listen_port "$file")"
    [ -n "$NET_IPV6" ] && tell "  服务端 IPv6: [$NET_IPV6]:$(jq -r .listen_port "$file")"
    tell "  公钥: $(jq -r .public_key "$file")"
    tell "  IPv4 前缀: $(jq -r '.address[0]' "$file" | cut -d. -f1-3)"
    tell "  IPv6 前缀: $(jq -r '.address[1]' "$file" | sed 's/::1\/64/::\/64/')"
  fi
}
...
wg_setup(){
  ...
  [ -n "$private" ] || { tell_warn "密钥生成失败"; wait_key; return; }
  while :; do
    prefix=$(prompt "IPv4 内网网段（前三段）" "10.7.0")
    if [[ $prefix =~ ^([0-9]+)\.([0-9]+)\.([0-9]+)$ ]] && [ "${BASH_REMATCH[1]}" -le 255 ] && [ "${BASH_REMATCH[2]}" -le 255 ] && [ "${BASH_REMATCH[3]}" -le 255 ]; then break; fi
    tell_warn "IPv4 网段前三段必须为 0-255 的数字"
  done
  local ipv6_prefix ipv6_input
  while :; do
    ipv6_input=$(prompt "IPv6 内网 /64（例如 fd00:7::/64）" "fd00:7::/64")
    ipv6_input=${ipv6_input%/64}
    if [[ $ipv6_input =~ ^[0-9A-Fa-f:]+$ ]] && [[ $ipv6_input == *:* ]]; then
      ipv6_prefix=${ipv6_input%::}
      ipv6_prefix=${ipv6_prefix%:}
      ipv6_prefix="${ipv6_prefix}::"
      break
    fi
    tell_warn "IPv6 内网必须是合法的十六进制前缀，例如 fd00:7::/64"
  done
  if [ "$role" = server ]; then
    ...
    body=$(jq -n --arg private "$private" --arg public "$public" --arg a4 "$address4" --arg a6 "$address6" \
     ...
     {role:"server",enabled:false,private_key:$private,public_key:$public,
      ...
      peer_public_key:$peer_key,peer_ip4:$peer4,peer_ip6:$peer6,peer_ip:$peer4,peer_host:"",peer_port:0,
    ...
    while :; do
      peer_host=$(prompt "服务端地址（IPv4 / IPv6 / 域名均可）" "1.1.1.1")
      peer_host=${peer_host#\[}
      peer_host=${peer_host%\]}
      if [[ $peer_host =~ ^([0-9]{1,3}\.){3}[0-9]{1,3}$ ]] ||
         [[ $peer_host =~ ^[0-9A-Fa-f:]+$ ]] && [[ $peer_host == *:* ]] ||
         [[ $peer_host =~ ^[A-Za-z0-9][A-Za-z0-9.-]*[A-Za-z0-9]$ ]]; then
        break
      fi
      tell_warn "服务端地址只能填写 IPv4、IPv6 或域名"
    done
    tell "  IPv6 示例：2001:db8::1；域名示例：wg.example.com"
    ...
    body=$(jq -n --arg private "$private" --arg public "$public" --arg a4 "$address4" --arg a6 "$address6" \
     ...
     {role:"client",enabled:false,private_key:$private,public_key:$public,
      ...
      peer_public_key:$peer_key,peer_ip4:$peer4,peer_ip6:$peer6,peer_ip:$peer4,peer_host:$host,peer_port:$port,
      endpoint:{type:"wireguard",tag:"wireguard",system:true,name:$iface,
        mtu:1280,
...
wg_fill_peer_key(){
  ...
  json_edit "$WG_CONF" 'if .role=="server" then .peer_public_key=$k|.endpoint.peers=[{public_key:$k,allowed_ips:[(.peer_ip4+"/32"),(.peer_ip6+"/128")]}] else .peer_public_key=$k|.endpoint.peers[0].public_key=$k end' --arg k "$key" || { tell_warn "覆盖失败"; wait_key; return; }
...
run_update(){
  ...
  if [ -z "$latest" ]; then tell_warn "无法获取 sing-box 最新版本信息，请检查网络或 GitHub 访问"
  elif [ -z "$current" ]; then
