...
warp_probe(){
  ...
  jq -n --argjson port "$port" --argjson endpoint "$endpoint" '{log:{level:"error"},inbounds:[{type:"mixed",tag:"warp-probe",listen:"127.0.0.1",listen_port:$port}],outbounds:[{type:"direct",tag:"direct"}],endpoints:[$endpoint],route:{rules:[{inbound:["warp-probe"],action:"route",outbound:"cf-warp"}],final:"direct",auto_detect_interface:true}}' >"$tmp_conf" 2>/dev/null || { rm -f "$tmp_conf" "$tmp_log"; return 1; }
...
menu_warp(){
  while :; do
    ...
    tell ""; tell "  1. 获取出口"; tell "  2. 启用 / 关闭"; tell "  3. WARP 信息"; tell "  4. 清除数据"; tell "  0. 返回"
    tell "${CYAN}--------------------------------${PLAIN}"
    case $(prompt "请选择") in
      1)
      ...
      3)
        screen_clear; tell "${BRIGHT_CYAN}${BOLD}>> WARP 信息 <<${PLAIN}"; if [ -s "$(warp_file)" ]; then
          ...
          tell "  模式：$(warp_mode)"; tell "  IPv4：$(jq -r '.public_ipv4 // .config.interface.addresses.v4 // "-"' "$(warp_file)")"; tell "  IPv6：$(jq -r '.public_ipv6 // .config.interface.addresses.v6 // "-"' "$(warp_file)")"; tell "  WARP IPv4：$(jq -r '.config.interface.addresses.v4 // "-"' "$(warp_file)")"; tell "  WARP IPv6：$(jq -r '.config.interface.addresses.v6 // "-"' "$(warp_file)")"; tell "  Endpoint：$(jq -r '.config.peers[0].endpoint.host // "-"' "$(warp_file)")"; tell "  Client ID：$(jq -r '.config.client_id // "-"' "$(warp_file)")"
...
bootstrap(){
  ...
  check_dependencies
  chmod 700 "$SELF" 2>/dev/null
  ln -sf "$SELF" "$SHORTCUT" 2>/dev/nullload_net_cache
  probe_network_stack_async
  if [ ! -x "$CORE" ]; then
