...
build_config(){
  ...
  local iface_v4 iface_v6
  
  selected=$(state_get exit); domain=$(state_get domain)
  final=direct; use_tun=0; endpoints='[]'; peer_host=""; providers='[]'; tls_extra='{}'
  
  iface_v4=$(ip -4 route show default 2>/dev/null | awk '/default/ {print $5}' | head -1)
  iface_v6=$(ip -6 route show default 2>/dev/null | awk '/default/ {print $5}' | head -1)
  ...
  if [ -n "$iface_v4" ] && [ -n "$iface_v6" ] && [ "$iface_v4" != "$iface_v6" ]; then
    outbounds=$(jq -n --argjson base "$outbounds" --arg v4 "$iface_v4" --arg v6 "$iface_v6" '
      $base + [
        {"type":"direct","tag":"direct-v4","bind_interface":$v4},
        {"type":"direct","tag":"direct-v6","bind_interface":$v6}
      ]')
  fi
  ...
  rules=$(jq -n --arg host "$peer_host" --arg v4 "$iface_v4" --arg v6 "$iface_v6" '
    [{action:"sniff"},
     {protocol:"dns",action:"hijack-dns"},
     {port:53,action:"hijack-dns"}]
    + (if $host=="" then [] 
       elif ($host | test("^[0-9]+\\.[0-9]+\\.[0-9]+\\.[0-9]+$")) then 
         [{ip_cidr:[($host+"/32")],action:"route",outbound:(if $v4!="" and $v6!="" and $v4!=$v6 then "direct-v4" else "direct" end)}]
       elif ($host | test("^[0-9a-fA-F:]+$")) then 
         [{ip_cidr:[($host+"/128")],action:"route",outbound:(if $v4!="" and $v6!="" and $v4!=$v6 then "direct-v6" else "direct" end)}]
       else 
         [{domain:[$host],action:"route",outbound:"direct"}] 
       end)
    + [{ip_is_private:true,action:"route",outbound:"direct"}]
    + (if $v4!="" and $v6!="" and $v4!=$v6 then 
          [{ip_version:4,action:"route",outbound:"direct-v4"},
           {ip_version:6,action:"route",outbound:"direct-v6"}] 
       else [] end)')
  ...
  jq -n --argjson inbounds "$inbounds" --argjson outbounds "$outbounds" \
    ...
    {log:{level:"warn",timestamp:true},
     ...
     inbounds:$inbounds,
     outbounds:$outbounds,
     route:{rules:$rules,final:$final,default_domain_resolver:"dns-local"}}
...
get_ip_info(){
  ...
  res=$(curl -s -${mode} -m 5 https://ipinfo.io/json 2>/dev/null)
  ip=$(jq -r '.ip // empty' <<<"$res" 2>/dev/null)
  if [ -n "$ip" ]; then
    country=$(jq -r '.country // empty' <<<"$res" 2>/dev/null)
    asn=$(jq -r '.org // empty' <<<"$res" 2>/dev/null | awk '{print $1}')
    name=$(jq -r '.org // empty' <<<"$res" 2>/dev/null | cut -d' ' -f2-)
    IP_INFO_IP="$ip"
    IP_INFO_C="$country"
    IP_INFO_ASN="$asn"
    IP_INFO_NAME="$name"
