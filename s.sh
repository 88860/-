...
default_iface_v4(){ ip -4 route show default 2>/dev/null | awk 'NR==1{print $5}'; }
default_iface_v6(){ ip -6 route show default 2>/dev/null | awk 'NR==1{print $5}'; }
  ...
  (
    ...
    while :; do
      ip monitor link route address 2>/dev/null | while IFS= read -r line; do
        case "$line" in
          *" lo "*|*"lo:"*|*"docker"*|*"veth"*|*"br-"*|*"virbr"*|*"$TUN_IF"*|*"$WG_IF"*) continue ;;
        esac
        sleep 3
...
build_config(){
  ...
  local dns_direct_server dns_remote_server bridge_iface
  ...
  dns_block=$(jq -n --arg detour "$final" --arg host "$peer_host" --arg strategy "$strategy" \
    ...
    {
      servers: [
        { type: "udp", tag: "dns-direct", server: $direct_srv, domain_strategy: $strategy },
        (if $detour == "direct" then
          { type: "udp", tag: "dns-remote", server: $remote_srv, domain_strategy: $strategy }
        else
          { type: "udp", tag: "dns-remote", server: $remote_srv, detour: $detour, domain_strategy: $strategy }
        end)
      ],
      rules: [
...
run_watchdog(){
  ...
  [ "$target_exit" = "wireguard" ] && probe_override="wg-direct"
