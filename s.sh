...
build_uri(){
  ...
  case $kind in
    shadowsocks)
      ...
      # SIP002 has no standard network=tcp/udp parameter. Keep the extension only
      # for clients that understand this script's network hint; standard clients
      # may ignore unknown query parameters as allowed by SIP002.
      if [ -n "$ss_network" ]; then uri="${uri}?network=${ss_network}"; fi
...
wg_config_apply_transaction(){
  local body=$1 mode=${2:-} force_apply=${3:-false} old_body rollback_rc=1
...
protocol_edit_option3(){
  local file=$1 kind=$2 value len current_network default_network
  [ -n "$file" ] && [ -f "$file" ] || return 1
  case $kind in
    shadowsocks)
      menu_item 1 "TCP + UDP"; menu_item 2 "TCP"; menu_item 3 "UDP"
      current_network=$(jq -r '.meta.network // .inbound.network // ""' "$file")
      case "$current_network" in "") default_network=1;; tcp) default_network=2;; udp) default_network=3;; *) default_network=1;; esac
      while :; do
        case $(prompt "客户端连接方式" "$default_network") in
      ...
      case $kind in
        ...
        *-grpc) value=$(prompt "gRPC Service Name" "$(jq -r '.meta.service_name//"TunService"' "$file")"); protocol_set "$file" '.meta.service_name=$v|.inbound.transport.service_name=$v' --arg v "$value" || return 1 ;;
        ...
        vmess-tls) menu_item 1 "Auto"; menu_item 2 "AES-128-GCM"; menu_item 3 "ChaCha20-Poly1305"
          ...
          while :; do case $(prompt "加密方式" "$default_security") in 1) security=auto; break;;2) security=aes-128-gcm; break;;3) security=chacha20-poly1305; break;;*) tell_warn "输入无效，请重新选择";;esac; done
