...
install_deps(){
  ...
  if is_alpine; then
    for c in jq openssl timeout ip ping tar gzip sha256sum ss flock getent wget; do
      case "$c" in
        timeout) command -v timeout >/dev/null 2>&1 || miss+=("coreutils");;
        ip) command -v ip >/dev/null 2>&1 || miss+=("iproute2");;
        ping) command -v ping >/dev/null 2>&1 || miss+=("iputils");;
        ss) command -v ss >/dev/null 2>&1 || miss+=("iproute2");;
...
install_singbox(){
  local version arch tmpdir f url
