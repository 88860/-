...
config_build_rules(){
  ...
  case "$mode" in
    ipv4) family_rules='[{"ip_cidr":["::/0"],"action":"bypass"}]' ;;
    ipv6) family_rules='[{"ip_cidr":["0.0.0.0/0"],"action":"bypass"}]' ;;
    *) family_rules='[]' ;;
  esac
...
build_config(){
  ...
  if [ "$selected" = warp ]; then
    ...
    case "$WARP_API_FAMILY" in
      4) dns_direct_tag="dns-direct-v4"; dns_remote_tag="dns-direct-v4"; dns_strategy="prefer_ipv4" ;;
      6) dns_direct_tag="dns-direct-v6"; dns_remote_tag="dns-direct-v6"; dns_strategy="prefer_ipv6" ;;
      *) return 1 ;;
    esac
  fi
...
warp_endpoint(){
  ...
  mode=$(warp_mode_get)
  warp_transport_select || return 1
  bind_interface="$WARP_API_INTERFACE"
  ...
  jq -n --arg resolver "$resolver" --arg host "$host" --argjson port "$port" --arg private "$private" --arg peer "$peer" \
    --arg bind "$bind_interface" --argjson addresses "$addresses" --argjson allowed "$allowed" \
...
warp_delete(){
  ...
  if [ -n "$id" ] && [ -n "$token" ]; then
    warp_transport_select || { tell_warn "没有可用的 WARP 上游网络，未删除 Cloudflare 注册"; wait_key; return 1; }
