...
CERT_TAG=acme-cert
BYPASS_PREF=90
BYPASS_STATE=$SBM_DIR/bypass_rules
HOP_TABLE=sbm_hop
...
probe_network_stack(){
  ...
  if [ -n "$local_v4" ]; then
    ...
    (curl -4 -s -m 2 --interface "$NET_IF_V4" https://ipv4.icanhazip.com 2>/dev/null > "$tmp4") &
  fi
  if [ -n "$local_v6" ]; then
    tmp6=$(mktemp); TMP_FILES="$TMP_FILES $tmp6"
    (curl -6 -s -m 2 --interface "$NET_IF_V6" https://ipv6.icanhazip.com 2>/dev/null > "$tmp6") &
  fi
  wait
  [ -n "$tmp4" ] && { ip4=$(tr -d '\n ' < "$tmp4"); rm -f "$tmp4"; }
  [ -n "$tmp6" ] && { ip6=$(tr -d '\n ' < "$tmp6"); rm -f "$tmp6"; }
...
sync_bypass_rules(){
  local port new_ports owned4 owned6
  new_ports=$(protected_ports)
  owned4=$(mktemp) || return 1
  owned6=$(mktemp) || { rm -f "$owned4"; return 1; }
  if [ -f "$BYPASS_STATE" ]; then
    awk '$1==4 {print $2}' "$BYPASS_STATE" 2>/dev/null | sort -u >"$owned4"
    awk '$1==6 {print $2}' "$BYPASS_STATE" 2>/dev/null | sort -u >"$owned6"
  fi
  for port in $new_ports; do
    if ! ip -4 rule show pref "$BYPASS_PREF" 2>/dev/null | grep -q "sport $port lookup main"; then
      if ip -4 rule add pref "$BYPASS_PREF" sport "$port" lookup main 2>/dev/null; then
        printf '%s\n' "$port" >>"$owned4"
      fi
    fi
    if ! ip -6 rule show pref "$BYPASS_PREF" 2>/dev/null | grep -q "sport $port lookup main"; then
      if ip -6 rule add pref "$BYPASS_PREF" sport "$port" lookup main 2>/dev/null; then
        printf '%s\n' "$port" >>"$owned6"
      fi
    fi
  done
  if [ -f "$BYPASS_STATE" ]; then
    while read -r port; do
      [ -n "$port" ] && [ "$port" != fwmark ] || continue
      grep -qx "$port" <<<"$new_ports" || ip -4 rule del pref "$BYPASS_PREF" sport "$port" lookup main 2>/dev/null
    done < <(sort -un "$owned4" 2>/dev/null)
    while read -r port; do
      [ -n "$port" ] && [ "$port" != fwmark ] || continue
      grep -qx "$port" <<<"$new_ports" || ip -6 rule del pref "$BYPASS_PREF" sport "$port" lookup main 2>/dev/null
    done < <(sort -un "$owned6" 2>/dev/null)
  fi
  if ! ip -4 rule show pref "$BYPASS_PREF" 2>/dev/null | grep -q 'fwmark 255 lookup main'; then
    if ip -4 rule add pref "$BYPASS_PREF" fwmark 255 lookup main 2>/dev/null; then printf 'fwmark\n' >>"$owned4"; fi
  elif grep -qx 'fwmark' "$owned4"; then
    :
  fi
  if ! ip -6 rule show pref "$BYPASS_PREF" 2>/dev/null | grep -q 'fwmark 255 lookup main'; then
    if ip -6 rule add pref "$BYPASS_PREF" fwmark 255 lookup main 2>/dev/null; then printf 'fwmark\n' >>"$owned6"; fi
  elif grep -qx 'fwmark' "$owned6"; then
    :
  fi
  { while read -r port; do [ -n "$port" ] && printf '4 %s\n' "$port"; done < <(sort -u "$owned4"); while read -r port; do [ -n "$port" ] && printf '6 %s\n' "$port"; done < <(sort -u "$owned6"); } >"$BYPASS_STATE.tmp"
  install -m600 "$BYPASS_STATE.tmp" "$BYPASS_STATE" 2>/dev/null || true
  rm -f "$owned4" "$owned6" "$BYPASS_STATE.tmp"
}
...
acme_options(){
  ...
  body=$(jq -n --arg d "$domain" --arg e "$(state_get email)" --arg dir "$ACME_DIR" \
    '{domain:[$d],default_server_name:$d,email:$e,data_directory:$dir,provider:"letsencrypt"}')
...
save_node(){
  ...
  if apply_config; then
    ...
    rm -f "$file"; printf '%s\n' "$old_state" | json_write "$STATE"; apply_config_quiet; tell_warn "配置应用失败，已回滚"; fi
  wait_key
}
...
menu_change_domain(){
  local new_domain old_state file count=0 email mode tmp pid log_file had_new_assets=0
  ...
  validate_domain "$new_domain" || { printf '%s\n' "$old_state" | json_write "$STATE"; prompt_yes "解析验证存在异常，是否继续" || { wait_key; return; }; }
  state_set domain "$new_domain"
  tmp=$(mktemp) || { printf '%s\n' "$old_state" | json_write "$STATE"; tell_warn "临时配置创建失败"; wait_key; return; }
  TMP_FILES="$TMP_FILES $tmp"
  if ! jq -n --argjson p "$(acme_options "$new_domain")" '{log:{level:"warn",timestamp:true},certificate_providers:[($p+{type:"acme",tag:"acme-temporary"})]}' >"$tmp" 2>/dev/null; then
    rm -f "$tmp"; printf '%s\n' "$old_state" | json_write "$STATE"; tell_warn "证书临时配置生成失败"; wait_key; return
  fi
  if ! "$CORE" check -c "$tmp" >/dev/null 2>&1; then
    tell_warn "新域名 ACME 配置校验失败"
    rm -f "$tmp"; printf '%s\n' "$old_state" | json_write "$STATE"; wait_key; return
  fi
  log_file=$(mktemp) || { printf '%s\n' "$old_state" | json_write "$STATE"; tell_warn "证书日志文件创建失败"; wait_key; return; }
  TMP_FILES="$TMP_FILES $log_file"
  ...
  if wait_for_certificate "$new_domain" "$pid" "$log_file"; then
    ...
    if apply_config; then
      cleanup_old_domain_assets "$(printf '%s\n' "$old_state" | jq -r .domain)"
  ...
  while :; do
    ...
    if [ "$(jq -r .tag "$PICKED")" = "$previous_exit" ]; then
      ...
      if apply_config restart; then
        ...
        json_save "$PICKED" "$old_json"; state_set exit "$previous_exit"; apply_config restart >/dev/null 2>&1; sync_watchdog
...
wg_tunnel_state(){
  ...
  [ -d "/sys/class/net/$WG_IF" ] || { echo fail; return; }
  if [ -n "$peer_ip6" ]; then
...
wg_reinit_rollback(){
  ...
  apply_config_quiet
  [ "$previous" = wireguard ] && sync_watchdog
}

wg_status(){
  local file=$WG_CONF role enabled state ms color public ipv4 ipv6 listen_port peer_host peer_port peer_key endpoint_ips
  clear
  if [ ! -f "$file" ]; then
  ...
  case "$role" in
    client)
      ...
      tell ""
      tell "本机公钥：$public"
      listen_port=$(jq -r '.listen_port // 0' "$file" 2>/dev/null)
      if [ "$listen_port" -gt 0 ] 2>/dev/null; then
        if [ "$enabled" = true ] && ss -Hlun 2>/dev/null | awk '{print $5}' | sed 's/.*://' | grep -qx "$listen_port"; then
          tell "监听端口：$listen_port  ${GREEN}已监听${PLAIN}"
        else
          tell "监听端口：$listen_port  ${RED}未监听${PLAIN}"
        fi
      else
        tell "监听端口：自动"
      fi
      ...
      if [ "$enabled" = true ] && [ -d "/sys/class/net/$WG_IF" ]; then
        ...
        if [[ "$state" == ok\|* ]] && [[ "$ms" =~ ^[0-9]+$ ]]; then
          if [ "$ms" -lt 100 ]; then color="$GREEN"; elif [ "$ms" -lt 150 ]; then color="$YELLOW"; else color="$BROWN"; fi
          tell "连接：${GREEN}正常${PLAIN}"
          tell "延迟：${color}${ms}ms${PLAIN}"
        else
          tell "连接：${RED}异常${PLAIN}"
          tell "延迟：${RED}超时${PLAIN}"
        fi
      elif [ "$enabled" = false ]; then
        tell "连接：${YELLOW}已停止${PLAIN}"
      else
        tell "连接：${RED}异常${PLAIN}"
      fi
      tell ""
    ...
    server)
      ...
      tell "本机公钥：$public"
      tell ""
      local peer_count peer_index peer_name peer_ipv4 peer_ipv6 peer_pub handshake now peer_state
      peer_count=$(jq '.endpoint.peers | length' "$file" 2>/dev/null)
      ...
      if [ "$peer_count" -gt 0 ]; then
        ...
        for ((peer_index=0; peer_index<peer_count; peer_index++)); do
          ...
          tell "公钥：$peer_pub"
          peer_state="未连接"
          if [ "$enabled" = true ] && [ -d "/sys/class/net/$WG_IF" ] && command -v wg >/dev/null 2>&1; then
            handshake=$(wg show "$WG_IF" latest-handshakes 2>/dev/null | awk -v k="$peer_pub" '$1==k{print $2;exit}')
            if [[ "$handshake" =~ ^[0-9]+$ ]] && [ "$handshake" -gt 0 ] && [ $((now-handshake)) -le 180 ]; then
              peer_state="已连接"
            fi
          fi
          if [ "$peer_state" = "已连接" ]; then tell "状态：${GREEN}已连接${PLAIN}"; else tell "状态：${YELLOW}未连接${PLAIN}"; fi
        done
      fi
      tell ""
      if [ "$enabled" = true ] && [ -d "/sys/class/net/$WG_IF" ]; then tell "连接：${GREEN}正常${PLAIN}"; elif [ "$enabled" = false ]; then tell "连接：${YELLOW}已停止${PLAIN}"; else tell "连接：${RED}异常${PLAIN}"; fi
      tell ""
      tell "${CYAN}========================================${PLAIN}"
      ;;
    *)
...
wg_init_client(){
  ...
  if ! apply_config; then
    ...
    apply_config_quiet
    [ "$previous_exit" = wireguard ] && sync_watchdog
    rm -rf "$backup_wg"
    tell_warn "配置校验失败，已恢复旧配置"
    wait_key
    return
...
wg_client_add(){
  ...
  body=$(jq --arg name "$name" --arg ipv4 "$ipv4" --arg ipv6 "$ipv6" --arg pub "$pub" \
    '.clients += [{name:$name,ipv4:$ipv4,ipv6:$ipv6,public_key:$pub}] |
     .endpoint.peers += [{public_key:$pub,allowed_ips:[$ipv4,$ipv6]}]' "$WG_CONF") || { tell_warn "配置生成失败"; wait_key; return; }
...
wg_client_edit(){
  local index count name ipv4 ipv6 pub old_body body
  ...
  old_body=$(cat "$WG_CONF")
  body=$(jq --argjson i "$index" --arg name "$name" --arg ipv4 "$ipv4" --arg ipv6 "$ipv6" --arg pub "$pub" \
    '.clients[$i]={name:$name,ipv4:$ipv4,ipv6:$ipv6,public_key:$pub} |
     .endpoint.peers[$i]={public_key:$pub,allowed_ips:[$ipv4,$ipv6]}' "$WG_CONF") || { tell_warn "配置生成失败"; wait_key; return; }
  wg_server_client_apply "$body" || { wait_key; return; }
  tell_ok "客户端已修改"
  wait_key
}
    ...
    if [ ! -f "$WG_CONF" ]; then
      ...
      if [ "$role" = client ] && [ "$enabled" = true ]; then
        if [ ! -d "/sys/class/net/$WG_IF" ]; then
          ...
          if [[ "$state" == ok\|* ]] && [[ "$ms" =~ ^[0-9]+$ ]]; then
            if [ "$ms" -lt 100 ]; then color="$GREEN"; elif [ "$ms" -lt 150 ]; then color="$YELLOW"; else color="$BROWN"; fi
            tell "  模式：客户端|${GREEN}已启动${PLAIN}|${color}${ms} ms${PLAIN}"
          else
            tell "  模式：客户端|${RED}异常${PLAIN}"
          fi
        fi
  ...
  if [ -f "$BYPASS_STATE" ]; then
    while read -r family selector value; do
      case "$family:$selector:$value" in
        4:sport:*) ip -4 rule del pref "$BYPASS_PREF" sport "$value" lookup main 2>/dev/null || true ;;
        6:sport:*) ip -6 rule del pref "$BYPASS_PREF" sport "$value" lookup main 2>/dev/null || true ;;
        4:fwmark:fwmark) ip -4 rule del pref "$BYPASS_PREF" fwmark 255 lookup main 2>/dev/null || true ;;
        6:fwmark:fwmark) ip -6 rule del pref "$BYPASS_PREF" fwmark 255 lookup main 2>/dev/null || true ;;
      esac
    done < <(awk '$2=="sport"{print $1,"sport",$3} $2=="fwmark"{print $1,"fwmark","fwmark"}' "$BYPASS_STATE" 2>/dev/null)
  fi
