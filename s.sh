...
config_build_rules(){
  local peer_host=$1 mode=${2:-} ssh_rule_ports ssh_client_ips family_rules='[]'
  ...
  jq -n --arg host "$peer_host" --argjson ssh_ports "$ssh_rule_ports" --argjson ssh_client_ips "$ssh_client_ips" --argjson family_rules "$family_rules" '
    (if ($ssh_client_ips|length)>0 then
       [{ip_cidr:$ssh_client_ips,action:"route",outbound:"direct"}]
     else [] end)
