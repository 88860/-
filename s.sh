...
probe_network_stack(){
  ...
  # Stack detection is based on real outbound connectivity, not whether the
  # interface happens to have a public address. This also handles NAT/CGNAT IPv4.
  if [ -n "$local_v4" ] && curl --interface "$NET_IF_V4" -4 -k -sS -o /dev/null -m 3 --connect-timeout 2 https://1.1.1.1/ >/dev/null 2>&1; then
    v4_ok=1
  fi
  if [ -n "$local_v6" ] && curl --interface "$NET_IF_V6" -6 -k -sS -o /dev/null -m 3 --connect-timeout 2 https://[2606:4700:4700::1111]/ >/dev/null 2>&1; then
    v6_ok=1
  fi
...
build_config(){
  ...
  local dns_strategy auto_detect icmp_out dns_resolver
  ...
  jq -n --argjson inbounds "$inbounds" --argjson outbounds "$outbounds" \
        ...
        --arg final "$final" --arg dns_resolver "$dns_resolver" --argjson auto_detect "$auto_detect" '
    {log:{level:"warn",timestamp:true},
     dns:$dns,
     inbounds:$inbounds,
     outbounds:$outbounds,
     route:{rules:$rules,final:$final,
            auto_detect_interface:$auto_detect,default_domain_resolver:$dns_resolver}}
  ...
  case $URI_SCHEME in
    vless)
      ...
      if [ "$security" = reality ]; then
        local pbk sid
        pbk=$(query_value pbk); sid=$(query_value sid)
        [ -n "$pbk" ] && [ -n "$sid" ] || return 1
        [[ "$sid" =~ ^[0-9A-Fa-f]{0,8}$ ]] || return 1
        outbound=$(jq --arg sni "$sni" --arg fp "$fingerprint" --arg pbk "$pbk" --arg sid "$sid" \
    ...
    hysteria2|hy2)
      ...
      if [ -n "$obfs_pw" ]; then
        ...
        case "$obfs_type" in salamander|gecko) ;; *) obfs_type="salamander" ;; esac
      ...
      case "$bbr_profile" in conservative|standard|aggressive) ;; *) bbr_profile=standard ;; esac
      ...
      if [ -n "$hop_range" ]; then
        if [[ "$hop_range" =~ ^[0-9]+-[0-9]+$ ]]; then
          local hop_low=${hop_range%-*} hop_high=${hop_range#*-}
          if [ "$hop_low" -ge 1 ] && [ "$hop_high" -le 65535 ] && [ "$hop_low" -lt "$hop_high" ]; then
            outbound=$(jq --arg r "${hop_low}:${hop_high}" '.server_ports=[$r]|del(.server_port)|.hop_interval="30s"' <<<"$outbound")
          fi
        elif [[ "$hop_range" =~ ^[0-9]+:[0-9]+$ ]]; then
          local hop_low=${hop_range%:*} hop_high=${hop_range#*:}
          if [ "$hop_low" -ge 1 ] && [ "$hop_high" -le 65535 ] && [ "$hop_low" -lt "$hop_high" ]; then
            outbound=$(jq --arg r "$hop_range" '.server_ports=[$r]|del(.server_port)|.hop_interval="30s"' <<<"$outbound")
          fi
        fi
      fi
      min_pkt=$(query_value min_packet_size); max_pkt=$(query_value max_packet_size)
      if [ "$obfs_type" = "gecko" ] && { [ -n "$min_pkt" ] || [ -n "$max_pkt" ]; }; then
        min_pkt=${min_pkt:-512}; max_pkt=${max_pkt:-1200}
        if [[ "$min_pkt" =~ ^[0-9]+$ && "$max_pkt" =~ ^[0-9]+$ ]] && [ "$min_pkt" -le "$max_pkt" ]; then
    ...
    tuic)
      ...
      case "$congestion" in cubic|new_reno|bbr) ;; *) congestion=bbr ;; esac
      alpn=$(query_value alpn); [ -n "$alpn" ] || alpn=h3
      udp_mode=$(query_value udp_relay_mode); [ -n "$udp_mode" ] || udp_mode=native
      case "$udp_mode" in native|quic) ;; *) udp_mode=native ;; esac
      outbound=$(jq -n --arg tag "$tag" --arg server "$URI_HOST" --argjson port "$URI_PORT" \
    ...
    snell)
      ...
      case "$s_mode" in default|unshaped|unsafe-raw) ;; *) s_mode=default ;; esac
      outbound=$(jq -n --arg tag "$tag" --arg server "$URI_HOST" --argjson port "$URI_PORT" \
