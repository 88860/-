...
warp_status(){
  ...
  if [ "$(state_exit_get 2>/dev/null)" = warp ] && systemctl is-active --quiet sing-box 2>/dev/null && ip link show "$TUN_IF" >/dev/null 2>&1; then
    echo running
  else
    echo stopped
  fi
...
warp_start(){
  ...
  endpoint=$(warp_endpoint "$( [ "$(network_ipv6_ok)" = 1 ] && printf dns-direct-v6 || printf dns-direct-v4 )") || { tell_warn "WARP 配置不完整"; wait_key; return 1; }
