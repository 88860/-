...
detect_env() {
  if command -v systemctl >/dev/null 2>&1 && command -v apt-get >/dev/null 2>&1; then
    ...
    SYS_ENV="openrc"
    ENV_DISP="OpenRC (极限低内存)"
    SERVICE_UNIT=/etc/init.d/sing-box
    
    # LXC 下的 /tmp 通常是内存盘(tmpfs)，64M机器极易被大文件撑爆OOM
    # 强制将所有临时操作转移到物理硬盘目录，并施加严格的 Go 内存限制
    mkdir -p /etc/sbm/tmp
    export TMPDIR="/etc/sbm/tmp"
...
remote_version(){ curl -fsSL -m 15 "$GH_API/releases/latest" 2>/dev/null | grep '"tag_name":' | head -n 1 | awk -F'"' '{print $4}' | sed 's/^v//'; }
...
install_core(){
  local version url asset candidate tmp found
  version=$(remote_version)
  [ -n "$version" ] || { tell_warn "获取版本信息失败或遭遇 API 限流"; return 1; }
  
  for candidate in $(asset_candidates); do
    if curl -fsSL -I -m 10 "https://github.com/SagerNet/sing-box/releases/download/v${version}/sing-box-${version}-${candidate}.tar.gz" >/dev/null 2>&1; then
      asset=$candidate
      break
    fi
  done
  [ -n "$asset" ] || { tell_warn "未找到匹配架构的安装包"; return 1; }
  ...
  # 分步流式下载并深度释放内核缓存，彻底防爆内存
  if ! curl -4 -fL --retry 3 --connect-timeout 15 -m 240 "$url" -o "$tmp/core.tar.gz" 2>/dev/null; then
    curl -fL --retry 3 --connect-timeout 15 -m 240 "$url" -o "$tmp/core.tar.gz" 2>/dev/null || { rm -rf "$tmp"; tell_warn "安装包下载失败"; return 1; }
  fi
  
  [ "$SYS_ENV" = "openrc" ] && { sync; sleep 1; }
...
run_watchdog(){
  ...
  while :; do
    ...
    if [ "$SYS_ENV" = "systemd" ]; then
      ...
      if env http_proxy="http://127.0.0.1:2080" curl -s -o /dev/null --connect-timeout 4 -m 6 "$PROBE_URL"; then fail_count=0; else fail_count=$((fail_count+1)); fi
    fi
...
get_ip_info(){
  ...
  if [ -n "$raw_ip" ]; then
    res=$(env http_proxy="$proxy_env" curl -s -m 5 "http://ipwho.is/${raw_ip}" 2>/dev/null)
...
wg_tunnel_state(){
  ...
  if env http_proxy="$proxy_env" curl --interface "$WG_IF" -s -o /dev/null --connect-timeout 4 -m 6 "$PROBE_URL" 2>/dev/null; then
    echo up; return;
  fi
  echo down
}
...
run_update(){
  ...
  if curl -4 -fL --retry 3 --connect-timeout 15 -m 30 "$script_url" -o "$script_tmp" 2>/dev/null || curl -fL --retry 3 --connect-timeout 15 -m 30 "$script_url" -o "$script_tmp" 2>/dev/null; then
