...
probe_network_stack(){
  ...
  # Stack detection is based on real outbound connectivity, not public-IP discovery.
  # Use literal IP endpoints here so DNS itself cannot affect the result.
  if [ -n "$local_v4" ] && curl --interface "$NET_IF_V4" -4 -sS -k -m 3 --connect-timeout 2 \
      https://1.1.1.1/cdn-cgi/trace >/dev/null 2>&1; then
    v4_ok=1
  fi
  if [ -n "$local_v6" ] && curl --interface "$NET_IF_V6" -6 -sS -k -m 3 --connect-timeout 2 \
      'https://[2606:4700:4700::1111]/cdn-cgi/trace' >/dev/null 2>&1; then
    v6_ok=1
  fi
...
uri_to_outbound(){
  ...
  case $URI_SCHEME in
    ...
    hysteria2|hy2)
      ...
      # Gecko packet-size fields are valid only when Gecko obfuscation is enabled.
