...
warp_tunnel_select(){
  ...
  if [[ "$host" =~ ^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
    ...
    resolved4=$(getent ahostsv4 "$host" 2>/dev/null | awk 'NR==1{print $1}')
    resolved6=$(getent ahostsv6 "$host" 2>/dev/null | awk 'NR==1{print $1}')
    if [ -n "$resolved4" ] && [ -n "$NET_IF_V4" ]; then
      WARP_TUNNEL_FAMILY=4
      WARP_TUNNEL_INTERFACE="$NET_IF_V4"
    elif [ -n "$resolved6" ] && [ -n "$NET_IF_V6" ]; then
      WARP_TUNNEL_FAMILY=6
      WARP_TUNNEL_INTERFACE="$NET_IF_V6"
    else
      return 1
