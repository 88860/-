...
wg_status(){
  ...
  clear
  [ -f "$file" ] || { tell "未配置"; wait_key; return; }
  role=$(jq -r '.role // ""' "$file")
  [ "$role" = client ] || [ "$role" = server ] || { tell "未配置"; wait_key; return; }
  if [ "$(jq -r '.enabled//false' "$file")" = true ]; then
    status="已启动"
  else
    status="已停止"
  fi
  if [ "$role" = client ] && [ "$status" = "已启动" ]; then
  ...
  while :; do
    ...
    if [ ! -f "$WG_CONF" ]; then
      ...
      if [ "$role" = client ] && [ "$enabled" = true ]; then
        ...
        if [ "$enabled" = true ]; then tell "  模式：服务端 状态：${GREEN}已启动${PLAIN}"; else tell "  模式：服务端 状态：${RED}未启动${PLAIN}"; fi
      else
        if [ "$enabled" = true ]; then tell "  模式：客户端 状态：${GREEN}已启动${PLAIN}"; else tell "  模式：客户端 状态：${RED}未启动${PLAIN}"; fi
      fi
    fi
    tell ""
    tell "  1. 初始化配置"
