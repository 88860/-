...
warp_api_request(){
  ...
  if [ "$method" = POST ]; then
    code=$(curl -sS --location --tlsv1.3 --connect-timeout 8 --max-time 20 -o "$out" -w '%{http_code}' -X POST "$url" \
      ...
      -H 'Content-Type: application/json' --data "$body" 2>/dev/null); curl_rc=$?
  else
...
warp_keypair(){
  local private public keypair
  if command -v wg >/dev/null 2>&1; then
    private=$(wg genkey 2>/dev/null) || return 1
    public=$(printf '%s\n' "$private" | wg pubkey 2>/dev/null) || return 1
  else
    keypair=$("$CORE" generate wg-keypair 2>/dev/null) || keypair=
...
warp_register(){
  ...
  if warp_registered; then
    tell "当前：${GREEN}已注册${PLAIN}"
    tell "IPv4：$(warp_config_get '.config.interface.addresses.v4 // "-"')"
    tell "IPv6：$(warp_config_get '.config.interface.addresses.v6 // "-"')"
    prompt_yes "重新注册" || return
  else
    tell "当前：${DIM}未注册${PLAIN}"
    local register_confirm
    register_confirm=$(prompt "开始注册" "y")
    [[ "$register_confirm" =~ ^[yY]$ ]] || return
  fi
  ...
  mkdir -p "$WARP_DIR" || { tell_warn "目录创建失败"; wait_key; return; }
  ...
  keypair=$(warp_keypair) || { tell_warn "WireGuard 密钥生成失败"; wait_key; return; }
  private=${keypair%%$'\t'*}; public=${keypair#*$'\t'}
  response_file=$(mktemp) || { tell_warn "临时文件创建失败"; wait_key; return; }
  ...
  if ! jq --arg pk "$private" --arg pub "$public" --arg iid "$WARP_INSTALL_ID" '{id:.id,token:.token,private_key:$pk,public_key:$pub,install_id:$iid,account:.account,config:.config,warp_mode:"dual",registered_at:(now|todateiso8601)}' "$response_file" | json_write "$WARP_CONF"; then
  ...
  tell_ok "注册成功"
  warp_select_mode_and_start || { wait_key; return 1; }
}
...
warp_select_mode_and_start(){
  ...
  warp_mode_set "$mode" || { tell_warn "模式保存失败"; return 1; }
  tell "正在启用 WARP $(warp_mode_label) ..."
  if warp_start; then
    tell_ok "WARP 已启用，当前出口：$(warp_mode_label)"
    return 0
  fi
  tell_warn "WARP 启用失败"
  return 1
...
warp_fetch_exit(){
  ...
  local mode info4 info6 ip4 ip6 loc4 loc6 colo4 colo6 warp4 warp6 again choice
  mode=$(warp_mode_get)
  case "$mode" in
    ipv4)
  ...
  [ -n "$ip4" ] && tell "  IPv4：$ip4 | ${loc4:-??} | ${colo4:-未知} | WARP=${warp4:-off}" || [ "$mode" != ipv4 ] || tell_warn "IPv4 出口获取失败"
  [ -n "$ip6" ] && tell "  IPv6：$ip6 | ${loc6:-??} | ${colo6:-未知} | WARP=${warp6:-off}" || [ "$mode" != ipv6 ] || tell_warn "IPv6 出口获取失败"
  if [ "$mode" = dual ]; then
    [ -n "$ip4" ] || tell_warn "IPv4 出口获取失败"
    [ -n "$ip6" ] || tell_warn "IPv6 出口获取失败"
  fi
  if [ -n "$ip4$ip6" ]; then
    tell_ok "已获取当前出口 IP"
  else
    tell_warn "未获取到当前出口 IP"
  fi
  tell ""
