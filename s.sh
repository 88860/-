...
render_share_uri(){
  ...
  if [ -n "$NET_IF_V4" ]; then
    physical_v4=$(ip -4 addr show dev "$NET_IF_V4" scope global 2>/dev/null | awk '/inet /{print $2;exit}' | cut -d/ -f1)
  fi
  if [ -n "$NET_IF_V6" ]; then
    physical_v6=$(ip -6 addr show dev "$NET_IF_V6" scope global 2>/dev/null | awk '!/temporary/{print $2;exit}' | cut -d/ -f1)
  fi
