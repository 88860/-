...
# 强制重置终端缓冲，根治 Alpine / OpenRC 界面的残留残影问题
clear_screen() {
...
# 极小内存专用版：舍弃臃肿的 jq 全量解析，改用基于文本流式的截取，防止容器秒爆内存
remote_version(){
  local url="https://api.github.com/repos/SagerNet/sing-box/releases/latest"
  if command -v curl >/dev/null 2>&1; then
    curl -fsSL -m 15 "$url" 2>/dev/null | grep '"tag_name":' | head -n 1 | sed 's/.*"tag_name": *"v//;s/".*//'
  else
    wget -qO- -T 15 "$url" 2>/dev/null | grep '"tag_name":' | head -n 1 | sed 's/.*"tag_name": *"v//;s/".*//'
  fi
}
...
install_core(){
  ...
  if [ "$SYS_ENV" = "openrc" ]; then
    ...
    # OpenRC (LXC) 保护机制: 启用流式管道解压，绝对不在 /tmp 落盘导致撑爆 tmpfs
    if wget -qO- -T 120 "$url" 2>/dev/null | tar -xzf - -C "/usr/local/bin" --strip-components=1 "sing-box-${version}-${asset}/sing-box" 2>/dev/null; then
      ...
      tell_warn "安装包下载或解压失败 (可能遭遇网络中断或剩余内存极端不足)"
      return 1
    fi
  else
    ...
    # Systemd (KVM) 全速机制: 正常落盘缓存并校验
    if command -v curl >/dev/null 2>&1; then
      curl -4 -fL --retry 3 --connect-timeout 15 -m 240 "$url" -o "$tmp/core.tar.gz" 2>/dev/null || \
      curl -fL --retry 3 --connect-timeout 15 -m 240 "$url" -o "$tmp/core.tar.gz" 2>/dev/null || \
      { rm -rf "$tmp"; tell_warn "安装包下载失败"; return 1; }
    else
      wget -qO "$tmp/core.tar.gz" -T 120 "$url" 2>/dev/null || { rm -rf "$tmp"; tell_warn "安装包下载失败"; return 1; }
    fi
...
build_config(){
  ...
  if [ "$SYS_ENV" = "systemd" ]; then
    # 系统级路由挂载（MTU固定 1400 确保 UDP/QUIC / Hysteria2 防断流丢包）
  ...
  # 1.14.0 官方规范强制适配：只保留 address，杜绝泄漏，移除旧版的 type / server / default_domain_resolver
  dns_block=$(jq -n --arg detour "$final" --arg host "$peer_host" --arg strategy "$strategy" --arg direct_srv "$dns_direct_server" --arg remote_srv "$dns_remote_server" '
    {
      servers: [
        {tag: "dns-direct", address: $direct_srv, detour: "direct"},
        {tag: "dns-remote", address: $remote_srv, detour: (if $detour == "direct" then "direct" else $detour end)}
      ],
      rules: [
...
run_watchdog(){
  ...
  while :; do
    ...
    if [ "$SYS_ENV" = "systemd" ]; then
      if command -v curl >/dev/null 2>&1; then
        curl -s -o /dev/null --connect-timeout 4 -m 6 "$PROBE_URL" && fail_count=0 || fail_count=$((fail_count+1))
      else
        wget -q -O /dev/null -T 6 "$PROBE_URL" 2>/dev/null && fail_count=0 || fail_count=$((fail_count+1))
      fi
    elif [ "$SYS_ENV" = "openrc" ]; then
...
save_node(){
  ...
  else out_warn "配置应用失败。原文件修改已强制覆盖且不回滚。"
  fi
  wait_key
}
...
peer_delete(){
  ...
  rm -f "$PICKED"
  apply_config && tell_ok "已删除" || tell_warn "删除节点强制执行成功，但应用异常"
...
menu_status(){
  while :; do
    ...
    tell "  环境架构: ${GREEN}${ENV_DISP}${PLAIN} 模式"
...
while :; do
  ...
  tell "${CYAN}================================${PLAIN}"
  tell "${CYAN}             s管理              ${PLAIN}"
  tell "   [ 当前运行模式: ${GREEN}${ENV_DISP}${PLAIN} ]   "
  tell "${CYAN}================================${PLAIN}"
  tell "  1. 服务端管理"
  tell "  2. 客户端管理"
  tell "  3. WireGuard 管理"
