...
build_config(){
  ...
  if [ "$use_tun" = 1 ]; then
    if [ "$final" != "wireguard" ]; then
      if [ -n "$NET_IF_V4" ] && [ -n "$NET_IF_V6" ] && [ "$NET_IF_V4" != "$NET_IF_V6" ]; then
        outbounds=$(jq -n --argjson base "$outbounds" \
          --arg if4 "$NET_IF_V4" --arg if6 "$NET_IF_V6" '
          $base + [
            {"type":"bridge","tag":"bridge-v4","interface":$if4},
            {"type":"bridge","tag":"bridge-v6","interface":$if6}
          ]')
      elif [ -n "$NET_IF_V4" ]; then
        outbounds=$(jq -n --argjson base "$outbounds" --arg iface "$NET_IF_V4" \
          '$base + [{"type":"bridge","tag":"bridge-out","interface":$iface}]')
      elif [ -n "$NET_IF_V6" ]; then
        outbounds=$(jq -n --argjson base "$outbounds" --arg iface "$NET_IF_V6" \
          '$base + [{"type":"bridge","tag":"bridge-out","interface":$iface}]')
      else
        outbounds=$(jq -n --argjson base "$outbounds" \
          '$base + [{"type":"bridge","tag":"bridge-out"}]')
      fi
    fi
  ...
  if [ "$use_tun" = 1 ] && [ "$final" != "wireguard" ]; then
    if [ -n "$NET_IF_V4" ] && [ -n "$NET_IF_V6" ] && [ "$NET_IF_V4" != "$NET_IF_V6" ]; then
      rules=$(jq -n --argjson list "$rules" '
        $list + [
          {ip_version:4,inbound:["tun-in"],action:"route",outbound:"bridge-v4",preferred_by:["bridge-v4"]},
          {ip_version:6,inbound:["tun-in"],action:"route",outbound:"bridge-v6",preferred_by:["bridge-v6"]}
        ]')
    else
      rules=$(jq -n --argjson list "$rules" '
        $list + [{inbound:["tun-in"],action:"route",outbound:"bridge-out",preferred_by:["bridge-out"]}]')
    fi
  fi
