...
wg_status(){
  local file=$WG_CONF role enabled public ipv4 ipv6 listen_port peer_key peer_host peer_port peer_ip4 peer_ip6 state ms color
  clear
  ...
  tell "  模式：$([ "$role" = client ] && printf '客户端' || printf '服务端')"
  tell "  本机公钥：${public:-未配置}"
  tell "  IPv4：${ipv4:-未配置}"
  tell "  IPv6：${ipv6:-未配置}"
  if [ "$role" = server ]; then
    listen_port=$(jq -r '.listen_port // empty' "$file" 2>/dev/null)
    peer_key=$(jq -r '.peer_public_key // empty' "$file" 2>/dev/null)
    tell "  监听端口：${listen_port:-未配置}"
    tell "  客户端公钥：${peer_key:-未配置}"
  else
    peer_host=$(jq -r '.peer_host // empty' "$file" 2>/dev/null)
    peer_port=$(jq -r '.peer_port // empty' "$file" 2>/dev/null)
    peer_key=$(jq -r '.peer_public_key // empty' "$file" 2>/dev/null)
    tell "  服务端：${peer_host:-未配置}"
    tell "  服务端端口：${peer_port:-未配置}"
    tell "  服务端公钥：${peer_key:-未配置}"
  fi
  if [ "$enabled" = true ]; then
    if [ -d "/sys/class/net/$WG_IF" ]; then
      tell "  状态：${GREEN}已启动${PLAIN}"
    else
      tell "  状态：${RED}异常${PLAIN}"
    fi
  else
    tell "  状态：${RED}已停止${PLAIN}"
  fi
  if [ "$role" = client ] && [ "$enabled" = true ] && [ -d "/sys/class/net/$WG_IF" ]; then
    peer_ip4=$(jq -r '.peer_ip // empty' "$file" 2>/dev/null)
    peer_ip6=$(jq -r '.peer_ip6 // empty' "$file" 2>/dev/null)
  ...
  while :; do
    ...
    if [ ! -f "$WG_CONF" ]; then
      ...
      if [ "$role" = client ] && [ "$enabled" = true ]; then
        if [ ! -d "/sys/class/net/$WG_IF" ]; then
          tell "  模式：客户端 状态：${RED}异常${PLAIN}"
        else
          state=$(wg_tunnel_state "$(jq -r '.peer_ip // empty' "$WG_CONF")" "$(jq -r '.peer_ip6 // empty' "$WG_CONF")")
          ms=${state#*|}
          if [[ "$state" == ok\|* ]] && [[ "$ms" =~ ^[0-9]+$ ]]; then
            if [ "$ms" -lt 100 ]; then color="$GREEN"; elif [ "$ms" -lt 150 ]; then color="$YELLOW"; else color="$BROWN"; fi
            tell "  模式：客户端 状态：${GREEN}已启动${PLAIN} 延迟：${color}${ms} ms${PLAIN}"
          else
            tell "  模式：客户端 状态：${RED}异常${PLAIN}"
          fi
        fi
        ...
        if [ "$enabled" = true ]; then
          if [ -d "/sys/class/net/$WG_IF" ]; then tell "  模式：服务端 状态：${GREEN}已启动${PLAIN}"; else tell "  模式：服务端 状态：${RED}异常${PLAIN}"; fi
        else
          tell "  模式：服务端 状态：${RED}已停止${PLAIN}"
        fi
      else
        if [ "$enabled" = true ]; then
          if [ -d "/sys/class/net/$WG_IF" ]; then tell "  模式：客户端 状态：${GREEN}已启动${PLAIN}"; else tell "  模式：客户端 状态：${RED}异常${PLAIN}"; fi
        else
          tell "  模式：客户端 状态：${RED}已停止${PLAIN}"
        fi
      fi
    fi
    tell ""
