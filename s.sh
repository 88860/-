...
build_config(){
  ...
  if [ "$selected" != direct ] && [ "$selected" != wireguard ] && [ -f "$PEER_DIR/$selected.json" ]; then
    ...
    if [ "$wgrole" = client ] && [ "$selected" = wireguard ]; then final=wireguard; peer_host=$(jq -r '.peer_host//""' "$wg_source"); transparent_proxy_available && use_tun=1; fi
  fi
...
wg_system_enabled(){
  is_lxc && printf 'false' || printf 'true'
}
