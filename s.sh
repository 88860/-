...
menu_change_domain(){
  local new_domain old_state file count=0 email mode
  ...
  validate_domain "$new_domain" || prompt_yes "解析验证存在异常，是否继续切换" || { wait_key; return; }
  if prompt_yes "同步重置验证机制"; then
    email=$(prompt "ACME 通知邮箱" "admin@$new_domain"); [ -n "$email" ] || { printf '%s\n' "$old_state" | json_write "$STATE"; wait_key; return; }
    printf '\n  %b选择域名证书验证方式:%b\n' "${CYAN}" "${PLAIN}"
    echo "  1. HTTP         (推荐 需放行80端口)"
    echo "  2. TLS-ALPN     (推荐 需放行443端口)"
    echo "  3. Cloudflare API"
