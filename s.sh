...
init_dirs(){
  ...
  [ -f "$PKG_LOG" ] || : >"$PKG_LOG"
  [ -f "$RULE_STATE" ] || printf '%s\n' '{"ports4":[],"ports6":[]}' | json_write "$RULE_STATE"
}
...
build_dns_block(){
  local strat="$1" final="$2" iface="$3"
  local bserver="1.1.1.1"
  [ "$strat" = "ipv6_only" ] && bserver="2606:4700:4700::1111"
  jq -n --arg strat "$strat" --arg final "$final" --arg bserver "$bserver" --arg iface "$iface" '{
    servers: [
      {
  ...
  if $iface != "" then .servers |= map(. + {bind_interface:$iface}) else . end |
...
build_config(){
  ...
  if [ ${#node_files[@]} -gt 0 ] && [ -n "$domain" ]; then
    ...
    providers=$(jq -n --arg d "$domain" --arg e "$(state_get email)" --arg dir "$ACME_DIR" --argjson dns01 "$dns01" --arg chal "$chal" '
      [{
        type:"acme", tag:$CERT_TAG, domain:[$d], email:$e,
        data_directory:$dir, provider:"letsencrypt"
      }
      | if $chal|startswith("dns_") then .dns01_challenge=$dns01
        elif $chal=="http" then .disable_tls_alpn_challenge=true
        elif $chal=="alpn" then .disable_http_challenge=true
        else . end
      ]')
  fi
    ...
    inbounds=$(jq -s 'map(
      ...
      if $node.tls_mode == "acme" then
        $node.inbound | .tls = ({enabled:true, certificate_provider:"acme-cert"}
          + (if $node.alpn != null then {alpn: $node.alpn} else {} end))
      else
        $node.inbound
      end
  ...
  rules=$(build_route_rules "$final" direct "$probe_target")
  ...
  local dns_block
  dns_block=$(build_dns_block "$strat" "$final" "$NET_IF_V4")
...
collect_mgmt_dests(){
  ...
  done < <(jq -r '"\(.outbound.server//"-")|\(.management_port // 22)"' "${files[@]}" 2>/dev/null)
}

clear_rules(){
  while ip -4 rule del pref "$PREF_MAIN" 2>/dev/null; do :; done
  while ip -6 rule del pref "$PREF_MAIN" 2>/dev/null; do :; done
  while ip -4 rule del pref "$PREF_DEST" 2>/dev/null; do :; done
  while ip -6 rule del pref "$PREF_DEST" 2>/dev/null; do :; done
}
...
sync_mgmt_rules(){
  ...
  jq -n --argjson p4 "$(printf '%s\n' "$new_ports" | jq -Rsc 'split("\n")|map(select(length>0)|tonumber)')" \
        --argjson p6 "$(printf '%s\n' "$new_ports" | jq -Rsc 'split("\n")|map(select(length>0)|tonumber)')" \
        '{ports4:$p4,ports6:$p6}' >"$state_tmp" 2>/dev/null
...
build_uri(){
  ...
  case $kind in
    ...
    socks)
      uri=$(jq -r --arg n "$name" --arg h "$host" --arg p "$port" 'def u(s): s|@uri; "socks://" + u(.username) + ":" + u(.password) + "@\($h):\($p)#" + u($n)' <<<"$meta") ;;
    snell)
    ...
    case $(prompt "请选择") in
      ...
      4)
        if [ "$kind" = socks ]; then
          ...
          if [ "$value" = "0" ]; then
            json_edit "$PICKED" '.hopping=""' || { tell_warn 失败; wait_key; continue; }
            tell_ok "已关闭端口跳跃"
          else
