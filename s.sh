...
init_dirs(){
  ...
  if [ ! -f "$STATE" ]; then
    printf '%s\n' '{"exit":"direct","domain":"","email":"","challenge":"http","asset":""}' | json_write "$STATE" || return 1
  fi
  if [ ! -f "$PKG_LOG" ]; then
    : >"$PKG_LOG" || return 1
  fi
  if [ ! -f "$BYPASS_STATE" ]; then
    printf '%s\n' '{"v4_ports":[],"v6_ports":[]}' | json_write "$BYPASS_STATE" || return 1
  fi
}
...
check_dependencies(){
  ...
  local deps=(
    ...
    "systemctl:systemd" "journalctl:systemd" "modprobe:kmod" "wg:wireguard-tools"
  )
  local missing=0
  for item in "${deps[@]}"; do
    cmd=${item%%:*}
...
probe_network_stack_async(){
  (
    ...
    local tmp
    tmp=$(mktemp "${NET_CACHE}.XXXXXX") || exit 1
...
management_ssh_destinations(){
  ...
  if [ -f "$WG_CONF" ]; then
    ...
    port=$(jq -r '.peer_port // 0' "$WG_CONF" 2>/dev/null)
    [[ "$port" =~ ^[1-9][0-9]{0,4}$ ]] || port=0
    [ "$port" -le 65535 ] || port=0
...
restore_bypass_snapshot(){
  local old4="$1" old6="$2" oldd4="$3" oldd6="$4" p item ip rc=0
  ...
  while IFS= read -r p; do [ -n "$p" ] || continue; ip -4 rule add pref "$BYPASS_PREF" sport "$p" lookup main 2>/dev/null || rc=1; done <<< "$old4"
  while IFS= read -r p; do [ -n "$p" ] || continue; ip -6 rule add pref "$BYPASS_PREF" sport "$p" lookup main 2>/dev/null || rc=1; done <<< "$old6"
  while IFS= read -r item; do [ -n "$item" ] || continue; ip=${item%|*}; p=${item##*|}; ip -4 rule add pref "$((BYPASS_PREF-1))" to "$ip" dport "$p" lookup main 2>/dev/null || rc=1; done <<< "$oldd4"
  while IFS= read -r item; do [ -n "$item" ] || continue; ip=${item%|*}; p=${item##*|}; ip -6 rule add pref "$((BYPASS_PREF-1))" to "$ip" dport "$p" lookup main 2>/dev/null || rc=1; done <<< "$oldd6"
  return "$rc"
}
  ...
  while IFS= read -r item; do
    ...
    if ! grep -Fqx "$item" <<<"$oldd4"; then
      ...
      if ip -4 rule add pref "$((BYPASS_PREF-1))" to "$ip" dport "$port" lookup main 2>/dev/null; then
        ...
        while IFS= read -r rollback_item; do [ -n "$rollback_item" ] || continue; ip=${rollback_item%|*}; port=${rollback_item##*|}; ip -4 rule del pref "$((BYPASS_PREF-1))" to "$ip" dport "$port" lookup main 2>/dev/null || true; done <<< "$added_d4"
  ...
  while IFS= read -r item; do
    ...
    if ! grep -Fqx "$item" <<<"$oldd6"; then
      ...
      if ip -6 rule add pref "$((BYPASS_PREF-1))" to "$ip" dport "$port" lookup main 2>/dev/null; then
        ...
        while IFS= read -r rollback_item; do [ -n "$rollback_item" ] || continue; ip=${rollback_item%|*}; port=${rollback_item##*|}; ip -4 rule del pref "$((BYPASS_PREF-1))" to "$ip" dport "$port" lookup main 2>/dev/null || true; done <<< "$added_d4"
        while IFS= read -r rollback_item; do [ -n "$rollback_item" ] || continue; ip=${rollback_item%|*}; port=${rollback_item##*|}; ip -6 rule del pref "$((BYPASS_PREF-1))" to "$ip" dport "$port" lookup main 2>/dev/null || true; done <<< "$added_d6"
        restore_bypass_snapshot "$old4" "$old6" "$oldd4" "$oldd6"; return 1
      fi
    fi
  done <<< "$desiredd6"
  while IFS= read -r item; do
    ...
    if ! grep -Fqx "$item" <<<"$desiredd4"; then
      ...
      if ip -4 rule del pref "$((BYPASS_PREF-1))" to "$ip" dport "$port" lookup main 2>/dev/null; then
        ...
        while IFS= read -r rollback_item; do [ -n "$rollback_item" ] || continue; ip=${rollback_item%|*}; port=${rollback_item##*|}; ip -4 rule del pref "$((BYPASS_PREF-1))" to "$ip" dport "$port" lookup main 2>/dev/null || true; done <<< "$added_d4"
        while IFS= read -r rollback_item; do [ -n "$rollback_item" ] || continue; ip=${rollback_item%|*}; port=${rollback_item##*|}; ip -6 rule del pref "$((BYPASS_PREF-1))" to "$ip" dport "$port" lookup main 2>/dev/null || true; done <<< "$added_d6"
        while IFS= read -r rollback_item; do [ -n "$rollback_item" ] || continue; ip=${rollback_item%|*}; port=${rollback_item##*|}; ip -4 rule add pref "$((BYPASS_PREF-1))" to "$ip" dport "$port" lookup main 2>/dev/null || true; done <<< "$removed_d4"
        restore_bypass_snapshot "$old4" "$old6" "$oldd4" "$oldd6"; return 1
      fi
    fi
  done <<< "$oldd4"
  while IFS= read -r item; do
    ...
    if ! grep -Fqx "$item" <<<"$desiredd6"; then
      ...
      if ip -6 rule del pref "$((BYPASS_PREF-1))" to "$ip" dport "$port" lookup main 2>/dev/null; then
        ...
        while IFS= read -r rollback_item; do [ -n "$rollback_item" ] || continue; ip=${rollback_item%|*}; port=${rollback_item##*|}; ip -6 rule del pref "$((BYPASS_PREF-1))" to "$ip" dport "$port" lookup main 2>/dev/null || true; done <<< "$added_d6"
        while IFS= read -r rollback_item; do [ -n "$rollback_item" ] || continue; ip=${rollback_item%|*}; port=${rollback_item##*|}; ip -4 rule add pref "$((BYPASS_PREF-1))" to "$ip" dport "$port" lookup main 2>/dev/null || true; done <<< "$removed_d4"
        while IFS= read -r rollback_item; do [ -n "$rollback_item" ] || continue; ip=${rollback_item%|*}; port=${rollback_item##*|}; ip -6 rule add pref "$((BYPASS_PREF-1))" to "$ip" dport "$port" lookup main 2>/dev/null || true; done <<< "$removed_d6"
        restore_bypass_snapshot "$old4" "$old6" "$oldd4" "$oldd6"; return 1
      fi
    fi
  done <<< "$oldd6"
...
clear_bypass_rules(){
  local p item ip rc=0
  if [ -f "$BYPASS_STATE" ]; then
    while IFS= read -r p; do
      [ -n "$p" ] || continue
      ip -4 rule del pref "$BYPASS_PREF" sport "$p" lookup main 2>/dev/null || rc=1
    done < <(jq -r '.v4_ports[]? // empty' "$BYPASS_STATE" 2>/dev/null)
    while IFS= read -r p; do
      [ -n "$p" ] || continue
      ip -6 rule del pref "$BYPASS_PREF" sport "$p" lookup main 2>/dev/null || rc=1
    done < <(jq -r '.v6_ports[]? // empty' "$BYPASS_STATE" 2>/dev/null)
    while IFS= read -r item; do
      [ -n "$item" ] || continue
      ip=${item%|*}; p=${item##*|}
      ip -4 rule del pref "$((BYPASS_PREF-1))" to "$ip" dport "$p" lookup main 2>/dev/null || rc=1
    done < <(jq -r '.v4_dests[]? // empty' "$BYPASS_STATE" 2>/dev/null)
    while IFS= read -r item; do
      [ -n "$item" ] || continue
      ip=${item%|*}; p=${item##*|}
      ip -6 rule del pref "$((BYPASS_PREF-1))" to "$ip" dport "$p" lookup main 2>/dev/null || rc=1
    done < <(jq -r '.v6_dests[]? // empty' "$BYPASS_STATE" 2>/dev/null)
  fi
  printf '%s\n' '{"v4_ports":[],"v6_ports":[],"v4_dests":[],"v6_dests":[]}' | json_write "$BYPASS_STATE" >/dev/null 2>&1 || rc=1
  return "$rc"
}
...
acme_options(){
  ...
  body=$(jq -n --arg d "$domain" --arg e "$(state_get email)" --arg dir "$ACME_DIR" \
    '{domain:[$d],email:$e,data_directory:$dir,provider:"letsencrypt"}') || return 1
  body=$(jq '.key_type="p256"' <<<"$body") || return 1
  case $(state_get challenge) in
    alpn) body=$(jq '.disable_http_challenge=true' <<<"$body") || return 1 ;;
    dns_cloudflare) body=$(jq --arg t "$(state_get cf_token)" '.dns01_challenge={provider:"cloudflare",api_token:$t}' <<<"$body") || return 1 ;;
    dns_alidns) body=$(jq --arg k "$(state_get ali_key)" --arg s "$(state_get ali_secret)" '.dns01_challenge={provider:"alidns",access_key_id:$k,access_key_secret:$s}' <<<"$body") || return 1 ;;
    dns_acmedns) body=$(jq --arg u "$(state_get acmedns_user)" --arg p "$(state_get acmedns_pass)" --arg s "$(state_get acmedns_sub)" --arg r "$(state_get acmedns_url)" '.dns01_challenge={provider:"acmedns",username:$u,password:$p,subdomain:$s,server_url:$r}' <<<"$body") || return 1 ;;
    ...
    if [ "$wg_role" = "client" ]; then
      if [ "$selected" = "direct" ] || [ "$selected" = "wireguard" ]; then
        if [ "$wg_endpoint" = "yes" ]; then
          ...
          if printf '%s' "$peer_host" | grep -Eq '^[0-9]+(\.[0-9]+){3}$'; then
            wg_route_excludes=$(jq -n --arg ip "$peer_host" '[($ip + "/32")]') || return 1
          elif printf '%s' "$peer_host" | grep -Eq '^[0-9A-Fa-f:]+$'; then
            wg_route_excludes=$(jq -n --arg ip "$peer_host" '[($ip + "/128")]') || return 1
          else
            return 1
          fi
          final="wireguard"
  ...
  if [ "$selected" != "direct" ] &&
    ...
    if [ -f "$PEER_DIR/$selected.json" ]; then
      ...
      if [ -z "$selected_outbound" ] ||
        ...
        tell_warn "出口节点 $selected 配置无效，回退直连" >&2
        final="direct"
        use_tun=0
        peer_host=""
  ...
  if [ "$use_tun" = "1" ]; then
    ...
    inbounds=$(
      jq -n \
        ...
        --argjson exclude "$wg_route_excludes" \
...
_apply_config_locked(){
  ...
  local build_err="$tmp.err"
  if ! build_config >"$tmp" 2>"$build_err" || [ ! -s "$tmp" ]; then
    if [ -s "$build_err" ]; then
      tell_warn "配置生成失败:"
      sed -n '1,8p' "$build_err" | while IFS= read -r line; do tell "    $line"; done
    else
      tell_warn "配置生成失败"
    fi
    rm -f "$tmp" "$build_err"
  ...
  if ! sync_hopping_rules; then
    rollback_config_failure "$prev_conf" "$prev_active" "$prev_exists" "跳跃规则同步失败，正在回滚配置"
    return 1
  fi
...
stop_watchdog(){
  ...
  if systemctl is-active --quiet sbm-watchdog.service; then return 1; fi
...
validate_range(){
  ...
  done < <({ listening_ports u; protected_ports; printf '%s\n' 80 443; })
  return 0
}
...
select_node(){
  ...
  while :; do
    ...
    else menu_invalid; fi
  done
}

menu_invalid(){
  local msg="${1:-输入无效，请重新选择}"
  printf '\033[1A\r\033[2K  %b[×] %s%b' "${RED}" "$msg" "${PLAIN}" >&2
  sleep 1
  printf '\r\033[2K' >&2
}

menu_choice(){
  local prompt_text="$1" pattern="$2"
  while :; do
    MENU_CHOICE=$(prompt "$prompt_text")
    if [[ "$MENU_CHOICE" =~ $pattern ]]; then
      return 0
    fi
    menu_invalid
  done
...
menu_delete_protocol(){
  while :; do
    ...
    if ! select_node; then return; fi
    local node_name was_acme old_json
    node_name=$(jq -r '.name' "$PICKED") || { tell_warn "无法读取节点配置"; wait_key; continue; }
    prompt_yes "确认删除 $node_name" || continue
    was_acme=$(jq -r '.tls_mode' "$PICKED") || { tell_warn "无法读取节点配置"; wait_key; continue; }
    old_json=$(cat "$PICKED") || { tell_warn "无法读取节点配置"; wait_key; continue; }
    if ! rm -f "$PICKED"; then
      tell_warn "节点文件删除失败"; wait_key; continue
    fi
    if apply_config; then
      tell_ok "已删除: $node_name"
      if [ "$was_acme" = "acme" ]; then
        ...
        if [ "$acme_count" -eq 0 ] && [ -n "$(state_get domain)" ]; then
          ...
          if prompt_yes "是否连同域名和证书一起清理"; then
            if state_set domain "" && state_set email "" && state_set challenge "http"; then
              find "$ACME_DIR" -mindepth 1 -delete 2>/dev/null || true
              tell_ok "相关配置已清理"
            else
              tell_warn "域名状态清理失败"
            fi
      ...
      if ! json_save "$PICKED" "$old_json" || ! apply_config_quiet; then
        tell_warn "删除导致配置异常，回滚失败，请立即检查配置"
      else
...
menu_modify_protocol(){
  local kind value other current_hop up_mbps down_mbps obfs_type obfs_password old_json bbr_profile min_pkt max_pkt max_choice
  ...
  while :; do
    ...
    tell "  0. 返回"; tell "${CYAN}==============================${PLAIN}"
    old_json=$(cat "$PICKED") || { tell_warn "无法读取节点配置"; wait_key; return; }
    max_choice=4
    [ "$kind" = vless-reality ] || [ "$kind" = hysteria2 ] && max_choice=6
    menu_choice "请选择" "^[0-${max_choice}]$"
    case $MENU_CHOICE in
      1)
...
uri_to_outbound(){
  ...
  if [ "$insecure" = 1 ] || [ "$insecure" = true ]; then
    outbound=$(jq 'if .tls then .tls.insecure=true else . end' <<<"$outbound") || return 1
  fi
  printf '%s' "$outbound"
}
...
list_peers(){
  ...
  local tmp_dir; tmp_dir=$(mktemp -d); TMP_FILES+=("$tmp_dir")
  local idx=0 raw_data old_ifs; local test_pids=()
...
peer_delete(){
  while :; do
    ...
    if [ "$(jq -r '.tag' "$PICKED")" = "$previous_exit" ]; then
      if ! state_set exit direct; then tell_warn "出口状态写入失败，未删除当前节点"; wait_key; continue; fi
      if ! stop_watchdog; then tell_warn "watchdog 停止失败，未删除节点"; wait_key; continue; fi
      if ! rm -f "$PICKED"; then tell_warn "节点文件删除失败"; wait_key; continue; fi
      if apply_config; then
...
get_ip_info(){
  ...
  res=$(curl -fsS "-${mode}" -m 5 https://ipwho.is/ 2>/dev/null) || return 1
...
wg_validate_endpoint_config(){
  local role
  [ -f "$WG_CONF" ] || return 1
  role=$(jq -r '.role // ""' "$WG_CONF" 2>/dev/null) || return 1
  jq -e --arg role "$role" '
    .endpoint.type == "wireguard" and
    (.endpoint.address | type == "array" and length > 0) and
    (.endpoint.private_key | type == "string" and length > 0) and
    (.endpoint.peers | type == "array" and length > 0) and
    all(.endpoint.peers[];
      (.public_key | type == "string" and length > 0) and
      (.allowed_ips | type == "array" and length > 0) and
      (if $role == "client" then
        (.address | type == "string" and length > 0) and
        (.port | type == "number" and . >= 1 and . <= 65535)
       else true end)
    )
  ' "$WG_CONF" >/dev/null 2>&1 || return 1
  case "$NET_STACK" in
    v4)
      jq -e '
        all(.endpoint.address[]; test("^[0-9]+[.][0-9]+[.][0-9]+[.][0-9]+/[0-9]+$")) and
        all(.endpoint.peers[]; all(.allowed_ips[]; test("^[0-9]+[.][0-9]+[.][0-9]+[.][0-9]+/[0-9]+$")))
      ' "$WG_CONF" >/dev/null 2>&1 || return 1
      ;;
    v6)
      jq -e '
        all(.endpoint.address[]; test("^[0-9A-Fa-f:]+/[0-9]+$")) and
        all(.endpoint.peers[]; all(.allowed_ips[]; test("^[0-9A-Fa-f:]+/[0-9]+$")))
      ' "$WG_CONF" >/dev/null 2>&1 || return 1
      ;;
    both)
      ;;
  ...
  if [ "$role" = client ]; then
    local host
    host=$(jq -r '.peer_host // ""' "$WG_CONF" 2>/dev/null) || return 1
    if [ "$NET_STACK" = v4 ]; then
      printf '%s' "$host" | grep -Eq '^[0-9]{1,3}(\.[0-9]{1,3}){3}$' || return 1
    elif [ "$NET_STACK" = v6 ]; then
      printf '%s' "$host" | grep -Eq '^[0-9A-Fa-f:]+$' || return 1
    else
      printf '%s' "$host" | grep -Eq '^[0-9]{1,3}(\.[0-9]{1,3}){3}$|^[0-9A-Fa-f:]+$' || return 1
    fi
  fi
  return 0
}
...
wg_tunnel_state(){
  local peer_ip="${1:-}" peer_host="${2:-}" handshake now age
  [ -d "/sys/class/net/$WG_IF" ] || { echo down; return; }
  [ -d "/sys/class/net/$TUN_IF" ] || { echo down; return; }
  command -v wg >/dev/null 2>&1 || { echo down; return; }
  now=$(date +%s)
  handshake=$(wg show "$WG_IF" latest-handshakes 2>/dev/null | awk '$2 ~ /^[0-9]+$/ && $2 > max {max=$2} END{print max+0}')
  case "$handshake" in ''|*[!0-9]*) handshake=0 ;; esac
  if [ "$handshake" -gt 0 ] && [ "$handshake" -le "$now" ]; then
    age=$((now-handshake))
    if [ "$age" -le 30 ]; then
      if [ -n "$peer_ip" ] && timeout 3 ping -I "$WG_IF" -c1 -W2 "$peer_ip" >/dev/null 2>&1; then
        echo up
        return
      fi
      if timeout 4 curl --interface "$WG_IF" -fsS -o /dev/null --connect-timeout 2 -m 4 "$PROBE_URL"; then
        echo up
        return
      fi
    fi
...
wg_wait_ready(){
  local peer_ip="${1:-}" peer_host="${2:-}" i
  for i in $(seq 1 12); do
    if [ "$(wg_tunnel_state "$peer_ip" "$peer_host")" = up ]; then
      return 0
    fi
    [ "$i" -lt 12 ] && sleep 1
  done
...
render_wg_info(){
  local file="$1" role
  role=$(jq -r '.role' "$file") || { tell_warn "WireGuard 配置读取失败"; return 1; }
  ...
  if [ "$role" = client ]; then
    ...
    if [ "$NET_STACK" = v6 ]; then
      tell "  服务端: [$(jq -r '.address[0]' "$file" | cut -d/ -f1)]:$(jq -r '.listen_port' "$file")"
    else
      tell "  服务端: $(local_ipv4):$(jq -r '.listen_port' "$file")"
    fi
    tell "  公钥: $(jq -r '.public_key' "$file")"
    if [ "$NET_STACK" = v6 ]; then
      tell "  前缀: $(jq -r '.address[0]' "$file")"
    else
...
wg_setup(){
  local role keypair private public prefix peer_host peer_port peer_key body
  local address4 address6 peer_ip4 peer_ip6 peer_ip addr_json allowed_json
  local listen_port
  ...
  if ! modprobe wireguard 2>/dev/null; then
    command -v wg >/dev/null 2>&1 || { tell_warn "WireGuard 内核模块不可用"; wait_key; return; }
  fi
  if ! keypair=$("$CORE" generate wg-keypair 2>/dev/null); then
    tell_warn "密钥生成失败"; wait_key; return
  fi
  private=$(awk '/PrivateKey/{print $2}' <<<"$keypair")
  public=$(awk '/PublicKey/{print $2}' <<<"$keypair")
  [ -n "$private" ] && [ -n "$public" ] || { tell_warn "密钥生成结果无效"; wait_key; return; }
  while :; do
  ...
  case "$NET_STACK" in
    v4)
      ...
      addr_json=$(jq -cn --arg a "$address4" '[$a]')
      allowed_json=$(jq -cn '["0.0.0.0/0"]')
      ;;
    v6)
      address6="$ipv6_prefix::1/64"; peer_ip6="$ipv6_prefix::2"
      addr_json=$(jq -cn --arg a "$address6" '[$a]')
      allowed_json=$(jq -cn '["::/0"]')
      ;;
    both)
      address4="$prefix.1/24"; address6="$ipv6_prefix::1/64"
      peer_ip4="$prefix.2"; peer_ip6="$ipv6_prefix::2"
      addr_json=$(jq -cn --arg a4 "$address4" --arg a6 "$address6" '[$a4,$a6]')
      allowed_json=$(jq -cn '["0.0.0.0/0","::/0"]')
      ;;
    *)
      tell_warn "无法确定网络地址族，请先等待网络检测完成"
      wait_key; return
      ;;
  esac
  if [ "$role" = server ]; then
    ...
    if [ "$NET_STACK" = v4 ]; then
      body=$(jq -n --arg private "$private" --arg public "$public" --argjson addr "$addr_json" --argjson port "$listen_port" --arg peer_key "$peer_key" --arg peer4 "$peer_ip4" --arg iface "$WG_IF" '
        {role:"server",enabled:false,private_key:$private,public_key:$public,address:$addr,listen_port:$port,peer_public_key:$peer_key,peer_ip:$peer4,peer_host:"",peer_port:0,endpoint:{type:"wireguard",tag:"wireguard",system:true,name:$iface,mtu:1408,address:$addr,private_key:$private,listen_port:$port,peers:(if $peer_key!="" then [{public_key:$peer_key,allowed_ips:[($peer4+"/32")]}] else [] end)}}') || { tell_warn "WireGuard 配置生成失败"; wait_key; return; }
    elif [ "$NET_STACK" = v6 ]; then
      body=$(jq -n --arg private "$private" --arg public "$public" --argjson addr "$addr_json" --argjson port "$listen_port" --arg peer_key "$peer_key" --arg peer6 "$peer_ip6" --arg iface "$WG_IF" '
        {role:"server",enabled:false,private_key:$private,public_key:$public,address:$addr,listen_port:$port,peer_public_key:$peer_key,peer_ip:$peer6,peer_host:"",peer_port:0,endpoint:{type:"wireguard",tag:"wireguard",system:true,name:$iface,mtu:1408,address:$addr,private_key:$private,listen_port:$port,peers:(if $peer_key!="" then [{public_key:$peer_key,allowed_ips:[($peer6+"/128")]}] else [] end)}}') || { tell_warn "WireGuard 配置生成失败"; wait_key; return; }
    else
      body=$(jq -n --arg private "$private" --arg public "$public" --argjson addr "$addr_json" --argjson port "$listen_port" --arg peer_key "$peer_key" --arg peer4 "$peer_ip4" --arg peer6 "$peer_ip6" --arg iface "$WG_IF" '
        {role:"server",enabled:false,private_key:$private,public_key:$public,address:$addr,listen_port:$port,peer_public_key:$peer_key,peer_ip:$peer4,peer_host:"",peer_port:0,endpoint:{type:"wireguard",tag:"wireguard",system:true,name:$iface,mtu:1408,address:$addr,private_key:$private,listen_port:$port,peers:(if $peer_key!="" then [{public_key:$peer_key,allowed_ips:[($peer4+"/32"),($peer6+"/128")]}] else [] end)}}') || { tell_warn "WireGuard 配置生成失败"; wait_key; return; }
    fi
  else
    case "$NET_STACK" in
      v4) address4="$prefix.2/32"; peer_ip4="$prefix.1"; peer_ip="$peer_ip4" ;;
      v6) address6="$ipv6_prefix::2/128"; peer_ip6="$ipv6_prefix::1"; peer_ip="$peer_ip6" ;;
      both) address4="$prefix.2/32"; address6="$ipv6_prefix::2/128"; peer_ip4="$prefix.1"; peer_ip6="$ipv6_prefix::1"; peer_ip="$peer_ip4" ;;
    esac
    peer_host=$(prompt "服务端 IP" "1.1.1.1")
    if [ "$NET_STACK" = v4 ]; then
      if ! printf '%s' "$peer_host" | grep -Eq '^[0-9]{1,3}(\.[0-9]{1,3}){3}$'; then tell_warn "当前为纯 IPv4 网络，服务端地址必须为 IPv4"; wait_key; return; fi
    elif [ "$NET_STACK" = v6 ]; then
      if ! printf '%s' "$peer_host" | grep -Eq '^[0-9A-Fa-f:]+$'; then tell_warn "当前为纯 IPv6 网络，服务端地址必须为 IPv6"; wait_key; return; fi
    else
      if ! printf '%s' "$peer_host" | grep -Eq '^[0-9]{1,3}(\.[0-9]{1,3}){3}$|^[0-9A-Fa-f:]+$'; then tell_warn "服务端地址必须为 IP，不支持域名"; wait_key; return; fi
    fi
    if printf '%s' "$peer_host" | grep -Eq '^[0-9]{1,3}(\.[0-9]{1,3}){3}$'; then
      IFS=. read -r _a _b _c _d <<<"$peer_host"
      if [ "$_a" -gt 255 ] || [ "$_b" -gt 255 ] || [ "$_c" -gt 255 ] || [ "$_d" -gt 255 ]; then
        tell_warn "服务端地址不是有效 IPv4"; wait_key; return
      fi
    fi
    ...
    body=$(jq -n --arg private "$private" --arg public "$public" --arg host "$peer_host" --argjson port "$peer_port" --arg peer_key "$peer_key" --arg peer_ip "$peer_ip" --argjson addr "$addr_json" --argjson allowed "$allowed_json" --arg iface "$WG_IF" '
      {role:"client",enabled:false,private_key:$private,public_key:$public,address:$addr,listen_port:0,peer_public_key:$peer_key,peer_ip:$peer_ip,peer_host:$host,peer_port:$port,endpoint:{type:"wireguard",tag:"wireguard",system:true,name:$iface,mtu:1408,address:$addr,private_key:$private,peers:[{address:$host,port:$port,public_key:$peer_key,allowed_ips:$allowed,persistent_keepalive_interval:25}]}}') || { tell_warn "WireGuard 配置生成失败"; wait_key; return; }
  fi
...
wg_fill_peer_key(){
  local key old_json role peer_ip peer_ip6 allowed_json
  ...
  role=$(jq -r '.role // ""' "$WG_CONF" 2>/dev/null) || { tell_warn "读取角色失败"; wait_key; return; }
  if [ "$role" = server ]; then
    peer_ip=$(jq -r '.peer_ip // ""' "$WG_CONF" 2>/dev/null) || { tell_warn "读取客户端地址失败"; wait_key; return; }
    [ -n "$peer_ip" ] || { tell_warn "缺少客户端地址"; wait_key; return; }
    case "$NET_STACK" in
      v4)
        allowed_json=$(jq -cn --arg ip "$peer_ip" '[($ip+"/32")]')
        ;;
      v6)
        peer_ip6=$(jq -r '.peer_ip // ""' "$WG_CONF" 2>/dev/null)
        allowed_json=$(jq -cn --arg ip "$peer_ip6" '[($ip+"/128")]')
        ;;
      both)
        peer_ip6=$(jq -r '.endpoint.address[1] // ""' "$WG_CONF" 2>/dev/null)
        peer_ip6=${peer_ip6%%/*}
        [ -n "$peer_ip6" ] || { tell_warn "缺少客户端 IPv6 地址"; wait_key; return; }
        allowed_json=$(jq -cn --arg ip "$peer_ip" --arg ip6 "$peer_ip6" '[($ip+"/32"),($ip6+"/128")]')
        ;;
      *)
        tell_warn "无法确定网络地址族"; wait_key; return
        ;;
    esac
    json_edit "$WG_CONF" '.peer_public_key=$k|.endpoint.peers=(if (.endpoint.peers|length)>0 then .endpoint.peers else [{}] end)|.endpoint.peers[0].public_key=$k|.endpoint.peers[0].allowed_ips=$allowed' --arg k "$key" --argjson allowed "$allowed_json" || { tell_warn "覆盖失败"; wait_key; return; }
  else
  ...
  if [ "$(jq -r '.enabled' "$WG_CONF")" = true ]; then
    if apply_config; then
      if [ "$role" = client ] && ! wg_wait_ready "$(jq -r '.peer_ip' "$WG_CONF")" "$(jq -r '.peer_host // ""' "$WG_CONF")"; then
...
wg_toggle(){
  ...
  if [ "$(jq -r '.enabled' "$WG_CONF")" = true ]; then
    ...
    if ! wg_validate_endpoint_config; then
      tell_warn "WireGuard 配置不符合 sing-box Endpoint 要求：peer 必须包含有效公钥和 allowed_ips"
      wait_key
      return
    fi
    ...
    if apply_config; then
      if [ "$role" = client ]; then
        if ! wg_wait_ready "$(jq -r '.peer_ip' "$WG_CONF")" "$(jq -r '.peer_host // ""' "$WG_CONF")"; then
          printf '%s\n' "$old_json" | json_write "$WG_CONF"
    ...
    case $MENU_CHOICE in
      ...
      5) clear
         ...
         if prompt_yes "确认删除配置"; then
           ...
           if ! rm -f "$WG_CONF"; then
             tell_warn "WireGuard 配置删除失败"; wait_key; continue
           fi
           if [ "$wg_role" = server ] && ! wg_server_nat_remove; then
             printf '%s\n' "$wg_backup" | json_write "$WG_CONF" >/dev/null 2>&1 || true
             tell_warn "WireGuard NAT 清理失败，未删除配置"
             wait_key
             continue
           fi
...
run_uninstall(){
  ...
  rm -f "$SERVICE_UNIT" "$DROPIN" /etc/systemd/system/sbm-watchdog.service || { tell_warn "删除 systemd 配置失败，取消卸载"; return 1; }
  rmdir "$DROPIN_DIR" 2>/dev/null || true
  systemctl daemon-reload >/dev/null 2>&1 || { tell_warn "删除 service 后 daemon-reload 失败，取消卸载"; return 1; }
  rm -rf "$SB_DIR" "$SBM_DIR" /var/lib/sing-box "$CORE" "$SHORTCUT" || { tell_warn "程序文件清理失败，取消卸载"; return 1; }
  ...
  rm -f "$SELF" || { tell_warn "脚本自身删除失败，卸载未完全完成"; return 1; }
  tell_ok "清理完成"
  exit 0
}
...
bootstrap(){
  ...
  chmod 700 "$SELF" 2>/dev/null || exit 1
  ln -sf "$SELF" "$SHORTCUT" 2>/dev/null || exit 1
  ...
  if ! "$CORE" check -c "$CONFIG" >/dev/null 2>&1; then
    tell_warn "sing-box 配置校验失败"
    exit 1
  fi
  systemctl enable --now sing-box >/dev/null 2>&1 || exit 1
  systemctl is-active --quiet sing-box || exit 1
  sync_bypass_rules || exit 1
  sync_hopping_rules || exit 1
  start_net_monitor || exit 1
}

case $1 in
  --sync) init_dirs || exit 1; sync_bypass_rules || exit 1; sync_hopping_rules || exit 1; exit 0 ;;
