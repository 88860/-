...
prompt(){
  ...
  if [ -n "$def" ]; then
    printf "  ${CYAN}❯ %s [%s]: ${PLAIN}" "$msg" "$def" >"$out"
  else
    printf "  ${CYAN}❯ %s: ${PLAIN}" "$msg" >"$out"
  fi
  value=$(read_line "$def")
  echo "${value:-$def}"
}
...
config_build_tun_inbounds(){
  ...
  jq -n --argjson list "$inbounds" --arg name "$name" --argjson route "$route" '
    [{type:"tun",tag:"tun-in",interface_name:$name,
      ...
      strict_route:(if ($route|length)==2 then true else false end),
      mtu:1500} + (if ($route|length)>0 then {route_address:$route} else {} end)] + $list'
}

config_build_rules(){
  local peer_host=$1 mode=${2:-} ssh_rule_ports family_rules='[]'
  ssh_rule_ports=$(ssh_ports | jq -Rsc 'split("\n") | map(select(length > 0) | tonumber)')
  case "$mode" in
    ipv4) family_rules='[{"ip_cidr":["::/0"],"action":"route","outbound":"direct"}]' ;;
    ipv6) family_rules='[{"ip_cidr":["0.0.0.0/0"],"action":"route","outbound":"direct"}]' ;;
    *) family_rules='[]' ;;
  esac
  jq -n --arg host "$peer_host" --argjson ssh_ports "$ssh_rule_ports" --argjson family_rules "$family_rules" '
    (if ($ssh_ports|length)>0 then
       [{network:"tcp",source_port:$ssh_ports,action:"bypass"},
        {network:"tcp",port:$ssh_ports,action:"bypass"}]
     else [] end)
...
build_config(){
  ...
  if [ "$selected" = warp ]; then
    ...
    case "$warp_mode" in
      ipv4) dns_remote_tag="dns-direct-v4"; dns_strategy="prefer_ipv4" ;;
      ipv6)
        [ "$(network_ipv6_ok)" = 1 ] || return 1
        dns_remote_tag="dns-direct-v6"; dns_strategy="prefer_ipv6" ;;
      dual)
        if [ "$(network_ipv6_ok)" = 1 ]; then
          dns_remote_tag="dns-direct-v6"; dns_strategy="prefer_ipv6"
        else
          dns_remote_tag="dns-direct-v4"; dns_strategy="prefer_ipv4"
        fi
        ;;
      *) return 1 ;;
    esac
  ...
  rules=$(config_build_rules "$peer_host" "$( [ "$selected" = warp ] && warp_mode_get || printf dual )") || return 1
...
warp_status(){
  ...
  if [ "$(state_exit_get 2>/dev/null)" = warp ] && systemctl is-active --quiet sing-box 2>/dev/null && ip link show "$TUN_IF" >/dev/null 2>&1 && ip link show "$WARP_IF" >/dev/null 2>&1; then
    echo running
  else
    echo stopped
  fi
...
warp_register(){
  local new_ip_mode=${1:-} new_ip_target=${2:-}
  clear
  ...
  if warp_registered; then
    ...
    [ "$new_ip_mode" = newip ] || prompt_yes "重新注册" || return
  else
  ...
  tell_ok "注册成功"
  if [ "$new_ip_mode" = newip ]; then
    warp_mode_set "$new_ip_target" || { tell_warn "WARP 出口模式保存失败"; wait_key; return 1; }
    warp_start || { tell_warn "新 WARP 出口启用失败"; wait_key; return 1; }
  else
...
warp_fetch_exit(){
  ...
  warp_show_exit
  tell ""
  local again choice mode current
  again=$(prompt "是否重新获取出口" "n")
  if [[ "$again" =~ ^[yY]$ ]]; then
    current=$(warp_mode_get)
    while :; do
      choice=$(prompt "选择出口")
      case "$choice" in
        1) mode=ipv4; break ;;
    ...
    if [ "$mode" = "$current" ]; then
      if warp_register newip "$current"; then
        clear; tell "${CYAN}✦ WARP 出口${PLAIN}"; tell ""
        tell_ok "已重新获取新的 WARP $(warp_mode_label) 出口"
        tell ""; warp_show_exit
      else
        tell_warn "重新获取新的 WARP 出口失败"
      fi
      wait_key; return 0
    fi
    warp_mode_set "$mode" || { tell_warn "模式保存失败"; wait_key; return 1; }
    if warp_start; then
      clear; tell "${CYAN}✦ WARP 出口${PLAIN}"; tell ""
      tell_ok "出口模式已切换为 $(warp_mode_label)"; tell ""; warp_show_exit
    else
      warp_mode_set "$current" || true
      tell_warn "WARP 出口模式切换失败"
    fi
  fi
  wait_key
}
...
warp_start(){
  ...
  if [ "$mode" = ipv6 ] || [ "$mode" = dual ]; then
    if [ "$(network_ipv6_ok)" != 1 ]; then
      tell_warn "当前服务器没有可用的 IPv6 网络，无法启用 WARP $(warp_mode_label)"
      wait_key
      return 1
    fi
  fi
