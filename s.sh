...
build_config(){
  ...
  dns_block=$(jq -n --arg direct "$dns_direct_server" --arg strategy "$dns_strategy" --arg detour "$final" '
    ...
    rules:[
      ...
      {match_response:true,response_rcode:["NOERROR","NXDOMAIN"],action:"respond"},
...
bootstrap(){
  ...
  if "$CORE" check -c "$CONFIG" >/dev/null 2>&1; then
    if ! systemctl enable --now sing-box >/dev/null 2>&1; then
      tell_warn "sing-box 启动失败"
      systemctl status sing-box --no-pager -l 2>/dev/null || true
      exit 1
    fi
  else
    tell_warn "sing-box 配置检查失败"
    exit 1
  fi
  sync_bypass_rules
  sync_hopping_rules
