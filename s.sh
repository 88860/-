...
list_peers(){
  ...
  jq -s '
    [ range(0; length) as $i | .[$i].outbound | .tag = "peer-\($i+1)" ] as $outs |
    [ range(0; length) as $i | {type:"socks",tag:"in-\($i+1)",listen:"127.0.0.1",listen_port:(20000+$i+1)} ] as $ins |
    [ range(0; length) as $i | {inbound:["in-\($i+1)"],action:"route",outbound:"peer-\($i+1)"} ] as $rules |
    { log: {level: "fatal"}, inbounds: $ins, outbounds: ($outs + [{type:"direct",tag:"direct"}]), route: {rules: $rules, final: "direct"} }
  ' "$@" > "$sb_conf" 2>/dev/null
  ...
  if [ -n "$raw_data" ]; then
    ...
    while read -r file tag type name port; do
      ...
      (
        local res ms
        res=$(curl -x socks5h://127.0.0.1:$((20000+idx)) -s -o /dev/null -w "%{time_total}" -m 2.5 "http://cp.cloudflare.com/generate_204")
        if [ "$res" = "0.000" ] || [ -z "$res" ]; then
          echo "fail" > "$tmp_dir/res_$idx"
        else
  ...
  for i in $(seq 1 $PEER_COUNT); do
    local tag type name port mark ms color status_text left_part
    ...
    left_part=$(printf "%2d. [%-8s] %s|%s%s" "$i" "$type" "$name" "$port" "$mark")
    printf "  %b%-40s %s%b\n" "$color" "$left_part" "$status_text" "$PLAIN"
  done
  rm -rf "$tmp_dir"
  return 0
}
