...
probe_network_stack(){
  ...
  if [ -z "$local_v4" ] && [ -z "$local_v6" ]; then
    NET_IPV4=""; NET_IPV6=""; NET_STACK="none"; return
  fi
...
probe_network_stack_async(){
  (
    ...
    printf '%s\n%s\n%s\n%s\n%s\n' "$NET_IPV4" "$NET_IPV6" "$NET_IF_V4" "$NET_IF_V6" "$NET_STACK" > "$tmp" && mv -f "$tmp" "$NET_CACHE" || rm -f "$tmp"
...
load_net_cache(){
  ...
  if [ -z "$NET_STACK" ]; then
    probe_network_stack
    printf '%s\n%s\n%s\n%s\n%s\n' "$NET_IPV4" "$NET_IPV6" "$NET_IF_V4" "$NET_IF_V6" "$NET_STACK" > "$NET_CACHE" 2>/dev/null
  fi
}
...
sync_bypass_rules(){
  local port new_ports old_ports4 old_ports6 sport_ok=0
  ...
  ip -4 rule add pref "$BYPASS_PREF" sport 65535 lookup main 2>/dev/null && {
    ip -4 rule del pref "$BYPASS_PREF" sport 65535 lookup main 2>/dev/null
    sport_ok=1
  }

  if [ "$sport_ok" = 1 ]; then
    nft delete table inet "$MARK_TABLE" 2>/dev/null
    ...
    nft delete table inet "$MARK_TABLE" 2>/dev/null
    nft add table inet "$MARK_TABLE" 2>/dev/null
    nft add chain inet "$MARK_TABLE" output '{ type route hook output priority mangle; policy accept; }' 2>/dev/null
    for port in $new_ports; do
      nft add rule inet "$MARK_TABLE" output tcp sport "$port" meta mark set 255 2>/dev/null
      nft add rule inet "$MARK_TABLE" output udp sport "$port" meta mark set 255 2>/dev/null
    done
  fi
  ...
  ip -4 rule del pref "$ICMP_PREF" ipproto icmp lookup main 2>/dev/null
  ip -4 rule add pref "$ICMP_PREF" ipproto icmp lookup main 2>/dev/null
  ip -6 rule del pref "$ICMP_PREF" ipproto ipv6-icmp lookup main 2>/dev/null
  ip -6 rule add pref "$ICMP_PREF" ipproto ipv6-icmp lookup main 2>/dev/null
}
...
build_config(){
  ...
  local peer_host node_files tls_extra providers probe_target peer_domains peer_ips wg_host
  local _strategy=""
    ...
    inbounds=$(jq -s --arg d "$domain" --argjson x "$tls_extra" '
      [ .[] |
        ...
        if .tls_mode=="acme" then
          $in * {tls: ({server_name:$d,enabled:true}
  ...
  peer_domains='[]'
  peer_ips='[]'
  if ls "$PEER_DIR"/*.json 1> /dev/null 2>&1; then
    peer_domains=$(jq -s 'map(.outbound.server//empty) | map(select(test("^[0-9]+\\.[0-9]+\\.[0-9]+\\.[0-9]+$") | not)) | map(select(test("^[0-9a-fA-F:]+$") | not)) | unique' "$PEER_DIR"/*.json 2>/dev/null || echo '[]')
    peer_ips=$(jq -s 'map(.outbound.server//empty) | map(select(test("^[0-9]+\\.[0-9]+\\.[0-9]+\\.[0-9]+$") or test("^[0-9a-fA-F:]+$"))) | unique' "$PEER_DIR"/*.json 2>/dev/null || echo '[]')
  fi
  if [ -f "$WG_CONF" ] && [ "$(jq -r '.enabled//false' "$WG_CONF")" = true ] && [ "$(jq -r .role "$WG_CONF")" = client ]; then
    wg_host=$(jq -r '.peer_host//""' "$WG_CONF")
    if [ -n "$wg_host" ]; then
      if [[ "$wg_host" =~ ^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+$ ]] || [[ "$wg_host" =~ : ]]; then
        peer_ips=$(jq --arg h "$wg_host" '. + [$h] | unique' <<<"$peer_ips")
      else
        peer_domains=$(jq --arg h "$wg_host" '. + [$h] | unique' <<<"$peer_domains")
      fi
    fi
  fi

  rules=$(jq -n --argjson domains "$peer_domains" --argjson ips "$peer_ips" '
    [{action:"sniff"},
     {protocol:"dns",action:"hijack-dns"},
     {port:53,action:"hijack-dns"}]
    + (if ($domains|length)>0 then [{domain:$domains,action:"route",outbound:"direct"}] else [] end)
    + (if ($ips|length)>0 then [{ip_cidr:($ips | map(. + (if test(":") then "/128" else "/32" end))),action:"route",outbound:"direct"}] else [] end)
  ...
  dns_block=$(jq -n --argjson domains "$peer_domains" --arg final "$final" '
    (if ($domains|length)>0 then
      [{domain:$domains,server:"dns-direct-v4"}]
    else [] end) as $node_rules |
    [
      {tag:"dns-remote-v4",address:"1.1.1.1",detour:$final},
      {tag:"dns-remote-v4b",address:"8.8.8.8",detour:$final},
      {tag:"dns-remote-v6",address:"2606:4700:4700::1111",detour:$final},
      {tag:"dns-remote-v6b",address:"2001:4860:4860::8888",detour:$final},
      {tag:"dns-direct-v4",address:"1.1.1.1",detour:"direct"},
      {tag:"dns-direct-v4b",address:"8.8.8.8",detour:"direct"},
      {tag:"dns-direct-v6",address:"2606:4700:4700::1111",detour:"direct"},
      {tag:"dns-direct-v6b",address:"2001:4860:4860::8888",detour:"direct"}
    ] as $servers |
    {
      servers: $servers,
      rules: ($node_rules + [
        {action:"evaluate",server:"dns-remote-v4"},
        {match_response:true,query_type:["A"],action:"route",server:"dns-remote-v4"},
        {action:"evaluate",server:"dns-remote-v6"},
        {match_response:true,query_type:["AAAA"],action:"route",server:"dns-remote-v6"}
      ]),
      final: "dns-remote-v4"
    }')

  case "$NET_STACK" in
    both) _strategy="prefer_ipv6" ;;
    v4)   _strategy="ipv4_only" ;;
    v6)   _strategy="ipv6_only" ;;
  esac
  [ -n "$_strategy" ] && dns_block=$(jq --arg s "$_strategy" '. + {strategy:$s}' <<<"$dns_block")
  ...
  jq -n --argjson inbounds "$inbounds" --argjson outbounds "$outbounds" \
    ...
    {log:{level:"warn",timestamp:true},
     ...
     inbounds:$inbounds,
     outbounds:$outbounds,
     route:{rules:$rules,final:$final,default_domain_resolver:{server:"dns-direct-v4"}}}
...
run_watchdog(){
  ...
  sleep 3
  target_exit=$(state_get exit); probe_override="$target_exit"
  while :; do
    ...
    if [ "$current" = "direct" ]; then
      if ! ss -Hln 2>/dev/null | grep -q ':2081'; then WATCHDOG_PROBE="$probe_override" apply_config_quiet; fi
      if env http_proxy="$proxy" curl -s -o /dev/null --connect-timeout 4 -m 6 "$PROBE_URL" || env http_proxy="$proxy" curl -s -o /dev/null --connect-timeout 4 -m 6 "$probe2"; then
        fail_count=0; state_set exit "$target_exit"
        if ! apply_config_quiet; then state_set exit direct; WATCHDOG_PROBE="$probe_override" apply_config_quiet; fi
      ...
      if curl -s -o /dev/null --connect-timeout 4 -m 6 "$PROBE_URL" || curl -s -o /dev/null --connect-timeout 4 -m 6 "$probe2"; then fail_count=0; else
        ...
        if [ "$fail_count" -ge 3 ]; then
          ...
          if ! WATCHDOG_PROBE="$probe_override" apply_config_quiet; then state_set exit "$target_exit"; apply_config_quiet; fi
        fi
      fi
    fi
    sleep 120
...
save_node(){
  ...
  else rm -f "$file"; apply_config_quiet; out_warn "配置应用失败，已回滚"; fi
  wait_key
}
  ...
  body=$(jq -n --arg tag "$tag" --arg name "$name" --argjson port "$port" --arg uuid "$uuid" \
   ...
   {tag:$tag,name:$name,kind:"vless-reality",port:$port,proto:"t",hopping:"",
    ...
    inbound:{type:"vless",tag:$tag,listen:"::",listen_port:$port,
      ...
      tls:{server_name:$target,
        reality:{handshake:{server:$target,server_port:443},
...
create_hysteria2(){
  ...
  tell ""
  tell "  1. BBR     (自动调节，无需指定带宽)"
  tell "  2. Brutal  (需指定带宽上下行)"
  while :; do
    case $(prompt "请选择拥塞控制方式" 1) in
      1)
        tell ""
        tell "  已启用标准 BBR，无需设置带宽"
        break ;;
      2)
        tell ""
        while :; do
          up_mbps=$(prompt "上行带宽 (Mbps)" "100")
          [[ $up_mbps =~ ^[0-9]+$ ]] && [ "$up_mbps" -gt 0 ] && break || tell_warn "必须为大于 0 的整数"
        done
        while :; do
          down_mbps=$(prompt "下行带宽 (Mbps)" "100")
          [[ $down_mbps =~ ^[0-9]+$ ]] && [ "$down_mbps" -gt 0 ] && break || tell_warn "必须为大于 0 的整数"
        done
        tell ""
        tell "  已启用 Brutal (上行 ${up_mbps} / 下行 ${down_mbps} Mbps)"
        break ;;
      *) tell_warn "输入无效，请重新选择"; sleep 1 ;;
    esac
  done
  ...
  body=$(jq -n --arg tag "$tag" --arg name "$name" --argjson port "$port" \
        ...
        --arg min_pkt "$min_pkt" --arg max_pkt "$max_pkt" '
  ...
  if [ "$up_mbps" = 0 ] && [ "$down_mbps" = 0 ]; then
    body=$(jq 'del(.meta.up_mbps, .meta.down_mbps, .inbound.up_mbps, .inbound.down_mbps)' <<<"$body")
  fi
...
build_uri(){
  ...
  case $kind in
    vless-reality)
      uri="vless://$(jq -r .uuid <<<"$meta")@$host:$port?encryption=none&flow=xtls-rprx-vision&security=reality&sni=$(jq -r .target <<<"$meta")&fp=chrome&pbk=$(jq -r .public_key <<<"$meta")&sid=$(jq -r .short_id <<<"$meta")#$(uri_encode "$name")" ;;
    vless-tls)
      uri="vless://$(jq -r .uuid <<<"$meta")@$host:$port?encryption=none&flow=xtls-rprx-vision&security=tls&sni=$host&fp=chrome#$(uri_encode "$name")" ;;
    hysteria2)
      local obfs_type obfs_pw obfs_str hop_str
      ...
      hop_str=""
      [ -n "$hopping" ] && hop_str="&mport=$hopping"
      uri="hysteria2://$(uri_encode "$(jq -r .password <<<"$meta")")@$host:$port/?sni=$host&alpn=h3${obfs_str}${hop_str}#$(uri_encode "$name")" ;;
    tuic)
      uri="tuic://$(jq -r .uuid <<<"$meta"):$(uri_encode "$(jq -r .password <<<"$meta")")@$host:$port?congestion_control=bbr&alpn=h3&sni=$host#$(uri_encode "$name")" ;;
    trojan)
      uri="trojan://$(uri_encode "$(jq -r .password <<<"$meta")")@$host:$port?security=tls&sni=$host#$(uri_encode "$name")" ;;
    anytls)
...
render_share_uri(){
  ...
  if [ "$mode" = acme ]; then
    ...
    [ "$kind" = "snell" ] && out_alert "Snell 分享链接可能不被所有客户端识别，请手动复制 PSK"
    return
  fi
  if [ -n "$NET_IPV4" ]; then
  ...
  if [ -z "$NET_IPV4$NET_IPV6" ]; then out_warn "未识别到本机公网 IP，生成链接失败"; return; fi
  [ -n "$hopping" ] && out "跳跃配置: $hopping"
  [ "$kind" = "snell" ] && out_alert "Snell 分享链接可能不被所有客户端识别，请手动复制 PSK"
}
...
menu_modify_protocol(){
  local kind value other current_hop up_mbps down_mbps obfs_type obfs_password old_json
  local min_pkt max_pkt cur_up cur_down cur_mode
  ...
  while :; do
    ...
    case $(prompt "请选择") in
      ...
      3)
        if [ "$kind" = vless-reality ] || [ "$kind" = vless-tls ] || [ "$kind" = tuic ]; then
          ...
          json_edit "$PICKED" '.meta.uuid=$v|.inbound.users[0].uuid=$v' --arg v "$value" || { tell_warn 失败; wait_key; continue; }
          ...
          json_edit "$PICKED" '.meta.psk=$v|.inbound.psk=$v' --arg v "$value" || { tell_warn 失败; wait_key; continue; }
        else
      ...
      5)
        if [ "$kind" = vless-reality ]; then
          ...
          cur_up=$(jq -r '.meta.up_mbps//0' "$PICKED")
          cur_down=$(jq -r '.meta.down_mbps//0' "$PICKED")
          if [ "$cur_up" -gt 0 ] || [ "$cur_down" -gt 0 ]; then
            tell "  当前模式: Brutal (上行 ${cur_up} / 下行 ${cur_down} Mbps)"
            cur_mode=2
          else
            tell "  当前模式: BBR"
            cur_mode=1
          ...
          tell "  2. Brutal  (需指定带宽上下行)"
          while :; do
            case $(prompt "请选择拥塞控制方式" "$cur_mode") in
              1)
                json_edit "$PICKED" '
                  del(.meta.up_mbps, .meta.down_mbps, .inbound.up_mbps, .inbound.down_mbps)
                ' || { tell_warn 失败; wait_key; continue 2; }
                tell_ok "已启用标准 BBR"
                break ;;
              2)
                tell ""
                while :; do
                  up_mbps=$(prompt "上行带宽 (Mbps)" "${cur_up:-100}")
                  [[ $up_mbps =~ ^[0-9]+$ ]] && [ "$up_mbps" -gt 0 ] && break || tell_warn "必须为大于 0 的整数"
                done
                while :; do
                  down_mbps=$(prompt "下行带宽 (Mbps)" "${cur_down:-100}")
                  [[ $down_mbps =~ ^[0-9]+$ ]] && [ "$down_mbps" -gt 0 ] && break || tell_warn "必须为大于 0 的整数"
                done
                json_edit "$PICKED" '
                  .meta.up_mbps=$up | .meta.down_mbps=$down |
                  .inbound.up_mbps=$up | .inbound.down_mbps=$down
                ' --argjson up "$up_mbps" --argjson down "$down_mbps" || { tell_warn 失败; wait_key; continue 2; }
                tell_ok "已启用 Brutal (上行 ${up_mbps} / 下行 ${down_mbps} Mbps)"
                break ;;
              *) tell_warn "输入无效"; sleep 1 ;;
            esac
          done
    ...
    if apply_config; then tell_ok "已生效"; out_gap; render_share_uri "$PICKED"
...
menu_change_domain(){
  ...
  if [ "$new_domain" != "$old_domain" ]; then find "$ACME_DIR" -mindepth 1 -delete 2>/dev/null; fi
...
uri_to_outbound(){
  ...
  local disable_parrot s_mode client_meta min_pkt max_pkt _hop_arr=""
    ...
    vless)
      ...
      if [ "$security" = reality ]; then
        outbound=$(jq --arg sni "$sni" --arg fp "$fingerprint" --arg pbk "$(query_value pbk)" --arg sid "$(query_value sid)" \
          '.tls={server_name:$sni,utls:{fingerprint:$fp},reality:{public_key:$pbk,short_id:$sid},enabled:true}' <<<"$outbound")
      elif [ "$security" = tls ] || [ "$security" = xtls ]; then
        outbound=$(jq --arg sni "$sni" --arg fp "$fingerprint" \
          '.tls={server_name:$sni,utls:{fingerprint:$fp},enabled:true}' <<<"$outbound")
      fi
      case $network in
    ...
    hysteria2|hy2)
      outbound=$(jq -n --arg tag "$tag" --arg server "$URI_HOST" --argjson port "$URI_PORT" \
        ...
        '{type:"hysteria2",tag:$tag,server:$server,server_port:$port,password:$password,
          tls:{server_name:$sni,alpn:["h3"],enabled:true}}')
      ...
      if [ -n "$hop_range" ]; then
        _hop_arr=$(echo "$hop_range" | tr ',' '\n' | sed 's/-/:/g' | jq -R . | jq -s .)
        outbound=$(jq --argjson r "$_hop_arr" '.server_ports=$r|del(.server_port)|.hop_interval="30s"' <<<"$outbound")
      fi
      ...
      if [ -n "$min_pkt" ] || [ -n "$max_pkt" ]; then
        outbound=$(jq --arg mn "${min_pkt:-512}" --arg mx "${max_pkt:-1200}" \
          'if .obfs then .obfs.min_packet_size=($mn|tonumber) | .obfs.max_packet_size=($mx|tonumber) else . end' <<<"$outbound")
      fi ;;
    tuic)
      ...
      outbound=$(jq -n --arg tag "$tag" --arg server "$URI_HOST" --argjson port "$URI_PORT" \
        ...
        '{type:"tuic",tag:$tag,server:$server,server_port:$port,uuid:$uuid,password:$password,
          ...
          tls:{server_name:$sni,alpn:($alpn|split(",")),enabled:true}}') ;;
    trojan)
      outbound=$(jq -n --arg tag "$tag" --arg server "$URI_HOST" --argjson port "$URI_PORT" \
                       --arg password "$URI_USERINFO" --arg sni "$sni" \
        '{type:"trojan",tag:$tag,server:$server,server_port:$port,password:$password,
          tls:{server_name:$sni,enabled:true}}')
...
list_peers(){
  ...
  if [ -n "$raw_data" ]; then
    ...
    while read -r file tag type name port host; do
      ...
      (
        local ms="" v4_ms="" v6_ms="" r addrs ip
        addrs=$(resolve_addresses "$host" 2>/dev/null)
        [ -z "$addrs" ] && addrs="$host"
        for ip in $(echo "$addrs" | grep -E '^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+$' | head -3); do
          r=$(timeout 2 ping -4 -c 1 -W 1 -m 255 "$ip" 2>/dev/null | awk -F'/' '/^rtt|^round-trip/{print $5}')
          [ -n "$r" ] && { v4_ms="$r"; break; }
        done
        for ip in $(echo "$addrs" | grep ':' | head -3); do
          r=$(timeout 2 ping -6 -c 1 -W 1 -m 255 "$ip" 2>/dev/null | awk -F'/' '/^rtt|^round-trip/{print $5}')
          [ -n "$r" ] && { v6_ms="$r"; break; }
        done
        if [ -n "$v4_ms" ] && [ -n "$v6_ms" ]; then
          ms=$(awk "BEGIN {print ($v4_ms < $v6_ms) ? $v4_ms : $v6_ms}")
        elif [ -n "$v4_ms" ]; then ms="$v4_ms"
        elif [ -n "$v6_ms" ]; then ms="$v6_ms"
        else ms="fail"
        fi
        if [ -n "$ms" ] && [ "$ms" != "fail" ]; then
          ms=$(awk "BEGIN {print int($ms)}")
        fi
...
peer_select(){
  ...
  if wg_client_active; then
    ...
    json_edit "$WG_CONF" '.enabled=false'; wg_was_active=1
  fi
  state_set exit "$tag"; stop_watchdog
  if apply_config; then sync_watchdog; tell_ok "已接管: $(jq -r .name "$PICKED")"
  else
    state_set exit "$previous"
    if [ "$wg_was_active" = 1 ]; then json_edit "$WG_CONF" '.enabled=true' || out_warn "WireGuard 状态恢复失败，请手动检查"; fi
...
wg_tunnel_state(){
  ...
  if [ -n "$peer_ip6" ] && command -v ping6 >/dev/null 2>&1 && ping6 -c1 -W3 "$peer_ip6" >/dev/null 2>&1; then echo up; return; fi
  if curl --interface "$WG_IF" -s -o /dev/null --connect-timeout 4 -m 6 "$PROBE_URL"; then echo up; return; fi
  echo down
}

render_wg_info(){
  local file=$1 role peer_ip peer_ip6 v6_prefix
  role=$(jq -r .role "$file")
  ...
  peer_ip=$(jq -r .peer_ip "$file"); peer_ip6=$(jq -r '.peer_ip6//""' "$file")
  if [ -n "$peer_ip6" ]; then
    tell "  远端 IP:  $peer_ip / $peer_ip6"
  else
    tell "  远端 IP:  $peer_ip"
  fi
  if [ "$role" = client ]; then
    if [ "$(jq -r .enabled "$file")" = true ]; then
      if [ "$(wg_tunnel_state "$peer_ip" "$peer_ip6")" = up ]; then tell_ok "连接成功"; else tell_warn "连接中断"; fi
    fi
  else
    echo ""; tell "  ${YELLOW}请在客户端填入:${PLAIN}"
    tell "  服务端: $(local_ipv4):$(jq -r .listen_port "$file")"
    tell "  公钥: $(jq -r .public_key "$file")"
    tell "  IPv4 前缀: $(jq -r '.address[0]' "$file" | cut -d. -f1-3)"
    v6_prefix=$(jq -r '.address[1]' "$file" 2>/dev/null | sed 's/::1\/64$//;s/::1\/128$//')
    [ -n "$v6_prefix" ] && tell "  IPv6 前缀: $v6_prefix"
  fi
}

wg_setup(){
  local role keypair private public prefix address4 address6 peer_ip4 peer_ip6
  local ipv6_prefix listen_port peer_host peer_port peer_key body resolved
  while :; do
  ...
  [ -n "$private" ] || { tell_warn "密钥生成失败"; wait_key; return; }
  while :; do
    prefix=$(prompt "IPv4 网段前缀 (前三段)" "10.7.0")
  ...
  while :; do
    ipv6_prefix=$(prompt "IPv6 网段前缀" "fd00:7")
    if [[ $ipv6_prefix =~ ^[0-9a-fA-F:]+$ ]]; then break; fi
    tell_warn "IPv6 前缀格式示例: fd00:7"
  done
  if [ "$role" = server ]; then
    ...
    body=$(jq -n --arg private "$private" --arg public "$public" --arg a4 "$address4" --arg a6 "$address6" \
     ...
     {role:"server",enabled:false,private_key:$private,public_key:$public,
      ...
      endpoint:{type:"wireguard",tag:"wireguard",system:true,name:$iface,mtu:1408,
        local_address:[$a4,$a6],private_key:$private,listen_port:$port,
        peers:[{public_key:$peer_key,allowed_ips:[($peer4+"/32"),($peer6+"/128")]}]}}')
  else
    address4="$prefix.2/32"; address6="$ipv6_prefix::2/128"; peer_ip4="$prefix.1"; peer_ip6="$ipv6_prefix::1"
    peer_host=$(prompt "服务端 IP 或域名" "1.1.1.1")
    if ! [[ "$peer_host" =~ ^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+$ ]] && ! [[ "$peer_host" =~ : ]]; then
      resolved=$(resolve_addresses "$peer_host" | head -1)
      if [ -n "$resolved" ]; then
        tell_ok "域名 $peer_host 解析为 $resolved"
        peer_host="$resolved"
      else
        tell_warn "域名解析失败，请手动填写 IP"
        wait_key; return 1
      fi
    fi
    ...
    body=$(jq -n --arg private "$private" --arg public "$public" --arg a4 "$address4" --arg a6 "$address6" \
     ...
     {role:"client",enabled:false,private_key:$private,public_key:$public,
      ...
      endpoint:{type:"wireguard",tag:"wireguard",system:true,name:$iface,mtu:1408,
        local_address:[$a4,$a6],private_key:$private,
...
wg_toggle(){
  ...
  if [ "$(jq -r .enabled "$WG_CONF")" = true ]; then
    ...
    if apply_config; then tell_ok "已关闭隧道"; else out_warn "配置应用失败，请手动检查"; fi
  else
    ...
    if apply_config; then
      ...
      if [ "$role" = server ]; then wg_server_nat_remove; fi
      apply_config_quiet; sync_watchdog; tell_warn "冲突拦截"
    fi
  fi
  wait_key
}
    ...
    case $(prompt "请选择") in
      ...
      5) clear
         ...
         if prompt_yes "确认删除配置"; then
           ...
           rm -f "$WG_CONF"; wg_server_nat_remove
           if [ "$prev_exit" = wireguard ]; then state_set exit direct; stop_watchdog; fi
           if apply_config; then
...
run_uninstall(){
  ...
  while ip rule del pref "$ICMP_PREF" 2>/dev/null; do :; done
  while ip -6 rule del pref "$ICMP_PREF" 2>/dev/null; do :; done
...
menu_status(){
  while :; do
    ...
    tell "  网络栈 : ${NET_STACK:-未知} | IPv4: ${NET_IPV4:-无} | IPv6: ${NET_IPV6:-无}"
    tell ""
    render_client_ip_status
    tell ""
    tell "  1. 检测更新"
