...
install_core(){
  ...
  json=$(release_json)
  if ! echo "$json" | grep -q '"tag_name"'; then tell_warn "获取版本信息失败或遭遇 API 限流"; return 1; fi
  for candidate in $(asset_candidates); do
    url=$(echo "$json" | jq -r --arg s "$candidate.tar.gz" '.assets[]?|select(.name|endswith($s))|.browser_download_url' | head -1)
    [ -n "$url" ] && { asset=$candidate; break; }
  done
  [ -n "$url" ] || { tell_warn "未找到匹配架构的安装包"; return 1; }
  tmp=$(mktemp -d)
  
  if [ "$SYS_ENV" = "systemd" ]; then systemctl stop sing-box >/dev/null 2>&1; else rc-service sing-box stop >/dev/null 2>&1; fi
...
build_config(){
  ...
  # 1.14.0 官方规范: type/server 替代旧版 address, 防止泄漏剔除 default_domain_resolver
...
apply_config(){
  ...
  if ! error=$("$CORE" check -c "$tmp" 2>&1); then
    out_warn "配置语法校验未通过 (未执行回滚，强制执行，请仔细检查语法):"
...
peer_delete(){
  ...
  rm -f "$PICKED"
  apply_config && tell_ok "已删除" || tell_warn "删除成功但应用异常"
...
wg_toggle(){
  ...
  if [ "$(jq -r .enabled "$WG_CONF")" = "true" ]; then
    ...
    apply_config && tell_ok "已关闭隧道" || tell_warn "强制关闭隧道应用发生异常"
  else
