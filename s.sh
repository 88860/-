...
is_physical_iface(){
  case "$1" in
    ""|lo|"$TUN_IF"|"$WG_IF"|docker*|veth*|br-*|virbr*|tailscale*|zt*|wg*|tun*|tap*) return 1 ;;
  esac
...
is_public_v4(){
  local ip=$1 a b
  case "$ip" in
    ""|10.*|192.168.*|127.*|169.254.*|172.1[6-9].*|172.2[0-9].*|172.3[0-1].*) return 1 ;;
  esac
  IFS=. read -r a b _ <<<"$ip"
...
iface_has_public_v6(){
  local iface=$1 ip
  ip=$(ip -6 addr show dev "$iface" 2>/dev/null | awk '/inet6 /{print $2}' | cut -d/ -f1 | grep -v '^fe80')
  [ -n "$ip" ] || return 1
  is_public_v6 "$ip" || return 1
  printf '%s' "$ip"
  return 0
}
...
list_physical_ifaces(){
  ...
  for iface in /sys/class/net/*; do
    ...
    is_physical_iface "$iface" && printf '%s\n' "$iface"
  done | sort -u
}
...
default_iface_v4(){
  ...
  is_physical_iface "$iface" || iface=""
  if [ -z "$iface" ]; then
    iface=$(ip -4 route show table main default 2>/dev/null | awk '{for(i=1;i<=NF;i++)if($i=="dev"){print $(i+1);exit}}')
    is_physical_iface "$iface" || iface=""
  fi
  if [ -z "$iface" ]; then
    iface=$(list_physical_ifaces | while read -r i; do
      iface_has_public_v4 "$i" >/dev/null 2>&1 && { echo "$i"; break; }
    done)
  fi
  if [ -z "$iface" ]; then
    iface=$(list_physical_ifaces | while read -r i; do
      ip -4 addr show dev "$i" 2>/dev/null | grep -q 'inet ' && { echo "$i"; break; }
    done)
  fi
  printf '%s' "$iface"
}
...
default_iface_v6(){
  ...
  if [ -z "$iface" ]; then
    iface=$(list_physical_ifaces | while read -r i; do
      iface_has_public_v6 "$i" >/dev/null 2>&1 && { echo "$i"; break; }
    done)
  fi
  if [ -z "$iface" ]; then
    iface=$(list_physical_ifaces | while read -r i; do
      ip -6 addr show dev "$i" 2>/dev/null | grep -q 'inet6 ' && { echo "$i"; break; }
    done)
  fi
  printf '%s' "$iface"
}
...
install_core(){
  ...
  [ -n "$asset" ] || { tell_warn "内部错误：asset 未设置"; return 1; }
  tmp=$(mktemp -d)
  TMP_FILES="$TMP_FILES $tmp"
...
EOF
  ...
  nft add rule ip sbm_nat postrouting oifname != "$TUN_IF" oifname != "$WG_IF" oifname != "lo" masquerade 2>/dev/null
  ...
  nft add rule ip6 sbm_nat postrouting oifname != "$TUN_IF" oifname != "$WG_IF" oifname != "lo" masquerade 2>/dev/null
}
...
build_config(){
  ...
  local phys_iface bind_iface tun_route_target
  ...
  phys_iface=$(default_iface_v4)
  [ -z "$phys_iface" ] && phys_iface=$(default_iface_v6)
  bind_iface="$phys_iface"
    ...
    if [ "$(jq -r .role "$WG_CONF")" = client ]; then
      if [ "$selected" = "direct" ] || [ "$selected" = "wireguard" ]; then
        if [ -n "$bind_iface" ]; then
          endpoints=$(jq --arg bi "$bind_iface" '[.endpoint + {bind_interface:$bi}]' "$WG_CONF")
        else
  ...
  if [ "$use_tun" = 1 ]; then
    tun_route_target="$final"
    rules=$(jq -n --argjson list "$rules" --arg target "$tun_route_target" \
      '[{inbound:["tun-in"],action:"route",outbound:$target}] + $list')
  fi
  ...
  jq -n --argjson inbounds "$inbounds" --argjson outbounds "$outbounds" \
    ...
    | if $phys_iface != "" then .route.default_interface=$phys_iface else . end
...
create_vless_reality(){
  ...
  while :; do
    ...
    if [[ $short_id =~ ^[0-9a-fA-F]{1,16}$ ]]; then break; fi
    tell_warn "short_id 必须为 1-16 位十六进制字符"
  done
  ...
  body=$(jq -n --arg tag "$tag" --arg name "$name" --argjson port "$port" \
   ...
   {tag:$tag,name:$name,kind:"hysteria2",port:$port,proto:"u",hopping:$hopping,
    ...
    inbound:({type:"hysteria2",tag:$tag,listen:"::",listen_port:$port,
      users:[{password:$password}],bbr_profile:$bbr}
...
build_uri(){
  ...
  case $kind in
    ...
    hysteria2)
      local obfs_type obfs_pw obfs_str bbr
      obfs_type=$(jq -r '.obfs_type//""' <<<"$meta")
      obfs_pw=$(jq -r '.obfs_password//""' <<<"$meta")
      bbr=$(jq -r '.bbr_profile//"standard"' <<<"$meta")
      obfs_str=""
      [ -n "$obfs_type" ] && obfs_str="&obfs=${obfs_type}&obfs-password=$(uri_encode "$obfs_pw")"
      uri="hysteria2://$(uri_encode "$(jq -r .password <<<"$meta")")@$host:$port/?sni=$host&alpn=h3&bbr_profile=${bbr}${hopping:+&mport=$hopping}${obfs_str}#$(uri_encode "$name")" ;;
    tuic)
    ...
    case $(prompt "请选择") in
      ...
      3)
        if [ "$kind" = vless-reality ] || [ "$kind" = vless-tls ] || [ "$kind" = tuic ]; then
          value=$(prompt "新通讯 UUID (留空自动生成)"); [ -z "$value" ] && value=$(random_uuid)
  ...
  while :; do
    ...
    case $(prompt "请选择") in
      ...
      7)
         ...
         state_set exit direct
         stop_watchdog
         if systemctl stop sing-box 2>/dev/null; then
...
wg_setup(){
  ...
  local listen_port peer_host peer_port peer_key body bind_iface
  while :; do
    clear
  ...
  if [ "$role" = server ]; then
    ...
    if [[ "$peer_host" =~ : ]]; then
      bind_iface=$(default_iface_v6)
    else
      bind_iface=$(default_iface_v4)
    fi
    [ -n "$bind_iface" ] || bind_iface=$(default_iface_v4)
    [ -n "$bind_iface" ] || bind_iface=$(default_iface_v6)
    body=$(jq -n --arg private "$private" --arg public "$public" --arg a4 "$address4" --arg a6 "$address6" \
          --arg host "$peer_host" --argjson port "$peer_port" --arg peer_key "$peer_key" \
          --arg peer4 "$peer_ip4" --arg iface "$WG_IF" --arg bi "$bind_iface" '
     {role:"client",enabled:false,private_key:$private,public_key:$public,
      ...
      endpoint:({type:"wireguard",tag:"wireguard",system:true,name:$iface,mtu:1408,
        ...
        + (if $bi!="" then {bind_interface:$bi} else {} end))}')
  fi
...
case $1 in
  ...
  --reapply)
    ...
    install -m600 "$_new_conf" "$CONFIG"
    systemctl reload sing-box 2>/dev/null || systemctl restart sing-box 2>/dev/null
    sync_bypass_rules
    sync_hopping_rules
    flock -u 9
    exit 0 ;;
