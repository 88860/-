  ...
  (
    ...
    while :; do
      ip monitor link route address 2>/dev/null | while IFS= read -r line; do
        case "$line" in
          *" lo "*|*"lo:"*|*"docker"*|*"veth"*|*"br-"*|*"virbr"*|*"$TUN_IF"*|*"$WG_IF"*|*"tailscale"*|*"zt"*|*"tun"*) continue ;;
        esac
...
build_config(){
  ...
  local dns_direct_server dns_remote_server dns_remote_v6_server bridge_iface
  ...
  if [ -f "$WG_CONF" ] && [ "$(jq -r '.enabled//false' "$WG_CONF")" = true ]; then
    ...
    if [ "$(jq -r .role "$WG_CONF")" = client ]; then
      ...
      [ "$selected" = wireguard ] && { final="wireguard"; use_tun=0; }
    fi
  fi
...
run_watchdog(){
  ...
  [ "$target_exit" = "wireguard" ] && probe_override="wireguard"
