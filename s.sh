...
build_config(){
  ...
  if [ "$SYS_ENV" = "systemd" ]; then
    [ "$use_tun" = 1 ] && inbounds=$(jq -n --argjson list "$inbounds" --arg name "$TUN_IF" '
      [{type:"tun",tag:"tun-in",interface_name:$name,inet4_address:["172.19.0.1/30"],inet6_address:["fdfe:dcba:9876::1/126"],auto_route:true,strict_route:true,stack:"mixed",mtu:1400}] + $list')
