...
build_config(){
  ...
  dns_block=$(jq -n --arg direct "$dns_direct_server" --arg strategy "$dns_strategy" --arg detour "$final" '
    {servers:[
      ...
      {type:"https",tag:"dns-remote",server:"cloudflare-dns.com",server_port:443,path:"/dns-query",tls:{enabled:true,server_name:"cloudflare-dns.com"},domain_resolver:"dns-direct",detour:$detour}
    ],
    final:"dns-remote",
    strategy:$strategy}')
