...
# Shared static paths, service identifiers, and UI constants.
...
init_dirs(){
  mkdir -p "$SBM_DIR" "$NODE_DIR" "$PEER_DIR" "$WG_DIR" "$ACME_DIR" "$SB_DIR" /var/lib/sing-box || return 1
...
check_dependencies(){
  ...
  local deps=(
    ...
    "awk:mawk" "getent:libc-bin" "base64:coreutils" "ps:procps"
  )
  local missing=0
  for item in "${deps[@]}"; do
    cmd=${item%%:*}
...
# Common JSON persistence primitives.
...
render_share_uri(){
  ...
  if [ "$mode" = acme ]; then
    ...
    uri=$(build_uri "$kind" "$name" "$port" "$meta" "$hopping" "$host") || { tell_warn "分享链接生成失败"; return 1; }
    [ -n "$uri" ] || { tell_warn "分享链接生成失败"; return 1; }
    tell "${GREEN}$uri${PLAIN}"
    [ -n "$hopping" ] && tell "跳跃配置: $hopping"
    return 0
  fi
  if [ -n "$NET_IPV4" ]; then
    uri=$(build_uri "$kind" "$name" "$port" "$meta" "$hopping" "$NET_IPV4") || { tell_warn "分享链接生成失败"; return 1; }
    [ -n "$uri" ] || { tell_warn "分享链接生成失败"; return 1; }
    tell "${GREEN}$uri${PLAIN}"
  fi
  if [ -n "$NET_IPV6" ]; then
    uri=$(build_uri "$kind" "$name" "$port" "$meta" "$hopping" "[$NET_IPV6]") || { tell_warn "分享链接生成失败"; return 1; }
    [ -n "$uri" ] || { tell_warn "分享链接生成失败"; return 1; }
    tell "${GREEN}$uri${PLAIN}"
  fi
  if [ -z "$NET_IPV4$NET_IPV6" ]; then tell_warn "未识别到本机公网 IP，生成链接失败"; return 1; fi
  [ -n "$hopping" ] && tell "跳跃配置: $hopping"
...
NET_STACK=""
IPV6_OK=0
# Network runtime state is owned by this module. Callers use accessors below.
...
# Network/port/domain/certificate validation domain extracted from s.sh.
...
# ACME certificate provider options.
acme_options(){
...
# sing-box configuration build/apply domain extracted from s.sh.
# Expects core state/network helpers and globals to be initialized by caller.
...
# URI parsing/building domain extracted from s.sh.
# Expects generic helpers such as uri_encode/uri_decode and jq.

build_uri(){
  local kind=$1 name=$2 port=$3 meta=$4 hopping=$5 host=$6 uri="" sni_host
  sni_host="$host"
  if [[ "$sni_host" == \[*\] ]]; then sni_host=${sni_host#\[}; sni_host=${sni_host%\]}; fi
  case $kind in
    shadowsocks)
    ...
    vmess-tls|vmess-ws|vmess-grpc|vmess-http)
      ...
      vmess_json=$(jq -n --arg v "2" --arg ps "$name" --arg add "$host" --argjson port "$port" \
        ...
        --arg hosth "$sni_host" --arg service "$(jq -r '.service_name//""' <<<"$meta")" --arg tls "tls" --arg type "none" \
    ...
    vless-ws)
      ...
      uri="vless://$(jq -r .uuid <<<"$meta")@$host:$port?encryption=none&security=tls&sni=$(uri_encode "$sni_host")&fp=chrome&type=ws&path=$(uri_encode "$vless_path")&host=$(uri_encode "$sni_host")&allowInsecure=0#$(uri_encode "$name")" ;;
    vless-grpc)
      local vless_service
    ...
    hysteria2)
      ...
      if [ -n "$hopping" ]; then
        uri="hysteria2://$(uri_encode "$(jq -r .password <<<"$meta")")@$host:$hopping?sni=$(uri_encode "$sni_host")${bbr_str}${bandwidth_str}${obfs_str}${pkt_str}#$(uri_encode "$name")"
      else
        uri="hysteria2://$(uri_encode "$(jq -r .password <<<"$meta")")@$host:$port?sni=$(uri_encode "$sni_host")${bbr_str}${bandwidth_str}${obfs_str}${pkt_str}#$(uri_encode "$name")"
      fi ;;
    tuic)
      local tuic_cc
    ...
    trojan-ws)
      ...
      uri="trojan://$(uri_encode "$(jq -r .password <<<"$meta")")@$host:$port?security=tls&sni=$(uri_encode "$sni_host")&type=ws&path=$(uri_encode "$trojan_path")&host=$(uri_encode "$sni_host")&allowInsecure=0#$(uri_encode "$name")" ;;
    trojan-grpc)
...
parse_uri(){
  ...
  if [ "$scheme" = hysteria2 ] || [ "$scheme" = hy2 ]; then
    ...
    if [ -n "$port_spec" ]; then
      [[ "$port_spec" =~ ^[0-9]+$ ]] || return 1
      [ "$port_spec" -ge 1 ] && [ "$port_spec" -le 65535 ] || return 1
      port=$port_spec
    else
      port=443
    fi
...
uri_to_outbound(){
  ...
  case $(jq -r .scheme <<<"$context") in
    ...
    hysteria2|hy2)
      ...
      bbr_profile=$(query_value "$context" bbr_profile)
      ...
      if [ -n "$up_mbps" ] || [ -n "$down_mbps" ]; then
        ...
        # Brutal/bandwidth mode is mutually exclusive with bbr_profile in the server config.
        bbr_profile=""
      fi
...
# WireGuard domain extracted from s.sh.
# No initialization side effects. Expects core globals/helpers from s.sh.
...
wg_server_nat_remove(){
  ...
  if [ -f "$nat_state" ]; then
    ...
    # No saved forwarding state means this installation did not enable NAT.
    # Remove any stale nftables tables, but never overwrite the host's
    # pre-existing forwarding settings with an assumed value of 0.
    nft delete table ip sbm_nat 2>/dev/null || true
    nft delete table ip6 sbm_nat 2>/dev/null || true
    return 0
  fi
...
# Protocol/node mutation domain.
# Keeps JSON schema knowledge out of menu code while preserving the existing
# json_edit/json_write persistence primitives and caller-visible behavior.
...
# Apply a modified node and restore the exact previous JSON if validation or
# service reload fails. UI code decides how to report the result.
protocol_commit(){
...
# Interactive protocol-edit handlers. They own prompts/validation for each
# menu option; menu_modify_protocol only dispatches and commits the mutation.
protocol_edit_name(){
  local file=$1 value
...
# Transaction helpers for callers that need an atomic state snapshot/rollback.
state_snapshot(){
  jq -e . "$STATE" >/dev/null 2>&1 || return 1
  cat "$STATE"
}
...
# ACME/domain state contract. Callers should not depend on raw state keys.
state_acme_get(){
  state_get "$1"
}
...
watchdog_needed(){ local exit_node; exit_node=$(state_exit_get) || return 2; [ "$exit_node" != "direct" ]; }
...
# Server node repository and protocol creation
# Interactive protocol constructors persist node JSON through save_node.
...
# Client / peer repository and client UI
...
if [ "${BASH_SOURCE[0]}" = "$0" ]; then
  case $1 in
    ...
    --clear-hopping) clear_hopping_rules || exit 1; exit 0 ;;
    --reapply)
      exec 9>"$REAPPLY_LOCK"
      flock -w 5 9 || exit 0
