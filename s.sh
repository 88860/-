...
wg_status(){
  local file=$WG_CONF role state ms color
  clear
  [ -f "$file" ] || { tell_warn "未配置 WireGuard"; wait_key; return; }
  role=$(jq -r '.role // ""' "$file")
  tell "${CYAN}========== WireGuard 状态 ==========${PLAIN}"
  if [ "$role" = server ]; then tell "  角色: ${GREEN}服务端${PLAIN}"; else tell "  角色: ${GREEN}客户端${PLAIN}"; fi
  ...
  if [ "$(jq -r '.enabled//false' "$file")" = true ]; then tell "  状态: ${GREEN}已启动${PLAIN}"; else tell "  状态: ${RED}未启动${PLAIN}"; fi
...
menu_wireguard(){
  while :; do
    ...
    if [ ! -f "$WG_CONF" ]; then
      tell "  配置: ${RED}未配置${PLAIN}"
    else
      local role enabled state ms color
      role=$(jq -r '.role // ""' "$WG_CONF")
      enabled=$(jq -r '.enabled//false' "$WG_CONF")
      if [ "$role" = server ]; then tell "  角色: ${GREEN}服务端${PLAIN}"; else tell "  角色: ${GREEN}客户端${PLAIN}"; fi
      if [ "$enabled" = true ]; then tell "  状态: ${GREEN}已启动${PLAIN}"; else tell "  状态: ${RED}未启动${PLAIN}"; fi
