...
node_ports(){
  local files=("$NODE_DIR"/*.json)
  [ -e "${files[0]}" ] && jq -r '.port' "${files[@]}" 2>/dev/null | grep -E '^[1-9][0-9]*$'
}
...
list_nodes(){
  local index=0 proto
  NODE_FILES=("$NODE_DIR"/*.json)
  [ -e "${NODE_FILES[0]}" ] || { tell "  系统内暂无节点"; return 0; }
  while IFS=$'\t' read -r kind port name proto; do
    index=$((index+1))
    [ "$proto" = "u" ] && proto=UDP || proto=TCP
    printf '  %2d. [%-14s] 端口: %-5s | %s\n' "$index" "$kind" "$port" "$name"
  done < <(jq -r '(.kind) + "\t" + (.port|tostring) + "\t" + (.name) + "\t" + (.proto)' "${NODE_FILES[@]}" 2>/dev/null)
  return 0
}
...
list_peers(){
  local index=0 current mark
  current=$(state_get exit)
  PEER_FILES=("$PEER_DIR"/*.json)
  [ -e "${PEER_FILES[0]}" ] || { tell "  暂无外部节点"; return 0; }
  while IFS=$'\t' read -r tag type name port; do
    index=$((index+1))
    [ "$tag" = "$current" ] && mark="${GREEN}<= 当前生效${PLAIN}" || mark=""
    printf '  %2d. [%-10s] %b | %s (端口: %s)\n' "$index" "$type" "$mark" "$name" "$port"
  done < <(jq -r '(.tag) + "\t" + (.outbound.type) + "\t" + (.name) + "\t" + ((.outbound.server_port // .outbound.listen_port // "-") | tostring)' "${PEER_FILES[@]}" 2>/dev/null)
  return 0
}
