...
warp_register(){
  ...
  tell_ok "注册成功"
  tell "1. IPv4   2. IPv6   3. 双栈   0. 返回"
  case $(prompt "选择出口") in
    1) warp_mode_set ipv4 || tell_warn "模式保存失败" ;;
    2) warp_mode_set ipv6 || tell_warn "模式保存失败" ;;
    3) warp_mode_set dual || tell_warn "模式保存失败" ;;
  esac
}
