  ...
  if [ "$NET_STACK" = "v6" ]; then
    dns_block=$(jq -n --arg strategy "$dns_strategy" --arg detour "$final" '
      {servers:[{type:"local",tag:"dns-direct"},
        {type:"https",tag:"dns-direct-v6",server:"2606:4700:4700::1111",server_port:443,path:"/dns-query",tls:{enabled:true,server_name:"cloudflare-dns.com"},detour:"direct"},
    ...
    dns_block=$(jq -n --arg strategy "$dns_strategy" --arg detour "$final" '
      {servers:[{type:"local",tag:"dns-direct"},
        {type:"https",tag:"dns-direct-v4",server:"1.1.1.1",server_port:443,path:"/dns-query",tls:{enabled:true,server_name:"cloudflare-dns.com"},detour:"direct"},
        {type:"https",tag:"dns-direct-v6",server:"2606:4700:4700::1111",server_port:443,path:"/dns-query",tls:{enabled:true,server_name:"cloudflare-dns.com"},detour:"direct"},
