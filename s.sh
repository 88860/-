...
build_config(){
  ...
  # 标准 Sing-box 1.14.0 DNS API：取消废弃参数并确保不发生 DNS 泄漏
...
apply_config(){
  ...
  if ! error=$("$CORE" check -c "$tmp" 2>&1); then
    out_warn "配置语法校验未通过 (不执行回滚机制，请自行排查错误):"
...
menu_delete_protocol(){
  ...
  apply_config && tell_ok "已删除" || tell_warn "强行移除节点文件，但重启服务异常"
  wait_key
}
...
menu_modify_protocol(){
  ...
  while :; do
    ...
    else tell_warn "因配置冲突或语法异常应用失败 (文件已强行修改且不触发回滚，请注意排查)"
    fi
    wait_key
  done
}
...
menu_change_domain(){
  ...
  else tell_warn "部署应用失败 (强制不回滚)"; wait_key
  fi
}
