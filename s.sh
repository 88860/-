...
wg_actual_listen_port(){
  ...
  ss -Hlunp 2>/dev/null | awk -v p=":$configured" '$4 ~ (p"$") && /sing-box/ {found=1} END{if(found) print p}' | sed 's/^://'
}

wg_tunnel_state(){
  local ip4="" ip6=""
  [ "$(jq -r '.role // ""' "$WG_CONF" 2>/dev/null)" = client ] || { echo 'v4=|v6='; return; }
  [ "$(jq -r '.enabled // false' "$WG_CONF" 2>/dev/null)" = true ] || { echo 'v4=|v6='; return; }
  get_ip_info 4 wireguard >/dev/null 2>&1 && ip4=$IP_INFO_IP
  get_ip_info 6 wireguard >/dev/null 2>&1 && ip6=$IP_INFO_IP
  printf 'v4=%s|v6=%s
' "$ip4" "$ip6"
}

wg_direct_ip_status(){
  local family=$1 route_target dev ip
  if command -v ip >/dev/null 2>&1; then
    if [ "$family" = 4 ]; then
      route_target=1.1.1.1
      dev=$(ip -4 route show default 2>/dev/null | awk -v tun="${TUN_IF:-}" '$1=="default" && (tun=="" || $0 !~ "dev " tun "( |$)"){for(i=1;i<=NF;i++) if($i=="dev"){print $(i+1); exit}}')
      [ -n "$dev" ] || dev=$(ip -4 route get "$route_target" 2>/dev/null | awk -v tun="${TUN_IF:-}" 'NR==1 {for(i=1;i<=NF;i++) if($i=="dev" && (tun=="" || $(i+1)!=tun)){print $(i+1); exit}}')
    else
      route_target=2606:4700:4700::1111
      dev=$(ip -6 route show default 2>/dev/null | awk -v tun="${TUN_IF:-}" '$1=="default" && (tun=="" || $0 !~ "dev " tun "( |$)"){for(i=1;i<=NF;i++) if($i=="dev"){print $(i+1); exit}}')
      [ -n "$dev" ] || dev=$(ip -6 route get "$route_target" 2>/dev/null | awk -v tun="${TUN_IF:-}" 'NR==1 {for(i=1;i<=NF;i++) if($i=="dev" && (tun=="" || $(i+1)!=tun)){print $(i+1); exit}}')
    fi
    if [ -n "$dev" ] && command -v curl >/dev/null 2>&1; then
      if [ "$family" = 4 ]; then
        ip=$(env -u http_proxy -u https_proxy -u HTTP_PROXY -u HTTPS_PROXY -u all_proxy -u ALL_PROXY curl -4 -fsS --interface "$dev" -m 5 https://api4.ipify.org 2>/dev/null)
      else
        ip=$(env -u http_proxy -u https_proxy -u HTTP_PROXY -u HTTPS_PROXY -u all_proxy -u ALL_PROXY curl -6 -fsS --interface "$dev" -m 5 https://api6.ipify.org 2>/dev/null)
      fi
      if [ "$family" = 4 ]; then
        printf '%s' "$ip" | grep -Eq '^[0-9]+(\.[0-9]+){3}$' && { printf '%s' "$ip"; return 0; }
      else
        printf '%s' "$ip" | grep -q ':' && { printf '%s' "$ip"; return 0; }
      fi
    fi
  fi
  get_ip_info "$family" >/dev/null 2>&1 && [ -n "$IP_INFO_IP" ] && printf '%s' "$IP_INFO_IP"
}

wg_status(){
  local file=$WG_CONF role enabled public ipv4 ipv6 peer_host peer_port peer_key state local4 local6 tunnel4 tunnel6
  screen_clear
  if [ ! -f "$file" ]; then tell "${RED}WireGuard 尚未配置，请先初始化。${PLAIN}"; wait_key; return; fi
  role=$(jq -r '.role // ""' "$file" 2>/dev/null); enabled=$(jq -r '.enabled // false' "$file" 2>/dev/null)
  public=$(jq -r '.public_key // empty' "$file" 2>/dev/null); ipv4=$(jq -r '.address[0] // empty' "$file" 2>/dev/null); ipv6=$(jq -r '.address[1] // empty' "$file" 2>/dev/null)
  local4=$(wg_direct_ip_status 4); local6=$(wg_direct_ip_status 6)
  tell "${BRIGHT_CYAN}${BOLD}>> WireGuard 状态信息 <<${PLAIN}"
  case "$role" in
    server)
      local configured_port actual_port
      configured_port=$(jq -r '.listen_port // 0' "$file" 2>/dev/null); actual_port=$(wg_actual_listen_port "$configured_port")
      tell "运行模式：服务端"
      if [ "$enabled" = true ] && [ -n "$actual_port" ]; then tell "状态：${GREEN}已启动${PLAIN}"; elif [ "$enabled" = true ]; then tell "状态：${RED}异常${PLAIN}"; else tell "状态：${RED}已停止${PLAIN}"; fi
      if [ -n "$local4" ]; then tell "本机IPv4：${GREEN}${local4}${PLAIN}"; else tell "本机IPv4：${RED}无或不可用${PLAIN}"; fi
      if [ -n "$local6" ]; then tell "本机IPv6：${GREEN}${local6}${PLAIN}"; else tell "本机IPv6：${RED}无或不可用${PLAIN}"; fi
      tell "本机公钥：$public"; tell ""
      tell "监听端口：$configured_port"; tell "隧道IPv4：$ipv4"; tell "隧道IPv6：$ipv6"; tell ""
      tell "客户端数量：$(jq '.endpoint.peers | length' "$file" 2>/dev/null || echo 0)"; tell ""
      tell "IPv4外层：$(if [ -n "$local4" ]; then printf '%s正常%s' "$GREEN" "$PLAIN"; else printf '%s不可用%s' "$RED" "$PLAIN"; fi)"
      tell "IPv6外层：$(if [ -n "$local6" ]; then printf '%s正常%s' "$GREEN" "$PLAIN"; else printf '%s不可用%s' "$RED" "$PLAIN"; fi)"
      ;;
    client)
      tunnel4=""; tunnel6=""
      if [ "$enabled" = true ]; then
        state=$(wg_tunnel_state); tunnel4=${state#v4=}; tunnel4=${tunnel4%%|v6=*}; tunnel6=${state#*|v6=}
        tell "运行模式：客户端"
        if [ -n "$tunnel4" ] || [ -n "$tunnel6" ]; then tell "状态：${GREEN}已连接${PLAIN}"; else tell "状态：${RED}未连接${PLAIN}"; fi
      else tell "运行模式：客户端"; tell "状态：${RED}已停止${PLAIN}"; fi
      if [ -n "$local4" ]; then tell "本机IPv4：${GREEN}${local4}${PLAIN}"; else tell "本机IPv4：${RED}无或不可用${PLAIN}"; fi
      if [ -n "$local6" ]; then tell "本机IPv6：${GREEN}${local6}${PLAIN}"; else tell "本机IPv6：${RED}无或不可用${PLAIN}"; fi
      tell "本机公钥：$public"; tell ""
      peer_host=$(jq -r '.peer_host // empty' "$file" 2>/dev/null); peer_port=$(jq -r '.peer_port // empty' "$file" 2>/dev/null); peer_key=$(jq -r '.peer_public_key // empty' "$file" 2>/dev/null)
      tell "服务器：${GREEN}${peer_host}${PLAIN}:${YELLOW}${peer_port}${PLAIN}"
      if [ -n "$tunnel4" ] || [ -n "$tunnel6" ]; then tell "服务器状态：${GREEN}已连接${PLAIN}"; else tell "服务器状态：${RED}未连接${PLAIN}"; fi
      tell "服务器公钥：$peer_key"; tell ""
      tell "隧道IPv4：$ipv4"; tell "隧道IPv6：$ipv6"; tell ""
      tell "外层IPv4：$(if [ -n "$local4" ]; then printf '%s正常%s' "$GREEN" "$PLAIN"; else printf '%s不可用%s' "$RED" "$PLAIN"; fi)"
      tell "外层IPv6：$(if [ -n "$local6" ]; then printf '%s正常%s' "$GREEN" "$PLAIN"; else printf '%s不可用%s' "$RED" "$PLAIN"; fi)"
      tell "隧道IPv4：$(if [ -n "$tunnel4" ]; then printf '%s正常%s' "$GREEN" "$PLAIN"; else printf '%s不可用%s' "$RED" "$PLAIN"; fi)"
      tell "隧道IPv6：$(if [ -n "$tunnel6" ]; then printf '%s正常%s' "$GREEN" "$PLAIN"; else printf '%s不可用%s' "$RED" "$PLAIN"; fi)"
      ;;
    *) tell "${RED}WireGuard 配置无效，请重新初始化。${PLAIN}" ;;
  esac
  tell ""; tell "${CYAN}----------------------------------------${PLAIN}"; wait_key
}
menu_wireguard(){
  while :; do
    screen_clear; tell "${BRIGHT_CYAN}${BOLD}>> WireGuard <<${PLAIN}"
    if [ ! -f "$WG_CONF" ]; then tell "  模式：${RED}未配置${PLAIN}"; else
      local role enabled state configured_port actual_port
      role=$(jq -r '.role // ""' "$WG_CONF"); enabled=$(jq -r '.enabled // false' "$WG_CONF")
      if [ "$role" = client ]; then
        if [ "$enabled" = true ]; then
          state=$(wg_tunnel_state)
          case "$state" in
            v4=\|v6=) tell "  模式：客户端|${RED}未连接${PLAIN}";;
            v4=*\|v6=*) tell "  模式：客户端|${GREEN}已连接${PLAIN}";;
            *) tell "  模式：客户端|${RED}未连接${PLAIN}";;
          esac
        else tell "  模式：客户端|${RED}已停止${PLAIN}"; fi
      elif [ "$role" = server ]; then
        configured_port=$(jq -r '.listen_port // 0' "$WG_CONF"); actual_port=$(wg_actual_listen_port "$configured_port")
        if [ "$enabled" = true ] && [ -n "$actual_port" ]; then tell "  模式：服务端|${GREEN}已启动${PLAIN}"; elif [ "$enabled" = true ]; then tell "  模式：服务端|${RED}异常${PLAIN}"; else tell "  模式：服务端|${RED}已停止${PLAIN}"; fi
      else tell "  模式：${RED}配置无效${PLAIN}"; fi
    fi
    tell ""; tell "  1. 初始化配置"; tell "  2. 修改配置"; tell "  3. 启动/停止"; tell "  4. 状态信息"; tell "  5. 删除配置"; tell "  0. 退出"; tell "${CYAN}--------------------------------${PLAIN}"
    case $(prompt "请选择") in 1) wg_init ;; 2) wg_edit ;; 3) wg_toggle ;; 4) wg_status ;; 5) wg_delete ;; 0) return ;; *) tell_warn "输入无效"; sleep 1;; esac
  done
}
  ...
  if [ -n "$selected" ] && [ "$selected" != direct ]; then
    ...
    tmp_log=$(mktemp "$SBM_DIR/tmp/status-proxy-log.XXXXXX") || { rm -f "$tmp_conf"; return 1; }
      if [ "$selected" = wireguard ]; then
        ...
        jq -n --argjson port "$port" --arg final "$selected" --arg host "$(jq -r '.peer_host // empty' "$WG_CONF" 2>/dev/null)" --argjson endpoint "$(jq -c '.endpoint' "$WG_CONF" 2>/dev/null)" '
          {log:{level:"warn"},dns:{servers:[{type:"https",tag:"dns-direct-v4",server:"1.1.1.1",server_port:443,path:"/dns-query",tls:{enabled:true,server_name:"cloudflare-dns.com"}}],final:"dns-direct-v4",strategy:"prefer_ipv4"},inbounds:[{type:"mixed",tag:"status-in",listen:"127.0.0.1",listen_port:$port}],outbounds:[{type:"direct",tag:"direct"}],endpoints:[($endpoint|.tag=$final|.domain_resolver="dns-direct-v4")],route:{rules:(if $host=="" then [] elif ($host|test("^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+$")) then [{ip_cidr:[$host+"/32"],action:"route",outbound:"direct"}] elif ($host|test("^[0-9a-fA-F:]+$")) then [{ip_cidr:[$host+"/128"],action:"route",outbound:"direct"}] else [{domain:[$host],action:"route",outbound:"direct"}] end),final:$final,default_domain_resolver:"dns-direct-v4",auto_detect_interface:false}}
...
protected_ports(){
  { ssh_ports; node_ports; wg_listen_port; } | grep -E '^[1-9][0-9]*$' | sort -un
}

build_config(){
  local selected domain listen_addr dns_direct_tag dns_strategy final use_tun auto_detect
  local inbounds outbounds endpoints rules dns_block providers='[]' tls_extra='{}' peer_host
  local f acme_json pending_json wg_source wgob wgrole target_resolver ssh_rule_ports
  ...
  final=direct; use_tun=0; endpoints='[]'; peer_host=''; inbounds='[]'; outbounds='[{"type":"direct","tag":"direct"}]'
  ...
  elif { [ -f "$WG_CONF" ] || [ -n "${WG_OVERRIDE_FILE:-}" ]; } && [ "$(jq -r '.enabled//false' "${WG_OVERRIDE_FILE:-$WG_CONF}" 2>/dev/null)" = true ]; then
    wg_source=${WG_OVERRIDE_FILE:-$WG_CONF}
    wgob=$(jq -c --arg resolver "$dns_direct_tag" '.endpoint | if (.peers[0].address // "") != "" then .domain_resolver=$resolver else . end' "$wg_source") || return 1
    wgrole=$(jq -r '.role//""' "$wg_source")
    [ "$wgrole" = client ] || [ "$wgrole" = server ] || return 1
    endpoints=$(jq -n --argjson ep "$wgob" '[$ep]') || return 1
    if [ "$wgrole" = client ] && [ "$selected" = wireguard ]; then
      final=wireguard
      peer_host=$(jq -r '.peer_host//""' "$wg_source")
      transparent_proxy_available && use_tun=1
    elif [ "$wgrole" = server ]; then
      outbounds=$(jq -n --argjson base "$outbounds" '$base + [{type:"bridge",tag:"bridge"}]') || return 1
      final=bridge
    fi
  fi
  ...
  if [ "$final" = bridge ]; then
    rules=$(jq -n --argjson ssh_ports "$ssh_rule_ports" '
      (if ($ssh_ports|length)>0 then [{network:"tcp",port:$ssh_ports,action:"route",outbound:"direct"}] else [] end)
      + [{network:"icmp",action:"route",outbound:"direct"}]') || return 1
  else
  ...
  dns_block=$(jq -n --arg strategy "$dns_strategy" --arg final "$final" --arg direct "$dns_direct_tag" '
    {servers:[
     ...
     | .servers[2] |= (if $final=="wireguard" then . + {detour:$final} else . end)
     | .strategy=$strategy | .final="dns-remote"') || return 1
  target_resolver=$dns_direct_tag
  [ "$final" = direct ] || [ "$final" = wireguard ] || target_resolver=dns-direct-v4
  jq -n --argjson inbounds "$inbounds" --argjson outbounds "$outbounds" --argjson endpoints "$endpoints" --argjson rules "$rules" --argjson dns "$dns_block" --argjson providers "$providers" --arg final "$final" --arg resolver "$target_resolver" --argjson auto_detect "$auto_detect" '
    {log:{level:"warn",timestamp:true},dns:$dns,inbounds:$inbounds,outbounds:$outbounds,route:{rules:$rules,final:$final,default_domain_resolver:$resolver,auto_detect_interface:$auto_detect}}
...
wg_client_edit(){
  local index count choice value body ip4 ip6 pub
  count=$(jq '.endpoint.peers | length' "$WG_CONF" 2>/dev/null); count=${count:-0}
  [ "$count" -gt 0 ] || { tell_warn "暂无客户端"; wait_key; return; }
  screen_clear
  ...
  while :; do
    screen_clear
    tell "${BRIGHT_CYAN}${BOLD}>> 修改客户端 <<${PLAIN}"
    tell "  客户端：$(jq -r ".clients[$index].name // \"客户端 $((index+1))\"" "$WG_CONF")"
    tell "  1. 修改名称"
    tell "  2. 修改 IPv4"
    tell "  3. 修改 IPv6"
    tell "  4. 修改公钥"
    ...
    case "$choice" in
      1)
        value=$(prompt "客户端名称" "$(jq -r ".clients[$index].name // \"\"" "$WG_CONF")")
        [ -n "$value" ] || { tell_warn "客户端名称不能为空"; sleep 1; continue; }
        body=$(jq --argjson i "$index" --arg v "$value" '.clients[$i].name=$v' "$WG_CONF") || { tell_warn "配置生成失败"; sleep 1; continue; }
        ;;
      2)
        value=$(prompt "隧道 IPv4" "$(jq -r ".clients[$index].ipv4 // \"\"" "$WG_CONF")")
        { wg_valid_address "$value" && ! printf '%s' "$value" | grep -q ':'; } || { tell_warn "IPv4 地址无效"; sleep 1; continue; }
        if ! wg_client_address_conflict "$index" "$value" "$(jq -r ".clients[$index].ipv6 // \"\"" "$WG_CONF")"; then tell_warn "IPv4 地址已被其他客户端使用"; sleep 1; continue; fi
        ip4=${value%/*}; body=$(jq --argjson i "$index" --arg v "$value" --arg a "${ip4}/32" '.clients[$i].ipv4=$v | .endpoint.peers[$i].allowed_ips[0]=$a' "$WG_CONF") || { tell_warn "配置生成失败"; sleep 1; continue; }
        ;;
      3)
        value=$(prompt "隧道 IPv6" "$(jq -r ".clients[$index].ipv6 // \"\"" "$WG_CONF")")
        { wg_valid_address "$value" && printf '%s' "$value" | grep -q ':'; } || { tell_warn "IPv6 地址无效"; sleep 1; continue; }
        if ! wg_client_address_conflict "$index" "$(jq -r ".clients[$index].ipv4 // \"\"" "$WG_CONF")" "$value"; then tell_warn "IPv6 地址已被其他客户端使用"; sleep 1; continue; fi
        ip6=${value%/*}; body=$(jq --argjson i "$index" --arg v "$value" --arg a "${ip6}/128" '.clients[$i].ipv6=$v | .endpoint.peers[$i].allowed_ips[1]=$a' "$WG_CONF") || { tell_warn "配置生成失败"; sleep 1; continue; }
        ;;
      4)
        pub=$(prompt "客户端公钥" "$(jq -r ".clients[$index].public_key // \"\"" "$WG_CONF")")
        [ -n "$pub" ] || { tell_warn "客户端公钥不能为空"; sleep 1; continue; }
        if jq -e --arg k "$pub" --argjson i "$index" '.endpoint.peers | to_entries[] | select(.key != $i and .value.public_key == $k)' "$WG_CONF" >/dev/null 2>&1; then tell_warn "客户端公钥已存在"; sleep 1; continue; fi
        body=$(jq --argjson i "$index" --arg v "$pub" '.clients[$i].public_key=$v | .endpoint.peers[$i].public_key=$v' "$WG_CONF") || { tell_warn "配置生成失败"; sleep 1; continue; }
        ;;
      0) return ;;
      *) tell_warn "输入无效"; sleep 1; continue ;;
    esac
    if wg_apply_body "$body"; then tell_ok "已修改"; else tell_warn "修改失败，旧配置保持不变"; fi
    wait_key
  done
}
