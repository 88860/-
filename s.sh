    ...
    case $(prompt "请选择") in
      ...
      6)
         ...
         if systemctl is-active --quiet sing-box; then
           if sync_hopping_rules; then tell_ok "已重启"; else tell_warn "服务已重启，但跳跃规则同步失败"; fi
         else tell_warn "singbox 未运行"; tell_warn "重启失败"; fi
         wait_key ;;
      7)
         previous=$(state_exit_get)
         state_exit_set direct || { tell_warn "出口状态更新失败，未停止服务"; wait_key; continue; }
         if ! stop_watchdog; then state_exit_set "$previous" || true; sync_watchdog; tell_warn "看门狗停止失败，未停止服务"; wait_key; continue; fi
         if timeout 15 systemctl stop sing-box 2>/dev/null; then
           local stop_tmp
           stop_tmp=$(mktemp) || { state_exit_set "$previous" || true; sync_watchdog; tell_warn "临时配置文件创建失败，已回滚"; wait_key; continue; }
         ...
         else state_exit_set "$previous" || true; sync_watchdog || true; tell_warn "操作异常，已回滚"; fi
         wait_key ;;
      0) break ;;
      *) tell_warn "输入无效，请重新选择"; sleep 1 ;;
    esac
  ...
  if [ ! -s "$script_tmp" ]; then tell_warn "下载的脚本为空，放弃更新"
    ...
    if cmp -s "$script_tmp" "$SELF"; then tell_ok "脚本已是最新版本"
      ...
      if install -m700 "$script_tmp" "$SELF"; then
        rm -f "$script_tmp"; tell_ok "脚本更新成功，请重新运行本脚本生效"; exit 0
  ...
  while :; do
    ...
    case $(prompt "请选择") in
      2)
        ...
        if wg_server_client_apply "$body"; then tell_ok "已修改"; fi
        sleep 1
        ;;
      1) wg_server_clients ;;
      3) wg_edit_address ;;
...
protocol_edit_name(){
  local file=$1 value
  value=$(prompt "新识别名称" "$(jq -r .name "$file")")
  [ -n "$value" ] || return 1
...
protocol_edit_port(){
  local file=$1 value
  value=$(prompt_port "$(jq -r .proto "$file")" "$(jq -r .port "$file")") || return 1
...
menu_modify_protocol(){
  local choice kind old_json file
  clear; tell "${CYAN}========== 修改配置 ==========${PLAIN}"
  select_node file || return
  while :; do
    kind=$(jq -r .kind "$file")
    clear; tell "${CYAN}========== $(jq -r .name "$file") [$kind] ==========${PLAIN}"
...
select_node(){
  local index target=$1 count=0 raw_data old_ifs file kind port name proto
  local -a files=()
  [ -n "$target" ] || return 2
  set -- "$NODE_DIR"/*.json
  [ ! -e "$1" ] && { tell "  系统内暂无节点"; wait_key; return 1; }
  ...
  if [ -n "$raw_data" ]; then
    ...
    while read -r file kind port name proto; do
      ...
      printf "  %b%2d. [%-13s] %s:%s %b%s%b\n" "$GREEN" "$count" "$kind" "$name" "$port" "$color" "$status_text" "$PLAIN"
    done <<<"$raw_data"
    IFS="$old_ifs"
  fi
...
peer_stop(){
  clear
  if peer_stop_transaction; then tell_ok "已恢复直连"; else tell_warn "恢复直连失败，已回滚"; fi
  wait_key
}
