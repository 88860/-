...
protect_sshd() {
  local pid
  for pid in $(pidof sshd dropbear 2>/dev/null); do
...
random_uuid(){ cat /proc/sys/kernel/random/uuid 2>/dev/null || (tr -dc 'a-f0-9' </dev/urandom | head -c 32 | sed -E 's/(.{8})(.{4})(.{4})(.{4})(.{12})/----/'); }
...
check_dependencies(){
  ...
  command -v tar >/dev/null 2>&1 || to_install="$to_install tar"
...
remote_version(){
  curl -fsSL -o /dev/null -w '%{url_effective}' "https://github.com/SagerNet/sing-box/releases/latest" 2>/dev/null | awk -F'/v' '{print $NF}'
}
...
command_env="GOMEMLIMIT=20MiB GOGC=30"
...
stop_post() {
    $SHORTCUT --clear-hopping >/dev/null 2>&1 || true
}
EOF
...
ssh_ports(){
  {
    ...
    [ -f /etc/ssh/sshd_config ] && sed -n 's/^[[:space:]]*[Pp]ort[[:space:]]\+\([0-9]\+\).*//p' /etc/ssh/sshd_config 2>/dev/null
    for f in /etc/ssh/sshd_config.d/*.conf; do
      [ -e "$f" ] && sed -n 's/^[[:space:]]*[Pp]ort[[:space:]]\+\([0-9]\+\).*//p' "$f" 2>/dev/null
    done
    ss -Hlntp 2>/dev/null | awk '/sshd/{print $4}' | awk -F':' '{print $NF}'
...
listening_ports(){ ss -Hlnp"$1" 2>/dev/null | awk '{print $4}' | awk -F':' '{print $NF}' | grep -E '^[0-9]+$' | sort -u; }
hopping_node(){
  local file
...
resolve_addresses(){
  if command -v getent >/dev/null 2>&1; then
    ...
    nslookup "$1" 2>/dev/null | awk '/^Name:/ {f=1} f && /^Address:/ {print $2}' | sort -u
  fi
}
...
acme_options(){
  ...
  case $(state_get challenge) in
    alpn)
      body=$(echo "$body" | jq '.disable_http_challenge=true') ;;
    dns_cloudflare)
      body=$(echo "$body" | jq --arg t "$(state_get cf_token)"         '.dns01_challenge={provider:"cloudflare",api_token:$t}') ;;
    dns_alidns)
      body=$(echo "$body" | jq --arg k "$(state_get ali_key)" --arg s "$(state_get ali_secret)"         '.dns01_challenge={provider:"alidns",access_key_id:$k,access_key_secret:$s}') ;;
    dns_acmedns)
      body=$(echo "$body" | jq --arg u "$(state_get acmedns_user)" --arg p "$(state_get acmedns_pass)"                 --arg s "$(state_get acmedns_sub)" --arg r "$(state_get acmedns_url)"         '.dns01_challenge={provider:"acmedns",username:$u,password:$p,subdomain:$s,server_url:$r}') ;;
    *)
...
build_config(){
  local selected domain inbounds outbounds endpoints rules dns_block final
  local peer_host tls_extra providers strategy ipv6 node_exists probe_target
  
  selected=$(state_get exit); domain=$(state_get domain)
  probe_target=${WATCHDOG_PROBE:-}
  final=direct; endpoints='[]'; peer_host=""; providers='[]'; tls_extra='{}'
  
  ipv6=$(local_ipv6)
  if [ "$selected" = "direct" ]; then
    strategy="ipv4_only"; [ -n "$ipv6" ] && strategy="prefer_ipv4"
  else
  ...
  if [ "$selected" != "direct" ]; then
    inbounds=$(jq -n --argjson list "$inbounds" '
      [{type:"mixed",tag:"mixed-in",listen:"127.0.0.1",listen_port:2080}] + $list')
  fi
  ...
  jq -n --argjson inbounds "$inbounds" --argjson outbounds "$outbounds"         --argjson endpoints "$endpoints" --argjson rules "$rules"         --argjson dns "$dns_block" --argjson providers "$providers"         --arg final "$final" '
...
run_watchdog(){
  ...
  while :; do
    ...
    if curl -s -x socks5h://127.0.0.1:${probe_port} -o /dev/null -m 6 "$PROBE_URL" ||        curl -s -x socks5h://127.0.0.1:${probe_port} -o /dev/null -m 6 "$probe2"; then
      fail_count=0
      if [ "$(state_get exit)" = "direct" ] && [ "$target_exit" != "direct" ]; then
        state_set exit "$target_exit"
        build_config | json_write "$CONFIG" && rc-service sing-box restart
        sync_hopping_rules; sync_proxy_env
      fi
    else
...
prompt_port(){
  ...
  while :; do
    ...
    validate_port "$port" "$proto" "$current" >&2 && { printf '%s' "$port"; return 0; }
  done
}
...
validate_domain(){
  ...
  if ! { [ -n "$ipv4" ] && echo "$resolved" | grep -qx "$ipv4"; }      && ! { [ -n "$ipv6" ] && echo "$resolved" | grep -qx "$ipv6"; }; then
...
create_vless_reality(){
  ...
  port=$(prompt_port t "")
  uuid=$(prompt "通讯 UUID" "$(random_uuid)")
  while :; do
...
create_hysteria2(){
  ...
  if check_nft_permission; then
    ...
    tell "  ${YELLOW}当前 LXC 容器无 nftables 权限，已跳过端口跳跃设置${PLAIN}"
  fi
  
  tag=$(unique_tag "$name" in- "$NODE_DIR")
  body=$(jq -n --arg tag "$tag" --arg name "$name" --argjson port "$port"         --arg password "$password" --arg hopping "$hopping"         --argjson up "$up_mbps" --argjson down "$down_mbps" '
...
create_socks(){
  ...
  password=$(prompt "鉴权密码" "$(random_password)")
  ...
  while :; do
    ...
    if echo "$index" | grep -Eq '^[0-9]+$' && [ "$index" -ge 1 ] && [ "$index" -le "$NODE_COUNT" ]; then
      for file in "$NODE_DIR"/*.json; do
        [ -e "$file" ] || continue
        i=$((i+1))
        if [ "$i" -eq "$index" ]; then PICKED="$file"; return 0; fi
      done
    else
...
menu_modify_protocol(){
  local kind value other current_hop up_mbps down_mbps old_json
  ...
  while :; do
    ...
    case $(prompt "请选择") in
      ...
      7)
        [ "$kind" = hysteria2 ] || { tell_warn "输入无效"; sleep 1; continue; }
        while :; do
          up_mbps=$(prompt "上行带宽" "$(jq -r '.meta.up_mbps//0' "$PICKED")")
          if echo "$up_mbps" | grep -Eq '^[0-9]+$'; then break; else tell_warn "输入无效，请重新输入"; fi
        done
        while :; do
          down_mbps=$(prompt "下行带宽" "$(jq -r '.meta.down_mbps//0' "$PICKED")")
        ...
        json_edit "$PICKED" '.meta.up_mbps=$up | .meta.down_mbps=$down |
          ...
          if $down > 0 then .inbound.down_mbps=$down else del(.inbound.down_mbps) end'           --argjson up "$up_mbps" --argjson down "$down_mbps" || { tell_warn 失败; wait_key; continue; } ;;
      0) return ;;
...
menu_change_domain(){
  ...
  [ "$count" = 0 ] && tell "  无"
  echo ""
  new_domain=$(prompt "新域名"); [ -z "$new_domain" ] && return
  ...
  if [ "$new_domain" != "$old_domain" ]; then 
    mv "$ACME_DIR" "${ACME_DIR}.bak" 2>/dev/null
    mkdir -p "$ACME_DIR"
  fi
  
  if apply_config; then 
    rm -rf "${ACME_DIR}.bak" 2>/dev/null
    menu_server_info
  else
    tell_warn "部署异常已回滚"
    state_set domain "$old_domain"; state_set email "$old_email"; state_set challenge "$old_challenge"
    if [ -d "${ACME_DIR}.bak" ]; then
      rm -rf "$ACME_DIR" && mv "${ACME_DIR}.bak" "$ACME_DIR"
    fi
  ...
  case $URI_SCHEME in
    vless)
      ...
      if [ "$security" = reality ]; then
        outbound=$(echo "$outbound" | jq --arg sni "$sni" --arg fp "$fingerprint" --arg pbk "$(query_value pbk)" --arg sid "$(query_value sid)"           '.tls={enabled:true,server_name:$sni,utls:{enabled:true,fingerprint:$fp},reality:{enabled:true,public_key:$pbk,short_id:$sid}}')
      elif [ "$security" = tls ] || [ "$security" = xtls ]; then
        outbound=$(echo "$outbound" | jq --arg sni "$sni" --arg fp "$fingerprint"           '.tls={enabled:true,server_name:$sni,utls:{enabled:true,fingerprint:$fp}}')
      fi
      case $network in
        ws) outbound=$(echo "$outbound" | jq --arg path "${path:-/}" --arg host "$vhost"               '.transport=({type:"ws",path:$path}+(if $host=="" then {} else {headers:{Host:$host}} end))') ;;
        grpc) outbound=$(echo "$outbound" | jq --arg svc "$service" '.transport={type:"grpc",service_name:$svc}') ;;
        httpupgrade) outbound=$(echo "$outbound" | jq --arg path "${path:-/}" --arg host "$vhost"               '.transport={type:"httpupgrade",path:$path,host:$host}') ;;
      esac ;;
    hysteria2|hy2)
      outbound=$(jq -n --arg tag "$tag" --arg server "$URI_HOST" --argjson port "$URI_PORT"                        --arg password "$URI_USERINFO" --arg sni "$sni"         '{type:"hysteria2",tag:$tag,server:$server,server_port:$port,password:$password,
    ...
    tuic)
      ...
      outbound=$(jq -n --arg tag "$tag" --arg server "$URI_HOST" --argjson port "$URI_PORT"                        --arg uuid "$username" --arg password "$password" --arg cc "$congestion"                        --arg sni "$sni" --arg alpn "$alpn"         '{type:"tuic",tag:$tag,server:$server,server_port:$port,uuid:$uuid,password:$password,
          congestion_control:$cc,udp_relay_mode:"native",
          tls:{enabled:true,server_name:$sni,alpn:($alpn|split(","))}}') ;;
    trojan)
      outbound=$(jq -n --arg tag "$tag" --arg server "$URI_HOST" --argjson port "$URI_PORT"                        --arg password "$URI_USERINFO" --arg sni "$sni"         '{type:"trojan",tag:$tag,server:$server,server_port:$port,password:$password,
          tls:{enabled:true,server_name:$sni}}')
      if [ "$(query_value type)" = ws ]; then
        outbound=$(echo "$outbound" | jq --arg path "$(query_value path)" --arg host "$(query_value host)"           '.transport=({type:"ws",path:(if $path=="" then "/" else $path end)}
                       +(if $host=="" then {} else {headers:{Host:$host}} end))')
      fi ;;
    anytls)
      outbound=$(jq -n --arg tag "$tag" --arg server "$URI_HOST" --argjson port "$URI_PORT"                        --arg password "$URI_USERINFO" --arg sni "$sni"         '{type:"anytls",tag:$tag,server:$server,server_port:$port,password:$password,
          tls:{enabled:true,server_name:$sni}}') ;;
    socks5|socks)
      username=${URI_USERINFO%%:*}
      password=${URI_USERINFO#*:}
      [ "$password" = "$URI_USERINFO" ] && password=""
      outbound=$(jq -n --arg tag "$tag" --arg server "$URI_HOST" --argjson port "$URI_PORT"                        --arg user "$username" --arg pass "$password"         '{type:"socks",tag:$tag,server:$server,server_port:$port,version:"5"}
...
peer_add(){
  local name uri tag outbound probe ipv4 ipv6 domain port tmp
  ...
  for port in $(node_ports); do
    if [ "$URI_PORT" = "$port" ] && { { [ -n "$ipv4" ] && [ "$URI_HOST" = "$ipv4" ]; }        || { [ -n "$ipv6" ] && [ "$URI_HOST" = "$ipv6" ]; }        || { [ -n "$domain" ] && [ "$URI_HOST" = "$domain" ]; }; }; then
  ...
  jq -n --argjson ob "$outbound"     '{log:{level:"error"},
...
peer_delete(){
  ...
  local old_json; old_json=$(cat "$PICKED")
  rm -f "$PICKED"
...
get_ip_info(){
  local mode=$1 res ip country asn name proxy_arg=""
  
  if [ "$(state_get exit)" != "direct" ]; then
    proxy_arg="-x socks5h://127.0.0.1:2080"
  fi
  
  local raw_ip
  if [ "$mode" = 6 ]; then
    raw_ip=$(curl -s $proxy_arg -m 5 http://ipv6.icanhazip.com 2>/dev/null | tr -d '
 ')
  else
    raw_ip=$(curl -s $proxy_arg -m 5 http://ipv4.icanhazip.com 2>/dev/null | tr -d '
 ')
  fi
  
  if [ -n "$raw_ip" ]; then
    res=$(curl -s $proxy_arg -m 5 "https://ipwho.is/${raw_ip}" 2>/dev/null)
...
wg_tunnel_state(){
  if curl -x socks5h://127.0.0.1:2080 --connect-timeout 3 -m 5 -sI "http://cp.cloudflare.com/generate_204" >/dev/null 2>&1; then echo up; else echo down; fi
}
...
menu_wireguard(){
  local role_label tunnel_label old_json
  while :; do
    clear
...
run_update(){
  ...
  if curl -fsSL -m 15 "$script_url" -o "$script_tmp"; then
    if sh -n "$script_tmp" 2>/dev/null; then
...
menu_status(){
  while :; do
    ...
    local os=$(sed -n 's/^PRETTY_NAME="\(.*\)"//p' /etc/os-release 2>/dev/null || echo "Alpine Linux")
    local core=$(uname -r)
    local arch=$(uname -m)
...
case $1 in
  --sync) init_dirs; sync_hopping_rules; sync_proxy_env; exit 0 ;;
