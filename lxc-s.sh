...
build_config(){
  ...
  # 本地代理入口 (仅通过端口代理保证SSH不断，LXC环境禁用TUN)
  if [ "$selected" != "direct" ]; then
...
peer_select(){
  ...
  if apply_config; then
    ...
    printf '  %b[提示] 当前SSH窗口需执行 `source /etc/profile.d/sbm_proxy.sh` 才能立即全局生效 (新开窗口自动生效)%b\n' "${YELLOW}" "${PLAIN}" >&2
  else
...
peer_stop(){
  ...
  if apply_config; then
    tell_ok "已恢复直连"
    printf '  %b[提示] 当前SSH窗口需执行 `unset http_proxy https_proxy all_proxy` 以清除旧代理变量%b\n' "${YELLOW}" "${PLAIN}" >&2
  fi
  wait_key
}
...
wg_toggle(){
  ...
  if [ "$(jq -r .enabled "$WG_CONF")" = true ]; then
    ...
    if apply_config; then
      tell_ok "已关闭隧道"
      [ "$previous" = wireguard ] && printf '  %b[提示] 当前SSH窗口需执行 `unset http_proxy https_proxy all_proxy` 以清除旧代理变量%b\n' "${YELLOW}" "${PLAIN}" >&2
    fi
  else
    ...
    if apply_config; then
      ...
      [ "$role" = client ] && printf '  %b[提示] 当前SSH窗口需执行 `source /etc/profile.d/sbm_proxy.sh` 才能立即全局生效 (新开窗口自动生效)%b\n' "${YELLOW}" "${PLAIN}" >&2
    else
