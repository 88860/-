...
get_ip_info_iface(){
  local mode=$1 iface=$2 res ip country asn name url
  case "$mode" in
    4|6) url=https://ipwho.is/ ;;
    *) IP_INFO_IP=""; IP_INFO_C=""; IP_INFO_ASN=""; IP_INFO_NAME=""; return ;;
  esac
  [ -n "$iface" ] || { IP_INFO_IP=""; IP_INFO_C=""; IP_INFO_ASN=""; IP_INFO_NAME=""; return; }
  if command -v curl >/dev/null 2>&1; then
    res=$(env -u http_proxy -u https_proxy -u HTTP_PROXY -u HTTPS_PROXY -u all_proxy -u ALL_PROXY curl -s -"$mode" -m 5 --interface "$iface" "$url" 2>/dev/null)
  elif command -v wget >/dev/null 2>&1; then
    res=$(env -u http_proxy -u https_proxy -u HTTP_PROXY -u HTTPS_PROXY -u all_proxy -u ALL_PROXY wget -qO- -T 5 "$url" 2>/dev/null)
  else
    res=""
  fi
  ip=$(printf '%s' "$res" | jq -r '.ip // empty' 2>/dev/null)
...
render_status_ip(){
  local exit_node protocol mode local_if4 local_if6 local4 local6 local4_info local6_info
  ...
  tell "  出口：${CYAN}${mode}${PLAIN}"
  local_if4=$(default_iface 4)
  local_if6=$(default_iface 6)
  get_ip_info_iface 4 "$local_if4"
  local4="$IP_INFO_IP"; local4_info="$IP_INFO_C|$IP_INFO_NAME|$IP_INFO_ASN"
  get_ip_info_iface 6 "$local_if6"
  local6="$IP_INFO_IP"; local6_info="$IP_INFO_C|$IP_INFO_NAME|$IP_INFO_ASN"
  if [ -n "$local4" ]; then
    IFS='|' read -r IP_INFO_C IP_INFO_NAME IP_INFO_ASN <<EOF
$local4_info
EOF
    tell "  本机 IPv4: ${GREEN}${local4}${PLAIN} | ${YELLOW}${IP_INFO_C}${PLAIN}"
    tell "  ${CYAN}${IP_INFO_NAME}${PLAIN} | ASN: ${PURPLE}${IP_INFO_ASN}${PLAIN}"
  else
    tell "  本机 IPv4: ${RED}无或不可用${PLAIN}"
  fi
  if [ -n "$local6" ]; then
    IFS='|' read -r IP_INFO_C IP_INFO_NAME IP_INFO_ASN <<EOF
$local6_info
EOF
    tell "  本机 IPv6: ${GREEN}${local6}${PLAIN} | ${YELLOW}${IP_INFO_C}${PLAIN}"
    tell "  ${CYAN}${IP_INFO_NAME}${PLAIN} | ASN: ${PURPLE}${IP_INFO_ASN}${PLAIN}"
  else
    tell "  本机 IPv6: ${RED}无或不可用${PLAIN}"
  fi
  if [ "$mode" != "直连" ]; then
    get_ip_info 4
    if [ -n "$IP_INFO_IP" ]; then
      tell "  出口 IPv4: ${GREEN}${IP_INFO_IP}${PLAIN} | ${YELLOW}${IP_INFO_C}${PLAIN}"
      tell "  ${CYAN}${IP_INFO_NAME}${PLAIN} | ASN: ${PURPLE}${IP_INFO_ASN}${PLAIN}"
    else
      tell "  出口 IPv4: ${RED}无或不可用${PLAIN}"
    fi
    get_ip_info 6
    if [ -n "$IP_INFO_IP" ]; then
      tell "  出口 IPv6: ${GREEN}${IP_INFO_IP}${PLAIN} | ${YELLOW}${IP_INFO_C}${PLAIN}"
      tell "  ${CYAN}${IP_INFO_NAME}${PLAIN} | ASN: ${PURPLE}${IP_INFO_ASN}${PLAIN}"
    else
      tell "  出口 IPv6: ${RED}无或不可用${PLAIN}"
    fi
  fi
}
...
menu_client_status(){
  screen_clear
  tell "${BRIGHT_CYAN}${BOLD}>> 客户端状态 <<${PLAIN}"
