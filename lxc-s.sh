...
render_status_ip(){
  local exit_node protocol mode local_if4 local_if6 local4 local6
  ...
  local4=""; local6=""
  [ -n "$local_if4" ] && local4=$(ip -4 -o addr show dev "$local_if4" scope global 2>/dev/null | awk '{sub(/\/.*$/, "", $4); print $4; exit}')
  [ -n "$local_if6" ] && local6=$(ip -6 -o addr show dev "$local_if6" scope global 2>/dev/null | awk '$4 !~ /^fe80:/ {sub(/\/.*$/, "", $4); print $4; exit}')
  if [ -n "$local4" ]; then
    tell "  本机 IPv4: ${GREEN}${local4}${PLAIN}"
  else
    tell "  本机 IPv4: ${RED}无或不可用${PLAIN}"
  fi
  if [ -n "$local6" ]; then
    tell "  本机 IPv6: ${GREEN}${local6}${PLAIN}"
  else
