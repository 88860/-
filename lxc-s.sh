...
create_hysteria2(){
  local name port password tag body up_mbps down_mbps obfs_type obfs_password
  clear; tell "${CYAN}========== 创建协议 ==========${PLAIN}"
  ...
  body=$(jq -n --arg tag "$tag" --arg name "$name" --argjson port "$port" \
   ...
   {tag:$tag,name:$name,kind:"hysteria2",port:$port,proto:"u",hopping:"",
...
render_share_uri(){
  local file=$1 kind name port meta host uri ipv6 mode
  kind=$(jq -r .kind "$file"); name=$(jq -r .name "$file"); port=$(jq -r .port "$file")
  meta=$(jq -c .meta "$file"); mode=$(jq -r .tls_mode "$file")
  if [ "$mode" = acme ]; then
...
menu_modify_protocol(){
  local kind value other up_mbps down_mbps old_json obfs_type obfs_password
