...
install_core(){
  printf '  %b正在拉取 Github 最新版 sing-box (专为 64M 极低内存优化)...%b\n' "${CYAN}" "${PLAIN}"
  local version arch url target_dir found
  
  version=$(curl -sI "https://ghp.ci/https://github.com/SagerNet/sing-box/releases/latest" | grep -i "^location:" | sed -E 's/.*\/tag\/v([^[:space:]\r\n]+).*/\1/')
  ...
  url="https://ghp.ci/https://github.com/SagerNet/sing-box/releases/download/v${version}/sing-box-${version}-${arch}.tar.gz"
  
  target_dir="/var/lib/sbm_tmp"
  rm -rf "$target_dir" && mkdir -p "$target_dir"
  
  if curl -fsSL -m 240 "$url" | tar -xz -C "$target_dir"; then
    ...
    if [ -n "$found" ] && [ -x "$found" ]; then
      ...
      tell_ok "sing-box 内核已直装: v${version} [低内存流式无损装载]"
      return 0
    fi
  fi
...
while :; do
  ...
  tell "${CYAN}================================${PLAIN}"
  tell "${CYAN}            s管理               ${PLAIN}"
  tell "${CYAN}   [ 专为 64M 微内存架构打造 ]  ${PLAIN}"
  tell "${CYAN}================================${PLAIN}"
  tell "  1. 服务端管理"
  tell "  2. 客户端管理"
  tell "  3. WireGuard 管理"
