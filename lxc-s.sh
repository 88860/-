  ...
  while :; do
    ...
    case $(prompt "请选择") in
      ...
      6) 
         ...
         wait_key
         ;;
      7) if rc-service sing-box stop; then tell_ok "已停止"; else tell_warn "操作异常"; fi; wait_key ;;
      0) break ;;
      *) tell_warn "输入无效，请重新选择"; sleep 1 ;;
    esac
  done
  ...
  if [ -n "$raw_data" ]; then
    ...
    while read -r file tag type name port host; do
      ...
      (
        ...
        if [[ "$host" =~ : ]]; then
          if [ -n "$iface" ]; then
            ping_res=$(ping6 -I "$iface" -c 1 -W 2 -m 255 "$host" 2>/dev/null | awk -F'/' '/^rtt|^round-trip/{print $5}')
          else
            ping_res=$(ping6 -c 1 -W 2 -m 255 "$host" 2>/dev/null | awk -F'/' '/^rtt|^round-trip/{print $5}')
          fi
        else
          if [ -n "$iface" ]; then
            ping_res=$(ping -I "$iface" -c 1 -W 2 -m 255 "$host" 2>/dev/null | awk -F'/' '/^rtt|^round-trip/{print $5}')
          else
