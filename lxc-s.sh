...
protect_sshd() {
  ...
  for pid in $(pidof sshd dropbear 2>/dev/null); do
    sh -c "echo -1000 > /proc/$pid/oom_score_adj" 2>/dev/null || true
  done
}
...
install_core(){
  ...
  if curl -fsSL --limit-rate 2M -m 120 "$url" -o "$target_dir/core.tar.gz" 2>/dev/null; then
    free_mem
    if tar -xzf "$target_dir/core.tar.gz" -C "$target_dir" 2>/dev/null; then
      rm -f "$target_dir/core.tar.gz"
  ...
  case $(state_get challenge) in
    ...
    dns_cloudflare)
      body=$(echo "$body" | jq --arg t "$(state_get cf_token)" \
        '.dns01_challenge={provider:"cloudflare",api_token:$t}') ;;
    dns_alidns)
      body=$(echo "$body" | jq --arg k "$(state_get ali_key)" --arg s "$(state_get ali_secret)" \
        '.dns01_challenge={provider:"alidns",access_key_id:$k,access_key_secret:$s}') ;;
    dns_acmedns)
      body=$(echo "$body" | jq --arg u "$(state_get acmedns_user)" --arg p "$(state_get acmedns_pass)" \
        ...
        '.dns01_challenge={provider:"acmedns",username:$u,password:$p,subdomain:$s,server_url:$r}') ;;
    *)
...
run_watchdog(){
  ...
  while :; do
    ...
    if curl -s -x socks5h://127.0.0.1:${probe_port} -o /dev/null -m 6 "$PROBE_URL" || \
       curl -s -x socks5h://127.0.0.1:${probe_port} -o /dev/null -m 6 "$probe2"; then
      fail_count=0
      if [ "$(state_get exit)" = "direct" ] && [ "$target_exit" != "direct" ]; then
