...
apply_config(){
  ...
  while ! rc-service sing-box status >/dev/null 2>&1; do
...
probe_handshake_target(){
  ...
  result=$(curl -ksI --max-time 10 "https://$target" 2>/dev/null)
...
create_hysteria2(){
  ...
  obfs_type=""
  obfs_password=""
  if prompt_yes "是否配置协议混淆 (Obfuscation，建议开启)"; then
    tell "  1. Salamander (经典混淆)"
    tell "  2. Gecko (1.14 新增，更强抗特征识别)"
    while :; do
...
render_share_uri(){
  ...
  case $kind in
    ...
    hysteria2) 
      ...
      obfs_type=$(echo "$meta" | jq -r '.obfs_type//""')
      obfs_pw=$(echo "$meta" | jq -r '.obfs_password//""')
      obfs_str=""
      [ -n "$obfs_type" ] && obfs_str="&obfs=${obfs_type}&obfs-password=$(uri_encode "$obfs_pw")"
      uri="hysteria2://$(uri_encode "$(echo "$meta" | jq -r .password)")@$host:$port/?sni=$host&alpn=h3${obfs_str}#$(uri_encode "$name")" ;;
    tuic) uri="tuic://$(echo "$meta" | jq -r .uuid):$(uri_encode "$(echo "$meta" | jq -r .password)")@$host:$port?congestion_control=cubic&alpn=h3&udp_relay_mode=native&sni=$host#$(uri_encode "$name")" ;;
...
list_nodes(){
  ...
  raw_data=$(jq -r 'input_filename + "|" + .kind + "|" + (.port|tostring) + "|" + .name + "|" + .proto' "$@" 2>/dev/null)
  if [ -n "$raw_data" ]; then
    old_ifs="$IFS"
    IFS="|"
...
menu_modify_protocol(){
  local kind value other up_mbps down_mbps old_json
...
list_peers(){
  local index=0 current mark raw_data old_ifs
  current=$(state_get exit)
  PEER_COUNT=0
  set -- "$PEER_DIR"/*.json
  [ ! -e "$1" ] && { tell "  暂无外部节点"; return 0; }
  raw_data=$(jq -r 'input_filename + "|" + (.tag//"-") + "|" + (.outbound.type//"-") + "|" + (.name//"-") + "|" + ((.outbound.server_port // .outbound.listen_port // "-") | tostring)' "$@" 2>/dev/null)
  if [ -n "$raw_data" ]; then
    old_ifs="$IFS"
    IFS="|"
    while read -r file tag type name port; do
      ...
      [ "$tag" = "$current" ] && mark="${GREEN}<= 当前生效${PLAIN}" || mark=""
      printf '  %2d. [%-10s] %b | %s (端口: %s)\n' "$index" "$type" "$mark" "$name" "$port"
    done <<EOF
$raw_data
EOF
    IFS="$old_ifs"
