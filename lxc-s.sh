...
check_dependencies(){
  ...
  ensure_command ip iproute2 || { tell_warn "系统缺少 iproute2 组件"; exit 1; }
  ensure_command ss iproute2-ss
  ensure_command openssl openssl
  ensure_command nft nftables
  ensure_command ping iputils
  return 0
}
...
remote_version(){
  local version
  version=$(curl -sI "https://github.com/SagerNet/sing-box/releases/latest" | grep -i "^location:" | sed -E 's/.*\/tag\/v([^[:space:]\r\n]+).*/\1/' | tr -d '\r')
  printf '%s' "$version"
}
...
build_config(){
  ...
  if [ -f "$WG_CONF" ] && [ "$(jq -r '.enabled//false' "$WG_CONF")" = true ]; then
    if [ "$(jq -r .role "$WG_CONF")" = client ]; then
      ...
      outbounds=$(jq -n --argjson base "$outbounds" --slurpfile wg "$WG_CONF" '
        $base + [{"type":"wireguard","tag":"wg-direct","server":$wg[0].peer_host,"server_port":$wg[0].peer_port,"local_address":$wg[0].address,"private_key":$wg[0].private_key,"peer_public_key":$wg[0].peer_public_key,"mtu":1408}]')
      [ "$selected" = wireguard ] && final="wg-direct"
    else
...
run_update(){
  ...
  [ "$current" = "$latest" ] && { tell_ok "已是最新版本，无需更新"; wait_key; return; }
  prompt_yes "发现新版本 v$latest，是否立即更新" || return
