...
install_core(){
  printf '  %b正在从 Alpine 原生仓库安装 sing-box...%b\n' "${CYAN}" "${PLAIN}"
  sed -i 's/^#//g' /etc/apk/repositories
  apk update >/dev/null 2>&1
  apk add --no-cache sing-box >/dev/null 2>&1 || { tell_warn "原生 sing-box 安装失败，请检查网络或软件源"; return 1; }
  
  # 关键修复：确保 sing-box 包名被写入日志以备卸载时清除
  ...
  tell_ok "sing-box 内核已安装: $(core_version) [原生 musl]"
}
