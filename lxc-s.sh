...
check_dependencies(){
  ...
  command -v timeout >/dev/null 2>&1 || { tell_warn "系统缺少 timeout 命令"; exit 1; }
...
acme_options(){
  ...
  body=$(jq -n --arg d "$domain" --arg e "$(state_get email)" --arg dir "$ACME_DIR" \
    '{domain:[$d],email:$e,data_directory:$dir,key_type:"p256"}') || return 1
...
install_core(){
  ...
  if wget -qO- -T 120 "$url" 2>/dev/null | tar -xzf - -C "$tmpdir" 2>/dev/null; then
    if [ -f "$tmpdir/sing-box-${version}-${arch}/sing-box" ]; then
      local new_core="$tmpdir/sing-box-${version}-${arch}/sing-box"
      chmod 755 "$new_core"
      if ! "$new_core" version >/dev/null 2>&1; then
        tell_warn "下载的二进制无法执行，放弃安装"
        rm -rf "$tmpdir"
        return 1
      fi
      if ! "$new_core" version 2>/dev/null | grep -q "${version}"; then
        tell_warn "下载的二进制版本校验失败，放弃安装"
        rm -rf "$tmpdir"
        return 1
      fi
      if ! install -m755 "$new_core" "${CORE}.new"; then
        tell_warn "新内核写入失败"
        rm -f "${CORE}.new"
        return 1
      fi
      if [ -f "$CONFIG" ] && ! "${CORE}.new" check -c "$CONFIG" >/dev/null 2>&1; then
        rm -f "${CORE}.new"
        tell_warn "新内核无法校验当前配置，放弃安装"
        return 1
      fi
      mv -f "${CORE}.new" "$CORE" || { rm -f "${CORE}.new"; tell_warn "新内核切换失败"; return 1; }
...
acme_domain_assets_exist(){
  ...
  [ -n "$domain" ] || return 1
  while IFS= read -r file; do
    if openssl x509 -in "$file" -noout -checkhost "$domain" >/dev/null 2>&1; then return 0; fi
  done <<EOF
$(find "$ACME_DIR/certificates" -type f -name '*.crt' 2>/dev/null)
EOF
  return 1
...
save_node(){
  ...
  if [ "$was_active" = 0 ]; then
    if ! rc-service sing-box start >/dev/null 2>&1 || ! rc-service sing-box status >/dev/null 2>&1; then
      ...
      tell_warn "sing-box 启动失败，已移除刚创建的协议"
      wait_key
      return 1
    fi
  fi
  domain=$(state_get domain)
  if [ "$(jq -r '.tls_mode//""' "$file")" = acme ] && [ -n "$domain" ]; then wait_for_certificate "$domain" || { wait_key; return 1; }; fi
  tell_ok "协议已生效"; tell_gap; render_share_uri "$file"; wait_key
}
...
EOF
  ...
  dns=$(jq -n --arg strategy "$dns_strategy" --arg bootstrap "$bootstrap_tag" --arg server "$bootstrap_server" --arg final "$final" --argjson v6ok "$dns_v6" '
  ...
  rules=$(jq -n --arg final "$final" '[{action:"sniff"},{inbound:["mixed-in"],action:"route",outbound:$final}]') || return 1
  if [ "$final" = direct ]; then
    rules=$(jq -n '[{action:"sniff"}]') || return 1
  elif [ "$final" = wireguard ]; then
    rules=$(jq -n '[{action:"sniff"},{inbound:["mixed-in"],action:"route",outbound:"wireguard"}]') || return 1
  fi
...
apply_config(){
  ...
  rm -f "$tmp"
  if rc-service sing-box status >/dev/null 2>&1; then rc-service sing-box restart >/dev/null 2>&1 || return 1; rc-service sing-box status >/dev/null 2>&1 || return 1; fi
  return 0
}
...
menu_delete_protocol(){
  ...
  if apply_config; then
    ...
    if [ "$was_acme" = acme ]; then
      ...
      if [ "$acme_count" -eq 0 ] && [ -n "$(state_get domain)" ] && prompt_yes "是否连同域名和证书一起清理"; then
        state_set domain ""; state_set email ""; state_set challenge http; rm -rf "$ACME_DIR"/*; tell_ok "相关配置已清理"
      fi
    fi
  ...
  while :; do
    ...
    case $(prompt "请选择") in
      1)
        ...
        json_edit "$PICKED" '.name=$v' --arg v "$value" || { tell_warn "修改失败"; wait_key; continue; } ;;
      2)
        value=$(prompt_port "$(jq -r .proto "$PICKED")" "$(jq -r .port "$PICKED")") || continue
        json_edit "$PICKED" '.port=$v|.inbound.listen_port=$v' --argjson v "$value" || { tell_warn "修改失败"; wait_key; continue; } ;;
      3)
        if [ "$kind" = vless-reality ] || [ "$kind" = vless-tls ] || [ "$kind" = tuic ]; then
          value=$(prompt "新通信 UUID (留空自动生成)" "$(jq -r '.meta.uuid//""' "$PICKED")"); [ -n "$value" ] || value=$(random_uuid)
          json_edit "$PICKED" '.meta.uuid=$v|.inbound.users[0].uuid=$v' --arg v "$value"
        elif [ "$kind" = snell ]; then
          while :; do
            value=$(prompt "新 PSK (留空自动生成, 12-255字节)" "$(jq -r '.meta.psk//""' "$PICKED")"); [ -n "$value" ] || value=$(random_password)
          ...
          value=$(prompt "新连接密码 (留空自动生成)" "$(jq -r '.meta.password//""' "$PICKED")"); [ -n "$value" ] || value=$(random_password)
      ...
      4)
        if [ "$kind" = socks ]; then
          ...
          json_edit "$PICKED" '.meta.mode=$v|.inbound.mode=$v' --arg v "$value"
        elif [ "$kind" = anytls ]; then
          value=$(prompt "客户端元数据 (留空则为空)" "$(jq -r '.meta.client_metadata//""' "$PICKED")")
          if [ -n "$value" ]; then json_edit "$PICKED" '.meta.client_metadata=$v' --arg v "$value"; else json_edit "$PICKED" 'del(.meta.client_metadata)'; fi
          ...
          if [ -n "$bbr_profile" ]; then
            json_edit "$PICKED" '.meta.up_mbps=0|.meta.down_mbps=0|.meta.bbr_profile=$b|del(.inbound.up_mbps,.inbound.down_mbps)|.inbound.bbr_profile=$b' --arg b "$bbr_profile"
          else
            json_edit "$PICKED" '.meta.up_mbps=$up|.meta.down_mbps=$down|.meta.bbr_profile=""|if $up>0 then .inbound.up_mbps=$up else del(.inbound.up_mbps) end|if $down>0 then .inbound.down_mbps=$down else del(.inbound.down_mbps) end|del(.inbound.bbr_profile)' --argjson up "$up_mbps" --argjson down "$down_mbps"
          fi
        else
      ...
      5)
        if [ "$kind" = vless-reality ]; then
          ...
          [ -n "$value" ] || continue
          json_edit "$PICKED" '.meta.target=$v|.inbound.tls.server_name=$v|.inbound.tls.reality.handshake.server=$v' --arg v "$value" || { tell_warn "修改失败"; wait_key; continue; }
        elif [ "$kind" = hysteria2 ]; then
          if prompt_yes "是否配置并开启协议混淆 (选择 N 则关闭混淆)"; then
            ...
            [ -n "$obfs_password" ] || obfs_password=$(jq -r '.meta.password' "$PICKED")
            if [ "$obfs_type" = gecko ]; then
              ...
              json_edit "$PICKED" '.meta.obfs_type=$t|.meta.obfs_password=$p|.meta.min_packet_size=($mn|tonumber)|.meta.max_packet_size=($mx|tonumber)|.inbound.obfs={type:$t,password:$p,min_packet_size:($mn|tonumber),max_packet_size:($mx|tonumber)}' --arg t "$obfs_type" --arg p "$obfs_password" --arg mn "$min_pkt" --arg mx "$max_pkt"
            else
              json_edit "$PICKED" '.meta.obfs_type=$t|.meta.obfs_password=$p|del(.meta.min_packet_size,.meta.max_packet_size)|.inbound.obfs={type:$t,password:$p}' --arg t "$obfs_type" --arg p "$obfs_password"
            fi
          else
            json_edit "$PICKED" '.meta.obfs_type=""|.meta.obfs_password=""|del(.meta.min_packet_size,.meta.max_packet_size)|del(.inbound.obfs)'
          fi
        else
          tell_warn "输入无效"; continue
        fi || { tell_warn "修改失败"; wait_key; continue; } ;;
      6)
        if [ "$kind" = vless-reality ]; then
          ...
          json_edit "$PICKED" '.meta.short_id=$v|.inbound.tls.reality.short_id=[$v]' --arg v "$value" || { tell_warn "修改失败"; wait_key; continue; }
        else
          tell_warn "输入无效"; continue
        fi ;;
      0) return ;;
    ...
    if apply_config; then tell_ok "已生效"; tell_gap; render_share_uri "$PICKED"; else tell_warn "配置应用失败"; fi
    wait_key
  done
}
...
menu_change_domain(){
  local new_domain old_domain old_email old_mode mode email count=0 file
  ...
  [ "$count" = 0 ] && tell "  无"; tell_gap
  new_domain=$(prompt "新域名 (留空取消)"); [ -n "$new_domain" ] || return
  has_acme_support || { tell_warn "当前 sing-box 不包含 ACME 支持"; wait_key; return; }
  if prompt_yes "重设验证方式"; then
    email=$(prompt "ACME 通知邮箱" "${old_email:-admin@$new_domain}"); [ -n "$email" ] || return
    tell "  1. HTTP-01"; tell "  2. TLS-ALPN-01"; tell "  3. DNS-01 Cloudflare"; tell "  4. DNS-01 阿里云"; tell "  5. DNS-01 ACME-DNS"
    while :; do
      case $(prompt "请选择验证方式" 1) in
  ...
  else email="$old_email"; mode="$(state_get challenge)"; fi
  validate_domain "$new_domain" || { prompt_yes "解析验证存在异常，是否继续" || { wait_key; return; }; }
  state_set domain "$new_domain" || { tell_warn "域名写入失败"; wait_key; return; }
  state_set email "$email" || { state_set domain "$old_domain"; wait_key; return; }
  state_set challenge "$mode" || { state_set domain "$old_domain"; state_set email "$old_email"; wait_key; return; }
  if ! apply_config; then
    state_set domain "$old_domain"; state_set email "$old_email"; state_set challenge "$old_mode"
    apply_config >/dev/null 2>&1
    tell_warn "域名配置应用失败，已恢复原配置"
    wait_key
    return
  fi
  if [ "$count" -gt 0 ] && ! wait_for_certificate "$new_domain"; then
    state_set domain "$old_domain"
    state_set email "$old_email"
    state_set challenge "$old_mode"
    apply_config >/dev/null 2>&1
    tell_warn "新域名证书签发失败，已恢复原配置"
    wait_key
    return
  fi
...
peer_select(){
  local tag
  clear; tell "${CYAN}========== 节点选择 ==========${PLAIN}"; select_peer || return
  tag=$(jq -r .tag "$PICKED")
  if peer_target_is_local "$PICKED"; then tell_warn "节点目标指向本机服务端地址，拒绝形成本机环路"; wait_key; return; fi
  if wg_client_active; then prompt_yes "WireGuard 隧道运行中，是否断开" || return; json_edit "$WG_CONF" '.enabled=false' || { tell_warn "WireGuard 关闭失败"; wait_key; return; }; fi
  state_set exit "$tag" || { tell_warn "出口状态写入失败"; wait_key; return; }
  if apply_config; then tell_ok "已接管: $(jq -r .name "$PICKED")"; else tell_warn "配置应用失败"; fi
  wait_key
}

peer_delete(){
  while :; do
    clear; tell "${CYAN}========== 删除节点 ==========${PLAIN}"; select_peer "删除节点" || return
    local tag; tag=$(jq -r .tag "$PICKED")
    [ "$tag" = "$(state_get exit)" ] && state_set exit direct
    rm -f "$PICKED"
    if apply_config; then tell_ok "已删除节点"; else tell_warn "配置应用失败"; fi
    wait_key
  done
}
...
peer_stop(){
  clear
  state_set exit direct || { tell_warn "出口状态写入失败"; wait_key; return; }
  if apply_config; then tell_ok "已恢复直连"; else tell_warn "配置应用失败"; fi
  wait_key
}
...
render_wg_info(){
  local file=$1 role
  role=$(jq -r .role "$file")
  local enabled state_text delay
  enabled=$(jq -r .enabled "$file")
  state_text="已断开"; delay="超时"
  if [ "$enabled" = true ]; then
    if [ "$role" = client ]; then
      delay=$(wg_tunnel_state "$(jq -r .peer_ip "$file")")
      [ -n "$delay" ] && state_text="已连接" || state_text="已断开"
    else
      state_text="运行中"; delay="-"
    fi
  fi
  tell "${CYAN}========== 隧道状态信息 ==========${PLAIN}"
  if [ "$role" = client ]; then
    tell "  模式：客户端 状态：$state_text 延迟：${delay}$( [ "$delay" != "超时" ] && [ "$delay" != "-" ] && printf ' ms' )"
  else
    tell "  模式：服务端 状态：$state_text 延迟：-"
  fi
  echo ""
...
wg_toggle(){
  local role previous
  clear; [ -f "$WG_CONF" ] || { tell_warn "请先初始化"; wait_key; return; }
  role=$(jq -r .role "$WG_CONF"); previous=$(state_get exit)
  if [ "$(jq -r .enabled "$WG_CONF")" = true ]; then
    json_edit "$WG_CONF" '.enabled=false' || { tell_warn "WireGuard 状态写入失败"; wait_key; return; }
    ...
    [ "$role" = client ] && [ "$previous" != direct ] && { prompt_yes "隧道将接管网络，确定" || return; }
    json_edit "$WG_CONF" '.enabled=true' || { tell_warn "WireGuard 状态写入失败"; wait_key; return; }
    [ "$role" = client ] && state_set exit wireguard
    if apply_config; then
      if ! rc-service sing-box status >/dev/null 2>&1; then rc-service sing-box start >/dev/null 2>&1 || true; fi
      tell_ok "已开启隧道"; sleep 1; render_wg_info "$WG_CONF"
    else tell_warn "配置应用失败"; fi
  fi
  wait_key
}
    ...
    case $(prompt "请选择") in
      ...
      5) clear
         ...
         if prompt_yes "确认删除配置"; then
           ...
           rm -f "$WG_CONF"
           [ "$prev_exit" = wireguard ] && state_set exit direct
           if apply_config; then tell_ok "配置已删除"; else tell_warn "配置应用失败"; fi
           wait_key
         fi ;;
      0) break ;;
      *) tell_warn "输入无效，请重新选择"; sleep 1 ;;
...
update_core(){ install_core && apply_config; }
...
run_update(){
  ...
  if [ -z "$latest" ]; then tell_warn "无法获取最新版本，请检查网络或 GitHub 访问"; elif [ -z "$current" ]; then if prompt_yes "无法读取当前内核版本，是否重新下载并安装最新版 v$latest"; then if update_core; then tell_ok "内核更新完成"; else tell_warn "内核更新失败"; fi; fi; elif [ "$current" != "$latest" ]; then if prompt_yes "发现新版本 v$latest，是否立即更新"; then if update_core; then tell_ok "内核更新完成"; else tell_warn "内核更新失败"; fi; fi; else tell_ok "内核已是最新版本"; fi
  tell_gap; tell "正在检测 lxc-s 脚本更新..."; script_url="https://raw.githubusercontent.com/88860/-/main/lxc-s.sh"; script_tmp=$(mktemp); TMP_FILES="$TMP_FILES $script_tmp"
