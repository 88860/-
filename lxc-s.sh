...
get_ip_info(){
  local mode=$1 res ip country asn name url
  case "$mode" in
    4) url=https://ipwho.is/ ;;
    6) url=https://ipwho.is/ ;;
    *) IP_INFO_IP=""; IP_INFO_C=""; IP_INFO_ASN=""; IP_INFO_NAME=""; return ;;
  esac
  if command -v curl >/dev/null 2>&1; then
    res=$(curl -s -${mode} -m 5 "$url" 2>/dev/null)
  else
    res=$(wget -qO- -T 5 "$url" 2>/dev/null)
  fi
...
local_ipv4(){ env -u http_proxy -u https_proxy -u HTTP_PROXY -u HTTPS_PROXY -u all_proxy -u ALL_PROXY wget -qO- -T 5 http://ipv4.icanhazip.com 2>/dev/null | tr -d '\n '; }
local_ipv6(){ env -u http_proxy -u https_proxy -u HTTP_PROXY -u HTTPS_PROXY -u all_proxy -u ALL_PROXY wget -qO- -T 5 http://ipv6.icanhazip.com 2>/dev/null | tr -d '\n '; }
...
setup_certificate(){
  ...
  json_edit "$STATE" '.domain=$domain|.email=$email|.challenge=$mode|.cf_token=$cf|.ali_key=$ak|.ali_secret=$as|.acmedns_url=$au|.acmedns_user=$auu|.acmedns_pass=$aup|.acmedns_sub=$aus' --arg domain "$domain" --arg email "$email" --arg mode "$mode" --arg cf "$cf_token" --arg ak "$ali_key" --arg as "$ali_secret" --arg au "$acmedns_url" --arg auu "$acmedns_user" --arg aup "$acmedns_pass" --arg aus "$acmedns_sub" || { tell_warn "证书配置保存失败"; return 1; }
  return 0
}
...
certificate_ready(){
  ...
  while IFS= read -r file; do
    if openssl x509 -in "$file" -noout -checkhost "$domain" >/dev/null 2>&1 && openssl x509 -in "$file" -noout -checkend 0 >/dev/null 2>&1; then
      cert="$file"; break
    fi
  done <<EOF
...
wait_for_certificate(){
  local domain=$1 pid=${2:-} log_file=${3:-}
  tell "等待证书签发完成: $domain"
  while :; do
    certificate_ready "$domain" && { tell_ok "证书已成功签发并完成校验"; return 0; }
    if [ -n "$pid" ]; then
      kill -0 "$pid" 2>/dev/null || { tell_warn "ACME 签发进程已退出，证书签发失败"; [ -n "$log_file" ] && [ -s "$log_file" ] && { tell "ACME 最后错误信息:"; tail -n 12 "$log_file"; }; return 1; }
    elif ! rc-service sing-box status >/dev/null 2>&1; then
...
EOF
  ...
  [ "$final" != direct ] && inbounds=$(jq -cn --argjson list "$inbounds" '$list + [{type:"mixed",tag:"mixed-in",listen:"127.0.0.1",listen_port:2080}]') || [ "$final" = direct ] || return 1
  ...
  rules=$(jq -n --arg final "$final" '[{action:"sniff"}] + (if $final=="direct" then [] else [{inbound:["mixed-in"],action:"route",outbound:(if $final=="wireguard" then "wireguard" else $final end)}] end)') || return 1
...
sync_proxy_env(){
  ...
  if { [ "$selected" = wireguard ] && [ -f "$WG_CONF" ] && [ "$(jq -r '.role//""' "$WG_CONF" 2>/dev/null)" = client ]; } || { [ "$selected" != direct ] && [ -n "$selected" ] && [ -f "$PEER_DIR/$selected.json" ]; }; then
...
menu_modify_protocol(){
  ...
  while :; do
    ...
    clear; tell "${CYAN}========== $(jq -r .name "$NODE_EDIT_FILE") [$kind] ==========${PLAIN}"
...
menu_change_domain(){
  local new_domain old_domain old_email old_mode mode email count=0 file cf_token ali_key ali_secret acmedns_user acmedns_pass acmedns_sub acmedns_url
  clear; tell "${CYAN}========== 更换域名 ==========${PLAIN}"
  old_domain=$(state_get domain); old_email=$(state_get email); old_mode=$(state_get challenge)
  tell "  当前域名: ${old_domain:-未设置}"; tell "  绑定节点:"
  for file in "$NODE_DIR"/*.json; do [ -e "$file" ] || continue; [ "$(jq -r .tls_mode "$file")" = acme ] && { tell "  - $(jq -r .name "$file") [$(jq -r .kind "$file")]"; count=$((count+1)); }; done
  [ "$count" = 0 ] && tell "  无"; echo ""
  ...
  if prompt_yes "同步重置验证机制"; then
    ...
    printf '\n  %b选择证书验证方式:%b\n' "${CYAN}" "${PLAIN}"
    echo "  1. HTTP         (推荐 需放行80端口)"; echo "  2. TLS-ALPN     (推荐 需放行443端口)"; echo "  3. Cloudflare API"; echo "  4. 阿里云 DNS API"; echo "  5. ACME-DNS API"
    while :; do
      case $(prompt "请选择方式" 1) in
    ...
    email="$old_email"; mode="$old_mode"; cf_token=$(state_get cf_token); ali_key=$(state_get ali_key); ali_secret=$(state_get ali_secret); acmedns_url=$(state_get acmedns_url); acmedns_user=$(state_get acmedns_user); acmedns_pass=$(state_get acmedns_pass); acmedns_sub=$(state_get acmedns_sub)
  fi
  validate_domain "$new_domain" "$mode" || { prompt_yes "验证存在异常，是否强制继续" || { wait_key; return; }; }
  ...
  if [ "$count" -gt 0 ] && { ! apply_config || ! wait_for_certificate "$new_domain"; }; then
    ACME_PENDING_DOMAIN=""; ACME_PENDING_EMAIL=""; ACME_PENDING_MODE=""; ACME_PENDING_CF_TOKEN=""; ACME_PENDING_ALI_KEY=""; ACME_PENDING_ALI_SECRET=""; ACME_PENDING_ACMEDNS_URL=""; ACME_PENDING_ACMEDNS_USER=""; ACME_PENDING_ACMEDNS_PASS=""; ACME_PENDING_ACMEDNS_SUB=""
    tell_warn "新域名证书签发失败，旧域名保持不变"; wait_key; return
  fi
  BUILD_DOMAIN_OVERRIDE="$new_domain"
  if ! apply_config; then
    BUILD_DOMAIN_OVERRIDE=""
    ACME_PENDING_DOMAIN=""; ACME_PENDING_EMAIL=""; ACME_PENDING_MODE=""; ACME_PENDING_CF_TOKEN=""; ACME_PENDING_ALI_KEY=""; ACME_PENDING_ALI_SECRET=""; ACME_PENDING_ACMEDNS_URL=""; ACME_PENDING_ACMEDNS_USER=""; ACME_PENDING_ACMEDNS_PASS=""; ACME_PENDING_ACMEDNS_SUB=""
    tell_warn "新域名切换失败，旧域名保持不变"; wait_key; return
  fi
  json_edit "$STATE" '.domain=$domain|.email=$email|.challenge=$mode|.cf_token=$cf|.ali_key=$ak|.ali_secret=$as|.acmedns_url=$au|.acmedns_user=$auu|.acmedns_pass=$aup|.acmedns_sub=$aus' --arg domain "$new_domain" --arg email "$email" --arg mode "$mode" --arg cf "$cf_token" --arg ak "$ali_key" --arg as "$ali_secret" --arg au "$acmedns_url" --arg auu "$acmedns_user" --arg aup "$acmedns_pass" --arg aus "$acmedns_sub" || { BUILD_DOMAIN_OVERRIDE=""; tell_warn "域名配置写入失败"; wait_key; return; }
  BUILD_DOMAIN_OVERRIDE=""
  ACME_PENDING_DOMAIN=""; ACME_PENDING_EMAIL=""; ACME_PENDING_MODE=""; ACME_PENDING_CF_TOKEN=""; ACME_PENDING_ALI_KEY=""; ACME_PENDING_ALI_SECRET=""; ACME_PENDING_ACMEDNS_URL=""; ACME_PENDING_ACMEDNS_USER=""; ACME_PENDING_ACMEDNS_PASS=""; ACME_PENDING_ACMEDNS_SUB=""
  [ -n "$old_domain" ] && [ "$old_domain" != "$new_domain" ] && cleanup_old_domain_assets "$old_domain"
  tell_ok "域名配置已生效"; wait_key
}
...
peer_add(){
  ...
  clear; tell "${CYAN}========== 添加节点 ==========${PLAIN}"; name=$(prompt "识别名称" "RemoteNode"); [ -n "$name" ] || return; uri=$(prompt "节点链接"); [ -n "$uri" ] || return; parse_uri "$uri" || { tell_warn "节点链接格式无效"; wait_key; return; }
  ...
  if [ -n "$raw_data" ]; then
    ...
    while read -r file tag type name port host; do
      ...
      (
        ...
        case "$ms" in ''|*[!0-9]*) echo fail >"$tmp_dir/res_$idx" ;; *) echo "$ms" >"$tmp_dir/res_$idx" ;; esac
      ) &
      test_pids="$test_pids $!"
    done <<EOF
$raw_data
EOF
  ...
  while [ "$idx" -le "$PEER_COUNT" ]; do
    ...
    ms=$(cat "$tmp_dir/res_$idx" 2>/dev/null)
...
peer_delete(){
  while :; do
    ...
    local previous_exit tag backup
    previous_exit=$(state_get exit); tag=$(jq -r .tag "$PICKED")
    backup=$(mktemp "$TMPDIR/node-delete.XXXXXX") || { tell_warn "临时文件创建失败"; wait_key; continue; }
    if ! mv "$PICKED" "$backup"; then
      rm -f "$backup"; tell_warn "删除失败"; wait_key; continue
    fi
    if apply_config; then
      rm -f "$backup"
      if [ "$tag" = "$previous_exit" ]; then
        if state_set exit direct; then sync_proxy_env; tell_ok "已删除当前生效节点，已恢复直连"; else tell_warn "节点已删除，但状态保存失败"; fi
      else
        tell_ok "已删除节点"
      fi
    else
...
wg_actual_listen_port(){
  ...
  if command -v ss >/dev/null 2>&1; then
    ss -Hlun 2>/dev/null | awk -v p="$port" '{n=$4; sub(/^.*:/,"",n); if(n==p) found=1} END{exit !found}' && { printf '%s\n' "$port"; return 0; }
  elif command -v netstat >/dev/null 2>&1; then
    netstat -uln 2>/dev/null | awk -v p=":$port$" 'NR>2 && $4 ~ p {found=1} END{exit !found}' && { printf '%s\n' "$port"; return 0; }
  fi
  return 1
}
...
wg_status(){
  ...
  case "$role" in
    client)
      ...
      ms=""
      if [ "$enabled" = true ]; then
        ms=$(direct_ping_ms "$(jq -r '.peer_host // .peer_ip // .peer_ip6 // empty' "$file" 2>/dev/null)")
      fi
      if echo "$ms" | grep -Eq "^[0-9]+$"; then
        if [ "$ms" -lt 100 ]; then color="$GREEN"; elif [ "$ms" -lt 150 ]; then color="$YELLOW"; else color="$BROWN"; fi
      ...
      if printf "%s\n" "$endpoint_ips" | grep -q ':'; then tell "IPv6：${GREEN}正常${PLAIN}"; else tell "IPv6：${YELLOW}不可用${PLAIN}"; fi
      if printf '%s\n' "$endpoint_ips" | grep -Eq '^[0-9]+(\.[0-9]+){3}$'; then tell "IPv4：${GREEN}正常${PLAIN}"; else tell "IPv4：${YELLOW}不可用${PLAIN}"; fi
      tell ""
      tell "${CYAN}========================================${PLAIN}"
      ;;
    server)
      ...
      actual_port=$(wg_actual_listen_port "$configured_port" 2>/dev/null || true)
...
wg_init_server(){
  ...
  clear; tell "${CYAN}========== WireGuard 服务端引导配置 ==========${PLAIN}"
...
wg_init_client(){
  ...
  clear; tell "${CYAN}========== WireGuard 客户端 ==========${PLAIN}"
  wg_keypair || { tell_warn "密钥生成失败"; wait_key; return; }
...
wg_init(){
  clear; tell "${CYAN}========== 初始化配置 ==========${PLAIN}"; tell "  1. 服务端"; tell "  2. 客户端"; tell "  0. 返回"
  case $(prompt "请选择") in 1) wg_init_server ;; 2) wg_init_client ;; 0) return ;; *) tell_warn "输入无效"; sleep 1 ;; esac
}

wg_edit_address(){
  local body
  clear; [ -f "$WG_CONF" ] || { tell_warn "请先初始化配置"; wait_key; return; }
  tell "${CYAN}========== 内网双栈地址 ==========${PLAIN}"; tell "  IPv4: $(jq -r '.address[0] // ""' "$WG_CONF")"; tell "  IPv6: $(jq -r '.address[1] // ""' "$WG_CONF")"; echo ""
  wg_prompt_address
  ...
  if ! wg_apply_body "$body"; then wait_key; return; fi
...
wg_client_address_conflict(){
  local index=$1 ipv4=$2 ipv6=$3 ip4=${ipv4%/*} ip6=${ipv6%/*} c4 c6
  c4=$(jq -r --argjson i "$index" '.clients | to_entries[] | select(.key != $i) | .value.ipv4 // empty' "$WG_CONF" | awk -F/ -v ip="$ip4" '$1==ip{f=1} END{print f+0}')
  c6=$(jq -r --argjson i "$index" '.clients | to_entries[] | select(.key != $i) | .value.ipv6 // empty' "$WG_CONF" | awk -F/ -v ip="$ip6" '$1==ip{f=1} END{print f+0}')
  [ "$c4" = 0 ] && [ "$c6" = 0 ]
}
...
wg_client_list(){
  local count
  clear; tell "${CYAN}========== 客户端列表 ==========${PLAIN}"; count=$(jq '.endpoint.peers|length' "$WG_CONF" 2>/dev/null); count=${count:-0}
  [ "$count" -gt 0 ] && jq -r '.clients|to_entries[]|"  \(.key+1). \(.value.name)\n     IPv4：\(.value.ipv4)\n     IPv6：\(.value.ipv6)\n     公钥：\(.value.public_key)"' "$WG_CONF" || tell "暂无客户端"; wait_key
}

wg_server_clients(){
  while :; do clear; tell "${CYAN}========== 客户端管理 ==========${PLAIN}"; tell "  1. 添加客户端"; tell "  2. 修改客户端"; tell "  3. 删除客户端"; tell "  4. 查看客户端"; tell "  0. 返回"
...
wg_edit_server(){
  local value body
  while :; do clear; tell "${CYAN}========== 修改配置 ==========${PLAIN}"; tell "  1. 客户端管理"; tell "  2. 监听端口"; tell "  3. 隧道内网地址"; tell "  0. 返回"
    case $(prompt "请选择") in
      1) wg_server_clients ;; 2) value=$(prompt_port u "$(jq -r '.listen_port' "$WG_CONF")") || continue; body=$(jq --argjson p "$value" '.listen_port=$p|.endpoint.listen_port=$p' "$WG_CONF") || continue; wg_apply_body "$body" && tell_ok "已修改"; sleep 1 ;;
...
wg_edit_client(){
  local value body
  while :; do clear; tell "${CYAN}========== 修改配置 ==========${PLAIN}"; tell "  1. 服务端地址"; tell "  2. 服务端端口"; tell "  3. 服务端公钥"; tell "  4. 内网双栈地址"; tell "  0. 返回"
    case $(prompt "请选择") in
      1) value=$(prompt "服务端地址" "$(jq -r '.peer_host // ""' "$WG_CONF")"); [ -n "$value" ] || { tell_warn "服务端地址不能为空"; sleep 1; continue; }; body=$(jq --arg v "$value" '.peer_host=$v|.endpoint.peers[0].address=$v' "$WG_CONF") || continue; wg_apply_body "$body" && tell_ok "已修改"; sleep 1 ;;
      2) value=$(prompt_port u "$(jq -r '.peer_port // 0' "$WG_CONF")") || continue; body=$(jq --argjson p "$value" '.peer_port=$p|.endpoint.peers[0].port=$p' "$WG_CONF") || continue; wg_apply_body "$body" && tell_ok "已修改"; sleep 1 ;;
      3) value=$(prompt "服务端公钥" "$(jq -r '.peer_public_key // ""' "$WG_CONF")"); [ -n "$value" ] || { tell_warn "服务端公钥不能为空"; sleep 1; continue; }; body=$(jq --arg v "$value" '.peer_public_key=$v|.endpoint.peers[0].public_key=$v' "$WG_CONF") || continue; wg_apply_body "$body" && tell_ok "已修改"; sleep 1 ;;
...
wg_delete(){
  clear; [ -f "$WG_CONF" ] || { tell_warn "未配置 WireGuard"; wait_key; return; }
  prompt_yes "确认删除 WireGuard 配置" || return
  previous=$(state_get exit)
  local backup
  backup=$(mktemp -d "$TMPDIR/wg-delete.XXXXXX") || { tell_warn "临时目录创建失败"; wait_key; return; }
  mv "$WG_DIR" "$backup/wg" || { rm -rf "$backup"; tell_warn "删除失败"; wait_key; return; }
...
menu_wireguard(){
  while :; do clear; tell "${CYAN}========== WireGuard ==========${PLAIN}"
    if [ ! -f "$WG_CONF" ]; then tell "  模式：未配置"; else
      role=$(jq -r '.role // ""' "$WG_CONF"); enabled=$(jq -r '.enabled//false' "$WG_CONF")
      if [ "$role" = client ] && [ "$enabled" = true ]; then
        state=$(direct_ping_ms "$(jq -r '.peer_host // .peer_ip // .peer_ip6 // empty' "$WG_CONF")")
        if echo "$state" | grep -Eq '^[0-9]+$'; then
          if [ "$state" -lt 100 ]; then color=$GREEN; elif [ "$state" -lt 150 ]; then color=$YELLOW; else color=$BROWN; fi
          tell "  模式：客户端|${GREEN}已连接${PLAIN}|${color}${state} ms${PLAIN}"
        else tell "  模式：客户端|${YELLOW}未连接${PLAIN}"; fi
      elif [ "$role" = server ]; then
        [ "$enabled" = true ] && tell "  模式：服务端|${GREEN}已启动${PLAIN}" || tell "  模式：服务端|${YELLOW}已停止${PLAIN}"
      else tell "  模式：客户端|${YELLOW}已停止${PLAIN}"; fi
    fi
