...
resolve_addresses(){
  if command -v getent >/dev/null 2>&1; then
    { getent ahostsv4 "$1" 2>/dev/null; getent ahostsv6 "$1" 2>/dev/null; } | awk '{print $1}' | sort -u
  else
    ping -c1 "$1" 2>/dev/null | awk -F'[()]' '/PING/{print $2}'
  fi
}
...
run_watchdog(){
  ...
  while :; do
    sleep 30
    if curl -s -x socks5h://127.0.0.1:2080 -o /dev/null -m 6 "$PROBE_URL" || curl -s -x socks5h://127.0.0.1:2080 -o /dev/null -m 6 "$probe2"; then
      fail_count=0
...
run_update(){
  ...
  if [ -n "$latest" ] && [ "$current" != "$latest" ]; then
    if prompt_yes "发现新版本 v$latest，是否立即更新"; then
      install_core && apply_config && tell_ok "内核更新完成"
    fi
  else
    tell_ok "内核已是最新版本"
  fi
