...
build_config(){
  ...
  local inbounds outbounds dns rules providers='[]' mgmt4 mgmt6 selected_outbound
  ...
  if find "$NODE_DIR" -maxdepth 1 -type f -name '*.json' -print -quit 2>/dev/null | grep -q .; then
    inbounds=$(for f in "$NODE_DIR"/*.json; do [ -e "$f" ] || continue; cat "$f"; done | jq -s \
      --arg domain "$domain" --arg cert "$CERT_TAG" --arg listen "$listen_addr" '
      map(
        . as $node |
        $node.inbound
        | .listen=$listen
        | if $node.tls_mode=="acme" then
            .tls=({enabled:true} + (if $domain!="" then {server_name:$domain} else {} end) + {certificate_provider:$cert})
          else . end
      )
    ') || return 1
    [ -n "$inbounds" ] || inbounds='[]'
...
EOF
  ...
  elif [ "$selected" = wireguard ] && [ -f "$WG_CONF" ] && [ "$(jq -r '.enabled//false' "$WG_CONF")" = true ] && [ "$(jq -r '.role//""' "$WG_CONF")" = client ]; then
    local wgob
    wgob=$(jq -c '.endpoint' "$WG_CONF") || return 1
    outbounds=$(jq -c --argjson ob "$wgob" '.+[$ob]' <<EOF
$outbounds
EOF
) || return 1
    final=wireguard
...
EOF
  ...
  dns=$(jq -n --arg strategy "$dns_strategy" --arg bootstrap "$bootstrap_tag" --arg server "$bootstrap_server" --arg final "$final" --arg v4 "$mgmt4" --arg v6 "$mgmt6" '
    ...
    | if $final!="direct" and $final!="wireguard" then .servers |= map(if .tag=="dns-remote" then .detour=$final else . end) else . end
  ') || return 1
  ...
  jq -n --argjson inbounds "$inbounds" --argjson outbounds "$outbounds" --argjson dns "$dns" --argjson rules "$rules" --argjson providers "$providers" --arg final "$final" --arg resolver "$target_resolver" '
    {log:{level:"warn",timestamp:true},dns:$dns,inbounds:$inbounds,outbounds:$outbounds,route:{rules:$rules,final:$final,default_domain_resolver:$resolver}}
...
wg_toggle(){
  ...
  if [ "$(jq -r .enabled "$WG_CONF")" = true ]; then
    ...
    if apply_config; then [ "$role" = client ] && sync_watchdog; sleep 3; render_wg_info "$WG_CONF"; else json_edit "$WG_CONF" '.enabled=false' || true; state_set exit "$previous" || true; apply_config_quiet; sync_watchdog; tell_warn "冲突拦截，已回滚"; fi
  fi
  wait_key
}
