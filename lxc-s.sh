...
install_core(){
  local version arch url target_dir="/usr/local/bin" success=0
  ...
  if wget -qO- -T 120 "$url" 2>/dev/null | tar -xzf - -C "$target_dir" --strip-components=1 "sing-box-${version}-${arch}/sing-box" 2>/dev/null; then
...
arm_watchdog(){
  ...
  nohup sh -c 'run_watchdog' </dev/null >/dev/null 2>&1 &
  echo $! > "$WATCHDOG_PID"
}
  ...
  body=$(jq -n --arg tag "$tag" --arg name "$name" --argjson port "$port" \
   ...
   {tag:$tag,name:$name,kind:"tuic",port:$port,proto:"u",hopping:"",
    ...
    inbound:{type:"tuic",tag:$tag,listen:"::",listen_port:$port,
      users:[{uuid:$uuid,password:$password}],congestion_control:"cubic"}}')
  save_node "$NODE_DIR/$tag.json" "$body"
}
...
render_share_uri(){
  ...
  case $kind in
    ...
    hysteria2) uri="hysteria2://$(uri_encode "$(echo "$meta" | jq -r .password)")@$host:$port/?sni=$host&alpn=h3#$(uri_encode "$name")" ;;
  ...
  while :; do
    ...
    if echo "$index" | grep -Eq '^[0-9]+$' && [ "$index" -ge 1 ] && [ "$index" -le "$NODE_COUNT" ]; then
      ...
      for file in "$NODE_DIR"/*.json; do
        [ -e "$file" ] || continue
        i=$((i+1))
        if [ "$i" -eq "$index" ]; then PICKED="$file"; [ -f "$PICKED" ] && return 0; fi
      done
    else
...
menu_modify_protocol(){
  local kind value other up_mbps down_mbps old_json choice
  clear; tell "${CYAN}========== 修改配置 ==========${PLAIN}"
  select_node || return
  while :; do
    kind=$(jq -r .kind "$PICKED")
    clear
    tell "${CYAN}========== 修改节点: $(jq -r .name "$PICKED") [$kind] ==========${PLAIN}"
    tell "  1. 修改识别名称"
    tell "  2. 修改监听端口"
    case $kind in
      vless-reality|vless-tls) tell "  3. 修改通讯 UUID" ;;
      socks) tell "  3. 修改鉴权密码"; tell "  4. 修改鉴权账号" ;;
      *) tell "  3. 修改连接密码" ;;
    esac
    [ "$kind" = vless-reality ] && tell "  5. 修改握手目标域名"
    [ "$kind" = hysteria2 ] && tell "  6. 修改带宽限制"
    tell "  0. 返回上级"
    tell "${CYAN}==========================================${PLAIN}"
    
    old_json=$(cat "$PICKED")
    choice=$(prompt "请选择要修改的项")
...
uri_to_outbound(){
  ...
  case $URI_SCHEME in
    ...
    tuic)
      ...
      congestion=$(query_value congestion_control); [ -n "$congestion" ] || congestion=cubic
...
peer_add(){
  local name uri tag outbound probe ipv4 ipv6 domain port remote_ips ip
  ...
  remote_ips=$(resolve_addresses "$URI_HOST")
  for port in $(node_ports); do
    if [ "$URI_PORT" = "$port" ]; then
      ...
      for ip in $remote_ips; do
        if [ -n "$ipv4" ] && [ "$ip" = "$ipv4" ]; then tell_warn "禁止自环接入"; wait_key; return; fi
        if [ -n "$ipv6" ] && [ "$ip" = "$ipv6" ]; then tell_warn "禁止自环接入"; wait_key; return; fi
      done
    fi
  done
...
get_ip_info(){
  ...
  local raw_ip
  if [ "$mode" = 6 ]; then
    raw_ip=$(env http_proxy="$proxy_env" wget -qO- -T 5 http://ipv6.icanhazip.com 2>/dev/null | tr -d '\n ')
  else
    raw_ip=$(env http_proxy="$proxy_env" wget -qO- -T 5 http://ipv4.icanhazip.com 2>/dev/null | tr -d '\n ')
  fi
...
wg_tunnel_state(){
  if env http_proxy="http://127.0.0.1:2080" wget -q -O /dev/null -T 3 "http://cp.cloudflare.com/generate_204" >/dev/null 2>&1; then echo up; else echo down; fi
}
...
run_update(){
  ...
  if wget -q -O "$script_tmp" -T 15 "$script_url"; then
