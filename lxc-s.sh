...
sync_proxy_env(){
  ...
  case "$selected" in
    direct) valid=0 ;;
    wireguard) valid=1 ;;
    *) [ -f "$PEER_DIR/$selected.json" ] && valid=1 || valid=0 ;;
  esac
...
build_config(){
  ...
  rules=$(jq -n --arg host "$peer_host" --arg dds "$dns_direct_server" '
    ...
    + (if $host==""
       then []
       elif ($host | test("^[0-9]+\\.[0-9]+\\.[0-9]+\\.[0-9]+$"))
       then [{ip_cidr:[($host+"/32")],action:"route",outbound:"direct"}]
       elif ($host | test("^[0-9a-fA-F:]+$"))
       then [{ip_cidr:[($host+"/128")],action:"route",outbound:"direct"}]
       else [{domain:[$host],action:"route",outbound:"direct"}]
       end)
...
apply_config(){
  local tmp error guard=0 prev_conf=""
  tmp=$(mktemp)
  TMP_FILES="$TMP_FILES $tmp"
  if [ -f "$CONFIG" ]; then
    prev_conf=$(mktemp)
...
arm_watchdog(){
  stop_watchdog
  nohup "$SELF" --watchdog </dev/null >/dev/null 2>&1 &
  echo $! > "$WATCHDOG_PID"
}
...
watchdog_needed(){
  ...
  case "$exit_node" in
    direct) return 1 ;;
    wireguard) return 0 ;;
    *) [ -f "$PEER_DIR/$exit_node.json" ] && return 0 || return 1 ;;
  esac
}

sync_watchdog(){
  if watchdog_needed; then arm_watchdog; else stop_watchdog; fi
}

run_watchdog(){
  trap 'exit 0' TERM INT
  local fail_count=0 probe2="http://captive.apple.com/hotspot-detect.html"
  local target_exit probe_override current probe_port
  ...
  while :; do
    ...
    if [ "$current" = "direct" ]; then
      if ! netstat -tln 2>/dev/null | grep -q ':2081'; then
        WATCHDOG_PROBE="$probe_override" apply_config_quiet
      fi
      probe_port=2081
    fi

    if env http_proxy="http://127.0.0.1:${probe_port}" wget -q -O /dev/null -T 6 "$PROBE_URL" 2>/dev/null || \
       env http_proxy="http://127.0.0.1:${probe_port}" wget -q -O /dev/null -T 6 "$probe2" 2>/dev/null; then
      fail_count=0
      if [ "$current" = "direct" ]; then
      ...
      if [ "$fail_count" -ge 3 ] && [ "$current" != "direct" ]; then
        fail_count=0
        state_set exit direct
...
probe_handshake_target(){
  local target=$1
  if wget -qO /dev/null -T 5 "https://$target" 2>/dev/null; then
    tell_ok "目标端口连通性验证通过 (未校验底层TLS特征)"
    return 0
  fi
  tell_warn "目标域名 443 端口可能无法连通或返回异常"
  return 1
}
...
peer_add(){
  local name uri tag outbound probe ipv4 ipv6 domain port tmp resolved_hosts ip
