...
get_ip_info(){
  ...
  if command -v curl >/dev/null 2>&1; then
    res=$(env -u http_proxy -u https_proxy -u HTTP_PROXY -u HTTPS_PROXY -u all_proxy -u ALL_PROXY -u NO_PROXY -u no_proxy curl -s -"$mode" -m 5 "$url" 2>/dev/null)
  elif command -v wget >/dev/null 2>&1; then
    res=$(env -u http_proxy -u https_proxy -u HTTP_PROXY -u HTTPS_PROXY -u all_proxy -u ALL_PROXY -u NO_PROXY -u no_proxy wget -qO- -T 5 "$url" 2>/dev/null)
  else
    res=""
  fi
...
render_status_ip(){
  ...
  get_ip_info 4
  if [ -n "$IP_INFO_IP" ]; then
    tell "  出口IPv4: ${GREEN}${IP_INFO_IP}${PLAIN} | ${YELLOW}${IP_INFO_C}${PLAIN}"
    tell "  ${CYAN}${IP_INFO_NAME}${PLAIN} | ASN: ${PURPLE}${IP_INFO_ASN}${PLAIN}"
  else
    tell "  出口IPv4: ${RED}无或不可用${PLAIN}"
  fi
  get_ip_info 6
  if [ -n "$IP_INFO_IP" ]; then
    tell "  出口IPv6: ${GREEN}${IP_INFO_IP}${PLAIN} | ${YELLOW}${IP_INFO_C}${PLAIN}"
    tell "  ${CYAN}${IP_INFO_NAME}${PLAIN} | ASN: ${PURPLE}${IP_INFO_ASN}${PLAIN}"
  else
    tell "  出口IPv6: ${RED}无或不可用${PLAIN}"
  fi
}
...
build_config(){
  local selected domain listen_addr dns_strategy bootstrap_tag final='direct' use_tun=0 peer_host="" auto_detect=false
...
EOF
    ...
    if [ "$wgrole" = client ] && [ "$selected" = wireguard ]; then
      final=wireguard
      use_tun=1
      peer_host=$(jq -r '.peers[0].address // ""' "$wg_source")
    fi
  fi

  if [ "$use_tun" = 1 ]; then
    inbounds=$(jq -n --arg name "$TUN_IF" --argjson list "$inbounds" '
  ...
  jq -n --argjson inbounds "$inbounds" --argjson outbounds "$outbounds" --argjson endpoints "$endpoints" --argjson dns "$dns" --argjson rules "$rules" --argjson providers "$providers" --arg final "$final" --arg resolver "$target_resolver" --argjson auto_detect "$auto_detect" '
...
while :; do
  ...
  tell "  -- 运行环境：${BRIGHT_GREEN}${BOLD}${PLATFORM}${PLAIN} --"
