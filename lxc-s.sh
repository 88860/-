...
render_client_ip_status(){
  local exit_node probe_proxy=0
  exit_node=$(state_get exit)
  if [ "$exit_node" != direct ] && [ -n "$exit_node" ]; then probe_proxy=1; fi
  if [ "$probe_proxy" = 1 ]; then get_ip_info_via_lxc_proxy 4 "$exit_node"; else get_ip_info 4; fi
  if [ -n "$IP_INFO_IP" ]; then
  ...
  if [ "$probe_proxy" = 1 ]; then get_ip_info_via_lxc_proxy 6 "$exit_node"; else get_ip_info 6; fi
  if [ -n "$IP_INFO_IP" ]; then
...
get_ip_info_via_lxc_proxy(){
  local family=$1 selected=$2 port tmp_conf tmp_log pid res ip country asn name
  IP_INFO_IP=""; IP_INFO_C=""; IP_INFO_ASN=""; IP_INFO_NAME=""
  [ -n "$selected" ] && [ "$selected" != direct ] || return 1
  [ -s "$CONFIG" ] || return 1
  ...
  tmp_conf=$(mktemp "$SBM_DIR/tmp/status-proxy.XXXXXX.json") || return 1
  tmp_log=$(mktemp "$SBM_DIR/tmp/status-proxy.XXXXXX.log") || { rm -f "$tmp_conf"; return 1; }
  if ! jq --argjson port "$port" --arg final "$selected" '
    .inbounds=[{type:"mixed",tag:"status-in",listen:"127.0.0.1",listen_port:$port}]
    | .route.final=$final
  ...
  while [ "$i" -lt 20 ]; do
    ...
    res=$(env -u http_proxy -u https_proxy -u HTTP_PROXY -u HTTPS_PROXY -u all_proxy -u ALL_PROXY \
      curl -sS -"$family" --socks5-hostname "127.0.0.1:$port" -m 5 https://ipwho.is/ 2>/dev/null) && break
    i=$((i+1)); sleep 0.2
  done
