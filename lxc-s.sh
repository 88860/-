...
build_config(){
  ...
  dns_block=$(jq -n --arg detour "$final" --arg strategy "$strategy" '
    {servers:[{type:"local",tag:"dns-local",independent_cache:true},
              (if $detour == "direct" then {type:"udp",tag:"dns-remote",server:"1.1.1.1",independent_cache:true}
               else {type:"udp",tag:"dns-remote",server:"1.1.1.1",detour:$detour,independent_cache:true} end)],
     final:"dns-remote"}
...
uri_to_outbound(){
  ...
  if [ "$URI_SCHEME" = "vless" ] || [ "$URI_SCHEME" = "trojan" ] || [ "$URI_SCHEME" = "anytls" ]; then
    outbound=$(echo "$outbound" | jq '.multiplex={enabled:true,protocol:"smux",max_connections:4,padding:true}')
  fi
    ...
    body=$(jq -n --arg private "$private" --arg public "$public" --arg a4 "$address4" --arg a6 "$address6" \
     ...
     {role:"client",enabled:false,private_key:$private,public_key:$public,
      ...
      endpoint:{type:"wireguard",tag:"wireguard",system:false,name:$iface,mtu:1408,
        ...
        peers:[{server:$host,server_port:$port,public_key:$peer_key,
