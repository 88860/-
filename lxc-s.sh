...
case "$INIT_EXE" in
  ...
  */openrc-init|*/openrc) PLATFORM=openrc ;;
  *)
    printf '\n  [×] 不支持的 init 系统：%s\n' "${INIT_EXE:-未知}" >&2
    printf '      仅支持 OpenRC 或 systemd。\n\n' >&2
    exit 1
    ;;
esac
export PLATFORM
...
network_public_ip(){
  ...
  if command -v curl >/dev/null 2>&1; then
    if [ -n "$iface" ]; then
      curl -"$family" -fsS --connect-timeout 3 -m 5 --interface "$iface" "$url" 2>/dev/null | tr -d '\n '
    else
      curl -"$family" -fsS --connect-timeout 3 -m 5 "$url" 2>/dev/null | tr -d '\n '
    fi
  elif command -v wget >/dev/null 2>&1; then
