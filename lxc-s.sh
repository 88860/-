...
build_config(){
  ...
  if [ "$use_tun" = 1 ]; then
    if check_tun_support; then
      ...
      use_tun=0
      printf '  %b[提示] 当前 LXC 宿主缺失 TUN 权限，系统级代理已自动退化为环境变量模式%b\n' "${YELLOW}" "${PLAIN}" >&2
      printf '  %b       (本机作为服务端中继节点的功能不受任何影响)%b\n' "${YELLOW}" "${PLAIN}" >&2
    fi
  fi
...
apply_config(){
  ...
  sync_bypass_rules
  sync_hopping_rules
  # 应用配置时，完全自动判断是否需要写入/删除代理环境变量
  sync_proxy_env
  
  bypass_rules_present || printf '  %b[提示] 当前LXC宿主内核不支持 sport 路由规则，普通代理不受影响，TUN/自动路由功能可能不可用%b\n' "${YELLOW}" "${PLAIN}" >&2
  return 0
}
...
run_watchdog(){
  ...
  sync_bypass_rules; sync_hopping_rules; sync_proxy_env
  exit 0
}
...
get_ip_info(){
  ...
  # 现在由于脚本自带代理环境变量（如果在代理状态下），这里的 curl 会完美自动走节点出口
...
case $1 in
  --sync) init_dirs; sync_bypass_rules; sync_hopping_rules; sync_proxy_env; exit 0 ;;
