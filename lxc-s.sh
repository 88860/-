...
menu_change_domain(){
  ...
  screen_clear; tell "${CYAN}========== 更换域名 ==========${PLAIN}"
...
menu_client(){
  while :; do
    screen_clear; tell "${CYAN}========== 客户端管理 ==========${PLAIN}"
...
run_script_update(){
  local script_url script_tmp
  screen_clear
  tell "正在检测管理脚本更新..."
  case "$PLATFORM" in
    OpenRC) script_url="https://raw.githubusercontent.com/88860/-/main/lxc-s.sh" ;;
    systemd) script_url="https://raw.githubusercontent.com/88860/-/main/s.sh" ;;
    *) tell_warn "未知环境，无法确定脚本更新地址"; wait_key; return 1 ;;
  esac
  ...
  elif sh -n "$script_tmp" 2>/dev/null && { [ "$PLATFORM" = OpenRC ] || bash -n "$script_tmp" 2>/dev/null; }; then
...
menu_create_protocol(){
  while :; do
    screen_clear; tell "${CYAN}========== 创建协议 ==========${PLAIN}"
...
create_vless_tls(){
  ...
  screen_clear; tell "${CYAN}========== 创建协议 ==========${PLAIN}"; name=$(prompt "节点名称" "VLESS-TLS"); setup_certificate || return; port=$(prompt_port t ""); uuid=$(prompt "通信 UUID (留空自动生成)" "$(random_uuid)"); tag=$(unique_tag "$name" in- "$NODE_DIR")
...
create_tuic(){
  ...
  screen_clear; tell "${CYAN}========== 创建协议 ==========${PLAIN}"; name=$(prompt "节点名称" "TUIC"); setup_certificate || return; port=$(prompt_port u ""); uuid=$(prompt "通信 UUID (留空自动生成)" "$(random_uuid)"); password=$(prompt "连接密码 (留空自动生成)" "$(random_password)"); tag=$(unique_tag "$name" in- "$NODE_DIR")
...
create_trojan(){
  ...
  screen_clear; tell "${CYAN}========== 创建协议 ==========${PLAIN}"; name=$(prompt "节点名称" "Trojan"); setup_certificate || return; port=$(prompt_port t ""); password=$(prompt "连接密码 (留空自动生成)" "$(random_password)"); tag=$(unique_tag "$name" in- "$NODE_DIR")
...
create_anytls(){
  ...
  screen_clear; tell "${CYAN}========== 创建协议 ==========${PLAIN}"; name=$(prompt "节点名称" "AnyTLS"); setup_certificate || return; port=$(prompt_port t ""); password=$(prompt "连接密码 (留空自动生成)" "$(random_password)"); client_meta=$(prompt "客户端元数据 (留空则为空)" ""); tag=$(unique_tag "$name" in- "$NODE_DIR")
...
create_socks(){
  ...
  screen_clear; tell "${CYAN}========== 创建协议 ==========${PLAIN}"; name=$(prompt "节点名称" "Socks5"); username=$(prompt "鉴权账号" "admin"); password=$(prompt "鉴权密码 (留空自动生成)" "$(random_password)"); [ -n "$username" ] && [ -n "$password" ] || { tell_warn "必填项为空"; wait_key; return; }; port=$(prompt_port t ""); tag=$(unique_tag "$name" in- "$NODE_DIR")
...
create_snell(){
  ...
  screen_clear; tell "${CYAN}========== 创建协议 ==========${PLAIN}"; name=$(prompt "节点名称" "Snell"); port=$(prompt_port t "")
  ...
  while :; do
    ...
    if [ -f "$WG_CONF" ]; then
      ...
      if [ "$role" = server ]; then
        ...
        [ "$enabled" = true ] && tell "  模式：客户端|${GREEN}已启动${PLAIN}" || tell "  模式：客户端|${YELLOW}已停止${PLAIN}"
      else tell "  模式：未配置"; fi
    else tell "  模式：未配置"; fi
    tell ""
    tell "  1. 初始化配置"; tell "  2. 修改配置"; tell "  3. 启动/停止"; tell "  4. 删除配置"; tell "  0. 退出"
    tell "${CYAN}===============================${PLAIN}"
    case $(prompt "请选择") in
      1) wg_init ;; 2) wg_edit ;; 3) wg_toggle ;; 4) wg_delete ;; 0) return ;; *) tell_warn "输入无效"; sleep 1 ;;
    esac
  done
}
...
list_peers(){
  ...
  PEER_COUNT=0
  set -- "$PEER_DIR"/*.json
  [ ! -e "$1" ] && { screen_clear; tell "${CYAN}========== $title ==========${PLAIN}"; tell "  暂无外部节点"; return 0; }
...
EOF
  ...
  screen_clear; tell "${CYAN}========== $title ==========${PLAIN}"
  idx=1
...
wg_edit_server(){
  local value body
  while :; do screen_clear; tell "${CYAN}========== 修改配置 ==========${PLAIN}"; tell "  1. 客户端管理"; tell "  2. 监听端口"; tell "  3. 隧道内网地址"; tell "  0. 返回"
    case $(prompt "请选择") in
...
wg_edit_client(){
  local value body
  while :; do screen_clear; tell "${CYAN}========== 修改配置 ==========${PLAIN}"; tell "  1. 服务端地址"; tell "  2. 服务端端口"; tell "  3. 服务端公钥"; tell "  4. 内网双栈地址"; tell "  0. 返回"
    case $(prompt "请选择") in
...
wg_server_clients(){
  while :; do
    screen_clear; tell "${CYAN}========== 客户端管理 ==========${PLAIN}"; tell "  1. 添加客户端"; tell "  2. 修改客户端"; tell "  3. 删除客户端"; tell "  4. 查看客户端"; tell "  0. 返回"
...
wg_client_list(){
  local count
  screen_clear; tell "${CYAN}========== 客户端列表 ==========${PLAIN}"; count=$(jq '.endpoint.peers|length' "$WG_CONF" 2>/dev/null); count=${count:-0}
  if [ "$count" -eq 0 ]; then tell "暂无客户端"; else jq -r '.clients|to_entries[]|"  \(.key+1). \(.value.name)\n     IPv4：\(.value.ipv4)\n     IPv6：\(.value.ipv6)\n     公钥：\(.value.public_key)"' "$WG_CONF" 2>/dev/null; fi
  wait_key
}
...
EOF
      ...
      ;;
    systemd) rm -f /etc/profile.d/sbm_proxy.sh ;;
  esac
  return 0
}
...
default_iface_v6(){ default_iface 6; }

ipv6_egress_available(){
  local iface=${NET_IF_V6:-}
  [ -n "$iface" ] || iface=$(default_iface_v6)
  [ -n "$iface" ] || return 1
...
create_hysteria2(){
  ...
  screen_clear; tell "${CYAN}========== 创建协议 ==========${PLAIN}"; name=$(prompt "节点名称" "Hysteria2"); setup_certificate || return; port=$(prompt_port u ""); password=$(prompt "连接密码 (留空自动生成)" "$(random_password)"); bbr_profile=""; up_mbps=0; down_mbps=0
...
menu_delete_protocol(){
  screen_clear; tell "${CYAN}========== 删除协议 ==========${PLAIN}"
  select_node || return
...
menu_modify_protocol(){
  ...
  screen_clear; tell "${CYAN}========== 修改配置 ==========${PLAIN}"
  select_node || return
  ...
  while :; do
    ...
    screen_clear; tell "${CYAN}========== $(jq -r .name "$NODE_EDIT_FILE") [$kind] ==========${PLAIN}"
...
menu_server(){
  while :; do
    screen_clear; tell "${CYAN}========== 服务端管理 ==========${PLAIN}"
...
peer_add(){
  ...
  screen_clear; tell "${CYAN}========== 添加节点 ==========${PLAIN}"; name=$(prompt "识别名称" "RemoteNode"); [ -n "$name" ] || return; uri=$(prompt "节点链接"); [ -n "$uri" ] || return; parse_uri "$uri" || { tell_warn "节点链接格式无效"; wait_key; return; }
...
wg_init_server(){
  ...
  screen_clear; tell "${CYAN}========== WireGuard 服务端引导配置 ==========${PLAIN}"
...
wg_init_client(){
  ...
  screen_clear; tell "${CYAN}========== WireGuard 客户端 ==========${PLAIN}"
  wg_keypair || { tell_warn "密钥生成失败"; wait_key; return; }
...
wg_init(){
  screen_clear; tell "${CYAN}========== 初始化配置 ==========${PLAIN}"; tell "  1. 服务端"; tell "  2. 客户端"; tell "  0. 返回"
...
while :; do
  ...
  tell "${CYAN}================================${PLAIN}"
  tell "${CYAN}             s管理              ${PLAIN}"
  tell "  当前环境：${GREEN}${PLATFORM}${PLAIN}"
  tell "${CYAN}================================${PLAIN}"
  tell "  1. 服务端管理"
  tell "  2. 客户端管理"
  tell "  3. WireGuard 管理"
...
menu_modify_protocol(){
  ...
  while :; do
    ...
    screen_clear; tell "${CYAN}========== $(jq -r .name "$PICKED") [$kind] ==========${PLAIN}"
    tell "  1. 识别名称"; tell "  2. 监听端口"
    case $kind in
      vless-reality|vless-tls) tell "  3. 通信 UUID" ;;
      tuic) tell "  3. 通信 UUID"; tell "  4. 连接密码" ;;
...
peer_add(){
  ...
  screen_clear; tell "${CYAN}========== 添加节点 ==========${PLAIN}"
...
menu_client_status(){
  screen_clear; tell "${CYAN}========== 客户端状态 ==========${PLAIN}"
  local exit_node=$(state_get exit)
