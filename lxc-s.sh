...
peer_listener_has_port(){
  local proto=$1 port=$2
  case "$proto" in
    u)
      netstat -uln 2>/dev/null | awk -v p=":$port$" 'NR>2 && $4 ~ p {found=1} END{exit !found}'
      ;;
    t)
      netstat -tln 2>/dev/null | awk -v p=":$port$" 'NR>2 && $4 ~ p {found=1} END{exit !found}'
      ;;
    *)
      peer_listener_has_port t "$port" || peer_listener_has_port u "$port"
      ;;
  esac
}
...
peer_target_is_local(){
  ...
  peer_listener_has_port "$proto" "$port"
}
